...
function _MakePWDWidget()
{
	...
	code += "<input type='password' onkeyup='return show_bar();' autocomplete='off' class='pwdfield'  name='"+pwdname+"' id='"+this.pwdfieldid+"'>";

	this.pwdtxtfield=pwdname;
	...
	mark_bar += "<div class='pwdstrength' id='"+this.pwdstrengthdiv+"'>";

	this.pwdstrengthbar = pwdname + "_strength_bar";

	mark_bar += "<div class='pwdstrengthbar' id='"+this.pwdstrengthbar+"'></div>";

	this.pwdstrengthstr = pwdname + "_strength_str";

	mark_bar += "<div class='pwdstrengthstr' id='"+this.pwdstrengthstr+"'></div>"; 
	mark_bar += "</div>"; 
    
    document.getElementById('mark_bar').innerHTML=mark_bar;
...
{
	document.getElementById('brn_1').style.display='block';
