...
<page size="A4" style="margin-right: 0.25in;margin-left: 0.25in;">
  ...
  <hr>
  <canvas height="10vh" width="40vw"  id="chartContainer1"></canvas>
  <div class="row"> 
  <br>
        ...
        $.ajax({
            ...
            success: function(result) 
            {
              ...
              var st1=0;
               ...
               { 
                st1=+(start_point+i);
                score.TeamA.push({'x' : st1,'y' : data[i].value});
               }
           ...
           $.ajax({
             ...
             { 
              score.TeamA.push(data[i].value);
               if(data[i].value>1 && data[i].value<=2)
                 {
                    myColors[start_point-1+i]="yellow";
                 }
                 else if(data[i].value>2 && data[i].value<=4)
                 {
                    myColors[start_point-1+i]="lightgreen";
                 }
                 else
                 {
                    myColors[start_point-1+i]="green";
                 }
             }
              ...
              for (var i = 1; i <= 12; i++) { 
                if(data[i] !=undefined && start_point<=i)
                label.TeamB.push((start_point)+';');
                else
                label.TeamB.push((i)+';');
              }
            ...
            success: function(result) 
            {
              ...
              for (var i = 0; i < len; i++) 
               { 
                 st2=+(start_point+(+data[i].value_x));
                score.TeamA.push({'x' : st2,'y' : data[i].value});
               ...
               for (var i = 0; i < len; i++) 
               { 
                 st3=+(start_point+(+data[i].value_x));
                if(data[i].value_low_bp !=0)
                {
                  var range1 = { 
                    minRvalue : []
                  };
                  range1.minRvalue.push({'x' : st3,'y' : data[i].value_low_bp },{'x' : st3 ,'y' : data[i].value_high_bp});
                  rd.push({
