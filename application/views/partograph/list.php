...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '25',
        "ajax": {
            "url": "<?php echo base_url('partograph/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'partograph/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
$modal.load('<?php echo base_url().'partograph/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
function edit_partograph(id)
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'partograph/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function view_partograph(id)
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'partograph/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('partograph/deleteall');?>",
                      data: {row_id: allVals},
...
<?php } ?>
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered partograph_list_tbl" cellspacing="0" width="100%">
            <thead>
                <tr>
...
{ 
  ...
  $.ajax({url: "<?php echo base_url(); ?>partograph/reset_search/", 
    success: function(result)
    { 
...
$("#search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('partograph/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    { 
 ...
 ?>
 function delete_partograph(partograph_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('partograph/delete/'); ?>"+partograph_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('partograph');?>'; 
    }); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor" href="'<?php echo base_url("partograph/print_partographs"); ?>'" target="_blank" >Print</a>
