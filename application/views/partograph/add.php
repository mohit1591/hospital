...
<form id="partograph_setting_form">
  ...
  <div class="row">
    ...
    <?php
      ...
      {
        if(!empty($partograph_setting_list))
        {
          foreach($partograph_setting_list as $setting_list)
          {
    ...
    ?>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-4">
          <?php if($setting_list->setting_name=='PARTOGRAPH_PRINT_SETTING') { ?>
          </div>
        </div>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-3" style="padding-left:7px;">
             <textarea cols="45" readonly="">{patient_reg_no},{patient_name},{patient_age},{patient_address},{booking_code},{mobile_no},{booking_date},{booking_time},{doctor_name},{patient_bp},{patient_temp},{patient_weight},{patient_pr},{patient_spo},{patient_rbs},{prv_history}, {personal_history}, {chief_complaints}, {examination}, {diagnosis}, {patient_test_name}, {patient_pres}, {suggestion}, {remark}, {appointment_date}, {appointment_date},{parent_relation_type},{parent_relation_name}, {signature} </textarea>
        </div>
     </div>
     </div>
     </div>
...
$("#partograph_setting_form").on("submit", function(event) 
{
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>partograph_setting/",
    type: "post",
    data: $(this).serialize(),
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'partograph_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
