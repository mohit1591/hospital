...
var table;
<?php
if(in_array('2013',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/dosage/ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_dosage_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/dosage/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/dosage/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('2013',$users_data['permission']['action'])) {
        ?>
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               <?php if(in_array('2014',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" id="modal_add">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
               <?php if(in_array('2016',$users_data['permission']['action'])) {
               ?>
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('pediatrician/dosage/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/dosage/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
