...
<div class="overlay-loader" id="overlay-loader">
  ...
  <form  id="pedic_setting" class="form-inline">
...
$("#pedic_setting").on("submit", function(event) { 
  ...
  { 
    var path = 'edit/'+ids;
    var msg = 'Pediatrician setting successfully updated.';
  }
  else
  {
    var path = 'add/';
    var msg = 'Pediatrician setting successfully created.';
  } 
  //alert('ddd');return false;
  $.ajax({
    url: "<?php echo base_url('pediatrician/pediatrician_settings/'); ?>"+path,
    type: "post",
    data: $(this).serialize(),
    success: function(result) {
      if(result==1)
      {
        $('#load_add_pedic_settings_modal_popup').modal('hide');
      ...
      {
        $("#load_add_pedic_settings_modal_popup").html(result);
      }       
      $('#overlay-loader').hide();
    }
  });
...
{
   $.ajax({url: "<?php echo base_url(); ?>pediatrician/pediatrician_settings/blood_bank_settings_dropdown/", 
    success: function(result)
    {
