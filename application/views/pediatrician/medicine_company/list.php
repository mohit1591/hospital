...
var table;
<?php
if(in_array('2038',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/medicine_company/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$(document).ready(function(){
var $modal = $('#load_add_medicine_company_modal_popup');
$('#modal_add').on('click', function(){
$modal.load('<?php echo base_url().'pediatrician/medicine_company/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_medicine_company(id)
{
  var $modal = $('#load_add_medicine_company_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/medicine_company/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function view_medicine_company(id)
{
  var $modal = $('#load_add_medicine_company_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/medicine_company/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/medicine_company/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('2038',$users_data['permission']['action'])) {
       ?>
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered type_list" cellspacing="0" width="100%">
            <thead>
                <tr>
  	...
  	<div class="userlist-right">
  		<div class="btns">
          <?php if(in_array('2039',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('2041',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?>
          <?php if(in_array('1374',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('pediatrician/medicine_company/archive'); ?>'">
...
$('#selectAll').on('click', function () { 
 ...
 function delete_medicine_company(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/medicine_company/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$(document).ready(function() {
   $('#load_add_medicine_company_modal_popup').on('shown.bs.modal', function(e) {
      $('.inputFocus').focus();
   })
}); 
</script> 
...
<div id="load_add_medicine_company_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
</div><!-- container-fluid -->
</body>
</html>