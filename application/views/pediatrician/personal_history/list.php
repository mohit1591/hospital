...
var table;
<?php
if(in_array('1999',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/personal_history/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'pediatrician/personal_history/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_personal_history_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/personal_history/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_personal_history_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/personal_history/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/personal_history/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1999',$users_data['permission']['action'])) {
       ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
          <?php if(in_array('2000',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('2002',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?>
          <?php if(in_array('2003',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('pediatrician/personal_history/archive'); ?>'">
...
<script> 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/personal_history/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
