...
var table;
<?php
if(in_array('1978',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/diagnosis/ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_diagnosis_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/diagnosis/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/diagnosis/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1978',$users_data['permission']['action'])) {
        ?>
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               <?php if(in_array('1979',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" id="modal_add">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
               <?php if(in_array('1981',$users_data['permission']['action'])) {
               ?>
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('pediatrician/diagnosis/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/diagnosis/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
