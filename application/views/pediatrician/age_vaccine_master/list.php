...
var table;
<?php
if(in_array('1596',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/age_vaccine_master/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$(document).ready(function(){
var $modal = $('#load_add_age_vaccine_modal_popup');
...
function edit_age_vaccine(id)
{
  var $modal = $('#load_add_age_vaccine_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/age_vaccine_master/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/age_vaccine_master/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1596',$users_data['permission']['action'])) {
       ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
          <?php if(in_array('1597',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1599',$users_data['permission']['action'])) {
          ?>
          ...
          ?>
  			<button class="btn-update" onclick="window.location.href='<?php echo base_url('pediatrician/age_vaccine_master/archive'); ?>'">
...
$('#selectAll').on('click', function () { 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/age_vaccine_master/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$(document).ready(function() {
   $('#load_add_age_vaccine_modal_popup').on('shown.bs.modal', function(e) {
      $('.inputFocus').focus();
   })
}); 
</script> 
...
<div id="load_add_age_vaccine_modal_popup" class="modal fade modal-60" role="dialog" data-backdrop="static" data-keyboard="false"></div>
</div> <!-- container-fluid -->
</body>
</html>