...
var table;
<?php
if(in_array('1971',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/examination/ajax_list')?>",
            "type": "POST",
...
$(document).ready(function(){
var $modal = $('#load_add_examination_modal_popup');
$('#modal_add').on('click', function(){

$modal.load('<?php echo base_url().'pediatrician/examination/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_examination(id)
{
  var $modal = $('#load_add_examination_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/examination/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_examination_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/examination/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/examination/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1971',$users_data['permission']['action'])) {
        ?>
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               <?php if(in_array('1972',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" id="modal_add">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
               <?php if(in_array('1974',$users_data['permission']['action'])) {
               ?>
               ...
               <?php } ?>
               <?php if(in_array('1975',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('pediatrician/examination/archive'); ?>'">
...
<script>  
 ...
 function delete_examination(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/examination/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$(document).ready(function(){
  $('#load_add_examination_modal_popup').on('shown.bs.modal', function(e){
    $('.inputFocus').focus();
  });
});
...
<div id="load_add_examination_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
</div><!-- container-fluid -->
</body>
</html>