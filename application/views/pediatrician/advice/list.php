...
var table;
<?php
if(in_array('1950',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('pediatrician/advice/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'pediatrician/advice/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_advice_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/advice/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_advice_modal_popup');
  $modal.load('<?php echo base_url().'pediatrician/advice/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('pediatrician/advice/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1950',$users_data['permission']['action'])) {
       ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
          <?php if(in_array('1951',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1953',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?>
          <?php if(in_array('1954',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" onclick="window.location.href='<?php echo base_url('pediatrician/advice/archive'); ?>'">
...
$('#selectAll').on('click', function () { 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('pediatrician/advice/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
