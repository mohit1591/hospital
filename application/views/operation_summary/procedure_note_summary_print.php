...
if(!empty($signature_data))
{
    $signature_reprt_data .='<table border="0" cellpadding="4px" cellspacing="0" width="100%" style="border-collapse: collapse;font-size:13px;margin-top: 5%;"> 
	  ...
	  $signature_reprt_data.='<tr>
      <td width="70%"></td>
        <td valign="top" align="" style="padding-right:2em;"><div style="float:right;text-align: left;width:140px;">Dr.'.$signature_data->qualification.'</div></td>
      </tr>';
...
if($template!="" && !empty($template))
{
	...
	if(getProcedureNoteTabSetting('blood_transfusion')['print_status']){
	...
	if(getProcedureNoteTabSetting('blood_loss')['print_status']){
		$template= str_replace("{blood_loss}",$summary_data['blood_loss'],$template);
	}
	if(getProcedureNoteTabSetting('drain')['print_status']){
		$template= str_replace("{drain}",$summary_data['drain'],$template);
	}
	if(getProcedureNoteTabSetting('materials_submitted_for_histopathological_exam')['print_status']){
		$template= str_replace("{histopathological}",$summary_data['histopathological'],$template);
	}
	if(getProcedureNoteTabSetting('materials_submitted_for_microbiological_exam')['print_status']){
	...
	$template= str_replace("{procedure_data}",$procedure_data,$template);
