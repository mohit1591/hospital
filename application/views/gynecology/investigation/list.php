...
<?php
 ...
 if(in_array('1845',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('gynecology/investigation/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
function edit_investigation(id)
{
  var $modal = $('#load_add_gynecology_inves_modal_popup');
  $modal.load('<?php echo base_url().'gynecology/investigation/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('gynecology/investigation/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
      ...
      <div class="fixed">
  		<div class="btns">
          <?php if(in_array('1846',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1848',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
  				<i class="fa fa-trash"></i> Delete
  			</button>
          <?php } ?>
          <?php if(in_array('1845',$users_data['permission']['action'])) {
          ?>
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('gynecology/investigation/archive'); ?>'">
...
{ 
 ...
 function delete_investigation(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('gynecology/investigation/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
