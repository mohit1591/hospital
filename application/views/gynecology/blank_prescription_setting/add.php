...
<form id="prescription_setting_form">
  ...
  <div class="row">
    ...
    <?php
      if(in_array('1898',$users_data['permission']['action']))
      {
    ...
    ?>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-3" style="padding-left:7px;">
             <textarea cols="45" readonly="">{patient_reg_no},{patient_name},{patient_age},{patient_address},{booking_code},{mobile_no},{booking_date},{doctor_name},{vitals} {patient_history}{family_history} {personal_history} {menstrual_history} {medical_history} {obestetric_history} {current_medicine}{disease}{complaints}{allergy}{general_examination}{clinical_examination}{investigation}{medicine_data}{advice}{suggestion} {remark} {appointment_date} {signature}  </textarea>
        </div>
     </div>
     </div>
     </div>
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>gynecology/blank_prescription_setting/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
       flash_session_msg("blank prescription setting successfully updated");    
       $('.overlay-loader').hide();    
    }
  });
});
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'gynecology/blank_prescription_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
