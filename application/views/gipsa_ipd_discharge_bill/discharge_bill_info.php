...
//print_r($form_data);die;
 ...
 ?>
  ...
  <tr>
    <th style="text-align:left;" width="8%">Group</th>
    <td style="text-align:left;" width="8%">
      <select name="group" id="group" class="w-100px" onchange="reset_value1();" required>
                      <option value="0">Select Group</option>
                    <option value="100000" >Room & Nursing Charges</option>
                    <option value="200000" >ICU Charges</option>
                    <option value="300000" >OT Charges</option>
                    <option value="400000" >Medicine and Consumables</option>
                     <option value="500000" >Professional fees</option>
                     <option value="600000" >Investigation Charges</option>
                      <option value="700000" >Ambulance Charges</option>
                      <option value="800000" >Miscellaneous Charges</option>
                        <option value="900000" >Package Charges</option>
                  </select>
                </td>
    ...
    <td style="text-align:left;" width="10%"><input type="text" id="code" name="code" class="w-100px " readonly/><input type="hidden" id="particular_id" name="grp_code" class="w-100px"/></td>
    ...
    <th style="text-align:left;padding-left:20px;" width="7%">Rate </th>
    <td style="text-align:left;" width="7%"><input type="text" id="charge"  name="rate" class="w-100px price_float"/></td>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered table-responsive medicine_stock_list" cellspacing="0" width="100%">
            ...
            if(!empty($running_bill_data['CHARGES']))
            {
             ...
             //print '<pre>'; print_r($running_bill_data['CHARGES']); exit;
            ...
            <tr>
              <td><?php echo $i; ?></td>
              <td><?php echo $running_data['group_name']; ?>
              <td><?php echo $running_data['particular'];?></td>
              <td><?php echo $running_data['particular_code']; ?>
     ...
     <div id="confirm_print_bill" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-theme"><h4>Are You Sure?</h4></div>
          <div class="modal-footer">
             <a  type="button" data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("gipsa_ipd_discharge_bill/print_discharge_bill_paidamount/".$ipd_id.'/'.$patient_id); ?>');">Print</a>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a  type="button" data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("gipsa_ipd_discharge_bill/print_discharge_bill/".$ipd_id.'/'.$patient_id); ?>');">Print</a>
...
<?php $discharge_date= strtotime($form_data['discharge_date']);?> 
<script>
...
function edit_advance_payment(ipd_id,patient_id){
    ...
    $modal.load('<?php echo base_url().'gipsa_ipd_discharge_bill/update_advance_payment/' ?>'+ipd_id+'/'+patient_id,
    {
...
function update_perticulars(edit_id,update_row)
{
  ...
  var net_price_edit= $('#net_price_edit_'+edit_id).val();
      ...
      $.ajax({
            url : "<?php echo base_url('gipsa_ipd_discharge_bill/update_discharge_date/'); ?>",
            dataType: "json",
            method: 'post',
...
$(function () {
  payment_calc_all();
    ...
    var group_name=$('#group').val();
        ...
        row = i ;
        $.ajax({
        url : "<?php echo base_url('gipsa_ipd_discharge_bill/get_particular_data/'); ?>" + request.term +'/'+group_name,
        dataType: "json",
        method: 'post',
      data: {
...
function payment_calculate(edit_id){
  ...
  function payment_new_calc_all(update_row)
  {
        ...
        $.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>gipsa_ipd_discharge_bill/payment_calc_all/", 
        dataType: "json",
  ...
  function add_perticulars(){
    ...
    var group_code=$('#group option:selected').val();
   var code= $('#code').val();
    var date= $('#date').val();
    ...
    var amount= $('#amount').val();
     ...
     $.ajax({
        url : "<?php echo base_url('gipsa_ipd_discharge_bill/add_perticulars/'); ?>"+ '<?php echo $ipd_id ?>'+'/'+'<?php echo $patient_id;?>',
        dataType: "json",
        method: 'post',
        data: {
        ...
        success: function( data ) {
          if(data==1)
          {
             window.location.href="<?php echo base_url('gipsa_ipd_discharge_bill/discharge_bill_info');?>/"+'<?php echo $ipd_id ?>'+'/'+'<?php echo $patient_id ?>'+'/'+'<?php echo $discharge_date;?>';
          }
        }
     });
...
function payment_function(value,error_field){
           $('#updated_payment_detail').html('');
                $.ajax({
                     type: "POST",
                     url: "<?php echo base_url('gipsa_ipd_discharge_bill/get_payment_mode_data')?>",
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('gipsa_ipd_discharge_bill/delete_charges/'); ?>"+id+'/'+ipd_id+'/'+patient_id, 
                 success: function(result)
                 {
                     

                    window.location.href="<?php echo base_url('gipsa_ipd_discharge_bill/discharge_bill_info/'.$ipd_id.'/'.$patient_id.'/'.$form_data['discharge_date']);?>"
                 }
              });
    });     
 }
