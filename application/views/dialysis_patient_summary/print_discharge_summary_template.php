...
$pincode = $all_detail['discharge_list'][0]->pincode;
    ...
    $dialysis_date_time='';
    if(!empty($all_detail['discharge_list'][0]->dialysis_date) && $all_detail['discharge_list'][0]->dialysis_date!='0000-00-00')
    {
           // if($date_time_status==1)
           // {
                $time = '';
                if(date('h:i:s', strtotime($all_detail['discharge_list'][0]->dialysis_time))!='12:00:00')
                {
                    $time = date('h:i A', strtotime($all_detail['discharge_list'][0]->dialysis_time));
                }
                
                $dialysis_date_time = date('d-m-Y',strtotime($all_detail['discharge_list'][0]->dialysis_date)).' '.$time;
    ...
    $template_data = str_replace("{dialysis_date}",$booking_date_time,$template_data);
