...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "ajax": {
            "url": "<?php echo base_url('ipd_patient_discharge_summary/medicine_ajax_list/'.$summary_id)?>",
            "type": "POST",
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('ipd_patient_discharge_summary/deleteall_medicine');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
  ...
  <div class="row m-b-5">
    ...
    <form>
       <?php if(in_array('757',$users_data['permission']['action'])) {
        ?>
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered ipd_patient_discharge_summary_list" cellspacing="0" width="100%">
            <thead class="bg-theme">
                ...
                <th <?php  if($m=0){ ?> class="" <?php } ?> ><?php if(!empty($med_value->setting_value)) { echo $med_value->setting_value; } else { echo $med_value->var_title; } ?></th>
                <?php 
                $m++; 
                }
                ?>  
    ...
    <div class="userlist-right">
      <div class="btns">
               ...
               <?php if(in_array('759',$users_data['permission']['action'])) {
               ?>
             <button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
              <i class="fa fa-trash"></i> Delete
               ...
               <?php } ?>
                ...
                <?php if(in_array('758',$users_data['permission']['action'])) {
               ?>
            <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("ipd_patient_discharge_summary/print_discharge_summary_medicine/".$summary_id); ?>');"> <i class="fa fa-print"></i> Print</a>
 <?php } ?>
