...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('item_manage/ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'item_manage/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_item_manage(id)
{
  ...
  var $modal = $('#load_add_item_manage_modal_popup');
  $modal.load('<?php echo base_url().'item_manage/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_item_manage_modal_popup');
  $modal.load('<?php echo base_url().'item_manage/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            ...
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('item_manage/deleteall');?>",
                      data: {row_id: allVals},
...
<!--    <div style="border: 1px solid #ccc; float: right; padding:5px; font-weight: bold;">
      ...
      <div class="fixed">
    		<div class="btns">
            <?php if(in_array('985',$users_data['permission']['action'])) {
            ?>
    			<button class="btn-update" id="modal_add">
    				<i class="fa fa-plus"></i> New
    			</button>
            <?php } ?>
            <?php if(in_array('987',$users_data['permission']['action'])) {
            ?>
            ...
            <?php if(in_array('984',$users_data['permission']['action'])) {
           ?>
            <a href="<?php echo base_url('item_manage/item_manage_list_excel'); ?>" class="btn-anchor m-b-2">
            <i class="fa fa-file-excel-o"></i> Excel
            </a>

            <a href="<?php echo base_url('item_manage/item_manage_list_csv'); ?>" class="btn-anchor m-b-2">
            <i class="fa fa-file-word-o"></i> CSV
            </a>

            <a href="<?php echo base_url('item_manage/pdf_item_manage_list'); ?>" class="btn-anchor m-b-2">
            <i class="fa fa-file-pdf-o"></i> PDF
            </a>
          
                        
             <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("item_manage/print_item_manage_list"); ?>');"> <i class="fa fa-print"></i> Print</a>
             <?php }?>
            ...
            <?php } ?>
            <?php if(in_array('988',$users_data['permission']['action'])) {
            ?>
    			<button class="btn-update" onclick="window.location.href='<?php echo base_url('item_manage/archive'); ?>'">
    				<i class="fa fa-archive"></i> Archive
    			</button>
            <?php } ?>
			
			<?php  if(in_array('985',$users_data['permission']['action'])) { ?>
                <a href="<?php echo base_url('item_manage/inventory_item_sample_excel'); ?>" class="btn-anchor m-b-2">
...
function select_item_data(item_type){
  ...
  if(item_type==2){
   ...
   if(item_type==1){
    ...
    if(item_type==4){
     $('#room_type').html('<div class="row m-b-5"><div class="col-xs-7"><input type="text" name="rack_no"  placeholder="Rack No" onkeyup="return form_submit();"/></div>');  }
  ...
  $('#overlay-loader').show();
    ...
    $.ajax({
          url: "<?php echo base_url(); ?>item_manage/advance_search/",
          type: "post",
          data: $(this).serialize(),
          success: function(result) 
          {
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('item_manage/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{
  var $modal = $('#load_add_item_manage_modal_popup');
  $modal.load('<?php echo base_url().'item_manage/view_item/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
$(document).ready(function(){
  $('#load_add_item_manage_modal_popup').on('shown.bs.modal', function(e){
    $('.inputFocus').focus();
  });
 ...
 var $modal = $('#load_item_inventory_import_modal_popup');
      ...
      //  alert();
    $modal.load('<?php echo base_url().'item_manage/inventory_item_import_excel' ?>',
    { 
    },
    function(){
    $modal.modal('show');
