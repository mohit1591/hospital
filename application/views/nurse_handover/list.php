...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('nurse_handover/ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'nurse_handover/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_religion_modal_popup');
  $modal.load('<?php echo base_url().'nurse_handover/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_religion_modal_popup');
  $modal.load('<?php echo base_url().'nurse_handover/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('nurse_handover/deleteall');?>",
                      data: {row_id: allVals},
    ...
    <form>
  	...
  	<div class="userlist-right">
       ...
       <button class="btn-update" onclick="reload_table()">
        ...
        {
        ?>
        <!-- <button class="btn-exit" onclick="window.location.href='<?php echo base_url('nurse_handover/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('nurse_handover/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
<div id="confirm_print" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <!-- <div class="modal-body"></div> -->
          <div class="modal-footer">
            <a type="button" data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("nurse_handover/print_details"); ?>');" >Print</a>
...
<?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('nurse_handover');?>'; 
  ...
  <script>
    function combinedPrint()
    {
      ...
      if (patientCodes.size === 1) { // If the Set size is 1, all patient codes are identical
        if(allVals.length > 0) {
          var url = '<?php echo base_url(); ?>' + 'nurse_handover/print_details/' + allVals.join(',');
