...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('garbage_stock_item/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            ...
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('garbage_stock_item/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right">
          ...
          <?php } ?>
          <?php if(in_array('977',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?> 
               <button class="btn-exit" onclick="window.location.href='<?php echo base_url('garbage_stock_item'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('garbage_stock_item/restore/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('garbage_stock_item/trash/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            ...
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('garbage_stock_item/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
