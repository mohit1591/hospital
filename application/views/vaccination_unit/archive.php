...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('vaccination_unit/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('vaccination_unit/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered table-responsive medicine_unit_archive" cellspacing="0" width="100%">
            <thead>
                <tr>
  	...
  	<div class="userlist-right">
        ...
        <?php
         if(in_array('1020',$users_data['permission']['action'])) {
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('vaccination_unit'); ?>'">
...
?>
<script>  
 function restore_vaccination_unit(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccination_unit/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccination_unit/trash/'); ?>"+unit_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('vaccination_unit/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
