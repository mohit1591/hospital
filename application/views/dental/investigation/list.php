...
var table;
<?php
if(in_array('1696',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dental/investigation/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$(document).ready(function(){
var $modal = $('#load_add_dental_investigation_modal_popup');
$('#modal_add').on('click', function(){
$modal.load('<?php echo base_url().'dental/investigation/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_teeth_number(id)
{
  //alert(id);
  var $modal = $('#load_add_dental_investigation_modal_popup');
  $modal.load('<?php echo base_url().'dental/investigation/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
// {
//   var $modal = $('#load_add_dental_teeth_number_modal_popup');
//   $modal.load('<?php echo base_url().'dental/investigation/view/' ?>'+id,
//   {
//     //'id1': '1',
//     //'id2': '2'
//     },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dental/investigation/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
            ...
            <ul class="prescription_menu">
              ...
              <li class="per_menu_tab <?php if($uri_url=='duration'){ ?> active <?php } ?>">
               ...
               <li class="per_menu_tab <?php if($uri_url=='medicine'){ ?> active <?php } ?>">
                ...
                <strong><label>
                    <span onClick="window.location='<?php echo base_url('dental/type');?>';"> Medicine Type </span>
                    </label>
                 </strong>
                </li>
    ...
    <form>
       <?php if(in_array('1696',$users_data['permission']['action'])) {
       ?>
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered table-responsive investigation_list" cellspacing="0" width="100%">
            <thead>
                <tr>
                    <th width="40" align="center"> <input type="checkbox" name="selectall" class="" id="selectAll" value=""> </th> 
                    <th> Investigation Category</th> 
                     <th> Investigation Subcategory</th> 
                    <th> Status </th> 
                    <th> Action </th>
                </tr>
            </thead>  
      ...
      <div class="fixed">
  		<div class="btns">
          <?php if(in_array('1697',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1699',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?>
          <?php if(in_array('1700',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('dental/investigation/archive'); ?>'">
...
{ 
 ...
 function delete_teeth_number(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dental/investigation/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 $(document).ready(function() {
   $('#load_add_dental_investigation_modal_popup').on('shown.bs.modal', function(e) {
      $('.inputFocus').focus();
   })
}); 
</script> 
...
<div id="load_add_dental_investigation_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
</div><!-- container-fluid -->
</body>
</html>