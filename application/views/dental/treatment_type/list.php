...
var table;
<?php
if(in_array('1660',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dental/treatment_type/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
{
  var $modal = $('#load_add_dental_treatment_modal_popup');
  $modal.load('<?php echo base_url().'dental/treatment_type/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dental/treatment_type/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1660',$users_data['permission']['action'])) {
       ?>
      ...
      <div class="fixed">
  		<div class="btns">
          <?php if(in_array('1661',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1663',$users_data['permission']['action'])) {
          ?>
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('dental/treatment_type/archive'); ?>'">
...
{ 
 ...
 function delete_treatment(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dental/treatment_type/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
