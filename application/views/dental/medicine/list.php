...
var table;
<?php
if(in_array('1703',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dental/medicine/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$(document).ready(function(){
var $modal = $('#load_add_medicine_modal_popup');
$('#modal_add').on('click', function(){
$modal.load('<?php echo base_url().'dental/medicine/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_medicine_modal_popup');
  $modal.load('<?php echo base_url().'dental/medicine/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_medicine_modal_popup');
  $modal.load('<?php echo base_url().'eye/medicine/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dental/medicine/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1703',$users_data['permission']['action'])) {
       ?>
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered medicine_list" cellspacing="0" width="100%">
            <thead>
                <tr>
  	...
  	<div class="userlist-right">
  		<div class="btns">
          <?php if(in_array('1704',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1706',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?>
          <?php if(in_array('1707',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('dental/medicine/archive'); ?>'">
...
<script> 
 ...
 function delete_medicine(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dental/medicine/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$(document).ready(function() {
   $('#load_add_medicine_modal_popup').on('shown.bs.modal', function(e) {
      $('.inputFocus').focus();
   })
});  
</script> 
...
<div id="load_add_medicine_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
</div><!-- container-fluid -->
</body>
</html>