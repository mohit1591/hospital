...
var table;
<?php
if(in_array('1632',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dental/chief_complaints/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
{
  var $modal = $('#load_add_dental_chief_complaints_modal_popup');
  $modal.load('<?php echo base_url().'dental/chief_complaints/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dental/chief_complaints/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1632',$users_data['permission']['action'])) {
       ?>
      ...
      <div class="fixed">
  		<div class="btns">
          <?php if(in_array('1633',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1635',$users_data['permission']['action'])) {
          ?>
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('dental/chief_complaints/archive'); ?>'">
...
{ 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dental/chief_complaints/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
