...
var table;
<?php
if(in_array('1689',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dental/teeth_number/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'dental/teeth_number/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_teeth_number(id)
{
  //alert(id);
  var $modal = $('#load_add_dental_teeth_number_modal_popup');
  $modal.load('<?php echo base_url().'dental/teeth_number/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
// {
//   var $modal = $('#load_add_dental_teeth_number_modal_popup');
//   $modal.load('<?php echo base_url().'dental/teeth_number/view/' ?>'+id,
//   {
//     //'id1': '1',
//     //'id2': '2'
//     },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dental/teeth_number/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1689',$users_data['permission']['action'])) {
       ?>
      ...
      <div class="fixed">
  		<div class="btns">
          <?php if(in_array('1690',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="modal_add">
  				<i class="fa fa-plus"></i> New
  			</button>
          <?php } ?>
          <?php if(in_array('1692',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?>
          <?php if(in_array('1693',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('dental/teeth_number/archive'); ?>'">
...
{ 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dental/teeth_number/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 $(document).ready(function() {
   $('#load_add_dental_teeth_number_modal_popup').on('shown.bs.modal', function(e) {
      $('.inputFocus').focus();
   })
}); 
</script> 
...
<div id="load_add_dental_teeth_number_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
</div><!-- container-fluid -->
</body>
</html>