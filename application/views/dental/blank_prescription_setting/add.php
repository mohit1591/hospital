...
<form id="prescription_setting_form">
  ...
  <div class="row">
    ...
    <?php
      if(in_array('1741',$users_data['permission']['action']))
      {
    ...
    ?>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-3" style="padding-left:7px;">
             <textarea cols="45" readonly="">{patient_reg_no},{patient_name},{patient_age},{patient_address},{booking_code},{mobile_no},{booking_date},{doctor_name},{chief_complaints}
{prv_history} {allergy} {oral_habbit}{investigation}{diagnosis}{medicine}{treatement} {advice}  {appointment_date} {signature} </textarea>
        </div>
     </div>
     </div>
     </div>
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>dental/blank_prescription_setting/",
    type: "post",
    data: $(this).serialize(),
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'dental/blank_prescription_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
