...
{           
  ...
  function remove_particulars_vals(allVals)
  {
   ...
   if(allVals!="")
   {
      ...
      $.ajax({
              type: "POST",
              url: "<?php echo base_url('opd_charge_entry/remove_opd_particular');?>",
...
<input type="hidden" name="pannel_type" value="<?php echo $patient_details['pannel_type']; ?>" />
<input type="hidden" name="opd_type" value="<?php echo $patient_details['opd_type']; ?>" />
<input type="hidden" name="opd_id" value="<?php echo $patient_details['id']; ?>">
...
<div class="row">
  ...
  <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <select name="particulars" id="particular_id" class="" onchange="return get_particulars_data(this.value);">
            ...
            ?> 
             ...
             <a class="btn-new" id="patient_particular_add_modal" title="Add New Particular"> New </a>   
    ...
    <!--- Group 27-08-2019 ---->
      ...
      <div class="col-xs-8">
        <input type="text"  name="group_name"  id="group_name" readonly>
      </div>
    </div> 
      <div class="row m-b-5">
      <div class="col-xs-4">
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text" readonly name="charges" class="price_float" id="charges" value="">
      </div>
    </div> <!-- row -->
    <div class="row m-b-5">
      <div class="col-xs-4">
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text" readonly class="price_float" name="amount" id="amount" value="">
        <a class="btn-new" onclick="particular_payment_calculation();" title="Add To Charge"> Add </a>
      </div>
    </div> <!-- row -->
...
<?php $opd_particular_payment =  $this->session->userdata('opd_particular_payment'); ?>
  ...
  <div class="col-xs-4 media_100">
    ...
    <table class="table table-bordered table-striped opd_billing_table" id="particular_list">
      <thead class="bg-theme">
          ...
          $perticuller_list = $this->session->userdata('opd_particular_charge_billing'); 
             ...
             foreach($perticuller_list as $perticuller)
             {
              ...
              ?>
                <tr>
                  ...
                  <td><?php echo number_format($perticuller['quantity'],0); ?></td>
 ...
 <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text" readonly=""  class="price_float" name="particulars_charges" id="particulars_charges" value="<?php if(!empty($opd_particular_payment['particulars_charges'])) { echo number_format($opd_particular_payment['particulars_charges'],2,'.',''); } else{ echo $particulars_charges; } ?>">
      </div>
    </div> <!-- row -->
 ...
 <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        ...
        <button class="btn-update" type="button" onclick="window.location.href='<?php echo base_url('day_care'); ?>'">
...
$('#patient_particular_add_modal').on('click', function(){
...
function get_particulars_data(particulars_id)
{
    ...
    $.ajax({url: "<?php echo base_url(); ?>general/particulars_list/"+particulars_id, 
      success: function(result)
      {
        ...
        $('#code').val(result.particulars_code);
      } 
    });
  ...
  function get_particular_amount()
  {
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>opd_charge_entry/opd_particular_calculation/", 
            dataType: "json",
  ...
  if(particular !=''){
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>opd_charge_entry/particular_payment_calculation/", 
            dataType: "json",
            data: 'amount='+amount+'&quantity='+quantity+'&particular='+particular+'&particulars='+particulars+'&particulars_charges='+particulars_charges+'&charges='+charges+'&s_date='+s_date,
            success: function(result)
            {
              $('#particular_list').html(result.html_data);
              $('#total_amount').val(result.total_amount);
...
$("button[data-number=4]").click(function(){
    $('#daycare_charge_count').modal('hide');
   /* $(this).hide();*/
});

$("button[data-number=5]").click(function(){
    $('#remove_particulars').modal('hide');
});
...
<div id="load_add_particular_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
 ...
 <div id="daycare_charge_count" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content bg-r-border">
          ...
          <p><i class="fa fa-star" aria-hidden="true"></i><span class="text1">Please Select Particular.</span></p>
 ...
 <div id="remove_particulars" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content bg-r-border">
            <div class="modal-header bg-theme bg-red"><b><span>Notice</span></b><button type="button" class="close close1" data-dismiss="modal" data-number="5" aria-label="Close"><span aria-hidden="true">×</span></button></div>
          <div class="modal-footer  text-l">
          <p><i class="fa fa-star" aria-hidden="true"></i><span class="text1">Please Check Atleast One Particular.</span></p>
