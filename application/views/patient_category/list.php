...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('patient_category/ajax_list')?>",
            "type": "POST",
...
function edit_patient_category(id)
{
  var $modal = $('#load_add_patient_category_modal_popup');
  $modal.load('<?php echo base_url().'patient_category/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_patient_category_modal_popup');
  $modal.load('<?php echo base_url().'patient_category/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('patient_category/deleteall');?>",
                      data: {row_id: allVals},
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('patient_category/archive'); ?>'">
...
<script>  
 ...
 function delete_patient_category(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('patient_category/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
