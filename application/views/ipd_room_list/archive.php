...
var table;
<?php
if(in_array('674',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('ipd_room_list/archive_ajax_list')?>",
            "type": "POST",
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('ipd_room_list/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered ipd_room_list_archive" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
  		<div class="btns"> 
          <?php if(in_array('676',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
  				<i class="fa fa-window-restore" aria-hidden="true"></i> Restore
  			</button>
          <?php } ?>
          <?php if(in_array('675',$users_data['permission']['action'])) {
          ?>
...
<script>  
 ...
 function restore_ipd_room_list(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('ipd_room_list/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
  ...
  function trash(ipd_room_list_id)
 {     
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false, 
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('ipd_room_list/trash/'); ?>"+ipd_room_list_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('ipd_room_list/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
