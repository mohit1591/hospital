...
<div class="overlay-loader" id="overlay-loader">
  ...
  <form  id="ipd_room_list" class="form-inline">
...
$("#ipd_room_list").on("submit", function(event) { 
  ...
  { 
    var path = 'edit/'+ids;
    var msg = 'IPD Room List successfully updated.';
  }
  else
  {
    var path = 'add/';
    var msg = 'IPD Room List successfully created.';
  } 
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      {
        $("#load_add_ipd_room_list_modal_popup").html(result);
      }       
      $('#overlay-loader').hide();
    }
  });
...
function get_charges(room_category_id)
{
     if(room_category_id!='')
     {
          $.post('<?php echo base_url().'ipd_room_type/get_room_type_charges/' ?>',{'room_category_id':room_category_id},function(result){
               if(result!='')
               {
...
{
 $.ajax({url: "<?php echo base_url(); ?>ipd_room_list/get_room_type_list/", 
  success: function(result)
  {
...
<div id="load_add_ipd_room_type_modal_popup" class="modal fade modal-top45" role="dialog" data-backdrop="static" data-keyboard="false"></div>