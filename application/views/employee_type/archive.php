...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('employee_type/archive_ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'employee_type/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('employee_type/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right">
        ...
        ?>
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('employee_type'); ?>'">
...
<script>  
 ...
 function restore_employee_type(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('employee_type/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
  ...
  function trash(employee_type_id)
 {     
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false, 
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('employee_type/trash/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('employee_type/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
