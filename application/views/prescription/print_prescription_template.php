...
if($all_detail['prescription_list'][0]->prescription_header=='1')
{
    if($all_detail['prescription_list'][0]->seprate_header=='1')
    {
        //docotor headercontent
        $doctor_header = $all_detail['prescription_list'][0]->header_content;
...
$template_data = str_replace("{token_no}",$all_detail['prescription_list'][0]->token_no,$template_data);

$template_data = str_replace("{net_amount}",'Rs. '.$all_detail['prescription_list'][0]->net_amount,$template_data);

$booking_time1 = date('h:i A', strtotime($all_detail['prescription_list'][0]->booking_time)); 
$template_data = str_replace("{date_time}",date('d-m-Y',strtotime($all_detail['prescription_list'][0]->booking_date)).' '.$booking_time1,$template_data);
    ...
    if(!empty($all_detail['prescription_list'][0]->booking_date) && $all_detail['prescription_list'][0]->booking_date!='0000-00-00')
{
    ...
    $template_data = str_replace("{prescription_time}",$presc_time,$template_data);
     ...
     $validity_date='';
    if(!empty($all_detail['prescription_list'][0]->validity_date) && $all_detail['prescription_list'][0]->validity_date!='0000-00-00')
{
    $validity_date = date('d-m-Y',strtotime($all_detail['prescription_list'][0]->validity_date)); 
}
    ...
    if(!empty($all_detail['prescription_list'][0]->attended_doctor)) {
        $template_data = str_replace("{doctor_name}",'Dr. '.get_doctor_name($all_detail['prescription_list'][0]->attended_doctor),$template_data);    
    } else {
        $template_data = str_replace("{doctor_name}",'',$template_data);
    }
    $template_data = str_replace("{doctor_qualifications}",get_doctor_qualifications($all_detail['prescription_list'][0]->attended_doctor),$template_data);

if(!empty($all_detail['prescription_list'][0]->referral_doctor))
{
   $doc_name = get_doctor_name($all_detail['prescription_list'][0]->referral_doctor); 
}
else
{
  $doc_name =  $all_detail['prescription_list'][0]->ref_by_other ;
}
    $template_data = str_replace("{ref_doctor_name}",$doc_name,$template_data);
 ...
 $template_data = str_replace("{vitals}",$patient_vital,$template_data);
    ...
    $template_data = str_replace("{age}",$patient_age,$template_data);
...
//if(!empty($form_data['next_appointment_date'])){  ?>
  <?php if(!empty($form_data['next_appointment_date']) && $form_data['next_appointment_date']!='0000-00-00 00:00:00' && $form_data['next_appointment_date']!='1970-01-01' && date('d-m-Y',strtotime($form_data['next_appointment_date']))!='01-01-1970'){ 
       ...
       { 
          ...
          <div style="font-size:small;line-height:18px;">
            <b>'.$Appointment_date.' :</b>
            '.date('d-m-Y',strtotime($form_data['next_appointment_date'])).'
          </div>
          <div><br><b>Reason:</b><br>'.$form_data['next_reason'].'</div>
        </div>';
  ...
  ?>
      ...
      <div style="font-size:small;line-height:18px;">
        ...
        if(!empty($appointment_date->setting_value)) 
       { 
            $Appointment_date =  $value->setting_value; } else { $appointment =  $value->var_title;
        }
        
        
        if(!empty($appointment_date->setting_value)) { echo $appointment_date =  $value->setting_value; } else { echo  $appointment =  $value->var_title; } ?> :</b>
       < ?php   date('d-m-Y',strtotime($form_data['next_appointment_date'])); ?>
      </div>
    </div>-->
<?php }   }
