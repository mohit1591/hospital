...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('diagnose/ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'diagnose/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_simulation_modal_popup');
  $modal.load('<?php echo base_url().'diagnose/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function view_diagnose(id)
{
  var $modal = $('#load_add_simulation_modal_popup');
  $modal.load('<?php echo base_url().'diagnose/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('diagnose/deleteall');?>",
                      data: {row_id: allVals},
...
<script>  
 ...
 function delete_diagnose(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('diagnose/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
