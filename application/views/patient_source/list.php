...
var table;
<?php
if(in_array('916',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('patient_source/ajax_list')?>",
            "type": "POST",
...
function edit_patient_source(id)
{
  var $modal = $('#load_add_emp_type_modal_popup');
  $modal.load('<?php echo base_url().'patient_source/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('patient_source/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right">
  		<div class="btns">
      <?php
      if(in_array('922',$users_data['permission']['action'])) 
      {
      ?>
      ...
      {
      ?>
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('patient_source/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('patient_source/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
