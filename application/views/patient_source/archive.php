...
var table;
<?php
if(in_array('919',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('patient_source/archive_ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'patient_source/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('patient_source/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right">
        ...
        ?>
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('patient_source'); ?>'">
...
<script>  
 ...
 function restore_patient_source(patient_source_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('patient_source/restore/'); ?>"+patient_source_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
  ...
  function trash(patient_source_id)
 {     
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false, 
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('patient_source/trash/'); ?>"+patient_source_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('patient_source/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
