      ...
      <div class="modal-body">   
          <div class="row">
            <div class="col-md-12 m-b1">
              <div class="row">
                <div class="col-md-5">
                    <label>Patient source<span class="star">*</span></label>
                </div>
                <div class="col-md-7">
                    <input type="text" name="patient_source"  value="<?php echo $form_data['patient_source']; ?>" class="inputFocus source_firstCap">
                    
                    <?php if(!empty($form_error)){ echo form_error('patient_source'); } ?>
                </div>
              </div> <!-- innerrow -->
            </div> <!-- 12 -->
          </div> <!-- row -->  
...
$("#patient_source").on("submit", function(event) { 
  ...
  { 
    var path = 'edit/'+ids;
    var msg = 'Patient Source successfully updated.';
  }
  else
  {
    var path = 'add/';
    var msg = 'Patient Source successfully created.';
  } 
  //alert('ddd');return false;
  $.ajax({
    url: "<?php echo base_url('patient_source/'); ?>"+path,
    type: "post",
    data: $(this).serialize(),
...
{
   $.ajax({url: "<?php echo base_url(); ?>patient_source/patient_source_dropdown/", 
    success: function(result)
    {
