<html><head>
<title>Vendor Item Purchase Report</title>
<?php
if($print_status==1)
{
?>
<script type="text/javascript">
window.print();
window.onfocus=function(){ window.close();}
</script>
<?php 
}
?>
<style>
body
{
  font-size: 8px;
} 
td
{
  padding-left:5px !important;;
  font:14px Arial;
}
</style>
</head><body>
<table width="100%" cellpadding="0" cellspacing="0" border="1px" style="font:14px Arial !important;">
     <tr><td colspan="11" align="center"><h3>Vendor Item Purchase Report </h3></td></tr>
         <tr>
                    <th width="40" align="center">S.No.</th> 
                    <th>Item Code</th>
                    <th>Item Name</th>
                    <th>Serial Number</th>
                    <th>Vendor Name </th> 
                    <th>Quantity</th>
                    <th>MRP</th>
                    <th>Purchase Rate</th>
                    <th>Discount</th>
                    <th>Net Amount</th>
                    <th>Date</th>
        </tr>
 <?php
   if(!empty($report_list))
   {
       $grand_total_mrp =0;
	    $grand_total_purchase=0;
	    $grand_paid_discount=0;
	    $grand_total_amount=0;
	    $total_qty=0;
     //echo "<pre>";print_r($report_list); die;
     $i=1;
     $tot_data=count($report_list);
     foreach($report_list as $reports)
     {
         $grand_total_amount = $grand_total_amount + $reports->total_amount;
		 $total_qty = $total_qty+$reports->qty;
		 //$discounts = ($reports->discount/100)*($reports->per_pic_price*$reports->qty);
		 
		    if($reports->purchase_amount!='0.00')
            {
               $purchase_amount = $reports->purchase_amount;
            }
            else
            {
              $purchase_amount = $reports->per_pic_price;
            }
            
       ?>
      <tr>
        <td align="center"><?php echo $i; ?></td>
        <td><?php echo $reports->item_code; ?></td>
        <td><?php echo $reports->item_name; ?></td>
        <td><?php echo $reports->serial_numbers; ?></td>
        <td><?php echo $reports->vendor_name; ?></td>  
        <td><?php echo $reports->qty; ?></td>  
        <td><?php echo $reports->mrp; ?></td>  
        <td><?php echo $purchase_amount; ?></td>  
        <td><?php echo $reports->discount; ?></td>  
        <td><?php echo $reports->total_amount; ?></td>  
        <td><?php echo date('d-m-Y',strtotime($reports->purchase_date)); ?></td>  
     </tr>
       <?php
       $i++;  
     }
     ?>
     
     <tr>
        <td colspan="5">&nbsp;</td>
        
        <td><?php echo $total_qty; ?></td>  
        <td colspan="3">&nbsp;</td> 
        <td><?php echo $grand_total_amount; ?></td>  
        <td>&nbsp;</td>  
     </tr>

   <?php       
   }
 ?> 
</table></body></html>