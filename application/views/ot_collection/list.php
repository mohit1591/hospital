...
function reset_date_search()
  {
      ...
      $('#end_date').val('');
      $.ajax({
         url: "<?php echo base_url('ot_collection_report/reset_date_search/'); ?>",  
...
$modal.load('<?php echo base_url().'ot_collection_report/advance_search/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
if(in_array('1',$permission_section)): ?> 
    ...
    <?php }?>
      ...
      <div class="grp">
       ...
       <?php if(in_array('1452',$users_data['permission']['action'])): ?>
      ...
      <div class="fixed">
          ...
          <?php if(in_array('1456',$users_data['permission']['action'])): ?>
               <a href="<?php echo base_url('ot_collection_report/ot_report_excel');?>" class="btn-anchor m-b-2">
               <i class="fa fa-file-excel-o"></i> Excel
               </a>
          <?php endif;?>
          <?php if(in_array('1455',$users_data['permission']['action'])): ?>
               <a href="<?php echo base_url('ot_collection_report/ot_report_csv');?>" class="btn-anchor m-b-2">
               <i class="fa fa-file-word-o"></i> CSV
               </a>
          <?php endif;?>
         <?php if(in_array('1454',$users_data['permission']['action'])): ?>
               <a href="<?php echo base_url('ot_collection_report/pdf_ot_report'); ?>" class="btn-anchor m-b-2">
                    <i class="fa fa-file-pdf-o"></i> PDF
               </a>
          <?php endif;?>
          <?php if(in_array('1453',$users_data['permission']['action'])): ?>
              <!--  <a href="javascript:void(0)" class="btn-anchor m-b-2" id="deleteAll" onClick="return openPrintWindow('< ?php echo base_url("medicine_collection_report/print_medicine_report"); ?>', 'windowTitle', 'width=820,height=600');">
               <i class="fa fa-print"></i> Print
               </a> -->

              <a href="javascript:void(0)" class="btn-anchor m-b-2" onClick="return print_window_page('<?php echo base_url("ot_collection_report/print_ot_report"); ?>');">
              <i class="fa fa-print"></i> Print
              </a> 
...
/*$('.datepicker').datepicker({
  ...
  }).on("change", function(selectedDate) 
  {
    ...
    $.ajax({
           url: "<?php //echo base_url('medicine_collection_report/search_data/'); ?>", 
           type: 'POST',
...
function form_submit(vals){
    ...
    $.ajax({ 
           url: "<?php echo base_url('ot_collection_report/advance_search/'); ?>", 
           type: 'POST',
           data: { from_date: start_date, end_date : end_date,section_id:section_id,branch_id:branch_id,employee:employee} ,
           success: function(result)
           { 
                if(vals!="1")
                {
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "aaSorting": [],
        "ajax": {
            "url": "<?php echo base_url('ot_collection_report/ajax_list')?>",
            "type": "POST"
        }, 
        "columnDefs": [
        { 
