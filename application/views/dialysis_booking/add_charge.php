...
{           
  ...
  function remove_particulars_vals(allVals)
  {
   ...
   if(allVals!="")
   {
      ...
      $.ajax({
              type: "POST",
              url: "<?php echo base_url('dialysis_booking/remove_dialysis_particular');?>",
...
<input type="hidden" name="dialysis_id" value="<?php echo $dialysis_id; ?>" />
...
<div class="row">
  <div class="col-xs-4 media_50">
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text" readonly="" name="booking_code" value="<?php echo $patient_details['booking_code']; ?>" /> 
      </div>
    </div> <!-- row -->
  ...
  <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <select name="particulars" id="particular_id" class="" onchange="return get_particulars_data(this.value);">
            ...
            ?> 
             ...
             <a  class="btn-new" id="patient_dialysis_particular_add_modal"> New</a>   
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text"  name="s_date" id="s_date" class="datepicker" value="<?php echo date('d-m-Y'); ?>">
        <a href="javascript:void(0);" onclick="more_info()">More Info</a>
      </div>
    </div> <!-- row -->
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text"  name="start_date" id="start_date"  onchange="calculate_hrs();" class="w-130px datepicker" value="<?php echo date('d-m-Y'); ?>">
        <input type="text" name="start_time" id="start_time" onchange="calculate_hrs();" class="w-65px datepicker3 m_input_default" placeholder="Time" value="<?php echo date('H:i:s'); ?>">
      </div>
    </div> <!-- row -->
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text"  name="end_date" id="end_date" class="w-130px datepicker" onchange="calculate_hrs();" value="<?php echo date('d-m-Y'); ?>">
        <input type="text" name="end_time" id="end_time" class="w-65px datepicker3 m_input_default" onchange="calculate_hrs();" placeholder="Time" value="<?php echo date('H:i:s'); ?>">
      </div>
    </div> <!-- row -->
</div>
    <div class="row m-b-5">
...
<?php $dialysis_particular_payment =  $this->session->userdata('dialysis_particular_payment'); ?>
  ...
  <div class="col-xs-4 media_100">
    ...
    <table class="table table-bordered table-striped opd_billing_table" id="particular_list">
      <thead class="bg-theme">
          ...
          $perticuller_list = $this->session->userdata('dialysis_particular_charge_billing'); 
          ...
          if(!empty($perticuller_list))
          {
             ...
             foreach($perticuller_list as $perticuller)
             {
              ...
              ?>
                <tr>
                  <td>
                    <input type="checkbox" class="part_checkbox booked_checkbox" name="particular_id[]" value="<?php echo $i; //echo $perticuller['charge_id']; ?>" >
                  </td>
                  <td><?php echo $i; ?></td>
                  <td><?php echo $perticuller['particulars']; ?></td>
                  <td><?php echo $perticuller['doctor']; ?></td>
                  <td><?php echo $perticuller['s_date']; ?></td>
 ...
 <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        <input type="text" readonly=""  class="price_float" name="particulars_charges" id="particulars_charges" value="<?php if(!empty($dialysis_particular_payment['particulars_charges'])) { echo number_format($dialysis_particular_payment['particulars_charges'],2,'.',''); } else{ echo $particulars_charges; } ?>">
      </div>
    </div> <!-- row -->
...
<input type="hidden" name="data_id" value="<?php echo $data_id; ?>">
 <div class="row m-b-5">
      ...
      <div class="col-xs-8">
        ...
        <button class="btn-update" type="button" onclick="window.location.href='<?php echo base_url('dialysis_booking'); ?>'">
...
var $modal = $('#load_add_dialysis_perticular_modal_popup');

$('#patient_dialysis_particular_add_modal').on('click', function(){
$modal.load('<?php echo base_url().'dialysis_perticular/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function get_particulars_data(particulars_id)
{
    ...
    $.ajax({url: "<?php echo base_url(); ?>general/dialysis_particulars_list/"+particulars_id, 
      success: function(result)
      {
  ...
  function get_particular_amount()
  {
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>dialysis_booking/dialysis_particular_calculation/", 
            dataType: "json",
  ...
  function calculate_hrs()
  {
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>dialysis_booking/calculate_hrs/", 
            dataType: "json",
            data: 'start_date='+start_date+'&start_time='+start_time+'&end_time='+end_time+'&end_date='+end_date,
            success: function(result)
            { 
               $('#quantity').val(result.quantity); 
               $('.overlay-loader').css('display','none');
  ...
  function particular_payment_calculation()
  {
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>dialysis_booking/particular_payment_calculation/", 
            dataType: "json",
            data: 'amount='+amount+'&quantity='+quantity+'&particular='+particular+'&particulars='+particulars+'&particulars_charges='+particulars_charges+'&charges='+charges+'&s_date='+s_date+'&doctor='+doctor+'&doctor_id='+doctor_id,
            success: function(result)
            {
              $('#particular_list').html(result.html_data);
              $('#total_amount').val(result.total_amount);
...
$('#start_time').datetimepicker().on('dp.change', function (event) {
                calculate_hrs();
            });

$('#end_time').datetimepicker().on('dp.change', function (event) {
                calculate_hrs();
            });
...
<div id="load_add_dialysis_perticular_modal_popup" class="modal fade" role="dialog" data-backdrop="static" data-keyboard="false"></div>
