...
//echo "<pre>"; print_r($all_detail); exit;
    ...
    if(empty($address_setting_list))
    {
        ...
        $country = $all_detail['dialysis_list'][0]->country_name;    
        $state = $all_detail['dialysis_list'][0]->state_name;    
        $city = $all_detail['dialysis_list'][0]->city_name;    
    ...
    else
    {
        ...
        {
           $address .= $all_detail['dialysis_list'][0]->address1.' '; 
        }
        if($address_setting_list[0]->address2)
        {
           $address .= $all_detail['dialysis_list'][0]->address2.' '; 
        }
       
        if($address_setting_list[0]->address3)
        {
           $address .=  $all_detail['dialysis_list'][0]->address3.' '; 
        }
      
        if($address_setting_list[0]->city)
        {
           $address .=  $all_detail['dialysis_list'][0]->city_name.' '; 
        }
       
        if($address_setting_list[0]->state)
        {
           $address .= $all_detail['dialysis_list'][0]->state_name.' '; 
        }
        if($address_setting_list[0]->country)
        {
           $address .=  $all_detail['dialysis_list'][0]->country_name.' '; 
        }
        if($address_setting_list[0]->pincode)
        {
           $address .= $all_detail['dialysis_list'][0]->pincode; 
        }
        $template_data = str_replace("{patient_address}",$address,$template_data);
    }
    
   $template_data = str_replace("{dialysis_name}",$all_detail['dialysis_list'][0]->dialysiss_name,$template_data);
    
    
    $dialysis_date=date('d-m-Y' , strtotime($all_detail['dialysis_list'][0]->dialysis_date));
    $admission_time=date('H:i A' , strtotime($all_detail['dialysis_list'][0]->admission_time));

    if(!empty($time_setting[0]->ipd) && !empty($time_setting[0]->ipd))
    {
        // admission date time
        $template_data = str_replace("{dialysis_date_time}", $dialysis_date." ".$admission_time, $template_data);  
        // admission date time
    }
    else
    {
        // admission date time
        $template_data = str_replace("{dialysis_date_time}", $dialysis_date, $template_data);  
       // admission date time
    }
    
    
    $simulation = get_simulation_name($all_detail['dialysis_list'][0]->simulation_id);
    $template_data = str_replace("{patient_name}",$simulation.''.$all_detail['dialysis_list'][0]->patient_name,$template_data);
        ...
        $template_data = str_replace("{parent_relation_type}",$all_detail['dialysis_list'][0]->relation,$template_data);
        }
        else
        {
    ...
    if(!empty($all_detail['dialysis_list'][0]->relation_name))
        {
        $rel_simulation = get_simulation_name($all_detail['dialysis_list'][0]->relation_simulation_id);
        $template_data = str_replace("{parent_relation_name}",$rel_simulation.' '.$all_detail['dialysis_list'][0]->relation_name,$template_data);
        }
        else
        {
        ...
        // adhar no 
        $adhar_no=$all_detail['dialysis_list'][0]->adhar_no;
        $template_data = str_replace("{adhar_no}",$adhar_no,$template_data);  
        // adhar no

        // marital status
        $marital_status=$all_detail['dialysis_list'][0]->marital_status;
        $template_data = str_replace("{marital_status}",$marital_status,$template_data);
        // marital status

        // Anniversary
        $anniversary=date('Y-m-d', strtotime($all_detail['dialysis_list'][0]->anniversary));
        $template_data = str_replace("{anniversary}",$anniversary,$template_data);
        // Anniversary

        // Religion Name
        $religion_name= ucwords($all_detail['dialysis_list'][0]->religion_name);
        $template_data = str_replace("{religion}", $religion_name, $template_data);    
        // Religion Name

        // DOB starts here
        if($all_detail['dialysis_list'][0]->dob!='0000-00-00')
        {
            $dob=date('d-m-Y' ,strtotime($all_detail['dialysis_list'][0]->dob));
        ...
        // DOB Ends Here
         ...
         // Relation Name
            if($all_detail['dialysis_list'][0]->relation_type==1)
                $type="father/o";
            else if($all_detail['dialysis_list'][0]->relation_type==2)
                $type="husband/o";
            else if($all_detail['dialysis_list'][0]->relation_type==3)
                $type="baby/o";
            else if($all_detail['dialysis_list'][0]->relation_type==4)
                $type="son/o";
            else if($all_detail['dialysis_list'][0]->relation_type==5)
                $type="daughter/o";
            else
                $type="";
        $relation_name= ucwords($all_detail['dialysis_list'][0]->relation_name);
        $template_data = str_replace("{relation_name}", $type." ".$relation_name, $template_data);    
        // Relation Name

        // Mother Name
        $mother= ucwords($all_detail['dialysis_list'][0]->mother);
        $template_data = str_replace("{mother}", $mother, $template_data);     
        // Mother Name

        // Guardian Name
        $guardian_name= ucwords($all_detail['dialysis_list'][0]->guardian_name);
        $template_data = str_replace("{guardian_name}", $guardian_name, $template_data);     
        // Guardian Name

       // Guardian Email
        $guardian_email= ucwords($all_detail['dialysis_list'][0]->guardian_email);
        $template_data = str_replace("{guardian_email}", $guardian_email, $template_data);  
      // guardian Email 

        // Guardian Phone
        $guardian_phone= ucwords($all_detail['dialysis_list'][0]->guardian_phone);
        $template_data = str_replace("{guardian_phone}",$guardian_phone, $template_data);  
        // guardian Phone 


        // Guardian Relation
        $guardian_relation= ucwords($all_detail['dialysis_list'][0]->relation);
        $template_data = str_replace("{guardian_relation}",$guardian_relation, $template_data);  
        // guardian Relation 


        // Patient email
        $patient_email= ucwords($all_detail['dialysis_list'][0]->patient_email);
        $template_data = str_replace("{patient_email}", $patient_email, $template_data);  
        // patient Email 

        // Monthly Income
        $monthly_income= number_format(ucwords($all_detail['dialysis_list'][0]->monthly_income,2));
        $template_data = str_replace("{monthly_income}", $monthly_income, $template_data);  
        // Monthly Income

        // occupation
        $occupation= ucwords($all_detail['dialysis_list'][0]->occupation);
        $template_data = str_replace("{occupation}", $occupation, $template_data);      
        // occupation

        //insurance_type
        $insurance_type= ucwords($all_detail['dialysis_list'][0]->insurance_type);
        $template_data = str_replace("{insurance_type}", $insurance_type, $template_data);  
        //insurance_type

        //insurance_type_name
        $insurance_type_name= ucwords($all_detail['dialysis_list'][0]->insurance_type_name);
        ...
        //insurance_policy_no
        $insurance_policy_no= ucwords($all_detail['dialysis_list'][0]->insurance_policy_no);
        $template_data = str_replace("{insurance_policy_no}", $insurance_policy_no, $template_data);  
        //insurance_policy_no

          //insurance_tpa_id
        $insurance_tpa_id= ucwords($all_detail['dialysis_list'][0]->tpa_id);
        $template_data = str_replace("{insurance_tpa_id}", $insurance_tpa_id, $template_data);  
        //insurance_tpa_id


        //insurance_amount
        $insurance_amount= ucwords($all_detail['dialysis_list'][0]->insurance_amount);
        $template_data = str_replace("{insurance_amount}", $insurance_amount, $template_data);  
        //insurance_amount

        //auth_no
        $auth_no= ucwords($all_detail['dialysis_list'][0]->auth_no);
        ...
        // Pacakage Name
        $dialysis_package=get_dialysis_package_name($all_detail['dialysis_list'][0]->id);
        $dialysis_package_name=$dialysis_package[0]->assigned_particular;
        $dialysis_package_name=explode(',', $dialysis_package_name);
        //$dialysis_package_name=implode(', ', $dialysis_package_name);
        $template_data = str_replace("{package_name}",$dialysis_package_name , $template_data); 

        /*$package_name= ucwords($all_detail['dialysis_list'][0]->package_name);
        $template_data = str_replace("{package_name}", $package_name, $template_data);  */

        // Referred By
        $referrer= ucwords($all_detail['dialysis_list'][0]->referrered_by);
        $template_data = str_replace("{referred_by}", $referrer, $template_data);  
        // Referred By

        
        // Referrer Name
        $referrer_name= ucwords($all_detail['dialysis_list'][0]->referrer_name);
        $template_data = str_replace("{referrer_name}", $referrer_name, $template_data);  
        // Referrer Name
    
        // Attented Doctors
        $attented_doctor= ucwords($all_detail['dialysis_list'][0]->attented_doctor);
        ...
        $dialysis_assigneddoctors=get_dialysis_assigned_doctors_name($all_detail['dialysis_list'][0]->id);
        $dialysisassigned_doctors=$dialysis_assigneddoctors[0]->assigned_doctor;
        $dialysisassigned_doctors=explode(',', $dialysisassigned_doctors);
        $dialysisassigned_doctors=implode(', Dr. ', $dialysisassigned_doctors);
        $template_data = str_replace("{assigned_doctor}", "Dr. ".$dialysisassigned_doctors , $template_data);  
        // assigned doctors name

        // Advance Payment
        $advance_payment= number_format($all_detail['dialysis_list'][0]->advance_payment,0);
        $template_data = str_replace("{attented_doctors}", $advance_payment, $template_data);  
        // Advance Payment

        // Registration Charge
        $reg_charge= number_format($all_detail['dialysis_list'][0]->reg_charge,2);
  ...
  $template_data= str_replace("{mobile_no}",$all_detail['dialysis_list'][0]->mobile_no,$template_data);
    $template_data = str_replace("{patient_reg_no}",$all_detail['dialysis_list'][0]->patient_code,$template_data);
    if(!empty($all_detail['dialysis_list'][0]->booking_code))
    {
        $template_data = str_replace("{booking_code}",$all_detail['dialysis_list'][0]->booking_code,$template_data);
    }
    else
    {
         $template_data = str_replace("{booking_code}",'',$template_data);
    }

    if(!empty($all_detail['dialysis_list'][0]->dialysis_date))
    {
        
        $template_data = str_replace("{dialysis_date}",date('d-m-Y',strtotime($all_detail['dialysis_list'][0]->dialysis_date)),$template_data);
    }
    else
    {
    ...
    if(!empty($all_detail['dialysis_list'][0]->dialysis_booking_date))
    {
        ...
        $template_data = str_replace("{dialysis_booking_date}",date('d-m-Y',strtotime($all_detail['dialysis_list'][0]->dialysis_booking_date)),$template_data);
    }
    else
    {
         $template_data = str_replace("{dialysis_booking_date}",'',$template_data);
    }

    if(!empty($all_detail['dialysis_list'][0]->doctor_name))
    {
        $template_data = str_replace("{doctor_name}",'Dr.'.$all_detail['dialysis_list'][0]->doctor_name,$template_data);
    }
    else
    {
     ...
     if(!empty($all_detail['dialysis_list'][0]->doctor_spec))
    {
        $spec_name='';
        $specialization = get_specilization_name($all_detail['dialysis_list'][0]->doctor_spec);
    ...
    if(!empty($all_detail['dialysis_list'][0]->mlc))
    {
        
        $template_data = str_replace("{mlc_level}",$all_detail['dialysis_list'][0]->mlc,$template_data);
        $template_data = str_replace("{mlc}",$all_detail['dialysis_list'][0]->mlc,$template_data);
    }
    else
    {
    ...
    {
        $template_data = str_replace("{room_no}",$all_detail['dialysis_list'][0]->room_no,$template_data);
    ...
    {
        $template_data = str_replace("{bed_no}",$all_detail['dialysis_list'][0]->bad_name,$template_data);
    }
    else if(!empty($all_detail['dialysis_list'][0]->bad_no))
    {
        $template_data = str_replace("{bed_no}",$all_detail['dialysis_list'][0]->bad_no,$template_data);
    }
    else
    {
    ...
    {
        $template_data = str_replace("{room_type}",$all_detail['dialysis_list'][0]->room_category,$template_data);
    }
    else
    {
    ...
    {
       $template_data = str_replace("{remarks}",$all_detail['dialysis_list'][0]->remarks,$template_data);
    }
    else
    {
