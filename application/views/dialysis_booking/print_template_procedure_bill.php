...
else
{
 ...
 $template_data->setting_value = str_replace("{dialysis_name}",$get_ipd_patient_details['dialysiss_name'],$template_data->setting_value); 
 
 if(!empty($get_ipd_patient_details['dialysis_booking_date']))
    {
        
       
       $template_data->setting_value = str_replace("{dialysis_booking_date}",date('d-m-Y',strtotime($get_ipd_patient_details['dialysis_booking_date'])),$template_data->setting_value);
    }
    else
    {
        
         $template_data->setting_value = str_replace("{dialysis_booking_date}",'',$template_data->setting_value);
    }
    

    
  if($get_ipd_patient_details['dialysis_date']=="0000-00-00" || $get_ipd_patient_details['dialysis_date']=='0000-00-00 00:00:00')
  {
    $template_data->setting_value = str_replace("{dialysis_date}",'',$template_data->setting_value);
  }
  else
  {
      ...
      if($template_data->date_time_formate==1)
    {
        ...
        $template_data->setting_value = str_replace("{dialysis_date}",date('d-m-Y h:i A',strtotime($get_ipd_patient_details['dialysis_date'])),$template_data->setting_value);

    }
    else
    {
        $template_data->setting_value = str_replace("{dialysis_date}",date('d-m-Y',strtotime($get_ipd_patient_details['dialysis_date'])),$template_data->setting_value); 
    }
    
  }
  //for date and time
  $template_data->setting_value = str_replace("{dialysis_date_time}",date('d-m-Y h:i A',strtotime($get_ipd_patient_details['dialysis_date'])),$template_data->setting_value);
  
  
  $dialysis_time='';
        if(!empty($get_ipd_patient_details['dialysis_time']) && $get_ipd_patient_details['dialysis_time']!='00:00:00' && strtotime($get_ipd_patient_details['dialysis_time'])>0)
        {
            $dialysis_time = date('h:i A', strtotime($get_ipd_patient_details['dialysis_time'])); 
        }

        
        $template_data->setting_value = str_replace("{dialysis_date_time}",date('d-m-Y',strtotime($get_ipd_patient_details['dialysis_date'])).' '.$dialysis_time,$template_data->setting_value);
        
        
        
        $template_data->setting_value = str_replace("{procedure_date}",date('d-m-Y',strtotime($get_ipd_patient_details['procedure_created_date'])),$template_data->setting_value);
  ...
  //insurance_type
       // $insurance_type= ucwords($get_ipd_patient_details['insurance_type']);
        $template_data->setting_value = str_replace("{insurance_type}", '', $template_data->setting_value);  
        //insurance_type

        //insurance_type_name
        //$insurance_type_name= ucwords($get_ipd_patient_details['insurance_type_name']);
        $template_data->setting_value = str_replace("{insurance_type_name}", '', $template_data->setting_value);  
        //insurance_type_name

        //insurance_company_name
        //$insurance_company_name= ucwords($get_ipd_patient_details['insurance_company_name']);
        $template_data->setting_value = str_replace("{insurance_company_name}", '', $template_data->setting_value);  
        //insurance_company_name
  ...
  if(!empty($get_ipd_patient_details['procedure_remarks']))
    {
       $template_data->setting_value = str_replace("{remarks}",$get_ipd_patient_details['procedure_remarks'],$template_data->setting_value);
    }
    else
    {
     ...
     if($template_data->date_time_formate==1)
      {
        ...
        $template_data->setting_value = str_replace("{booking_date}",date('d-m-Y',strtotime($get_ipd_patient_details['dialysis_date'])).' '.$admission_time,$template_data->setting_value);

      }
      else
      {
        $template_data->setting_value = str_replace("{booking_date}",date('d-m-Y',strtotime($get_ipd_patient_details['dialysis_date'])),$template_data->setting_value);
      }
...
//echo $paid_amount_dis_bill;
 //$paid_amount_dis_bill //on 24 sep balance_final
    ...
    $template_data->setting_value = str_replace("{bill_no}",$get_ipd_patient_details['procedure_bill_no'],$template_data->setting_value);
    $template_data->setting_value = str_replace("{mobile_no}",$get_ipd_patient_details['mobile_no'],$template_data->setting_value);


    if(!empty($get_ipd_patient_details['booking_code']))
    {
       
        $template_data->setting_value = str_replace("{booking_code}",$get_ipd_patient_details['booking_code'],$template_data->setting_value);
    }

    if(!empty($get_ipd_patient_details['dialysis_date']))
    {
        $booking_date = '<div style="width:100%;display:inline-flex;">
                        <div style="width:40%;line-height:19px;font-weight:600;">IPD Reg. Date:</div>

            <div style="width:60%;line-height:19px;">'.date('d-m-Y',strtotime($get_ipd_patient_details['dialysis_date'])).'</div>
            </div>';
    ...
    {
        $room_no = $get_ipd_patient_details['room_no'];
    ...
    if(!empty($get_ipd_patient_details['bad_name']))
    {
        ...
        $template_data->setting_value = str_replace("{bed_no}",$get_ipd_patient_details['bad_name'],$template_data->setting_value);
    }
...
$payment->reciept_suffix.')</div>
   ...
   else
   {
    ...
    $admission_time='';
     ...
     if($get_ipd_patient_details['dialysis_date']=="0000-00-00" || $get_ipd_patient_details['dialysis_date']=='0000-00-00 00:00:00')
      {
        $dialysis_time = date('h:i A', strtotime($get_ipd_patient_details['dialysis_date'])); 
      }
    $template_data->setting_value = str_replace("{dialysis_time}",$dialysis_time,$template_data->setting_value);
