...
var table;
<?php
if(in_array('852',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('unit/archive_ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'unit/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('unit/restoreall');?>",
                      data: {row_id: allVals},
    ...
    <form>
  	...
  	<div class="userlist-right">
        ...
        <?php
          ...
          <?php } ?> 
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('unit'); ?>'">
...
<script> 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('unit/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('unit/trash/'); ?>"+unit_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('unit/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
