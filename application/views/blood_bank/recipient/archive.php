...
var table;
<?php
if(in_array('1510',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('blood_bank/recipient/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('blood_bank/recipient/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right">
  		<div class="btns"> 
          <?php if(in_array('1513',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
  				<i class="fa fa-window-restore" aria-hidden="true"></i> Restore
  			</button>
          <?php } ?>
          <?php if(in_array('1511',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('blood_bank/recipient'); ?>'">
...
<script>  
 ...
 function restore_recipient(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('blood_bank/recipient/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('blood_bank/recipient/trash/'); ?>"+medicine_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('blood_bank/recipient/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
