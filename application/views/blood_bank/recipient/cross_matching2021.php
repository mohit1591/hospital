 ...
 <input type="hidden" id="blood_grp_id_cross" name='blood_grp_id_cross' value="<?php echo $recepeint_detail['ward_bed_no']; ?>"/>
                    ...
                    <div class="col-xs-5">
                        <div class="row m-b-5">
                            ...
                            <div class="col-xs-8">
                              <select name="donor_id" id="donor_id" class="selectpicker" onchange="return get_donor_data(this.value);" data-live-search="true">
                        ...
                        <div class="row m-b-5">
                            <div class="col-xs-4"><strong>Blood Group</strong></div>
                            <div class="col-xs-8">
                                <input type="text" name="blood_group" value="" id="blood_group">
     ...
     <div class="table-responsive">
         <table class="table table-bordered table-striped advice_list" style="width: 97%;">
            <thead>
                    <tr>
                       <th>Vitals</th> 
                      <th>Before Transfusion</th> 
                      <th>30 mins,Post Trans.</th> 
                      <th>1 hour, Post Trans.</th> 
                      <th>24 hour,Post Trans.</th>
                    </tr>
                    <tbody>
                    <?php 
                      $vl_first='';
                    ...
                    $i=1;foreach($vitals_list as $vital)
                    {
                    ...
                    ?>
                      ...
                      <input name="vitals_ids[<?php echo $vital->id; ?>][first_val]" value="<?php echo $vl_first; ?>" type="text"></td>
                      <td><input name="vitals_ids[<?php echo $vital->id; ?>][second_val]" value="<?php echo $vl_second; ?>" type="text"></td>
                      <td><input  value="<?php echo $vl_third; ?>" type="text" name="vitals_ids[<?php echo $vital->id; ?>][third_val]"></td>
                      <td><input  value="<?php echo $vl_fourth; ?>" type="text" name="vitals_ids[<?php echo $vital->id; ?>][fourth_val]"></td></tr>
                    <?php } ?>
...
{   
  ...
  $.ajax({url: "<?php echo base_url(); ?>general/get_donar_data/"+new_split[0], 
      success: function(result)
      {

        var result = JSON.parse(result);
        $('#option').html(result.option);
         $('#donor_actual_id').val(new_split[0]);
        $('#blood_group').val(new_split[1]);
        $('#blood_group_id').val(new_split[2]);
      } 
    });
...
function bar_code_data(bar_code_id)
{
  ...
  $.ajax({url: "<?php echo base_url(); ?>general/get_stock_available/"+comp_id+'/'+d[0]+'/'+blood_group_id+'/'+bar_code_id, 
      success: function(result)
      { 
        var result = JSON.parse(result);
         if(result.success=1)
