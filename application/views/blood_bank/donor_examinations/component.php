  ...
  <div class="row mb-5">
    ...
    <div class="col-md-0"></div>
      ...
      <span id="component_error"></span>
       <span id="quantity_errors"></span>
       <span id="error_comp_error"></span>
...
{ 
  ...
  $.ajax({
            type: "POST",
            url: "<?php echo base_url('blood_bank/donor_examinations/get_components_by_bag_type');?>",
            data: {'bag_type_id':bag_type_id,'collection_date':cl_date, 'component_data':<?php echo json_encode($component_data); ?>},
            success: function(result) 
            {
    ...
    $.ajax({
          ...
          success: function(result) 
          {
            ...
            if(result.st==0)
            {
              ...
              $("#quantity_errors").html(result.quantity);
              $("#error_comp_error").html(result.error_comp);
              
            }
            else if(result.st==1)
            {
              $("#component_error").html('');
              $("#error_comp_error").html('');
