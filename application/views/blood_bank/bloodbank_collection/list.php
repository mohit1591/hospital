...
function reset_date_search()
  {
      ...
      $('#end_date').val('');
      $.ajax({
         url: "<?php echo base_url('blood_bank/bloodbank_collection_report/reset_date_search/'); ?>",  
...
$modal.load('<?php echo base_url().'blood_bank/bloodbank_collection_report/advance_search/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
<?php }?>
      ...
      <div class="fixed">
          ...
          <?php if(in_array('1568',$users_data['permission']['action'])): ?>
               <a href="<?php echo base_url('blood_bank/bloodbank_collection_report/bloodbank_report_excel');?>" class="btn-anchor m-b-2">
               <i class="fa fa-file-excel-o"></i> Excel
               </a>
          <?php endif;?>
          <?php if(in_array('1567',$users_data['permission']['action'])): ?>
               <a href="<?php echo base_url('blood_bank/bloodbank_collection_report/bloodbank_report_csv');?>" class="btn-anchor m-b-2">
               <i class="fa fa-file-word-o"></i> CSV
               </a>
          <?php endif;?>
         <?php if(in_array('1569',$users_data['permission']['action'])): ?>
               <a href="<?php echo base_url('blood_bank/bloodbank_collection_report/pdf_bloodbank_report'); ?>" class="btn-anchor m-b-2">
          ...
          <?php if(in_array('1569',$users_data['permission']['action'])): ?>
              ...
              <a href="javascript:void(0)" class="btn-anchor m-b-2" onClick="return print_window_page('<?php echo base_url("blood_bank/bloodbank_collection_report/print_all_bloodbank_report"); ?>');">
...
$("#new_search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url(); ?>blood_bank/bloodbank_collection_report/advance_search/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
...
{ 
  ...
  $.ajax({
         url: "<?php echo base_url('blood_bank/bloodbank_collection_report/advance_search/'); ?>", 
         type: 'POST',
         data: { from_date: start_date, end_date : end_date,branch_id:branch_id,component_id:component_id,employee:employee} ,
         success: function(result)
         { 
            if(vals!="1")
            {
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "aaSorting": [],
        "ajax": {
            "url": "<?php echo base_url('blood_bank/bloodbank_collection_report/ajax_list')?>",
            "type": "POST"
        }, 
        "columnDefs": [
        { 
