...
<div class="overlay-loader" id="overlay-loader">
  ...
  <form  id="notice_board" class="form-inline">
 
      <div class="modal-header">
          <button type="button" class="close"  data-number="1" aria-label="Close"><span aria-hidden="true">×</span></button>
          <h4><?php echo $page_title; ?></h4> 
          <input type="hidden" value="<?php echo $form_data['data_id'];?>" name="data_id" id="data_id"/>
      </div>
...
{
$("#notice_board").on("submit", function(event) { 
  event.preventDefault(); 
  $('#overlay-loader').show();
  var ids = $('#data_id').val();
  if(ids!="" && !isNaN(ids))
  { 
    var path = 'open_popup_page_edit/'+ids;
    var msg = 'Notice board successfully updated.';
  }
  else
  {
    var path = 'open_popup_page/';
    var msg = 'Notice board successfully created.';
  } 
  //alert('ddd');return false;
  $.ajax({
    url: "<?php echo base_url('notice_board_page/'); ?>"+path,
    type: "post",
    data: $(this).serialize(),
    success: function(result) {
      if(result==1)
      {
        $('#load_notice_board_page').modal('hide');
        flash_session_msg(msg);
        reload_table();
      } 
      else
      {
        $("#load_notice_board_page").html(result);
      }       
      $('#overlay-loader').hide();
    }
  });
