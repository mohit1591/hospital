...
var table;
<?php
if(in_array('1191',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dialysis_schedule_list/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'dialysis_schedule_list/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_ot_booking(id)
{
  ...
  window.location.href='<?php echo base_url().'dialysis_schedule_list/edit/';?>'+id;
...
{
  var $modal = $('#load_add_ot_booking_modal_popup');
  $modal.load('<?php echo base_url().'dialysis_schedule_list/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dialysis_schedule_list/deleteall');?>",
                      data: {row_id: allVals},
...
<form id="ot_booking_search_form">
    ...
    <form>
       <?php if(in_array('1191',$users_data['permission']['action'])) {
       ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
           ...
           <?php if(in_array('1205',$users_data['permission']['action'])) {
          ?>
           <a href="<?php echo base_url('dialysis_schedule_list/dialysis_list_excel'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-excel-o"></i> Excel
              </a>
              <?php } ?>
               <?php if(in_array('1206',$users_data['permission']['action'])) {
          ?>
              <a href="<?php echo base_url('dialysis_schedule_list/dialysis_list_csv'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-word-o"></i> CSV
              </a>
              <?php } ?>
 <?php if(in_array('1207',$users_data['permission']['action'])) {
          ?>
              <a href="<?php echo base_url('dialysis_schedule_list/pdf_dialysis_list'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-pdf-o"></i> PDF
              </a>
          <?php } ?>
                      
           <?php if(in_array('1192',$users_data['permission']['action'])) {
                    ?>
              <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("dialysis_schedule_list/print_dialysis_list"); ?>');"> <i class="fa fa-print"></i> Print</a>
              <?php } ?>
          ...
          ?>
  		<!-- 	<button class="btn-exit" onclick="window.location.href='<?php// echo base_url('dialysis_schedule_list/archive'); ?>'">
...
function reset_search()
{
  ...
  $('#start_date_ot_l').val('');
  $('#end_date_ot_l').val('');
  $.ajax({url: "<?php echo base_url(); ?>dialysis_schedule_list/reset_search/", 
    success: function(result)
    {
      //$("#search_form").reset();
      reload_table();
...
$modal.load('<?php echo base_url().'dialysis_schedule_list/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dialysis_schedule_list/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
<script>
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('dialysis_schedule_list');?>'; 
    }) ;
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a  type="button" data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("dialysis_schedule_list/print_dialysis_booking_report"); ?>');">Print</a>
...
function form_submit()
  ...
  $.ajax({
      url: "<?php echo base_url(); ?>dialysis_schedule_list/advance_search/",
      type: "post",
      data: $(this).serialize(),
      success: function(result) 
      {
