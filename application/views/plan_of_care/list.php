...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '25',
        "ajax": {
            "url": "<?php echo base_url('plan_of_care/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'plan_of_care/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_diagnosis_modal_popup');
  $modal.load('<?php echo base_url().'plan_of_care/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_diagnosis_modal_popup');
  $modal.load('<?php echo base_url().'plan_of_care/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('plan_of_care/deleteall');?>",
                      data: {row_id: allVals},
...
if(in_array('449',$permission_action) || in_array('121',$permission_section)){
          ...
          ?>
  			<!-- <button class="btn-exit" onclick="window.location.href='<?php echo base_url('plan_of_care/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('plan_of_care/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
