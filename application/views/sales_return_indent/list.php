...
function reset_search()
  { 
    ...
    $('#refered_by').val('');
    $('#branch_id').val('');
    $.ajax({url: "<?php echo base_url(); ?>sales_return_indent?>/reset_search/", 
...
function edit_sales_return_indent(id)
{
  window.location.href='<?php echo base_url().'sales_return_indent/edit/';?>'+id
}

function view_medicine_entry(id)
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'sales_return_indent/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('sales_return_indent/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
 ...
 <form id="new_search_form">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered sales_return_indent_list" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('sales_return_indent/add'); ?>'">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
                 <a href="<?php echo base_url('sales_return_indent/medicine_sales_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>

                <a href="<?php echo base_url('sales_return_indent/medicine_sales_csv'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-word-o"></i> CSV
                </a>

                <a href="<?php echo base_url('sales_return_indent/pdf_medicine_sales'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-pdf-o"></i> PDF
                </a>
                <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("sales_return_indent/print_medicine_sales"); ?>');"> <i class="fa fa-print"></i> Print</a>
               ...
               ?>
  			     <button class="btn-exit" onclick="window.location.href='<?php echo base_url('sales_return_indent/archive'); ?>'">
...
<script>  
 ...
 function delete_sales_return_indent(id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('sales_return_indent/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$('documnet').ready(function(){
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('sales_return_indent');?>'; 
    }) ;
...
$modal.load('<?php echo base_url().'sales_return_indent/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
function form_submit(vals)
{
   ...
   $.ajax({
       url: "<?php echo base_url(); ?>sales_return_indent/advance_search/",
       type: 'POST',
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('sales_return_indent/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('sales_return_indent/total_calc_return');?>",
                      success: function(result) 
                      {
