...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('packages/archive_ajax_list')?>",
            "type": "POST",
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('packages/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right">
  		<div class="btns"> 
          <?php if(in_array('577',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
  				<i class="fa fa-window-restore" aria-hidden="true"></i> Restore
  			</button>
          <?php } ?>
          <?php if(in_array('576',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('packages'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('packages/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('packages/trash/'); ?>"+Vendor_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('packages/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
