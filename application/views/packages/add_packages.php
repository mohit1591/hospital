...
//echo "<pre>"; print_r($medicine_kit_data);
?>
<!DOCTYPE html>
<html>
<head>
        ...
        <div class="pro-right">
               <div class="grp">
                    <div class="row m-b-5">
                         ...
                         <div class="col-xs-7">
                              <input type="text" autocomplete="off" name="medicine_name" onkeyup="return get_medicine_search_data(this.value);">
                         </div>
                    </div> 
               </div>

            <div class="pframe2">
                <table class="table table-bordered table-striped" id="medicine_list">
                    <thead class="bg-theme">
                        <tr>
                    <th align="center" width="40"><input type="checkbox"  name="getmedicineselectAll" class="" id="getmedicineselectAll" value=""></th>
                            <th>Medicine Code</th>
                            <th>Medicine Name</th>
...
<div class="profile-master2">
  <div class="proleft">
          ...
          <table class="table table-bordered table-striped" id="medicine_add_list">
               <thead class="bg-theme">
                    <tr>
                         <th align="center" width="40"><input type="checkbox" onclick="med_check()" name="addmedicineselectAll" class="" id="addmedicineselectAll" value=""></th>
                         <th>Medicine Code</th>
                         <th>Medicine Name</th>
                         <th>Medicine Company</th>
               ...
               <tbody id="medicine_add">
                    <?php echo $added_medicine; ?>
               </tbody>
          </table>
    ...
    <div class="proright">
        <div class="btns">
            ...
            <button type="submit" class="btn-save" name="" id="" onclick="return addmedicinecheckboxValues();"><i class="fa fa-floppy-o"></i> Save</button>
            <button type="button" class="btn-save" name="" id="" onclick="window.location.href='<?php echo base_url('packages'); ?>'"><i class="fa fa-sign-out"></i> Exit</button>
...
$(document).ready(function(){
     <?php  $array = $this->session->userdata('medicine_kit_data');
     ...
     get_select_kit_medicine(checkStr);
     var profId = $("#pack_id").val();
    
});
$(document).ready(function(){
   $('#addmedicineselectAll').on('click', function () { 
     ...
     $('#getmedicineselectAll').on('click', function () { 
          if ($(this).hasClass('allChecked')) {
               $('.checklist').prop('checked', false);
          } else {
          $('.checklist').prop('checked', true);
...
var table;
  ...
  function getMedicineList(){
     var getChildCount = document.getElementById("medicine_list").rows.length;
     ...
     $.post('<?php echo base_url(); ?>packages/get_added_medicine_list/',{},function(result){
...
{    
    $('tbody#medicine_add tr#nodata').remove();
     var allVals = [];
     ...
     var checkStr ="onadd";
     allmedicineadd(allVals,checkStr);
}
...
{    
     ...
     $('.medicinechecklist:checkbox').each(function() 
     {
     ...
     allmedicinedelete(allVals);
}
function add_interpretation()
{
  var $modal = $('#load_add_interpretation_modal_popup');
  $modal.load('<?php echo base_url().'packages/add_interpretation/'; ?>',
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function addmedicinecheckboxValues() 
{    
     ...
     save_added_medicine(allVals);
}
var testChildTable;
//add the child test list into selected child test list 
function allmedicineadd(allVals,checkStr){ 

     if(allVals!=""){
          $.post('<?php echo base_url(); ?>packages/listalladdedmedicine/',{row_id: allVals},function(result){
         

                    getMedicineList();
                    $("#medicine_add").html(result);
          });
          
     }
}
//delete the selected child list
function allmedicinedelete(allVals){  
     $('tbody#medicine_add tr').remove();
     
     if(allVals!=""){
          $.post('<?php echo base_url(); ?>packages/deletealllistedmedicine/',{row_id: allVals},function(result){
...
function set_qty_mkit(mid,type,qty)
{
     if(qty!='')
     {
          $.post('<?php echo base_url(); ?>packages/set_qty_mkit/'+mid+'/'+type+'/'+qty,function(result)
          {         
...
function get_select_kit_medicine(medicine_kit_data)
{
     $.post('<?php echo base_url(); ?>packages/set_row_medicine_kit',function(result)
     {         
        $("#medicine_add").html(result);
     });
}

function save_added_medicine(allVals){
 ...
 function med_check()
 {
     if($('#addmedicineselectAll').is(':checked')) 
     {    
          $('.medicinechecklist').prop('checked', false);
     }
     else
 ...
 function get_medicine_search_data(val){
     
      var totalRows= $('tbody#medicine tr').length;
      var emptyRows= $('tbody#medicine tr#nodata').length;
      if(totalRows!=emptyRows){
            $('tbody#medicine tr#nodata').remove();
      }
     $.post('<?php echo base_url(); ?>packages/get_medicine_search_data',{'search_data':val},function(result){
            $("#medicine").html(result);
            if(totalRows!=emptyRows){
                $('tbody#medicine tr#nodata').remove();
            }
