...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('gipsa_ipd_patient_discharge_summary/ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'gipsa_ipd_patient_discharge_summary/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_ipd_patient_discharge_summary_modal_popup');
  $modal.load('<?php echo base_url().'gipsa_ipd_patient_discharge_summary/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_ipd_patient_discharge_summary_modal_popup');
  $modal.load('<?php echo base_url().'gipsa_ipd_patient_discharge_summary/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('gipsa_ipd_patient_discharge_summary/deleteall');?>",
                      data: {row_id: allVals},
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('gipsa_ipd_patient_discharge_summary/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('gipsa_ipd_patient_discharge_summary/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$(document).ready(function() {
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('gipsa_ipd_patient_discharge_summary');?>'; 
    }); 
 ...
 function print_summary(id)
{
  ...
  $modal.load('<?php echo base_url().'gipsa_ipd_patient_discharge_summary/print_template/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("gipsa_ipd_patient_discharge_summary/print_discharge_summary"); ?>');">Print</a>
    ...
    <div id="confirm_medicine_print" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <!-- <div class="modal-body"></div> -->
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("gipsa_ipd_patient_discharge_summary/print_discharge_summary_medicine"); ?>');">Print</a>
