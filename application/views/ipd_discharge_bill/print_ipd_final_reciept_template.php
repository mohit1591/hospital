...
//echo $all_detail['ipd_list'][0]->reciept_prefix; die;
...
if(!empty($get_ipd_patient_details['discharge_bill_no']) && !empty($get_ipd_patient_details['discharge_bill_no']))
{
    $template_data->template = str_replace("{reciept_no}",$get_ipd_patient_details['discharge_bill_no'],$template_data->template);    
    
     //$template_data->template = str_replace("{hospital_receipt_no}",$get_ipd_patient_details['discharge_bill_no'],$template_data->template);
}
else
{
    ...
    $template_data->template = str_replace("Receipt No. :",'',$template_data->template);
                ...
                if(!empty($get_ipd_patient_details['paid_amount_dis_bill']))
                    {
                        $total_amount = $get_ipd_patient_details['paid_amount_dis_bill'];
                    }
                    else
                    {
                        $total_amount = '0.00';
...
///////////////////////////////////////////
   $template_data->template = str_replace("{insurance_company}",'',$template_data->template); 
...
//if(in_array('218',$users_data['permission']['section'])  )
    //{
     if((!empty($all_detail['ipd_list'][0]->reciept_suffix) || !empty($all_detail['ipd_list'][0]->reciept_prefix)))
      {
    ...
    $tr = str_replace("{particular}",'IPD Discharge Bill Receipt',$tr);
  ...
  $template_data->template = str_replace("{balance}",'',$template_data->template);
    ...
    else
    {
     ...
     $template_data->template = str_replace("{advance_amount}",$get_ipd_patient_details['advance_payment_dis_bill'],$template_data->template);
...
//if(in_array('218',$users_data['permission']['section']))
   // {
     if(!empty($all_detail['ipd_list'][0]->reciept_prefix) && !empty($all_detail['ipd_list'][0]->reciept_suffix))
      {
        $template_data->template = str_replace("{hospital_receipt_no}",$all_detail['ipd_list'][0]->reciept_prefix.$all_detail['ipd_list'][0]->reciept_suffix,$template_data->template);
      }
       else
    ...
    if(!empty($all_detail['ipd_list'][0]->discharge_date))
    {
        $created_date = '<br><b>Receipt Date :</b>'.date('d-m-Y',strtotime($all_detail['ipd_list'][0]->discharge_date));
   ...
   /* }
    ...
    if(!empty($all_detail['ipd_list'][0]->discharge_date))
    {
        $receipt_date = '<div style="width:100%;display:inline-flex;">
            ...
            <div style="width:60%;line-height:19px;">'.date('d-m-Y',strtotime($all_detail['ipd_list'][0]->discharge_date)).'</div>
            </div>';
