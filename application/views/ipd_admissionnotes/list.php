...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('ipd_admissionnotes/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'ipd_admissionnotes/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
$modal.load('<?php echo base_url().'ipd_admissionnotes/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'ipd_admissionnotes/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'ipd_admissionnotes/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function allbranch_delete(allVals)
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('ipd_admissionnotes/deleteall');?>",
                      data: {row_id: allVals},
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'ipd_admissionnotes/upload_prescription/' ?>'+id,
    {
      //'id1': '1',
      //'id2': '2'
      },
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'ipd_admissionnotes/view_files/' ?>'+prescription_id,
    {
      //'id1': '1',
      //'id2': '2'
      },
    ...
    <div class="userlist-right relative">
      <div class="fixed">
        <div class="btns">
               ...
               <?php if(in_array('121',$users_data['permission']['section'])){
                ?>
                 <button class="btn-exit" onclick="window.location.href='<?php echo base_url('ipd_admissionnotes/archive'); ?>'">
...
function consolidate_model()
{
     ...
     $.ajax({
         url: "<?php echo base_url('ipd_admissionnotes/check_patient_ids/'); ?>"+allVals, 
         dataType: "json",
         success: function(result)
         {
             ...
             {
                window.open('<?php echo base_url('ipd_admissionnotes/print_consolidate_prescription?') ?>pres_ids='+allVals,'mywin','width=800,height=600');  
             }
             else
             {
...
{ 
  ...
  $.ajax({url: "<?php echo base_url(); ?>ipd_admissionnotes/reset_search/", 
    success: function(result)
    { 
...
$("#search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('ipd_admissionnotes/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    { 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('ipd_admissionnotes/delete/'); ?>"+prescription_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('ipd_admissionnotes');?>'; 
    }); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("ipd_admissionnotes/print_prescriptions"); ?>');">Print</a>
