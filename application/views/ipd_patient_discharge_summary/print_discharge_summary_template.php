...
$pincode = $all_detail['discharge_list'][0]->pincode;
    ...
    $template_data = str_replace("{doctor_name}","Dr. ".$all_detail['discharge_list'][0]->doctor_name,$template_data);
...
if($test_list!=""){
    ...
    $template_data = str_replace("{discharge_date}",$discharge_date,$template_data);
       ...
       $template_data = str_replace("{assigned_doctor}",$ipd_assigned_doctors , $template_data);
    ...
    {
        $template_data = str_replace("{room_no}",$all_detail2['ipd_list'][0]->room_no,$template_data);
    ...
    {
        $template_data = str_replace("{bed_no}",$all_detail2['ipd_list'][0]->bad_name,$template_data);
    }
    else if(!empty($all_detail2['ipd_list'][0]->bad_no))
    {
        $template_data = str_replace("{bed_no}",$all_detail2['ipd_list'][0]->bad_no,$template_data);
    }
    else
    {
    ...
    {
        $template_data = str_replace("{room_type}",$all_detail2['ipd_list'][0]->room_category,$template_data);
    }
    else
    {
   ...
   if(strcmp(strtolower($value['setting_name']),'review_time_and_date')=='0')
   {
       ...
       {
            if($all_detail['discharge_list'][0]->summery_type != 5){
...
//seprated vitals
    ...
    $cause_of_death = "";
    if(!empty($all_detail['discharge_list'][0]->cause_of_death)){
        $cause_of_death .= '<div style="float:left;width:100%;margin:1em 0;">
          <div style="font-size:small;line-height:18px;">
            <b>Cause of Death :</b>
            '.$all_detail['discharge_list'][0]->cause_of_death.'
          </div>
        </div>';
    }
    // dd($cause_of_death);
    if($all_detail['discharge_list'][0]->summery_type=='5'){ 
       $template_data = str_replace("{death_date_time}",$death_time_and_date.$cause_of_death,$template_data); 
    }
