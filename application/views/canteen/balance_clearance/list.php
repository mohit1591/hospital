          		...
          		<div id="previous_data"> 
                     <div  id="search_box_patient_name" style="display:none;">
                      ...
                      <?php if(!empty($form_error)){ echo form_error('patient_name'); } ?>
                       <div  id="search_box_customer_name" style="display:none;">
...
<script>
 ...
 { 
   ...
   { 
    $.ajax({
           url: "<?php echo base_url('canteen/balance_clearance/search_data/'); ?>", 
           type: 'POST',
  ...
  { 
     $.post('<?php echo base_url('canteen/balance_clearance/get_allsub_branch_list/'); ?>',{},function(result){
...
function get_balance_clearance_list(vals)
{
    ...
    $.post('<?php echo base_url('canteen/balance_clearance/balance_list/'); ?>',{'sub_branch_id':sub_branch_id,'patient_name':patientName,'customer_name':customerName,'mobile_no':mobileNo,'type':vals,'vendor_name':vendorName,'vendor_mobile_no':vendorMobileNo},function(result)
    { 
          if(result!='')
          {
...
<script>
  function get_patient_drop_down(type)
  {
    ...
    $.ajax({
                 type: "POST",
                 url: "<?php echo base_url('canteen/balance_clearance/get_drop_down_value/'); ?>", 
                 data: {type: type},
                 success: function(result)
                 {
...
{ 
    ...
    $modal.load('<?php echo base_url().'canteen/balance_clearance/pay_now/' ?>'+id+'/'+parent_id+'/'+section_id+'/'+branch_id,
    {'bal': balance},
     function(){
          $modal.modal('show');
     });
