...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('canteen/purchase_return/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('canteen/purchase_return/total_calc_return');?>",
...
function edit_purchase_return(id)
{
  ...
  window.location.href='<?php echo base_url().'canteen/purchase_return/edit/';?>'+id
...
function view_medicine_entry(id)
{
  ...
  $modal.load('<?php echo base_url().'canteen/purchase_return/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('canteen/purchase_return/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
      ...
      <table class="ptl_tbl">
        <tr>
          <td><label>From Date</label>
            <input type="text" name="start_date" placeholder="From Date" id="start_date_p" class="datepicker"  onkeyup="return form_submit();" value="<?php echo $form_data['start_date'];?>"></td>
          <td><label>To Date</label>
            <input type="text" name="end_date" placeholder="To Date" id="end_date_p" class="datepicker"  onkeyup="return form_submit();" value="<?php echo $form_data['end_date'];?>"></td>
        ...
        <tr>
          <td><label>Purchase No.</label>
            <input type="text" name="purchase_no" id="purchase_no" placeholder="Purchase No." value="<?php echo $form_data['purchase_no'];?>" class=""  onkeyup="return form_submit();" autofocus=""></td>
          <td><label>Invoice No.</label>
            <input type="text" name="invoice_id" id="invoice_id"  placeholder="Invoice No." value="<?php echo $form_data['invoice_id'];?>" class=""  onkeyup="return form_submit();"></td>
          <td></td>
        </tr>
        <tr>
    ...
    <form>
  	...
  	<div class="userlist-right ">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('canteen/purchase_return/add'); ?>'">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php // } ?>
               <?php // if(in_array('395',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
  				    <i class="fa fa-trash"></i> Delete
  			     </button>

                <a href="<?php echo base_url('canteen/purchase_return/item_purchase_return_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>

                <a href="<?php echo base_url('canteen/purchase_return/item_purchase_return_csv'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-word-o"></i> CSV
                </a>

                <a href="<?php echo base_url('canteen/purchase_return/pdf_item_purchase_return'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-pdf-o"></i> PDF
                </a>
				
                <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("canteen/purchase_return/print_item_purchase_return"); ?>');"> <i class="fa fa-print"></i> Print</a>
...
function reset_search()
  { 
    ...
    $('#invoice_id').val('');
    $('#branch_id').val('');
    $.ajax({url: "<?php echo base_url(); ?>canteen/purchase_return/reset_search/", 
      success: function(result)
      { 
        reload_table();
      } 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('canteen/purchase_return/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a  type="button" data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("canteen/purchase_return/print_purchase_return_recipt"); ?>');">Print</a>
...
$modal.load('<?php echo base_url().'canteen/purchase_return/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
$("#new_search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url(); ?>/canteen/purchase_return/advance_search/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
