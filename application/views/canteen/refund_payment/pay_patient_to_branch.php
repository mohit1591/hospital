...
<section class="userlist">
    <div class="userlist-box">
		...
		<?php $amount='';?>
		 ...
		 <form action="<?php echo base_url('canteen/refund_payment/save_refund_details/'.$form_data['pid'].'/'.$form_data['section_id'].'/'.$form_data['parent_id'].'/'.$form_data['branch_id']); ?>" method="post">
       ...
       <table id="dataTable" class="table table-striped table-bordered refund_payment_list dataTable no-footer" role="grid" aria-describedby="table_info" cellspacing="0">
        ...
        ?>
           ...
           <?php } } elseif ($form_data['section_id']==4){
            $i=1; 
             if(!empty($billing_data)){ //print_r($billing_data); die; print_letterhead_setting/template/1?>
		...
		<div class="row" id="inner_table" style="display:none;">
			...
			<div class="col-sm-4">
				<div class="row m-b-5">
					...
					<div class="col-sm-7"> <input type="text" name="refund_date" id="refund_date" value="" class="datepicker"/></div>
				</div> <!-- innerRow -->
			</div> <!-- 4 -->
			<div class="col-sm-4">
...
function pay_fields(value)
{
   $('#updated_payment_detail').html('');
     $.ajax({
        type: "POST",
        url: "<?php echo base_url('canteen/refund_payment/get_payment_mode_data')?>",
