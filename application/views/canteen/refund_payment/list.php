...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('canteen/refund_payment/ajax_list')?>",
            "type": "POST",
...
<section class="userlist">
    <div class="userlist-box">
                    ...
                    <label></label>  
                      ...
                      if (array_key_exists("permission",$users_data)){
                       ...
                       <input type="radio" <?php if(!empty($refund_search_data) && in_array('2',$refund_search_data['section_id'])){ echo 'checked="checked"'; } ?> name="section_id[]" onclick="return form_submit();" value="2"> OPD
                        ...
                        if(in_array('85',$permission_section)){ 
                       ?>
                       <input type="radio" <?php if(!empty($refund_search_data) && in_array('4',$refund_search_data['section_id'])){ echo 'checked="checked"'; } ?> name="section_id[]" onclick="return form_submit();" value="4"> OPD Billing
                        <?php } 
                    ...
                    <div id="previous_data">
                     ...
                     <div class="grp" id="search_box_patient_name" style="display:none;"><input  onkeyup="return form_submit();" type="text" name="patient_name"  id="patient_name" /></div>
                      <?php if(!empty($form_error)){ echo form_error('patient_name'); } ?>
                       <div class="grp" id="search_box_customer_name" style="display:none;"><input  onkeyup="return form_submit();" type="text" name="customer_name"  id="customer_name" /></div>
                      <?php if(!empty($form_error)){ echo form_error('customer_name'); } ?>
     ...
     <div class="userlist-right">
      ...
      <div class="btns">
              <button class="btn-exit" onclick="window.location.href='<?php echo base_url('canteen/refund_payment/refund_list'); ?>'">
...
<script>
 ...
 { 
   ...
   { 
    ...
    $.ajax({
           url: "<?php echo base_url('canteen/refund_payment/search_data/'); ?>", 
           type: 'POST',
  ...
  { 
     $.post('<?php echo base_url('canteen/refund_payment/get_allsub_branch_list/'); ?>',{},function(result){
...
{ 
      ...
      {
             document.getElementById("search_box_customer_name").style.display="block";
              document.getElementById("search_box_vendor_mobile_no").style.display="none";
      }
      else
      {
             document.getElementById("search_box_patient_name").style.display="none";
              document.getElementById("search_box_mobile_no").style.display="none";
              document.getElementById("search_box_customer_name").style.display="none";
             $('#patient_name').val('');
      }
}
...
/*function get_balance_clearance_list(vals)
{
    ...
    $.post('< ?php echo base_url('canteen/refund_payment/balance_list/'); ?>',{'sub_branch_id':sub_branch_id,'patient_name':patientName,'mobile_no':mobileNo,'type':vals,'vendor_name':vendorName,'vendor_mobile_no':vendorMobileNo},function(result)
    { 
          if(result!='')
          {
...
<script>
  function get_patient_drop_down(type)
  {
    ...
    $.ajax({
                 type: "POST",
                 url: "<?php echo base_url('canteen/refund_payment/get_drop_down_value/'); ?>", 
                 data: {type: type},
                 success: function(result)
                 {
...
{ 
    ...
    $modal.load('<?php echo base_url().'canteen/refund_payment/pay_now/' ?>'+id+'/'+parent_id+'/'+section_id+'/'+branch_id,
    {'bal': balance},
     function(){
          $modal.modal('show');
     });
