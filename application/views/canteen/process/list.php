...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('canteen/process/ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'canteen/process/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
var $modal = $('#load_item_inventory_import_modal_popup');
      ...
      //  alert();
    $modal.load('<?php echo base_url().'canteen/process/inventory_item_import_excel' ?>',
    { 
    },
    function(){
    $modal.modal('show');
...
function edit_stock_item(id)
{
  ...
  var $modal = $('#load_item_inventory_import_modal_popup');
  $modal.load('<?php echo base_url().'canteen/process/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_item_inventory_import_modal_popup');
  $modal.load('<?php echo base_url().'canteen/process/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('canteen/process/deleteall');?>",
                      data: {row_id: allVals},
...
function select_item_data(item_type){
  ...
  $('#overlay-loader').show();
    ...
    $.ajax({
          url: "<?php echo base_url(); ?>canteen/process/advance_search/",
          type: "post",
          data: $(this).serialize(),
          success: function(result) 
          {
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('canteen/process/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
