...
function reset_date_search()
  {
      ...
      $('#end_date').val('');
      $.ajax({
         url: "<?php echo base_url('testoutsource_reports/reset_date_search/'); ?>",  
...
$modal.load('<?php echo base_url().'testoutsource_reports/advance_search/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
$modal.load('<?php echo base_url().'testoutsource_reports/advance_report/' ?>',
{
//'id1': '1',
//'id2': '2'
},
    ...
    <form> 
    		...
    		<div class="grp_box m-b-5">
					<div class="grp" id="selection_criteria">Searching Criteria 
					        <select id="selection_criteria_list" name="selection_criteria_list" onchange="get_selected_value_by_user(this.value);form_submit();">
					        <option value=''>Select Option</option>
					        <option value="patient_name">Patient Name</option>
					        <option value="p_mobile_no">Mobile No.</option>
					        <option value="uhid_no">UHID No.</option>
					        <option value="lab_ref_no">Lab Ref. No.</option>
					        </select>
					</div>
       ...
       <?php
          if(in_array('1930',$users_data['permission']['action'])){?>
      ...
      <div class="fixed">
          ...
          <?php if(in_array('1931',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('testoutsource_reports/path_report_excel'); ?>" class="btn-anchor">
               <i class="fa fa-file-excel-o"></i> Excel
               </a>
          <?php } ?>
          <?php if(in_array('1932',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('testoutsource_reports/path_report_csv'); ?>" class="btn-anchor">
                    <i class="fa fa-file-word-o"></i> CSV
               </a>
          <?php } ?>
          <?php if(in_array('1933',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('testoutsource_reports/pdf_path_report'); ?>" class="btn-anchor">
                    <i class="fa fa-file-pdf-o"></i> PDF
               </a>
          <?php } ?>
          <?php if(in_array('1934',$users_data['permission']['action'])){?>
               <a href="javascript:void(0)" class="btn-anchor" id="deleteAll" onClick="return print_window_page('<?php echo base_url("testoutsource_reports/print_path_report"); ?>');">
               <i class="fa fa-print"></i> Print
               </a>
          <?php } ?>
          <?php if(in_array('1930',$users_data['permission']['action'])){?>
...
function form_submit()
{
	...
	var lab_ref_no = $('#lab_ref_no').val();
  	$.ajax({
	         url: "<?php echo base_url('testoutsource_reports/search_data/'); ?>", 
	         type: 'POST',
	         data: { patient_name: patient_name,uhid_no: uhid_no,lab_ref_no: lab_ref_no, start_date: start_date, end_date : end_date} ,
	         success: function(result)
	         { 
	            reload_table(); 
	         }
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20', 
        "ajax": {
            "url": "<?php echo base_url('testoutsource_reports/ajax_list')?>",
            "type": "POST"
        }, 
        "columnDefs": [
        { 
...
function clear_form_elements(ele) {
   $.ajax({url: "<?php echo base_url(); ?>testoutsource_reports/reset_date_search/", 
      success: function(result)
      { 
