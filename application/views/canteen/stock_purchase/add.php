        ...
        <div class="row">
          <div class="col-md-11 pr-0">
              ...
              <table class="table table-bordered table-striped purchase_stock_item_list" id="item_list">
                ...
                <tbody>
                <?php //print_r($stock_purchase_item_list) ;
                if(isset($stock_purchase_item_list) && !empty($stock_purchase_item_list))
                {
                  $i=1;foreach($stock_purchase_item_list as $stock_p_list)
                {
                  ?>
                  <tr>
                    <td><input type="checkbox" name="item_id[]" value="<?php echo $stock_p_list['item_id']; ?>"  class="booked_checkbox"></td>
                    <td><?php echo $i;?><input type="hidden" value="<?php echo $stock_p_list['item_id'];?>" id="item_id_<?php echo $stock_p_list['item_id'];?>"/><input type="hidden" value="<?php echo $stock_p_list['category_id'];?>" id="category_id_<?php echo $stock_p_list['item_id'];?>"/></td>
                   ...
                   <td><?php echo $stock_p_list['item_name'];?><input type="hidden" value="<?php echo $stock_p_list['item_name'];?>" id="item_name_<?php echo $stock_p_list['item_id'];?>"/><input type="hidden" value="<?php echo $stock_p_list['category_id'];?>" id="category_id_<?php echo $stock_p_list['item_id'];?>"/></td>
      ...
      <div class="fixed">
        <div class="btns">
         ...
         <!--  <button class="btn-save" type="submit" name="">
          ...
          <a href="<?php echo base_url('canteen/stock_purchase'); ?>" class="btn-anchor"><i class="fa fa-sign-out"></i> Exit</a>
        </div>
      </div>
    </div> 
...
<script> 
 ...
 function payment_function(value,error_field){
    $('#updated_payment_detail').html('');
     $.ajax({
        type: "POST",
        url: "<?php echo base_url('canteen/stock_purchase/get_payment_mode_data')?>",
...
{
    $.ajax({
          url: "<?php echo base_url('canteen/stock_purchase/get_vandor_data/'); ?>"+vendor_id, 
          success: function(result)
          {
 ...
 function item_payment_calculation()
  {
    ...
    if(item_name!='')
    {
     $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>canteen/stock_purchase/item_payment_calculation/", 
            dataType: "json",
...
function add_vendor(vendor_id)
{
  ...
  function payment_new_calc_all(update_row)
  {
     ...
     var data_id= '<?php echo $form_data['data_id'];?>';
      ...
      var pay_amount = $('#pay_amount').val();
        $.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>canteen/stock_purchase/payment_calc_all/", 
        dataType: "json",
  ...
  function payment_cal_perrow(ids){
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>canteen/stock_purchase/payment_cal_perrow/", 
            dataType: "json",
...
function remove_item(allVals)
  { 
   if(allVals!="")
   {
      $.ajax({
              type: "POST",
              url: "<?php echo base_url('canteen/stock_purchase/remove_stock_purchase_item_list');?>",
               dataType: "json",
...
$(".txt_firstCap").on('keyup', function(){
  ...
  <!--new css-->
    ...
    var getData = function (request, response) { 
        row = i ;
        $.ajax({
        url : "<?php echo base_url('canteen/stock_purchase/get_item_values/'); ?>" + request.term,
        dataType: "json",
        method: 'post',
       data: {
