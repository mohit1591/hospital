...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('canteen/employee_salary/ajax_list')?>",
            "type": "POST",
...
$modal.modal('show');
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('canteen/employee_salary');?>'; 
    }) ;
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'canteen/employee_salary/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'canteen/employee_salary/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('canteen/employee_salary/deleteall');?>",
                      data: {row_id: allVals},
...
if(in_array('1',$permission_section)): ?> 
 ...
 <?php endif;?>
      ...
      <?php
       ...
       {
        ?>
          <button class="btn-exit" onclick="window.location.href='<?php echo base_url('canteen/employee_salary/archive'); ?>'">
...
$("#search_form_emplist").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('canteen/employee_salary/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('canteen/employee_salary/delete/'); ?>"+employee_salary_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
