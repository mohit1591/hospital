...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('canteen/sale/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('canteen/sale/total_calc_return');?>",
...
function edit_sale(id)
{
  ...
  window.location.href='<?php echo base_url().'canteen/sale/edit/';?>'+id
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('canteen/sale/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
...
<section class="userlist">
    ...
    <table class="ptl_tbl">
      <tr>
        <td><label>From Date</label> <input type="text" placeholder="From Date" name="start_date" id="start_date_p" class="datepicker"  onkeyup="return form_submit();" value="<?php echo $form_data['start_date'] ?>"></td>
        <td><label>To Date</label> <input type="text" placeholder="To Date" name="end_date" id="end_date_p" class="datepicker"  onkeyup="return form_submit();" value="<?php echo $form_data['end_date'] ?>" ></td>
      ...
      <tr>
        <td><label>Sale No.</label><input type="text" placeholder="Sale No." id="purchase_no" value="<?php echo $form_data['purchase_no'] ?>" name="purchase_no" class=" "  onkeyup="return form_submit();" autofocus=""></td>
        <td><label>Invoice No.</label><input type="text" id="invoice_id" placeholder="Invoice No." value="<?php echo $form_data['invoice_id'] ?>" name="invoice_id" class=" "  onkeyup="return form_submit();"></td>
      </tr>
      <tr>
        <td><label>Paid Amount</label>
            <input type="text" name="paid_amount_to" id="paid_amount_to" placeholder="" value="<?php echo $form_data['paid_amount_to'] ?>" class="w-90px"  onkeyup="return form_submit();"> To
            <input type="text" name="paid_amount_from" id="paid_amount_from" placeholder="" value="<?php echo $form_data['paid_amount_from'] ?>" class="w-90px"  onkeyup="return form_submit();"></td>
            <td><label>Balance</label>
            <input type="text" name="balance_to" id="balance_to" class="w-90px" placeholder=""   value="<?php echo $form_data['balance_to'] ?>" onkeyup="return form_submit();"> To
            <input type="text" name="balance_from" id="balance_from" class="w-90px" placeholder=""  value="<?php echo $form_data['balance_from'] ?>" onkeyup="return form_submit();"></td>
      </tr>
      <tr><td>  <?php  
                $users_data = $this->session->userdata('auth_users'); 
    ...
    <form>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('canteen/sale/add'); ?>'">
               ...
               ?>
                ...
                <a href="<?php echo base_url('canteen/sale/item_sale_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>

                <a href="<?php echo base_url('canteen/sale/item_sale_csv'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-word-o"></i> CSV
                </a>

                <a href="<?php echo base_url('canteen/sale/pdf_item_sale'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-pdf-o"></i> PDF
                </a>

                 <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("canteen/sale/print_item_sale"); ?>');"> <i class="fa fa-print"></i> Print</a>
               ...
               <?php // if(in_array('389',$users_data['permission']['action'])) {
               ?>
  			    <!-- <button class="btn-update" onclick="window.location.href='<?php echo base_url('canteen/sale/archive'); ?>'">
...
function reset_search()
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('canteen/sale/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$('documnet').ready(function(){
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('canteen/sale');?>'; 
    }) ;
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <!-- <a type="button" data-dismiss="modal" class="btn-anchor" id="delete" onClick="return openPrintnewWindow('< ?php echo base_url("canteen/sale/print_purchase_recipt"); ?>', 'windowTitle', 'width=820,height=600');" target="_blank">Print</a> -->
...
$modal.load('<?php echo base_url().'canteen/sale/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
