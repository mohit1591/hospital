...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('canteen/purchase_estimate/ajax_list_estimate')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('canteen/purchase_estimate/total_calc_return');?>",
...
function edit_purchase(id)
{
  ...
  window.location.href='<?php echo base_url().'canteen/purchase_estimate/edit/';?>'+id
...
function view_medicine_entry(id)
{
  ...
  $modal.load('<?php echo base_url().'canteen/purchase_estimate/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('canteen/purchase_estimate/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
...
<section class="userlist">
    ...
    <table class="ptl_tbl">
      ...
      <tr>
        <td><label>Purchase No.</label><input type="text" id="purchase_no" placeholder="Purchase No." value="<?php echo $form_data['purchase_no'] ?>" name="purchase_no" class=" "  onkeyup="return form_submit();" autofocus=""></td>
    ...
    <form>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('canteen/purchase_estimate/add'); ?>'">
               ...
               ?>
                ...
                <a href="<?php echo base_url('canteen/purchase_estimate/item_purchase_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>

                <a href="<?php echo base_url('canteen/purchase_estimate/item_purchase_csv'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-word-o"></i> CSV
                </a>

                <a href="<?php echo base_url('canteen/purchase_estimate/pdf_item_purchase'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-pdf-o"></i> PDF
                </a>

                 <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("canteen/purchase_estimate/print_item_purchase"); ?>');"> <i class="fa fa-print"></i> Print</a>
               ...
               ?>
  			    <!-- <button class="btn-update" onclick="window.location.href='<?php echo base_url('canteen/purchase_estimate/archive'); ?>'">
...
function reset_search()
  { 
    ...
    $('#invoice_id').val('');
    $('#branch_id').val('');
    $.ajax({url: "<?php echo base_url(); ?>canteen/purchase_estimate/reset_search/", 
      success: function(result)
      { 
        reload_table();
      } 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('canteen/purchase_estimate/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$('documnet').ready(function(){
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('canteen/purchase');?>'; 
    }) ;
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <!-- <a type="button" data-dismiss="modal" class="btn-anchor" id="delete" onClick="return openPrintnewWindow('< ?php echo base_url("canteen/purchase_estimate/print_purchase_recipt"); ?>', 'windowTitle', 'width=820,height=600');" target="_blank">Print</a> -->
...
$modal.load('<?php echo base_url().'canteen/purchase_estimate/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
$("#new_search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url(); ?>/canteen/purchase_estimate/advance_search/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
