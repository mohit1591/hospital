...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dialysis_prescription/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'dialysis_prescription/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
$modal.load('<?php echo base_url().'dialysis_prescription/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'dialysis_prescription/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'dialysis_prescription/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function allbranch_delete(allVals)
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dialysis_prescription/deleteall');?>",
                      data: {row_id: allVals},
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'dialysis_prescription/upload_prescription/' ?>'+id,
    {
      //'id1': '1',
      //'id2': '2'
      },
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'dialysis_prescription/view_files/' ?>'+prescription_id,
    {
      //'id1': '1',
      //'id2': '2'
      },
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'dialysis_prescription/share_video_link/' ?>'+id+'/'+booking_id,
    {
    },
    function(){
    $modal.modal('show');
...
$("#search_form").on("submit", function(event) { 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dialysis_prescription/delete/'); ?>"+prescription_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('dialysis_prescription');?>'; 
    }); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("dialysis_prescription/print_prescriptions"); ?>');">Print</a>
