...
$template_data->template = str_replace("Paid Amount:",'',$template_data->template);

$template_data->template = str_replace("Balance :",$transaction_id[0]->field_value,$template_data->template);

$template_data->template = str_replace("Received with Thanks a sum of Rs.{paid_amount} from {patient_name}",'',$template_data->template);

$template_data->template = str_replace("Payment Mode:",'',$template_data->template);

$template_data->template = str_replace("Cash Receipt",'Proforma Invoice',$template_data->template);

$template_data->template = str_replace("Lab Reg. No. :",'Proforma No.',$template_data->template);

$template_data->template = str_replace("Barcode :",'',$template_data->template);
if(empty($address_setting_list))
    {
        $address = $all_detail['booking_list'][0]->address;
        $pincode = $all_detail['booking_list'][0]->pincode;    
        $country = $all_detail['booking_list'][0]->country_name;    
        $state = $all_detail['booking_list'][0]->state_name;    
        $city = $all_detail['booking_list'][0]->city_name;    
        $patient_address = $address.' '.$country.','.$state.' '.$city.' - '.$pincode;
        $template_data->template = str_replace("{address}",$address, $template_data->template);
    }
    else
    {
        $address='';
        ...
        {
           $address .= $all_detail['booking_list'][0]->address1.' '; 
        }
        if($address_setting_list[0]->address2)
        {
           $address .= $all_detail['booking_list'][0]->address2.' '; 
        }
       
        if($address_setting_list[0]->address3)
        {
           $address .=  $all_detail['booking_list'][0]->address3.' '; 
        }
      
        if($address_setting_list[0]->city)
        {
           $address .=  $all_detail['booking_list'][0]->city_name.' '; 
        }
       
        if($address_setting_list[0]->state)
        {
           $address .= $all_detail['booking_list'][0]->state_name.' '; 
        }
        if($address_setting_list[0]->country)
        {
           $address .=  $all_detail['booking_list'][0]->country_name.' '; 
        }
        if($address_setting_list[0]->pincode)
        {
           $address .= $all_detail['booking_list'][0]->pincode; 
        }
         $template_data->template = str_replace("{address}",$address, $template_data->template);
    }
//echo "<pre>"; print_r($all_detail['booking_list']); exit;
$del = ','; 
$address_n='';
$address_re='';
...
if($template_data->printer_id==2)
{
   ...
   //  }
       ...
       if(!empty($all_detail['booking_list'][0]->relation))
        ...
        $template_data->template = str_replace("{booking_code}",$all_detail['booking_list'][0]->lab_invoice_no,$template_data->template);
...
// added on 07-Feb-2018
    ...
    $template_data->template = str_replace("{gst_amount}",$all_detail['booking_list'][0]->gst_amount,$template_data->template);
