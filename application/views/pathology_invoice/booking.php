...
$child_branch_list = $this->session->userdata('invoice_sub_branches_data');
$profile_session_list = $this->session->userdata('invoice_set_profile');
...
<form id="booking_form" action="<?php echo current_url(); ?>" method="post">
...
<div class="row">
  <div class="col-md-4">
    ...
    <div class="row m-b-5">
      ...
      <div class="col-sm-8">
        <input type="text" readonly="" class="m_input_default" name="lab_invoice_no" value="<?php echo $form_data['lab_invoice_no']; ?>"/> 
      </div>  
    </div>
...
<?php //if(in_array('38',$users_data['permission']['section']) && in_array('174',$users_data['permission']['section'])) { ?>
  ...
  <div class="row m-b-5" id="doctor_div" <?php if($form_data['referred_by']==0){  }else{ ?> style="display: none;" <?php  } ?>>
      ...
      <div class="col-sm-8">
        <select name="referral_doctor" class="m_select_btn" id="refered_id" onChange="check_doctor_type(this.value); get_profile_test(); get_others(this.value);" style="width: 146px;">
    ...
    <div class="row m-b-5">
      ...
      <div class="col-sm-8">
        <select name="sample_collected_by" class="m_input_default">
                <option value=""> Select </option>
                <?php
    ...
    <div class="row m-b-5">
      ...
      <div class="col-sm-8">
        <select name="staff_refrenace_id" class="m_input_default">
...
<div class="more_content" id="patient_info" style="display: none;"> 
 ...
 <sup class="info"><a href="javascript:void(null)" class="small info"> ?<span>DEFINITION of 'SEC Form F-1' SEC Form F-1 is a filing with the Securities and Exchange Commission (SEC) required for the registration of certain securities by foreign issuers. SEC Form F-1 is required to register securities issued by foreign issuers for which no other specialized form exists or is authorized .</span>
    ...
    <div class="row m-b-5">
    <div class="col-sm-4">
    <label>Panel Type  <span class="star">*</span>
  ...
  <div class="row m-b-5">
      <div class="col-sm-4">
      <label>TPA ID <span class="star">*</span></label>
      </div>
        <div class="col-md-8">
      ...
      <div class="row m-b-5">
        <div class="col-sm-4">
        <label>Insurance Amount. <span class="star">*</span></label>
        </div>
        <div class="col-md-8">
      ...
      <div class="row m-b-5">
        <div class="col-sm-4">
        <label>Authorization No.<span class="star">*</span></label>
        </div>
        <div class="col-md-8">
  ...
  <div class="row">
	 ...
	 <div class="col-md-12">
        ...
        <div class="bx-right">
            <?php
              $profile = $this->session->userdata('invoice_set_profile'); 
  ...
  <div class="box">
    ...
    <div class="bk-tst-dtl">
      <table class="pb-tbl5" id="test_select">
        ...
        $booked_arr = $this->session->userdata('invoice_book_test');
          ...
          <div class="col-md-11">
            ...
            <div class="row">
              ...
              <div class="col-md-4 p-r-0">
                ...
                <input type="hidden" name="payment_mode" value="<?php echo $payment_mode[0]->id;?>">
            ...
            ?>
                ...
                <div class="row m-b-5">
                    ...
                    <div class="col-md-7">
						...
						<a href="<?php echo base_url('pathology_invoice');?>" class="btn-anchor"><i class="fa fa-sign-out"></i> Exit</a>
                    </div>
                </div>
...
function check_panel_type(panel_val)
{
    ...
    $.ajax({
          type:"POST",
          url: "<?php echo base_url(); ?>pathology_invoice/check_test_panel_type/",
          data: 'insurance_type_id='+insurance_type_id+'&panel_type='+panel_type+'&ins_company_id='+ins_company_id+'&polocy_no='+polocy_no+'&tpa_id='+tpa_id+'&ins_amount='+ins_amount+'&ins_authorization_no='+ins_authorization_no, 
            ...
            test_head = 0;
             ...
             profile_id = 0;
             $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/test_list/"+test_head+"/"+profile_id, 
              success: function(result)
              { 

                 $('#test_list').html(result);
                 $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/reset_booked_test/",
                    success: function(result)
                    { 
...
$(document).ready(function() {
  ...
  function payment_function(value,error_field){
           $('#updated_payment_detail').html('');
                $.ajax({
                     type: "POST",
                     url: "<?php echo base_url('pathology_invoice/get_payment_mode_data')?>",
...
{
   $.ajax({
      url: "<?php echo base_url(); ?>pathology_invoice/check_doctor_type/2/"+did, 
      success: function(response)
      {  
             var test_head = $('#dept_parent_test').val(); 
             if(test_head=="")
...
function set_branch(bid)
{
  if(bid>0)
  {
     $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/booking_set_branch/"+bid, 
      success: function(result)
      {
  ...
  {  
     ...
     $.ajax({
         type:'POST',
      url: "<?php echo base_url(); ?>pathology_invoice/set_profile/"+profile_id, 
      ...
      success: function(result)
      {
         ...
         {
          $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/list_booked_test/",
                success: function(result)
                { 
                   $('#test_select').html(result);
                   payment_calc();
...
$('#test_search').keyup(delay(function (e) {
 ...
 function test_search_list()
 {
      ...
      var url ="<?php echo base_url(); ?>pathology_invoice/test_list";
      $.post(url,
  ...
  $('#dept_id').change(function(){
      ...
      $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/dept_test_heads/"+dept_id, 
        success: function(result)
        { 
  ...
  $('#dept_parent_test').change(function(){  
      ...
      $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/test_list/"+head_id, 
        success: function(result)
        {
           $('#test_list').html(result); 
        } 
 ...
 function doctor_modal()
  ...
  { 
     ...
     ///// Profile Price
      $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/profile_price/"+profile_id+'/'+panel_id,  
        success: function(result)
        {
  ...
  {     
   if(allVals!="")
   {
      $.ajax({
              type: "POST",
              url: "<?php echo base_url('pathology_invoice/remove_booked_test');?>",
  ...
  {     
    ...
    $.ajax({
      url: "<?php echo base_url('pathology_invoice/set_booking_test'); ?>",
      type: "post",
  ...
  function payment_calc(vals)
  {
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>pathology_invoice/payment_calc/"+vals, 
            dataType: "json",
...
<script type="text/javascript">
  ...
  {   
     ...
     if(allVals!="")
     {
        $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/set_prescription_booking_test/"+allVals, 
        success: function(result)
        {
           $('#test_select').html(result); 
           payment_calc();
...
<script type="text/javascript">
 ...
 if(isset($flash_success) && !empty($flash_success))
 {
  ...
  $test_book_ids = $this->session->userdata('invoice_test_booking_id');
 ?>
...
$('document').ready(function(){
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && isset($test_book_ids)){ ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('pathology_invoice/booking');?>'; 
    }); 
 ...
 function get_patient_detail_by_mobile() {
  var val = $('#mobile_no').val();
   if(val.length==10)
   {
    ...
    $.ajax({
      url: "<?php echo site_url('pathology_invoice/get_patient_detail_no_mobile'); ?>/"+val,
      type: 'POST',
      async : false,
  ...
  //neha 25-2-2019
    $(document).ready(function(){
        ...
        $("#proceed").click(function(){
            ...
            {
              var action_url = '<?php echo site_url('pathology_invoice/booking/'); ?>'
              var radioValue = $("input[name='patient_id']:checked").val();
               $.ajax({
                    url: "<?php echo site_url('pathology_invoice/get_patient_detail_byid'); ?>/"+radioValue,
                    type: 'POST',
                    async : false,
                   success: function(datas) {
                   var data = $.parseJSON(datas);
     ...
     ///// Profile Price
      $.ajax({url: "<?php echo base_url(); ?>pathology_invoice/profile_price/"+profile_id+'/'+panel_id,  
        ...
        { 
            $.ajax({
                    type:'POST',
                    url: "<?php echo base_url(); ?>pathology_invoice/set_profile/"+<?php echo $profile["profile_id"];?>, 
...
if(isset($_GET['lid']) && !empty($_GET['lid']) && $_GET['lid']>0 && !empty($lead_test_list))
{
  ...
  ?>
      $.ajax({
                url: "<?php echo base_url('pathology_invoice/set_booking_test2/').$test->id.'/'.$test->rate; ?>",
                type: "post",
                data: $('#booking_form').serialize(),
                success: function(result) 
                {  
