...
?>
 ...
 array_sort_by_column($running_bill_data['CHARGES'], 'start_date');
            ...
            foreach($running_bill_data['CHARGES'] as $running_data)
              {
                if(!empty($running_data['room_category']))
                {
                  $room_category = ' ('.$running_data['room_category'].')';
                }
                else
                {
                    $room_category = '';
            ...
            <tr>
              <td><?php echo $i; ?></td>
              <td><?php echo $running_data['particular'].$room_category.$doctor_name;?></td>
   ...
   <?php /* $charge_entry_amount=0;
            ...
            foreach($running_bill_data['pathology_payment'] as $payment)
              {?>
                ...
                <td><button type ="button" class="btn-custom" id="deleteAll" onclick="return delete_test_charges('<?php echo $payment->id;?>');">
              <i class="fa fa-trash"></i> Delete
             </button></td>
                </tr>
            <?php $i++;} 
...
$modal.modal('show');
 ...
 {    
  ...
  function delete_test_charges(id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('ipd_running_bill/delete_test/'); ?>"+id, 
                 success: function(result)
                 {
