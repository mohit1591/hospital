      ...
      return false;
       ...
       function payment_function(value,error_field){
           $('#updated_payment_detail').html('');
                $.ajax({
                     type: "POST",
                     url: "<?php echo base_url('ambulance/enquiry/get_payment_mode_data')?>",
...
function payment_calc(obj)
{
    ...
    $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>ambulance/enquiry/calculate_payment/", 
            dataType: "json",
...
$("#confirm_booking").on("submit", function(event) {  
  ...
  { 
        ...
        $.ajax({
                url: "<?php echo base_url('ambulance/enquiry/confirm_booking/'); ?>"+ids,
                type: "post",
                data: $(this).serialize(),
                success: function(result) {
                  if(result==1)
                  {
                    var msg = 'Payment successfully updated.';
                    $('#load_add_modal_popup').modal('hide');
                    flash_session_msg(msg); 
                    window.location ="<?php echo base_url('ambulance/enquiry'); ?>";
                  } 
                  else
                  {
