...
<div class="overlay-loader" id="overlay-loader">
  ...
  <form  id="dialysis_pacakge_management_form" class="form-inline">
  <input type="hidden" name="data_id" id="type_id" value="<?php echo $form_data['data_id']; ?>" /> 
      ...
      <div class="modal-body">   
          <div class="row">
            <div class="col-md-12 m-b1">
              ...
              <div class="row m-b-5">
                  ...
                  <div class="col-xs-5">
                      ...
                      <div class="col-xs-7">
                        <select name="dialysis_type_id" id="dialysis_type_id" class="m10 w-145px">
                          <option value=""> Select Dialysis Type</option>
                          <?php
                          if(!empty($dialysis_type_list))
                          {
                            foreach($dialysis_type_list as $ot_type)
                            {

                             ?>
                               <option value="<?php echo $ot_type->id; ?>" <?php if($ot_type->id==$form_data['dialysis_type_id']){ echo 'selected="selected"'; } ?>><?php echo $ot_type->dialysis_type; ?></option>
                             <?php  
                            }
                          }
                          ?>
              ...
              <div class="row m-b-5">
                  <div class="col-md-5"> <label>Dialysis Details<span class="star">*</span></label></div>
           ...
           <div class="row m-b-5">
                <div class="col-md-5">
                    <label>Dialysis Procedure</label>
                </div>
                <div class="col-md-7">
...
{
  $.ajax({
    url: "<?php echo base_url('dialysis_pacakge_management/set_field_valid/'); ?>"+vals+'/'+i, 
...
$(function () {
    var getData = function (request, response) { 
        $.getJSON(
            "<?php echo base_url('dialysis_pacakge_management/get_vals/'); ?>" + request.term,
            function (data) {
                response(data);
            });
    };
...
//alert(names[1]);
       $('#dialysis_pacakge_management_types').val(names[0]);
          $('#ot_type_id').val(names[0]);

       // $("#dialysis_pacakge_management_types").val(ui.item.value);
        return false;
    }

    $("#dialysis_pacakge_management_types").autocomplete({ 
        source: getData,
        select: selectItem,
        minLength: 1,
        change: function() {  
...
$("#dialysis_pacakge_management_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('dialysis_pacakge_management/'); ?>"+path,
    type: "post",
    data: $(this).serialize(),
    success: function(result) {
      if(result==1)
      {
        $('#load_add_dialysis_pacakge_management_modal_popup').modal('hide');
        flash_session_msg(msg);
        get_dialysis_pacakge_management();
        reload_table();
      } 
      else
      {
        $("#load_add_dialysis_pacakge_management_modal_popup").html(result);
      }       
      $('#overlay-loader').hide();
    }
  });
...
function get_dialysis_type()
{
   $.ajax({url: "<?php echo base_url(); ?>dialysis_type/dialysis_type_dropdown/", 
    success: function(result)
    {
      $('#dialysis_type_id').html(result); 
    } 
  });
}
function get_dialysis_pacakge_management()
{
   $.ajax({url: "<?php echo base_url(); ?>dialysis_pacakge_management/ot_summary_dropdown/", 
    success: function(result)
    {
      $('#dialysis_pacakge_management_id').html(result); 
    } 
  });
}
