...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('ipd_prescription/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'ipd_prescription/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
$modal.load('<?php echo base_url().'ipd_prescription/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
    ...
    <form>
      ...
      <div class="fixed">
  		<div class="btns">
               ...
               <?php if(in_array('121',$users_data['permission']['section'])){
                ?>
  			     <button class="btn-exit" onclick="window.location.href='<?php echo base_url('ipd_prescription/archive'); ?>'">
...
function consolidate_model()
{
     ...
     $.ajax({
         url: "<?php echo base_url('ipd_prescription/check_patient_ids/'); ?>"+allVals, 
         dataType: "json",
         success: function(result)
         {
             ...
             {
                window.open('<?php echo base_url('ipd_prescription/print_consolidate_prescription?') ?>pres_ids='+allVals,'mywin','width=800,height=600');  
             }
             else
             {
...
{ 
  ...
  $.ajax({url: "<?php echo base_url(); ?>ipd_prescription/reset_search/", 
    success: function(result)
    { 
...
$("#search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('ipd_prescription/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    { 
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('ipd_prescription');?>'; 
    }); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("ipd_prescription/print_prescriptions"); ?>');">Print</a>
