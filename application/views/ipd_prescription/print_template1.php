...
$booking_date='';
      ...
      <div style="float:right;width:32%;min-height:100px;">
        ...
        <?php 
         ...
         $booking_time ='';
    if(!empty($all_detail['prescription_list'][0]->admission_time) && $all_detail['prescription_list'][0]->admission_time!='00:00:00' && strtotime($all_detail['prescription_list'][0]->admission_time)>0)
    {
        $booking_time = date('h:i A', strtotime($all_detail['prescription_list'][0]->admission_time));    
    }
    ...
    <table  width="100%" cellpadding="0" cellspacing="0" border="" style="border-collapse:collapse;">
      <tr>
        ...
        if(!empty($vitals_list))
        {
          ...
          {
            $vital_val = get_vitals_value($vitals->id,$id,6);
            ?>
