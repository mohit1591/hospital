...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('sms_template/ajax_list')?>",
            "type": "POST",
...
function edit_sms_template(id)
{
  ...
  var $modal = $('#load_add_elem_temp_modal_popup');
  $modal.load('<?php echo base_url().'sms_template/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('sms_template/deleteall');?>",
                      data: {row_id: allVals},
...
<script>  
 ...
 function delete_sms_template(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('sms_template/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
<div id="load_add_elem_temp_modal_popup" class="modal-45 modal fade " role="dialog" data-backdrop="static" data-keyboard="false"></div>
