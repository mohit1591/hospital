...
<div class="overlay-loader" id="overlay-loader">
  ...
  <form  id="ot_detail" class="form-inline">
  <input type="hidden" name="data_id" id="type_id" value="<?php echo $form_data['data_id']; ?>" /> 
    <input type="hidden" name="ot_mgt_id" id="ot_mgt_id" value="<?php echo $form_data['ot_mgt_id']; ?>" />
    <input type="hidden" name="ot_amount" id="ot_amount" value="<?php echo $form_data['ot_amount']; ?>" /> 
            ...
            <div class="col-md-12 m-b1">
             ...
             <div class="col-md-5">
                ...
                <div id="particular_div_0" style="display:none">
                    <div class="col-md-7">
                    <input type="text" name="doctor_wise[0][code_p][]" value="" id="" class="w-90px code" placeholder="Code"/>
...
function check_type(divsize,type_data)
{
    ...
    if(type_data==1)
    {
       ...
       else
       {
        ...
        $('#particular_div_'+divsize).html('<div class="col-md-7"><input type="text" name="doctor_wise['+divsize+'][code_p][]" value="" id="code_'+divsize+'" class="w-90px code" placeholder="Code"/> <input type="text" name="doctor_wise['+divsize+'][particular_name][]" value="" id="particular_name_'+divsize+'" class="w-90px particular_name" placeholder="Name"/><input type="hidden" name="doctor_wise['+divsize+'][particular_id][]" id="particular_id_'+divsize+'" value=""/> <select name="doctor_wise['+divsize+'][master_type][]" class="w-50px master_type"><option value="0" > Rs </option><option value="1" > % </option></select> <input type="text" name="doctor_wise['+divsize+'][master_rate][]"  class="w-50px rate_plan" value=""> <a href="javascript:void(0)" class="btn-new" onclick="'+onclick_event+'"><i class="fa '+css_c+'"></i></a></div>');
...
$(document).ready(function() {
  $('#load_add_ot_detail_modal_popup').on('shown.bs.modal', function(e){
...
$("#ot_detail").on("submit", function(event) { 
  event.preventDefault(); 
  //////// Check Validtion
  var pack_amount = $('#ot_amount').val();
   if(pack_amount!="")
   {
       var total_val = 0;
  ...
  { 
    ...
    var msg = 'Operation successfully updated.';
  }
  else
  {
    alert('add');
    var path = 'add/';
    var msg = 'Operation  successfully created.';
  } 
  //alert('ddd');return false;
  $.ajax({
    url: "<?php echo base_url('ot_detail/'); ?>"+path,
    type: "post",
    data: $(this).serialize(),
    success: function(result) {
      if(result==1)
      {
        $('#load_add_ot_detail_modal_popup').modal('hide');
      ...
      {
        $("#load_add_ot_detail_modal_popup").html(result);
      }       
      $('#overlay-loader').hide();
    }
  });
