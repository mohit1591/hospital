...
<form id="ipd_admission_print_setting_form">
  ...
  <div class="row">
    ...
    <?php
      ...
      {
        if(!empty($ipd_running_bill_print_setting_list))
        {
          foreach($ipd_running_bill_print_setting_list as $setting_list)
          {
    ...
    ?>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          <div class="col-xs-3">
            <label>IPD Running Bill Print Setting</label>
          </div>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-9 p-l-0 p-r-0">
             <textarea cols="45"  class="print_textarea" readonly="">{patient_reg_no},{patient_name},{patient_age},{patient_address},{ipd_no},{mobile_no},{booking_date},{doctor_name},{room_type},{bed_no},{mlc},{remarks}, {signature} </textarea>
        </div>
     </div>
     </div>
     </div>
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>ipd_running_bill_print_setting/",
    type: "post",
    data: $(this).serialize(),
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'ipd_running_bill_print_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
