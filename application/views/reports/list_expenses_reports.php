...
<page size="A4">
    ...
    <div style="float:left; margin-top:10px; width:100%;border:1px solid #111;border-top:none;font-size:13px;">       
          ...
          <div style="float:left;width:100%;border:1px solid #111;border-left:none;border-right:none;font-size:13px;">
            ...
            <!--<div style="float:left;width:15%;font-weight:600;padding:4px;"><u>Exp. Category</u></div> -->
            
            
            <div style="float:left;width:10%;font-weight:600;padding:4px;"><u>Paid To</u></div> 
            
            <div style="float:left;width:20%;font-weight:600;padding:4px;"><u>Remarks</u></div> 
     ...
     if(!empty($all_expense_list['expense_list']))
     {
        ...
        foreach($all_expense_list['expense_list'] as $expenses)
        {
            ...
            if($expenses->new_type>0)
            {
              $expenses_type = $expenses->expenses_type;
            }
          ?> 
          <div style="float:left;width:100%; min-height: 35px;  margin-bottom: 3px;">
              <div style="float:left;width:15%;padding:1px 4px;"><?php echo date('d-m-Y',strtotime($expenses->expenses_date)); ?></div>
              <div style="float:left;width:15%;padding:1px 4px;"><?php echo $expenses->vouchar_no; ?>  </div>
              <div style="float:left;width:15%;padding:1px 4px;"><?php echo $expenses_type.' '. $expenses->patient_name.' '.$expenses->book_code; ?></div> 
              <!--<div style="float:left;width:15%;padding:1px 4px;">< ?php echo $expenses_type.' '.$expenses->emp_names; 
                  if(!empty(trim($expenses->remarks)))
                   {
                       if($users_data['parent_id']=='156')
                       {
                          echo ' ('.substr($expenses->remarks,0,50).')'; 
                       }
                       else
                       {
              ...
              <div style="float:left;width:10%;padding:1px 4px;"><?php echo $expenses->paid_to_name; ?></div>
              
              <div style="float:left;width:20%;padding:1px 4px;"><?php echo $expenses->remarks; ?></div>
              
              <div style="float:left;width:10%;padding:1px 4px;">
              <?php 
              ...
              <div style="float:left;width:15%;padding:1px 4px;text-align:right;"><?php echo number_format($expenses->paid_amount,2); ?></div>
            </div> 
  <?php
      $i++;
      }
