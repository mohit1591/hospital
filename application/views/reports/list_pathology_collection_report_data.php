...
<body>
	...
	<page size="A4" style="font:13px Arial;">
        ...
        $data_empty = 0;
        if(!empty($pathology_doctor_collection_list))
        {
        	$data_empty = 1;
        ?>
		...
		<div style="float:left;width:100%;border:1px solid #111;border-top:none;font-size:13px;">	
				...
				<?php
                $j=1;
                $doctors_counter = count($pathology_doctor_collection_list);
                $doctor_total = '0.00';
                foreach($pathology_doctor_collection_list as $doctors)
                {
		...
		<!-- Doctor list end -->
				...
				<?php
                if(!empty($pathology_self_collection_list) || !empty($pathology_branch_collection_list))
                {
                 $data_empty = 1;
                ?>
		        ...
		        {  
		        	$data_empty = 1;
		          $branch_names = [];	
		          foreach($pathology_branch_collection_list as $names)
		          {
