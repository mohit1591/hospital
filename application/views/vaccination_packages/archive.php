...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('vaccination_packages/archive_ajax_list')?>",
            "type": "POST",
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('vaccination_packages/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered packages_archive" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
  		<div class="btns"> 
          <?php if(in_array('1065',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('vaccination_packages'); ?>'">
...
?>
<script>  
 function restore_packages(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccination_packages/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccination_packages/trash/'); ?>"+Vendor_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('vaccination_packages/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
