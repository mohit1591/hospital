...
var table;
<?php
if(in_array('1067',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
  form_submit();
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('vaccination_packages/vaccination_kit_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'vaccination_packages/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function allbranch_delete(allVals)
 ...
 function reset_search()
  { 
    ...
    $.ajax({url: "<?php echo base_url('vaccination_packages/reset_search');?>", 
      success: function(result)
      { 
        reload_table();
        $("#sub_branch_id").val("");
...
<section class="userlist">
    ...
    <form>
       <?php if(in_array('1067',$users_data['permission']['action'])) {
       ?>
      ...
      <?php if(in_array('587',$users_data['permission']['action'])) { ?>
             ...
             //if(in_array('582',$users_data['permission']['action'])) {
               ...
               ?>
                <a href="<?php echo base_url('vaccination_packages/vaccination_kit_stock_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>
                <?php //} ?>
               <?php //if(in_array('579',$users_data['permission']['action'])) {
               ?>
                    <a href="<?php echo base_url('vaccination_packages/vaccination_kit_stock_csv'); ?>" class="btn-anchor m-b-2">
                         <i class="fa fa-file-word-o"></i> CSV
                    </a>
               <?php //} ?>
               <?php //if(in_array('580',$users_data['permission']['action'])) {
               ?>
                    <a href="<?php echo base_url('vaccination_packages/pdf_vaccination_kit_stock'); ?>" class="btn-anchor m-b-2">
                    <i class="fa fa-file-pdf-o"></i> PDF
                    </a>
               <?php // } ?>
               <?php //if(in_array('581',$users_data['permission']['action'])) {
               ?>       
                    <a href="javascript:void(0)" class="btn-anchor m-b-2" id="deleteAll" onClick="return print_window_page('<?php echo base_url("vaccination_packages/print_vaccination_kit_stock"); ?>');">
 ...
 function load_allot_model()
{
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccination_packages/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
function package_quantity_add(id){
     ...
     $modal.load('<?php echo base_url().'vaccination_packages/add_medicine_kit_quantity/' ?>'+id+'/'+opt,
     {
          //'id1': '1',
          //'id2': '2'
    },
...
$("#new_search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url(); ?>vaccination_packages/search_vaccination_kit_stock_data/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    { 
...
function packages_manage(id){
     window.location.href="<?php echo base_url(); ?>vaccination_packages/add_vaccination_kit_qty_manage/"+id;
}
