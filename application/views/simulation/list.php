...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('simulation/ajax_list')?>",
            "type": "POST",
...
function edit_simulation(id)
{
  var $modal = $('#load_add_simulation_modal_popup');
  $modal.load('<?php echo base_url().'simulation/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('simulation/deleteall');?>",
                      data: {row_id: allVals},
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('simulation/archive'); ?>'">
...
<script>  
 ...
 function delete_simulation(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('simulation/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
