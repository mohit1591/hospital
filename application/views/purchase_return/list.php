...
{
  window.location.href='<?php echo base_url().'purchase_return/edit/';?>'+id
}

function view_medicine_entry(id)
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'purchase_return/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('purchase_return/deleteall');?>",
                      data: {row_id: allVals},
 ...
 <form id="new_search_form">
      <table class="ptl_tbl">
        <tr>
            ...
            <input type="text" name="start_date" id="start_date_p" class="datepicker start_datepicker"  onkeyup="return form_submit();" value="<?php echo $form_data['start_date'];?>"></td>
          <td><label>To Date</label>
            <input type="text" name="end_date" id="end_date_p" class="datepicker end_datepicker"  onkeyup="return form_submit();" value="<?php echo $form_data['end_date'];?>"></td>
    ...
    <form>
  	...
  	<div class="userlist-right ">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('vendor'); ?>'">
              ...
              <a href="<?php echo base_url('purchase_return/medicine_purchase_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>

                <a href="<?php echo base_url('purchase_return/medicine_purchase_csv'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-word-o"></i> CSV
                </a>

                <a href="<?php echo base_url('purchase_return/pdf_medicine_purchase'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-pdf-o"></i> PDF
                </a>
              <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("purchase_return/print_medicine_purchase"); ?>');"> <i class="fa fa-print"></i> Print</a>
               ...
               <?php } ?>
               <?php if(in_array('396',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-exit" onclick="window.location.href='<?php echo base_url('purchase_return/archive'); ?>'">
...
function reset_search()
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('purchase_return/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
<div id="confirm_print" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <!-- <div class="modal-body"></div> -->
          <div class="modal-footer">
            <!-- <a type="button" data-dismiss="modal" class="btn-anchor" id="delete" onClick="return openPrintWindow('< ?php echo base_url("purchase_return/print_purchase_return_recipt"); ?>', 'windowTitle', 'width=820,height=600');" target="_blank">Print</a> -->
...
$modal.load('<?php echo base_url().'purchase_return/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('purchase_return/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('purchase_return/total_calc_return');?>",
                      success: function(result) 
                      {
