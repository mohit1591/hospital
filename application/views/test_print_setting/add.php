...
<div class="row">
  <div class="col-md-12 add_test_print_setting">
    <?php if(in_array('902',$users_data['permission']['action'])) {?>
    <form action="<?php echo base_url('test_print_setting/add'); ?>" method="post">
    <div class="row m-b-5">
      <div class="col-xs-2">
    ...
    <div class="row m-b-5">
      ...
      <?php echo $form_data['short_code'];?> , {patient_category_name} , {authorize_person_name}</textarea>
      </div>
    </div> <!-- rowClose -->
    ...
    <div class="row m-b-5">
      ...
      <div class="col-xs-10">
       <button class="btn-save" type="submit" name="remove_levels" onClick=" validateForm();" ><i class="fa fa-floppy-o"></i> Save</button>
              <a class="btn-anchor" onclick="window.location.href='<?php echo base_url(); ?>test'"><i class="fa fa-sign-out"></i> Exit</a>
      </div>
    </div> <!-- rowClose -->
...
<script>
  function get_template_according(value){
    ...
    $.ajax({
      type:"POST",
      url: "<?php echo base_url(); ?>test_print_setting/test_printtemplate_dropdown/", 
      data: {value: value},
      datatype:'JSON',
