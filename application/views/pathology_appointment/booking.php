...
<div class="row">
  <div class="col-md-4">
    ...
    <div class="row m-b-5">
      ...
      <div class="col-sm-8">
        <input type="text" readonly="" class="m_input_default" id="lab_reg_no" name="lab_reg_no" value="<?php echo $form_data['lab_reg_no']; ?>"/> 
      </div>  
    </div>
    ...
    <div class="row m-b-5">
      ...
      <div class="col-sm-8">
        <select class="mr m_mr" name="simulation_id" id="simulation_id" onChange="find_gender(this.value)"> 
              ...
              <input type="text" name="patient_name"  id="patient_name" value="<?php echo $form_data['patient_name']; ?>" class="w-134px m_name txt_firstCap" style="width: 134px;margin-left: 5px;" autofocus=""/>
  ...
  <div class="row m-b-5" id="doctor_div" <?php if($form_data['referred_by']==0){  }else{ ?> style="display: none;" <?php  } ?>>
      ...
      <div class="col-sm-8">
        <select name="referral_doctor" class="m_select_btn" id="refered_id" onChange="check_doctor_type(this.value); get_profile_test(); get_others(this.value);" style="width: 146px;">
               ...
               ?>
              <a href="javascript:void(0)" onClick="doctor_modal()" class="btn-new"><i class="fa fa-plus"></i> New</a>
              <?php
              }
              ?>
    ...
    <div class="row m-b-5" id="ref_by_other" <?php if(!empty($form_data['ref_by_other']) && $form_data['referral_doctor']=='0'){ }else{ ?> style="display: none;width: 146px;" <?php } ?>>
    ...
    <div class="row m-b-5" id="hospital_div" <?php if($form_data['referred_by']==1){  }else{ ?> style="display: none;width: 146px;" <?php  } ?>>
      <div class="col-sm-4">
        <label>Referred By Hospital <span class="star">*</span></label>
      </div>  
      <div class="col-sm-8">
        <select name="referral_hospital" class="m_select_btn" id="referral_hospital">
              ...
              <a href="javascript:void(0)" onClick="hospital_add_modal()" class="btn-new"><i class="fa fa-plus"></i> New</a> 
   ...
   <div class="col-md-4">
    ...
    <div class="col-md-12">
       <div class="row">
         <div class="col-md-4" style="text-align: left;"> <a href="javascript:void(0);" onclick="more_patient_info()">More Info</a></div>
         <div class="col-md-8" >
...
<?php // echo "<pre>"; print_r($form_data); ?>
  ...
  <div class="row">
     ...
     <?php $edit_url=$this->uri->segment(2);
    // $edit_id=$this->uri->segment(3);!empty($edit_id) || $edit_id!=''
     if($edit_url=='edit_booking'){ $lab_dis="disabled";}else { $lab_dis="";}
      ?>
	 <div class="col-md-12">
         <div class="col-md-3 p-l-0">
             <label>Department </label>
             <select name="dept_id" <?php echo $lab_dis; ?> id="dept_id" class="m_input_default">
                ...
                <?php if($edit_url=='edit_booking'){?>
                  <input type="hidden" name="dept_id" value="<?php echo $form_data['dept_id'];?>">
                <?php }?>
              <?php if(!empty($form_error)){ echo form_error('dept_id'); } ?>
         </div>

         <div class="col-md-5 p-l-1in">
             <label>Search </label>
             <input type="text" class="m_input_default" name="test_search" id="test_search" value=""> 
         </div> 
         
         <div class="col-md-4 p-l-1in2">
              <?php
              $profile = $this->session->userdata('set_profile'); 
              $profile_print_status = get_profile_print_status('test_booking');
              ?>
              <label>Profile </label>
             <select name="profile_id" class="m_input_default" id="profile_id" onChange="get_profile_test();">
...
$profile_status = $profile_print_status->profile_status;
             ...
             &nbsp;&nbsp;<a class="btn-new" onClick="add_profile();">
              <i class="fa fa-plus"></i> Add
            </a>   
              <div class="text-danger" id="profile_error" style="margin-left:4em;"></div>
         </div> 
          ...
          foreach($profile_data as $profile_dat)
          {
             $profile_row .= '<tr>
                              <td width="40" align="center">
                                 <input type="checkbox" name="test_id[]" class="booked_checkbox" value="p_'.$profile_dat['id'].'" >
                              </td>
          ...
          <?php 
              ...
              ?>
                  <div class="row m-b-5">
                    ...
                    <div class="col-md-7">
                    <input type="text" name="home_collection_amount" id="home_collection_amount" value="<?php echo $home_collection[0]->charge; ?>"  onkeyup=payment_calc(0);>
                    </div>
                  </div>
              <?php
                }
...
function set_branch(bid)
{
 ...
 function simulation_modal()
  ...
  $("#test_search").keyup(function(){ 
     ...
     /* $.ajax({
               type: "POST",
               url: "<?php echo base_url(); ?>test/test_list/"+test_head_id+"/0/"+search_text+"/"+dept_id, 
               success: function(msg)
               {
                  $("#test_list").html(msg);
               }
  ...
  $('#dept_id').change(function(){
      ...
      test_list_vals_other();
      var url_bookid="<?php echo $this->uri->segment(3); ?>";
	  
	  //$edit_id=$this->uri->segment(3);!empty($edit_id) || $edit_id!=''
     //if($edit_url=='edit_booking'){ $lab_dis="disabled";}else { $lab_dis="";}
     if( url_bookid != 'edit_booking')
     {
        $.ajax({url: "<?php echo base_url(); ?>test/dept_ref_no/"+dept_id, 
          success: function(result)
          { 
             $('#lab_reg_no').val(result);  
          } 
        }); 
      }
            ...
            {  //alert(result.balance);
               ...
               <?php 
                if(isset($form_data['ipd_id']) && !empty($form_data['ipd_id']))
                  ...
                  {
                    //echo "$('#paid_amount').val(result.paid_amount); ";
                  } 
               ?>
