...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('banking/ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_banking_modal_popup');
  $modal.load('<?php echo base_url().'banking/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('banking/delete/'); ?>"+rate_id, 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('banking/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
...
$("#bank_search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url(); ?>banking/advance_search/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
...
<section class="userlist">
    ...
    <form>
      ...
      <?php
       ...
       <?php if(in_array('619',$users_data['permission']['action'])) { ?>
        <a href="<?php echo base_url('banking/banking_excel'); ?>" class="btn-anchor m-b-2">
        <i class="fa fa-file-excel-o"></i> Excel
        </a>
        <?php }  if(in_array('620',$users_data['permission']['action'])) { ?>
        <a href="<?php echo base_url('banking/banking_csv'); ?>" class="btn-anchor m-b-2">
        <i class="fa fa-file-word-o"></i> CSV
        </a>
        <?php }  if(in_array('621',$users_data['permission']['action'])) { ?>
        <a href="<?php echo base_url('banking/pdf_banking'); ?>" class="btn-anchor m-b-2">
    ...
    <?php }  if(in_array('615',$users_data['permission']['action'])) 
      ...
      {
      ?>
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('banking/archive'); ?>'">
