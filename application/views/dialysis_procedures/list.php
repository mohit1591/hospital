...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dialysis_procedures/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'dialysis_procedures/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
$modal.load('<?php echo base_url().'dialysis_procedures/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'dialysis_procedures/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'dialysis_procedures/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function allbranch_delete(allVals)
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dialysis_procedures/deleteall');?>",
                      data: {row_id: allVals},
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'dialysis_procedures/upload_prescription/' ?>'+id,
    {
      //'id1': '1',
      //'id2': '2'
      },
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'dialysis_procedures/view_files/' ?>'+prescription_id,
    {
      //'id1': '1',
      //'id2': '2'
      },
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'dialysis_procedures/share_video_link/' ?>'+id+'/'+booking_id,
    {
    },
    function(){
    $modal.modal('show');
...
{ 
  ...
  $.ajax({url: "<?php echo base_url(); ?>dialysis_procedures/reset_search/", 
    success: function(result)
    { 
...
$("#search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('dialysis_procedures/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    { 
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dialysis_procedures/delete/'); ?>"+prescription_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('dialysis_procedures');?>'; 
    }); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("dialysis_procedures/print_prescriptions"); ?>');">Print</a>
