...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('item_opening_stock/ajax_list')?>",
            "type": "POST",
...
function edit_item_manage(id)
{
  ...
  var $modal = $('#load_add_item_manage_modal_popup');
  $modal.load('<?php echo base_url().'item_opening_stock/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_item_manage_modal_popup');
  $modal.load('<?php echo base_url().'item_opening_stock/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            ...
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('item_opening_stock/deleteall');?>",
                      data: {row_id: allVals},
...
<!--    <div style="border: 1px solid #ccc; float: right; padding:5px; font-weight: bold;">
      ...
      <div class="fixed">
           ...
           ?>
             ...
             <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("item_opening_stock/print_item_manage_list"); ?>');"> <i class="fa fa-print"></i> Print</a>
             <?php }?>
            ...
            ?>
    			<button class="btn-update" onclick="window.location.href='<?php echo base_url('item_opening_stock/archive'); ?>'">
...
function select_item_data(item_type){
  ...
  $('#overlay-loader').show();
    ...
    $.ajax({
          url: "<?php echo base_url(); ?>item_opening_stock/advance_search/",
          type: "post",
          data: $(this).serialize(),
          success: function(result) 
          {
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('item_opening_stock/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{
  var $modal = $('#load_add_item_manage_modal_popup');
  $modal.load('<?php echo base_url().'item_opening_stock/view_item/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
