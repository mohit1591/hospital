...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('admissionnotes/examination/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('admissionnotes/examination/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       <?php //if(in_array('443',$users_data['permission']['action'])) {
        if(in_array('443',$permission_action) || in_array('121',$permission_section)){
        ?>
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered examination_archive" cellspacing="0" width="100%">
            <thead>
                <tr>
  	...
  	<div class="userlist-right">
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('examination'); ?>'">
...
?>
<script>  
 function restore_examination(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('admissionnotes/examination/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('admissionnotes/examination/trash/'); ?>"+examination_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('admissionnotes/examination/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
