...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('test_master/archive_ajax_list')?>",
            "type": "POST",
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('test_master/restoreall');?>",
                      data: {row_id: allVals},
...
<div class="col-md-12">
    ...
    <form> 
       ...
       <?php if(in_array('859',$users_data['permission']['action'])): ?>
             <table id="table" class="table table-striped table-bordered test_master_archive" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
  		<div class="btns"> 
          <?php if(in_array('861',$users_data['permission']['action'])): ?>
                  <button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
                    <i class="fa fa-window-restore" aria-hidden="true"></i> Restore
                  </button>
             <?php endif;?>
             <?php if(in_array('860',$users_data['permission']['action'])): ?>
            ...
            <?php endif;?>
                  <button class="btn-exit" onclick="window.location.href='<?php echo base_url('test_master'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('test_master/restore/'); ?>"+test_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('test_master/trash/'); ?>"+religion_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('test_master/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
