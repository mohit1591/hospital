...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('day_care/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'day_care/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
<?php 
  $checkbox_list = get_checkbox_coloumns('13');
 ...
 { ?>
    ...
    <div class="row">
      <div class="col-sm-4">
        ...
        <div class="row m-b-5">
          <div class="col-xs-5"><label>Day Care No.</label></div>
          <div class="col-xs-7">
      ...
      <div class="col-sm-4">
        ...
        <?php  
            ...
            $branch_list_id = implode(',', $ids); 
            $option= '<option value="'.$branch_list_id.'">All</option>';
            }
  	...
  	<div class="userlist-right relative">
        <div class="fixed">
  		<div class="btns opd_booking_list_right_btns">
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('day_care/booking'); ?>'">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
              

               <a href="<?php echo base_url('day_care/day_care_excel'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-excel-o"></i> Excel
              </a>

              <a href="<?php echo base_url('day_care/day_care_csv'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-word-o"></i> CSV
              </a>

              <a href="<?php echo base_url('day_care/day_care_pdf'); ?>" class="btn-anchor m-b-2">
              ...
              <?php if(in_array('2058',$users_data['permission']['action'])) 
               {
               ?>
                <a href="<?php echo base_url('day_care/sample_import_opd_excel'); ?>" class="btn-anchor m-b-2">
              ...
              <!-- <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return openPrintWindow('< ?php echo base_url("day_care/opd_print"); ?>', 'windowTitle', 'width=820,height=600');">
              <i class="fa fa-print"></i> Print
              </a> -->
              <a href="javascript:void(0)" class="btn-anchor m-b-2" onClick="return print_window_page('<?php echo base_url("day_care/day_care_print"); ?>');">
              <i class="fa fa-print"></i> Print
              </a> 
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('day_care/archive'); ?>'">
...
<script>
  ...
  $(document).ready(function()
  {
      var $modal = $('#load_opd_import_modal_popup');
        ...
        $modal.load('<?php echo base_url().'day_care/import_opd_excel' ?>',
        { 
...
function reset_search(ele) {
   $.ajax({url: "<?php echo base_url(); ?>day_care/reset_search/", 
      success: function(result)
      { 
...
{ 
  ...
  $.ajax({
         url: "<?php echo base_url('day_care/advance_search/'); ?>", 
         type: 'POST',
         data: { start_date: start_date, end_date : end_date, booking_code : booking_code, branch_id : branch_id, mobile_no : mobile_no, patient_name : patient_name } ,
         success: function(result)
         { 
            if(vals!="1")
            {
...
$('document').ready(function(){
 ...
 ?>
  ...
  {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'day_care/confirm_booking/' ?>'+id,
    {
      //'id1': '1',
      //'id2': '2'
      },
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('day_care/delete_booking/'); ?>"+booking_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer">
              ...
              if($users_data['parent_id']=='60')
                {
                    $day_booking_id = $this->session->userdata('day_booking_id');
                    ?>
                    <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("prescription/print_blank_prescriptions/".$day_booking_id."/".$users_data['parent_id']); ?>');">Print</a>
                    <?php
                }
                ...
                {
                    ?>
                    <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("day_care/print_booking_report"); ?>');">Print</a>
                    <?php 
                }
              ...
              ?>
                ...
                <button type="button" onclick="window.location.href='<?php echo base_url("day_care"); ?>'" data-dismiss="modal" class="btn-cancel" id="cancel">Close</button>
          </div>
        </div>
      </div>  
    </div> 
              ...
              if($users_data['parent_id']=='60')
                {
                    $day_booking_id = $this->session->userdata('day_booking_id');
                    ?>
                    <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("prescription/print_blank_prescriptions/".$day_booking_id."/".$users_data['parent_id']); ?>');">Print1</a>
                    <?php
                }
                elseif($users_data['parent_id']=='64')
                {
                  $day_booking_id = $this->session->userdata('day_booking_id');
                    ?>
                    <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("prescription/print_blank_prescriptions/".$day_booking_id."/".$users_data['parent_id']); ?>');">Print2</a>
                    <?php  
                }
                else
                {
                    ?>
                    <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("day_care/print_booking_report"); ?>');">Print 5</a>
                    <?php 
                }
              
              ?>
            <!-- <a  data-dismiss="modal" class="btn-anchor" id="delete" onClick="return openPrintWindow('< ?php echo base_url("day_care/print_booking_report"); ?>', 'windowTitle', 'width=820,height=600');" target="_blank">Print</a> -->
...
<script type="text/javascript">
     ...
     $("#checkbox_data_form").on("submit", function(event) { 
      ...
      var sList = [];
       ...
       $.ajax({
        url: "<?php echo base_url(); ?>day_care/checkbox_list_save",
        type: "POST",
        ...
        { 
            flash_session_msg(result); 
            setTimeout(function () {
            window.location = "<?php echo base_url(); ?>day_care";
        }, 1300); 
        }
      });
    }); 
     ...
     <?php
    // echo "<pre>"; print_r($unchecked_column); die;
 ...
 function reset_coloumn_record()
    {
        ...
        {
          $.ajax({
                url: '<?php echo base_url(); ?>day_care/reset_coloumn_record',
 ...
 function doctor_checked_status(opd_id,doctor_status)
{
     ...
     $.ajax({
      url: '<?php echo base_url(); ?>day_care/update_doctor_status/'+opd_id+'/'+doctor_status, 
          success: function(result)
          { 
            if(result==0)
              {
...
{
     $.ajax({
        url: "<?php echo base_url(); ?>day_care/dilated_start",
        type: "POST",
        data: { booked_id:id },
        success: function(result) 
        { 
