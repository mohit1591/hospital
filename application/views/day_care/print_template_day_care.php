...
//echo "<pre>";print_r($all_detail['opd_list'][0]);die();
...
$template_data->template = str_replace("{booking_code}",$all_detail['opd_list'][0]->booking_code,$template_data->template);
  ...
  if($all_detail['opd_list'][0]->address!='' || $all_detail['opd_list'][0]->address2!='' || $all_detail['opd_list'][0]->address3!='')
   {
    $address = $all_detail['opd_list'][0]->address.', '.$all_detail['opd_list'][0]->address2.', '.$all_detail['opd_list'][0]->address3;
   }
  ...
  if(!empty($address)){ $address;
   ...
   // $patient_address = $address.' - '.$pincode;
    ...
    {
        $template_data->template = str_replace("{booking_level}",'Day Care No. :',$template_data->template);
 ...
 $template_data->template = str_replace("{Quantity_level}",'',$template_data->template);
    ...
    {       
        ...
        $tr = str_replace("{particular}",'Registration Charges',$tr);
        $tr = str_replace("{quantity}","",$tr);
    ...
    $card_no='';
    if(!empty($all_detail['opd_list'][0]->card_no))
    {
      $card_no = '('. $all_detail['opd_list'][0]->card_no.')';
    }

     $template_data->template = str_replace("{payment_mode}",ucfirst($all_detail['opd_list'][0]->payment_mode).$card_no,$template_data->template);
    ...
    $this->session->unset_userdata('day_booking_id');
...
if($template_data->printer_id==3)
{
  ...
  if(!empty($address)){ $address;
   ...
   // $template_data->template = str_replace("{opd_reg_date}",date('d-m-Y',strtotime($all_detail['opd_list'][0]->booking_date)),$template_data->template);
    ...
    {
        $receipt_code = '<div><br><b>Day Care No.</b>'.$all_detail['opd_list'][0]->booking_code.'</div>';
  ...
  if(!empty($address)){ $address;
    ...
    if(!empty($all_detail['opd_list'][0]->booking_code))
    {
        $receipt_code = '<div style="width:100%;display:inline-flex;">
                        <div style="width:40%;line-height:19px;font-weight:600;">Day Care No.:</div>
