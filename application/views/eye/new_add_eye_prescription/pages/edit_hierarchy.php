				...
				<div class="well">
					<div class="row form-group">
						<div class="col-md-8">
							...
							<input type="hidden"  name="data_id" id="data_id" value="<?php echo $icd_detail['data_id']; ?>" >
						...
						<?php if($icd_dropdowns!=""){ ?>
							<div class="col-md-4" id="dropdownlist">
								<input type="hidden" class="form-control" id="eye_side_name" name="eye_side_name" value="<?php echo $icd_detail['eye_side_name']; ?>" readonly="">
								<select name="eye_side"  class="form-control" id="eye_side" onchange="get_eye_side(this.value);" required>
									...
									<?php foreach($icd_dropdowns as $icd_dropdown){ 
										...
										if($icd_detail['icd_code']==$icd_dropdown['icd_id'])
										{
						...
						<div id="diagnosis_user_box">
							<div class="row form-group">
								...
								<div class="col-md-9 col-sm-8">
									<input type="text" class="form-control" name="diagnosis_comment" value="<?php echo $icd_detail['diagnosis_comment']; ?>">
								</div>
							</div>
							<div class="row form-group">
								<div class="col-md-3 col-sm-4">
								...
								<div class="col-md-9 col-sm-8">
									<textarea class="form-control" name="user_comment" value="<?php echo $icd_detail['user_comment']; ?>"><?php echo $icd_detail['user_comment']; ?></textarea>
								</div>
							</div>
						</div>
					<?php }  ?>
				...
				<div class="well">
					<div class="row form-group">
						...
						<div class="col-md-4">
							...
							<input type="text" class="form-control" name="entered_by" value="<?php echo $icd_detail['done_by']; ?>" readonly="">
						</div>
						<div class="col-md-4">
							...
							<ul class="dia_list">
								...
								<li>
									<ul>
										...
										if($second_step!=""){ ?>
											<li id="sec_id"><strong class="<?php if($third_step!=""){ echo "text-danger";} else{ echo "text-success";}?>" id="sec">↳</strong><a href="#"><?php echo $second_step['second_icd_code']; ?></a> <?php echo $second_step['second_icd_name']; ?></li>
										<?php } ?>
										<li>
											<?php 
											if(!empty($third_step)){ ?>
											...
											<?php } else if(!empty($second_step) || $second_step !='' && empty($third_step) || $third_step==''){ ?>
												<ul class="d-none" id="abc">
...
<script>
    	...
    	function get_eye_side(id)
    	{
    		...
    		$('#ice_code').val(id);
    		if($('#ice_code').val().length == 6)
    		{
    			$('#sec_id').hide();
    			$('#abc').show();
    		}
    		else if($('#ice_code').val().length ==7)
    		{
    			$('#def').hide();
    			$('#abc').show();
    		}
   	...
   	$.ajax({
   		...
   		success:function(result)
   		{
   			...
   			if(diagno.user_comment .length > 0)
   			{
   				...
   				$('#commonly_used_edit_modal').modal('hide');
