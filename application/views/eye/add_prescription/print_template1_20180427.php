...
if(strcmp(strtolower($tab_value->setting_name),'pictorial_test')=='0')
    {  
      $pictorial_test='';
 if(!empty($all_detail['prescription_list'][0]->right_eye_image) || !empty($all_detail['prescription_list'][0]->left_eye_image) || !empty($all_detail['prescription_list'][0]->right_eye_discussion) || !empty($all_detail['prescription_list'][0]->left_eye_discussion))
        {
...
if(strcmp(strtolower($tab_value->setting_name),'biometric_detail')=='0')
    ...
    if(!empty($ucva_bcva_data) || !empty($keratometer_data) || !empty($iol_data))
    {
         ...
         if(!empty($tab_value->setting_value)) { $biometeric =  $tab_value->setting_value; } else { $biometeric =  $tab_value->var_title; }
       ...
       foreach($all_detail['prescription_list']['cheif_compalin'] as $cheif_c_data)
       {
        $chief_complaints.='<tr>
            <td align="left" height="30">'.$cheif_c_data->cheif_complains.'</td>';
               if($cheif_c_data->cheif_complains==1)
            {
                $eye_su_l='Left Eye';
            }
            else
            ...
            $chief_complaints.='<td align="center" height="30">'.$eye_su_l;
             ...
             $chief_complaints.='</td>';
              if($cheif_c_data->cheif_complains==2)
            {
                $eye_su= 'Right Eye';
            }
            else
