...
var table;
<?php
if(in_array('1314',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('eye/dosage/ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_dosage_modal_popup');
  $modal.load('<?php echo base_url().'eye/dosage/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('eye/dosage/deleteall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
  ...
  <div class="row m-b-5">
    ...
    <form>
       <?php if(in_array('1314',$users_data['permission']['action'])) {
        ?>
     ...
     <?php } ?>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               <?php if(in_array('1315',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" id="modal_add">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
               <?php if(in_array('1317',$users_data['permission']['action'])) {
               ?>
               ...
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('eye/dosage/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('eye/dosage/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
