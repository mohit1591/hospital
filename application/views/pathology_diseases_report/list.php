...
function reset_date_search()
  {
      ...
      $('#end_date').val('');
      $.ajax({
         url: "<?php echo base_url('pathology_diseases_report/reset_date_search/'); ?>",  
...
$modal.load('<?php echo base_url().'pathology_diseases_report/advance_search/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
$modal.load('<?php echo base_url().'pathology_diseases_report/advance_report/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
<input type="hidden" name="employee" value="<?php echo $users_data['parent_id'];?>" />
      ...
      <div class="grp media_float_left">
              <b>Disease</b> 
              <select name="disease" class="m_input_default w-130px" id="disease" onchange="return form_submit();">
                  <option value="">Select Disease</option>
                  <?php 
                    if(!empty($diseases_list))
                    {
                      foreach($diseases_list as $dept)
                      {
                        echo '<option value="'.$dept->id.'">'.$dept->disease.'</option>';
                      }
                    }
                  ?> 
              </select>
      ...
      <div class="fixed">
          ...
          <?php if(in_array('891',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('pathology_diseases_report/path_report_excel'); ?>" class="btn-anchor">
               <i class="fa fa-file-excel-o"></i> Excel
               </a>
          <?php } ?>
          <?php if(in_array('893',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('pathology_diseases_report/pdf_path_report'); ?>" class="btn-anchor">
                    <i class="fa fa-file-pdf-o"></i> PDF
               </a>
          <?php } ?>
          <?php if(in_array('894',$users_data['permission']['action'])){?>
               <a href="javascript:void(0)" class="btn-anchor" id="deleteAll" onClick="return print_window_page('<?php echo base_url("pathology_diseases_report/print_path_report"); ?>');">
...
{ 
  ...
  $modal.load('<?php echo base_url().'pathology_diseases_report/send_reminder/' ?>',
  {
...
function clear_form_elements(ele) {
   $.ajax({url: "<?php echo base_url(); ?>pathology_diseases_report/reset_date_search/", 
      success: function(result)
      { 
...
function form_submit(vals)
{
  ...
  $.ajax({
         url: "<?php echo base_url('pathology_diseases_report/search_data/'); ?>", 
         type: 'POST',
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20', 
        "ajax": {
            "url": "<?php echo base_url('pathology_diseases_report/ajax_list')?>",
            "type": "POST"
        }, 
        "columnDefs": [
        { 
  ...
  $('#confirm_email').modal({
      ...
      keyboard: false
        ...
        { 
            ...
            $.ajax({
                 url: "<?php echo base_url('pathology_diseases_report/email_report/'); ?>", 
                 success: function(result)
                 {
