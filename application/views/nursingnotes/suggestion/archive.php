...
var table;
<?php
//if(in_array('513',$users_data['permission']['action'])){
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('nursingnotes/suggestion/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'nursingnotes/suggestion/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'nursingnotes/suggestion/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('nursingnotes/suggestion/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <div class="userlist-right">
      <div class="btns"> 
          <?php //if(in_array('515',$users_data['permission']['action'])) {
        ...
        <button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
          ...
          <?php } ?>
          <?php //if(in_array('514',$users_data['permission']['action'])) {
          ...
          <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('suggestion'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('nursingnotes/suggestion/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('nursingnotes/suggestion/trash/'); ?>"+suggetion_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('nursingnotes/suggestion/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
