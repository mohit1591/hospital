...
function edit_sales_return_medicine(id)
{
  ...
  window.location.href='<?php echo base_url().'sales_return_medicine/edit/';?>'+id
 }

function view_medicine_entry(id)
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'sales_return_medicine/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('sales_return_medicine/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
...
<div class="container-fluid">
 ...
 <form id="new_search_form">
			...
			<div class="row">
				...
				<div class="col-md-4">
					...
					<div class="row m-b-5" id="doctor_div" <?php if($form_data['referred_by']==0){  }else{ ?> style="display: none;" <?php  } ?>>
						...
						<div class="col-md-8">
							<select class="" name="refered_id" id="refered_id" onchange="return form_submit();">
        ...
        { 
          ?>
				...
				<div class="col-md-4">
					<div class="row m-b-5">
						<div class="col-md-12">
							<a class="btn-custom" id="reset_date" onclick="reset_search();">Reset</a> <a href="javascript:void(0)" class="btn-a-search" id="adv_medicine_ret_search_sale"><i class="fa fa-cubes" aria-hidden="true"></i> Advance Search</a> 
						</div>
					</div>
	...
	<div class="userlist-right">
  		<div class="btns">
               <?php if(in_array('408',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-update" onclick="window.location.href='<?php echo base_url('sales_return_medicine/add'); ?>'">
  				    <i class="fa fa-plus"></i> New
  			     </button>
               <?php } ?>
                 <a href="<?php echo base_url('sales_return_medicine/medicine_sales_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>

                <a href="<?php echo base_url('sales_return_medicine/medicine_sales_csv'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-word-o"></i> CSV
                </a>

                <a href="<?php echo base_url('sales_return_medicine/pdf_medicine_sales'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-pdf-o"></i> PDF
                </a>

                <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("sales_return_medicine/print_medicine_sales"); ?>');"> <i class="fa fa-print"></i> Print</a>

               <?php if(in_array('410',$users_data['permission']['action'])) {
               ?>
               ...
               <?php } ?>
               <?php if(in_array('411',$users_data['permission']['action'])) {
               ?>
  			     <button class="btn-exit" onclick="window.location.href='<?php echo base_url('sales_return_medicine/archive'); ?>'">
...
var $modal = $('#load_add_modal_popup');
  $('#adv_medicine_ret_search_sale').on('click', function(){
$modal.load('<?php echo base_url().'sales_return_medicine/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
function form_submit(vals)
{
  ...
  $.ajax({
       ...
       type: 'POST',
       data: { start_date: start_date, end_date : end_date,branch_id:branch_id,refered_id:refered_id,sale_no:sale_no,referred_by:referred_by} ,
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('sales_return_medicine/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('sales_return_medicine/total_calc_return');?>",
                      success: function(result) 
                      {
