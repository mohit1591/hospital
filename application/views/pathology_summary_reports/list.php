...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20', 
        "ajax": {
            "url": "<?php echo base_url('advance_pathology_summary/ajax_list')?>",
            "type": "POST"
        }, 
        "columnDefs": [
        { 
...
function reset_date_search()
  {
      ...
      $('#end_date').val('');
      $.ajax({
         url: "<?php echo base_url('advance_pathology_summary/reset_date_search/'); ?>",  
...
$modal.load('<?php echo base_url().'advance_pathology_summary/advance_search/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
$modal.load('<?php echo base_url().'advance_pathology_summary/advance_report/' ?>',
{
//'id1': '1',
//'id2': '2'
},
...
<input type="hidden" name="employee" value="<?php echo $users_data['parent_id'];?>" />
      ...
      <div class="fixed">
          ...
          <?php if(in_array('891',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('advance_pathology_summary/path_report_excel'); ?>" class="btn-anchor">
               <i class="fa fa-file-excel-o"></i> Excel
               </a>
          <?php } ?>
          <?php if(in_array('893',$users_data['permission']['action'])){?>
               <a href="<?php echo base_url('advance_pathology_summary/pdf_path_report'); ?>" class="btn-anchor">
                    <i class="fa fa-file-pdf-o"></i> PDF
               </a>
          <?php } ?>
          <?php if(in_array('894',$users_data['permission']['action'])){?>
               <a href="javascript:void(0)" class="btn-anchor" id="deleteAll" onClick="return print_window_page('<?php echo base_url("advance_pathology_summary/print_path_report"); ?>');">
...
function clear_form_elements(ele) {
   $.ajax({url: "<?php echo base_url(); ?>advance_pathology_summary/reset_date_search/", 
      success: function(result)
      { 
...
var employee = $('#employee').val();
  ...
  $.ajax({
         url: "<?php echo base_url('advance_pathology_summary/search_data/'); ?>", 
         type: 'POST',
         data: { branch_id: branch_id, start_date: start_date, end_date : end_date, department : department, employee : employee} ,
         success: function(result)
         { 
            reload_table(); 
         }
  ...
  $('#confirm_email').modal({
      ...
      keyboard: false
        ...
        { 
            ...
            $.ajax({
                 url: "<?php echo base_url('advance_pathology_summary/email_report/'); ?>", 
                 success: function(result)
                 {
