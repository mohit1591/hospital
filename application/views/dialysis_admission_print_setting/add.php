...
<form id="dialysis_admission_print_setting_form">
  ...
  <div class="row">
    ...
    <?php
        ...
        if(!empty($dialysis_admission_setting_list))
        {
          foreach($dialysis_admission_setting_list as $setting_list)
          {
    ...
    ?>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <input class="form-control text-uppercase"  name="data[<?php echo $setting_list->id; ?>][setting_name]" value="<?php echo $setting_list->setting_name; ?>"  onkeypress="return onlyAlphabets(event,this);" type="hidden" data-toggle="tooltipipd_admission_print_setting"  title="Allow only characters." class="tooltip-text" <?php if(!empty($setting_list->setting_name)){ echo 'readonly'; } ?>>
          </div>
          <div class="col-xs-4">
          <?php if($setting_list->setting_name=='DIALYSIS_PRINT_SETTING') { ?>
          </div>
        </div>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          <div class="col-xs-3">
           ...
           <textarea  type="text"  cols="45" class="print_textarea" readonly="">{patient_reg_no}  ,  {patient_name}  ,  {patient_age} , {patient_address}  , {booking_code}  {dialysis_name}, ,{package_name},{No_of_visit_req},{No_of_visit_done},{}, {mobile_no}  , {booking_date}  , {doctor_name}  , {room_type}  , {bed_no}  , {mlc}  , {remarks}  , {parent_relation_type}  , {parent_relation_name} ,  {marital_status} ,  {anniversary}  , {religion}  , {dob}  ,  {relation_name} , {mother} , {guardian_name}  , {guardian_email} , {guardian_phone} ,  {guardian_relation}  ,  {patient_email} , {occupation} ,  {insurance_type} ,  {monthly_income} , {insurance_type_name}  , {insurance_company_name} , {insurance_policy_no}  ,  {insurance_tpa_id}  , {insurance_amount}  , {auth_no} , {referred_by}  , {referrer_name} , {attented_doctors} , {assigned_doctor} , {dialysis_booking_date}   ,  {dialysis_date}  ,  {doctor_name}   ,  {room_no} , {bed_no} , {room_type} , {signature}  </textarea>
        </div>
     </div>
     </div>
     </div>
...
$("#dialysis_admission_print_setting_form").on("submit", function(event) 
{
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>dialysis_admission_print_setting/",
    type: "post",
    data: $(this).serialize(),
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'dialysis_admission_print_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
