        ...
        <?php if($form_data['user_type']==1) {?>
          ...
          <div class="col-md-4">
                  ...
                  <div class="row m-b-5">
                    ...
                    <div class="col-md-8">
                      <input type="text" name="customer_code" value="<?php echo $form_data['customer_code'];?>" id="customer_code">
                    </div>
                  </div>
            ...
            {?>
              ...
              <div class="row m-b-5">
                ...
                <div class="col-md-10">
                  ...
                  <input type="hidden" value=" <?php echo $form_data['user_name'];?>" name="mobile_no"/>
                </div>
              </div>
            <?php }?>
...
<!---- stock_issue_allotment_tbl_box  ----->
    ...
    <div class="purchase_medicine_bottom">
       ...
       </div> <!-- stock_issue_allotment bottom-->
      </div> <!-- 12 -->
    </div> <!-- Row -->
   </div> <!-- close -->
...
{
    $.ajax({
          url: "<?php echo base_url('stock_issue_allotment/get_vandor_data/'); ?>"+vendor_id, 
          success: function(result)
          {
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("stock_issue_allotment/print_purchase_recipt"); ?>');">Print</a>
...
{  
   ...
   // var vat = $('#vat').val();
    ...
    $.ajax({
       type: "POST",
       url: "<?php echo base_url('stock_issue_allotment/ajax_list_item')?>",
 ...
 function validation_item_code(id){
    ...
    {   
     ...
     if(allVals!="")
     {
      $.ajax({
        type: "POST",
        url: "<?php echo base_url('stock_issue_allotment/set_item');?>",
  ...
  {
    $.ajax({
            url: "<?php echo base_url(); ?>stock_issue_allotment/ajax_added_item", 
            dataType: "json",
           success: function(result)
            {
              $('#item_select').html(result.data); 
  ...
  { 
   if(allVals!="")
   {
      $.ajax({
              type: "POST",
              url: "<?php echo base_url('stock_issue_allotment/remove_item_list');?>",
               dataType: "json",
              data: {item_id: allVals},
              success: function(result) 
              {  
...
$('documnet').ready(function(){
 <?php if(isset($_GET['status']) && isset($allotment_id) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('stock_issue_allotment/add');?>'; 
    }) ;
 ...
 function validation_check(unit,id){
  ...
  $.ajax({
     type: "POST",
    url: "<?php echo base_url(); ?>stock_issue_allotment/check_stock_avability/", 
   dataType: "json",
    ...
    success: function(result)
    {
        //alert(result);
       if(result==1){
          $('#qty_error_'+id).html('No Available Quantity');
       }else{
           $('#qty_error_'+id).html('');
         }
              ...
              var radioValue = $("input[name='patient_id']:checked").val();
               $.ajax({
                  ...
                  { 
                    ...
                    $("#user_type_id option[value="+data.patient_detail.id+"]").attr('selected', 'selected');
                    
                    
                    
                    appned_user_detail(2,data.patient_detail.id);
