      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
         <div class="col-xs-2">
           ...
           <div class="col-xs-9">
             <textarea cols="45" readonly="" class="print_textarea" >{summary_type} {patient_reg_no},{patient_name},{patient_age},{patient_address},{ipd_no},{mobile_no},{admission_date},{discharge_date},{doctor_name},{specialization},{chief_complaints}, {h_o_presenting_illness} ,{on_examination} ,{vitals}, {provisional_diagnosis}, {final_diagnosis}, {course_in_hospital}, {investigation}, {condition_at_discharge_time}, {advise_on_discharge},{medicine_detail} , {review_time_and_date},{vitals_pulse},{vitals_chest},{vitals_bp},{vitals_cvs},{vitals_temp},{vitals_cns},{vitals_p_a},{mlc},{specialization},{signature} </textarea>
        </div>
...
function get_template(value){
  ...
  $.ajax({
      type:"POST",
      url: "<?php echo base_url(); ?>daycare_discharge_print_setting/get_according_to_section/", 
      data: {value: value},
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>daycare_discharge_print_setting/",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'daycare_discharge_print_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
