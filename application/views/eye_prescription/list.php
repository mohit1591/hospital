...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('eyes_prescription/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'eyes_prescription/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
$modal.load('<?php echo base_url().'eyes_prescription/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'eyes_prescription/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'eyes_prescription/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('eyes_prescription/deleteall');?>",
                      data: {row_id: allVals},
...
{ 
  ...
  $.ajax({url: "<?php echo base_url(); ?>eyes_prescription/reset_search/", 
    success: function(result)
    { 
      reload_table();
    } 
...
$("#search_form").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('eyes_prescription/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    { 
...
function delete_eye_prescription(prescription_id)
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print' && !isset($_GET['type'])) { ?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('eyes_prescription');?>'; 
    }); 
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <a  data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("eyes_prescription/print_prescriptions"); ?>');">Print</a>
