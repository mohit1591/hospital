...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('crm/call_status/ajax_list')?>",
            "type": "POST",
...
function edit_call_status(id)
{
  var $modal = $('#load_add_disease_modal_popup');
  $modal.load('<?php echo base_url().'crm/call_status/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
<section class="userlist">
  ...
  <div class="row m-b-5">
    ...
    <form>
       <?php if(in_array('2436',$users_data['permission']['action'])) {
        ?>
    ...
    <div class="userlist-right">
        <div class="btns">
               <?php if(in_array('2440',$users_data['permission']['action'])) {
               ?>
               ...
               <?php if(in_array('2439',$users_data['permission']['action'])) {
               ?>
...
{         
    $('#table').dataTable();
     ...
     alllead_delete(allVals);
}

function alllead_delete(allVals)
 {    
   if(allVals!="")
   {
       $('#confirm').modal({
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('crm/call_status/deleteall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
...
function delete_call_status(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('crm/call_status/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
