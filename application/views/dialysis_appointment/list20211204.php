...
var $modal = $('#load_add_dialysis_appointment_modal_popup');
$('#modal_add').on('click', function(){
$modal.load('<?php echo base_url().'dialysis_appointment/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
function edit_dialysis_appointment(id)
{
  
  window.location.href='<?php echo base_url().'dialysis_appointment/edit/';?>'+id;
  
}

function view_dialysis_appointment(id)
{
  var $modal = $('#load_add_ot_booking_modal_popup');
  $modal.load('<?php echo base_url().'dialysis_appointment/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('dialysis_appointment/deleteall');?>",
                      data: {row_id: allVals},
...
<form name="search_form_list"  id="search_form_list"> 
    ...
    <form>
  	...
  	<div class="userlist-right">
          ...
          ?>
           <a href="<?php echo base_url('dialysis_appointment/dialysis_list_excel'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-excel-o"></i> Excel
              </a>
              <?php } ?>
               <?php if(in_array('1201',$users_data['permission']['action'])) {
          ?>
              <a href="<?php echo base_url('dialysis_appointment/dialysis_list_csv'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-word-o"></i> CSV
              </a>
              <?php } ?>
 <?php if(in_array('1202',$users_data['permission']['action'])) {
          ?>
              <a href="<?php echo base_url('dialysis_appointment/pdf_dialysis_list'); ?>" class="btn-anchor m-b-2">
          ...
          <?php } ?>
           ...
           <?php if(in_array('1203',$users_data['permission']['action'])) {
                    ?>
              <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("dialysis_appointment/print_dialysis_list"); ?>');"> <i class="fa fa-print"></i> Print</a>
              <?php } ?>
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('dialysis_appointment/archive'); ?>'">
...
function clear_form_elements(ele) {
   $.ajax({url: "<?php echo base_url(); ?>dialysis_appointment/reset_search/", 
      success: function(result)
      { 
...
$modal.load('<?php echo base_url().'dialysis_appointment/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
 ...
 function delete_dialysis_appointment(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('dialysis_appointment/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
<script>
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('dialysis_appointment');?>'; 
    }) ;
...
{
  $.ajax({
    url: "<?php echo base_url('dialysis_appointment/advance_search/'); ?>",
    type: "post",
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('dialysis_appointment/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
