...
$template_data->template = str_replace("{patient_category_name}",$all_detail['sales_list'][0]->patient_category_name,$template_data->template);
$template_data->template = str_replace("{authorize_person_name}",$all_detail['sales_list'][0]->authorize_person_name,$template_data->template);

$template_data->template = str_replace("{amount_in_words}",AmountInWordsdisplay($all_detail['sales_list'][0]->paid_amount),$template_data->template);

$template_data->template = str_replace("{medicine_discount}",$all_detail['sales_list'][0]->medicine_discount,$template_data->template);
...
{ 
	...
	if(!empty($medicine_list->m_hsn_no))
	{
	    $hsn = $medicine_list->m_hsn_no;
	}
	else
	{
	   $hsn='&nbsp;';
	}
	if(!empty($medicine_list->m_batch_no))
	{
	    $batc = $medicine_list->m_batch_no;
	}
	else
	{
	   $batc='&nbsp;';
	}
	if(!empty($medicine_list->m_cgst))
	{
	    $cgs = $medicine_list->m_cgst;
	}
	else
	{
	   $cgs='&nbsp;';
        ...
        $tr = str_replace("{cgst}",$cgs,$tr);
        $tr = str_replace("{sgst}",$medicine_list->m_sgst,$tr);
        $tr = str_replace("{batch_no}",$batc,$tr);
        $tr = str_replace("{exp_date}",$exc,$tr);
        $tr = str_replace("{hsn_no}",$hsn,$tr);
...
$cel_val = ceil($all_detail['sales_list'][0]->paid_amount);

$round_h = $cel_val-$all_detail['sales_list'][0]->paid_amount;

$template_data->template = str_replace("{round_off}",$round_h,$template_data->template);
if(!empty($all_detail['sales_list'][0]->balance))
{
  $balance_amt =  number_format($all_detail['sales_list'][0]->balance, 2, '.', '');
}
else
{
   $balance_amt ='0.00'; 
}
$template_data->template = str_replace("{balance}",$balance_amt,$template_data->template);
...
$template_data->template = str_replace("Salesman",$data_patient_reg,$template_data->template);
...
{ 
    ...
    $tr = str_replace("{exp_date}",$exc,$tr); //$expiryDate
    $tr = str_replace("{medicine_per_vat}",$medicine_list->vat,$tr);
    $tr = str_replace("{hsn_no}",$medicine_list->m_hsn_no,$tr);
    $tr = str_replace("{igst}",$medicine_list->m_igst,$tr);
    $tr = str_replace("{medicine_per_discount}",$medicine_list->m_discount,$tr);
...
$template_data->template = str_replace("{total_amt_per}",$g_total,$template_data->template);
