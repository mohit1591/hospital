...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '25',
        "ajax": {
            "url": "<?php echo base_url('diagnosis/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'diagnosis/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_diagnosis_modal_popup');
  $modal.load('<?php echo base_url().'diagnosis/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_diagnosis_modal_popup');
  $modal.load('<?php echo base_url().'diagnosis/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('diagnosis/deleteall');?>",
                      data: {row_id: allVals},
...
if(in_array('449',$permission_action) || in_array('121',$permission_section)){
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('diagnosis/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('diagnosis/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
