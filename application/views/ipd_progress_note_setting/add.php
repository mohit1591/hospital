...
<form id="prescription_setting_form">
  ...
  <div class="row">
    ...
    <?php
      if(in_array('764',$users_data['permission']['action']))
      {
    ...
    ?>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-4">
          <?php if($setting_list->setting_name=='PROGRESS_REPORT_SETTING') { ?>
          </div>
        </div>
      ...
      <div class="col-xs-12 m-b-5 m12">
        <div class="row">
          ...
          <div class="col-xs-9 p-l-0 p-r-0">
             <textarea cols="45" readonly=""  class="print_textarea" >{patient_reg_no},{patient_name},{patient_age},{patient_address},{booking_code},{mobile_no},{progress_report_date},{ipd_no},{admission_date},{doctor_name},{room_type},{room_no},{bed_no},{progress_note},{signature} </textarea>
        </div>
     </div>
     </div>
     </div>
...
for (instance in CKEDITOR.instances) {
  ...
  $('.overlay-loader').show();
  $.ajax({
    url: "<?php echo base_url(); ?>ipd_progress_note_setting/",
    type: "post",
    data: $(this).serialize(),
...
function preview_template(id)
{
  ...
  $modal.load('<?php echo base_url().'ipd_progress_note_setting/preview/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
