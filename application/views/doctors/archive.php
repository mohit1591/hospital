...
<?php if(in_array('126',$users_data['permission']['action'])):?>
     $(document).ready(function() { 
          table = $('#table').DataTable({  
               ...
               "order": [], 
               "ajax": {
                    "url": "<?php echo base_url('doctors/archive_ajax_list')?>",
                    "type": "POST",
               }, 
               "columnDefs": [{ 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('doctors/restoreall');?>",
                      data: {row_id: allVals},
    ...
    <div class="userlist-box">
         ...
         <form>
               <?php if(in_array('126',$users_data['permission']['action'])): ?>
                    ...
                    <table id="table" class="table table-striped table-bordered doctor_archive" cellspacing="0" width="100%">
      ...
      <div class="fixed">
          ...
          <?php } ?> 
               <button class="btn-update" onclick="window.location.href='<?php echo base_url('doctors'); ?>'">
...
<script>  
 ...
 function restore_doctors(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('doctors/restore/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('doctors/trash/'); ?>"+doctor_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('doctors/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
