...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('ipd_nursingnotes/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
function view_prescription(id)
{
  ...
  $modal.load('<?php echo base_url().'ipd_nursingnotes/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('ipd_nursingnotes/restoreall');?>",
                      data: {row_id: allVals},
  	...
  	<div class="userlist-right">
  		<div class="btns"> 
              <?php if(in_array('121',$users_data['permission']['section'])) {
               ...
               <?php } ?> 
        <button class="btn-exit" onclick="window.location.href='<?php echo base_url('ipd_nursingnotes'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('ipd_nursingnotes/restore/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('ipd_nursingnotes/trash/'); ?>"+doctor_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('ipd_nursingnotes/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
