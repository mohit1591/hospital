...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "aaSorting": [],
        "ajax": {
            "url": "<?php echo base_url('ipd_particular_summary_report/ajax_list')?>",
            "type": "POST"
        }, 
        "columnDefs": [
        { 
...
function reset_date_search()
  {
      ...
      $('#employee').val('');
      $.ajax({
         url: "<?php echo base_url('ipd_particular_summary_report/reset_date_search/'); ?>",  
...
<section class="userlist">
    ...
    <div class="grp_box m-b-5">
      ...
      <div class="grp">
              ...
              <select name="charge_type" id="charge_type" class="m_select_btn" onchange="return form_submit(this.value);">
                  <option value="">Select Charge Type</option>
                  <?php
                  if(!empty($charge_type_list))
                  {
                    foreach($charge_type_list as $charge_t_list)
                    {
                      echo '<option value="'.$charge_t_list['charge_type'].'">'.$charge_t_list['charge_type'].'</option>';
                    }
                  }
                  ?> 
                </select>
...
<!-- bootstrap data table -->
    ...
    <div class="userlist-right relative" id="example_wrapper">
      <div class="fixed">
      <div class="btns"> 
               <a href="<?php echo base_url('ipd_particular_summary_report/ipd_part_summary_report_excel'); ?>" class="btn-anchor m-b-2">
                    <i class="fa fa-file-excel-o"></i> Excel
               </a>
               <a href="<?php echo base_url('ipd_particular_summary_report/pdf_ipd_part_summary_report'); ?>" class="btn-anchor m-b-2">
               <i class="fa fa-file-pdf-o"></i> PDF
               </a>
            <a href="javascript:void(0)" class="btn-anchor m-b-2" onClick="return print_window_page('<?php echo base_url("ipd_particular_summary_report/print_ipd_part_collection_report"); ?>');">
...
function clear_form_elements(ele) {
   $.ajax({url: "<?php echo base_url(); ?>ipd_particular_summary_report/reset_date_search/",
      success: function(result)
      {
...
function form_submit(){
    ...
    $.ajax({
           url: "<?php echo base_url('ipd_particular_summary_report/search_data/'); ?>", 
           type: 'POST',
           data: { start_date: start_date, end_date : end_date,branch_id:branch_id,particulars:particulars,particulars_name:particulars_name,charge_type:charge_type} ,
           success: function(result)
           { 
              reload_table(); 
           }
