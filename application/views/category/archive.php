...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('item_category/archive_ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'item_category/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('item_category/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered category_archive" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
          ...
          <?php } ?> 
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('item_category'); ?>'">
...
<script>  
 ...
 function restore_Category(employee_type_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('item_category/restore/'); ?>"+employee_type_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {     
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('item_category/trash/'); ?>"+Vendor_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('item_category/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
