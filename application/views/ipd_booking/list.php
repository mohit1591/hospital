    ...
    <div class="row m-b-2">
      <div class="col-md-4">
          <div class="row">
            ...
            <div class="col-md-4">
              <label> <input type="radio" value="2"  name="running" id="running"  onClick="return check_status();" <?php if($form_data['running']=='2'){ ?> checked="checked" <?php } ?>> All</label>
            </div>
...
$('documnet').ready(function(){
 ...
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  <?php if(isset($_GET['status']) && $_GET['status']=='print_nurses_note'){?>
  $('#confirm_print_nurse_note').modal({
      backdrop: 'static',
      keyboard: false
        })
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('ipd_booking');?>'; 
        // print_window_page('<?php echo base_url("ipd_booking/print_nurses_notes"); ?>')
    }) ;
...
<div id="confirm_print" class="modal fade dlt-modal">
    ...
    <div id="confirm_print_nurse_note" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header bg-theme"><h4>Are You Sure?</h4></div>
          <!-- <div class="modal-body"></div> -->
          <div class="modal-footer">
            <a type="button" data-dismiss="modal" class="btn-anchor"  onClick="return print_window_page('<?php echo base_url("ipd_booking/print_nurses_notes"); ?>');" >Print</a>
            <button type="button" data-dismiss="modal" class="btn-cancel" id="cancel">Close</button>
          </div>
        </div>
      </div>  
...
{    
  var book_id=$('#ipd_book_id').val();
   ...
   var allVals = [];
     ...
     $('#form_type :selected').each(function(){
     allVals.push($(this).val());
    });
     
     
   
   print_window_page('<?php echo base_url("ipd_booking/print_ipd_adminssion_consent_new/?"); ?>ipd_id='+book_id+'&forid='+allVals);
}
...
function print_label(id)
{
  ...
  $modal.load('<?php echo base_url().'ipd_booking/print_template/'; ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function print_label_sixteen(id)
{
  var $modal = $('#load_add_ipd_label_summary_print_modal_popup');
  $modal.load('<?php echo base_url().'ipd_booking/print_label_sixteen_template/'; ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function print_label_twentyfour(id)
{
  var $modal = $('#load_add_ipd_label_summary_print_modal_popup');
  $modal.load('<?php echo base_url().'ipd_booking/print_label_twentyfour_template/'; ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function print_label_thirtysix(id)
{
  var $modal = $('#load_add_ipd_label_summary_print_modal_popup');
  $modal.load('<?php echo base_url().'ipd_booking/print_label_thirtysix_template/'; ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
// Added By Nitin Sharma 02/02/2024   
 ...
 {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'ipd_booking/view_test_report/' ?>'+id,
    {
    },
    function(){
    $modal.modal('show');
...
<div id="nabh_print_form" class="modal fade dlt-modal" style="width:900px;">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">   
            <div class="row">
              <div class="col-md-12">
                <div class="row m-b-5">
                  <div class="col-md-5">      
                    <label><b>Template Type:</b></label><br>
                    <span>Press ctrl + Left click to select multiple forms at a time. </span>
                  </div>
                  <div class="col-md-7">
                    <select class="form-control" name="form_type" id="form_type"multiple=""  style="height: 100px;">
                      ...
                      <?php if($users_data['parent_id']=='166'){ ?>
