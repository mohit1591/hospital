...
$template_data->template = str_replace("{payment_due_date}",date('d-m-Y',strtotime($all_detail['purchases_list'][0]->payment_due_date)),$template_data->template);
$template_data->template = str_replace("{customer_code}",$all_detail['purchases_list'][0]->customer_code,$template_data->template);

if($template_data->printer_id==2)
{
	
	$template_data->template = str_replace("{vendor_gst}",$all_detail['purchases_list'][0]->vendor_gst,$template_data->template);
	...
	{ 
	        ...
	        $serial_no=array();
            $purchase_item_serial =get_purchase_serial_no_item($ids,$purchases_list->item_id);
            $l=1;
          ...
          $tr = str_replace("{purchase_amount}",$purchases_list->mrp,$tr);
          $tr = str_replace("{mrp}",$purchases_list->per_pic_price,$tr);
          $tr = str_replace("{item_name}",$purchases_list->item_name.$mnd,$tr);
...
if($template_data->printer_id==1)
{
	...
	{ 
	  //  print_r($purchases_list);
	       $serial_no=array();
            $purchase_item_serial =get_purchase_serial_no_item($purchases_list->purchase_id,$purchases_list->item_id);
            $l=1;
            foreach ($purchase_item_serial as  $serial) 
            {
                array_push($serial_no, $serial->serial_no);
