...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '25',
        "ajax": {
            "url": "<?php echo base_url('Examination/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'Examination/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_examination_modal_popup');
  $modal.load('<?php echo base_url().'examination/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_examination_modal_popup');
  $modal.load('<?php echo base_url().'Examination/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('Examination/deleteall');?>",
                      data: {row_id: allVals},
...
if(in_array('439',$permission_action) || in_array('121',$permission_section)){
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('Examination/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('Examination/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
