...
$(document).ready(function() { 
  form_submit();
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('medicine_stock/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'medicine_stock/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'medicine_stock/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'medicine_stock/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
...
function reset_medicine_stock(mid, batch_no,qty)
{
    $.ajax({
              type: "POST",
              url: "<?php echo base_url('medicine_stock/reset_medicine_stock');?>",
              data: {mid: mid, batch_no: batch_no, qty: qty},
    ...
    <table class="ptl_tbl">
      <tr>
       ...
       <?php if(in_array('1',$permission_section)): ?> 
        ...
        <td><a class="btn-custom" id="reset_date" onclick="reset_search();">Reset</a> <a href="javascript:void(0)" class="btn-a-search" id="adv_search_stock"><i class="fa fa-cubes" aria-hidden="true"></i> Advance Search</a> </td>
      </tr>
    

            <tr><td><label>According To</label>
            <select  onchange="return form_submit();" name="type"><option value="1" selected> Batch </option><option value="2"> Medicine </option></select></td></tr>
    ...
    <form>
  	...
  	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
                <a href="<?php echo base_url('medicine_stock/medicine_stock_excel'); ?>" class="btn-anchor m-b-2">
                <i class="fa fa-file-excel-o"></i> Excel
                </a>
                <?php } ?>
               <?php if(in_array('579',$users_data['permission']['action'])) {
               ?>
                    <a href="<?php echo base_url('medicine_stock/medicine_stock_csv'); ?>" class="btn-anchor m-b-2">
                         <i class="fa fa-file-word-o"></i> CSV
                    </a>
               <?php } ?>
               <?php if(in_array('580',$users_data['permission']['action'])) {
               ?>
                    <a href="<?php echo base_url('medicine_stock/pdf_medicine_stock'); ?>" class="btn-anchor m-b-2">
                    <i class="fa fa-file-pdf-o"></i> PDF
                    </a>
               <?php } ?>
               <?php if(in_array('581',$users_data['permission']['action'])) {
               ?>       
                    <!-- <a href="javascript:void(0)" class="btn-anchor m-b-2" id="deleteAll" onClick="return openPrintWindow('< ?php echo base_url("medicine_stock/print_medicine_stock"); ?>', 'windowTitle', 'width=820,height=600');">
                    <i class="fa fa-print"></i> Print
                    </a> -->

                    <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("medicine_stock/print_medicine_stock"); ?>');"> <i class="fa fa-print"></i> Print</a>
               <?php } ?>
               <?php if(in_array('415',$users_data['permission']['action'])) {
               ?>
...
function load_allot_model()
{
     ...
     $modal.load('<?php echo base_url('medicine_stock/allotement_to_branch/'); ?>',{'medicine_ids':allVals,'batch_no':batchNo},function(){
          $modal.modal('show');
     });
...
function change_quantity(id,batch_no)
  {
    var $modal = $('#load_add_modal_popup');
    $modal.load('<?php echo base_url().'medicine_stock/change_quantity/' ?>',
    {
      'id': id,
      'batch_no': batch_no
      },
