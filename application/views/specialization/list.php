...
var table;
<?php
if(in_array('43',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('specialization/ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_specialization_modal_popup');
  $modal.load('<?php echo base_url().'specialization/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('specialization/deleteall');?>",
                      data: {row_id: allVals},
    ...
    <form>
      ...
      ?>
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered specialization_list" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
        ...
        {
        ?>
  			<button class="btn-update" onclick="window.location.href='<?php echo base_url('specialization/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('specialization/delete/'); ?>"+specialization_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
    ...
    <div id="confirm" class="modal fade dlt-modal">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" data-dismiss="modal" class="btn-update" id="delete">Delete</button>
            <button type="button" data-dismiss="modal" class="btn-cancel">Close</button>
          </div>
        </div>
      </div>  
