...
var table;
<?php
if(in_array('47',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('specialization/archive_ajax_list')?>",
            "type": "POST",
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'specialization/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('specialization/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered specialization_archive" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
        ...
        ?>  
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('specialization'); ?>'">
...
<script> 
 ...
 function restore_specialization(specialization_id)
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('specialization/restore/'); ?>"+specialization_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
  ...
  function trash(specialization_id)
 {     
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false, 
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('specialization/trash/'); ?>"+specialization_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('specialization/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
