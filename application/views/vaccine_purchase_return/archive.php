...
var table;
<?php
if(in_array('1087',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('vaccine_purchase_return/archive_ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
$modal.load('<?php echo base_url().'vaccine_purchase_return/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('vaccine_purchase_return/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
  	...
  	<div class="userlist-right media_btns">
  		<div class="btns"> 
          <?php if(in_array('1086',$users_data['permission']['action'])) {
          ?>
  			<button class="btn-update" id="deleteAll" onclick="return checkboxValues();">
  				<i class="fa fa-window-restore" aria-hidden="true"></i> Restore
  			</button>
          <?php } ?>
          <?php if(in_array('1085',$users_data['permission']['action'])) {
          ?>
          ...
          <?php } ?> 
               <button class="btn-exit" onclick="window.location.href='<?php echo base_url('vaccine_purchase_return'); ?>'">
...
$(document).ready(function(){
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccine_purchase_return/restore/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('vaccine_purchase_return/trash/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('vaccine_purchase_return/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
