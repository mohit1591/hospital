...
<?php if(in_array('126',$users_data['permission']['action'])):?>
     $(document).ready(function() { 
          table = $('#table').DataTable({  
               ...
               "order": [], 
               "ajax": {
                    "url": "<?php echo base_url('hospital/archive_ajax_list')?>",
                    "type": "POST",
               }, 
               "columnDefs": [{ 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('hospital/restoreall');?>",
                      data: {row_id: allVals},
  ...
  <div class="col-md-12">
         ...
         <form>
               <?php if(in_array('126',$users_data['permission']['action'])): ?>
                    ...
                    <table id="table" class="table table-striped table-bordered hospital_archive" cellspacing="0" width="100%">
      ...
      <div class="fixed">
          ...
          <?php } ?> 
               <button class="btn-update" onclick="window.location.href='<?php echo base_url('hospital'); ?>'">
...
<script>  
 ...
 function restore_hospital(rate_id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('hospital/restore/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
{         
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('hospital/trash/'); ?>"+hospital_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('hospital/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
