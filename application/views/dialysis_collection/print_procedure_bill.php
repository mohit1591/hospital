...
$template_data->template = str_replace("{net_amount}",$all_detail['sales_list'][0]->debit,$template_data->template);
...
$template_data->template = str_replace("{amount}",$all_detail['sales_list'][0]->debit,$template_data->template);
...
$template_data->template = str_replace('{particular}',"Discharge",$template_data->template);
...
$template_data->template = str_replace('{medicine_name}',"Discharge",$template_data->template);
...
if($template_data->printer_id==1){ //echo $template_data->template; exit;
    $template_data->template = str_replace("{hospital_receipt_no}",'',$template_data->template);
...
$template_data->template = str_replace("{patient_reg_no}",$all_detail['sales_list'][0]->patient_code,$template_data->template);
    ...
    if(!empty($all_detail['sales_list'][0]->recepit_no))
    {
        $recepit_no = '<div style="width:100%;display:inline-flex;">
                        <div style="width:40%;line-height:19px;font-weight:600;">Booking Code:</div>

            <div style="width:60%;line-height:19px;">'.$all_detail['sales_list'][0]->recepit_no.'</div>
            </div>';
        $template_data->template = str_replace("{booking_code}",$recepit_no,$template_data->template);
    }

    if(!empty($all_detail['sales_list'][0]->date))
    {
        $booking_date = '<div style="width:100%;display:inline-flex;">
                        <div style="width:40%;line-height:19px;font-weight:600;">Dialysis Booking Date:</div>
