...
{
  window.location.href='<?php echo base_url().'sales_estimate/edit/';?>'+id
}

function view_medicine_entry(id)
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'sales_estimate/view/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('sales_estimate/deleteall');?>",
                      data: {row_id: allVals},
    ...
    <form>
 	...
 	<div class="userlist-right">
  		<div class="btns">
               ...
               ?>
  			     <button class="btn-update" onClick="window.location.href='<?php echo base_url('sales_estimate/add'); ?>'">
              ...
              <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("sales_estimate/print_medicine_sales"); ?>');"> <i class="fa fa-print"></i> Print</a>
...
function reset_search()
  { 
    ...
    $.ajax({url: "<?php echo base_url(); ?>sales_estimate?>/reset_search/", 
      success: function(result)
      { 
        reload_table();
      } 
 ...
 function delete_sales_estimate(id)
 {    
    $('#confirm').modal({
      backdrop: 'static',
      keyboard: false
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('sales_estimate/delete/'); ?>"+id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
$('documnet').ready(function(){
 <?php if(isset($_GET['status']) && $_GET['status']=='print'){?>
  ...
  .one('click', '#cancel', function(e)
    { 
        window.location.href='<?php echo base_url('sales_estimate');?>'; 
    }) ;
...
$modal.load('<?php echo base_url().'sales_estimate/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
  ...
  function form_submit(vals)
{
    ...
    var referred_by = $("input[name='referredby']:checked").val();
    
  $.ajax({
       url: "<?php echo base_url(); ?>sales_estimate/advance_search/",
       type: 'POST',
...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('sales_estimate/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
        ...
        "drawCallback": function() 
        {
            $.ajax({
                      dataType: "json",
                      url: "<?php echo base_url('sales_estimate/total_calc_return');?>",
                      success: function(result) 
                      {
