...
var table;
<?php
if(in_array('26',$users_data['permission']['action'])) 
{
?>
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "order": [], 
        "ajax": {
            "url": "<?php echo base_url('users/archive_ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'users/add/' ?>',
{
  //'id1': '1',
  //'id2': '2'
  },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('users/restoreall');?>",
                      data: {row_id: allVals},
...
<section class="userlist">
    ...
    <form>
       ...
       <!-- bootstrap data table -->
        <table id="table" class="table table-striped table-bordered users_archive" cellspacing="0" width="100%">
  	...
  	<div class="userlist-right">
        ...
        <button class="btn-update" onclick="window.location.href='<?php echo base_url('users'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('users/restore/'); ?>"+emp_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
...
function users_details(id)
{
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('users/trash/'); ?>"+users_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('users/trashall');?>",
                      data: {row_id: allVals},
                      success: function(result) 
                      {
                            flash_session_msg(result);
