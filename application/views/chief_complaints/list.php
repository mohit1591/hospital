...
$(document).ready(function() { 
    table = $('#table').DataTable({  
        ...
        "pageLength": '25',
        "ajax": {
            "url": "<?php echo base_url('chief_complaints/ajax_list')?>",
            "type": "POST",
        }, 
        "columnDefs": [
        { 
...
{
  var $modal = $('#load_add_chief_complaints_modal_popup');
  $modal.load('<?php echo base_url().'chief_complaints/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        .one('click', '#delete', function(e)
        {
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('chief_complaints/deleteall');?>",
                      data: {row_id: allVals},
...
if(in_array('436',$permission_action) || in_array('121',$permission_section)){
          ...
          ?>
  			<button class="btn-exit" onclick="window.location.href='<?php echo base_url('chief_complaints/archive'); ?>'">
...
<script>  
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('chief_complaints/delete/'); ?>"+rate_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
