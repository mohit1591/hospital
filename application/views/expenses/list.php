...
$(document).ready(function() {
    table = $('#table').DataTable({
        ...
        "pageLength": '20',
        "ajax": {
            "url": "<?php echo base_url('expenses/ajax_list')?>",
            "type": "POST",
...
$modal.load('<?php echo base_url().'expenses/advance_search/' ?>',
{ 
},
function(){
$modal.modal('show');
...
{
  var $modal = $('#load_add_modal_popup');
  $modal.load('<?php echo base_url().'expenses/edit/' ?>'+id,
  {
    //'id1': '1',
    //'id2': '2'
    },
   ...
   if(allVals!="")
   {
      ...
      keyboard: false
        ...
        { 
            $.ajax({
                      type: "POST",
                      url: "<?php echo base_url('expenses/deleteall');?>",
                      data: {row_id: allVals},
...
<a class="btn-custom" id="reset_date" onclick="reset_search();"><i class="fa fa-refresh"></i> Reset</a>
          ...
          <div class="col-xs-4"> <label>Expense Type:</label></div>
           ...
           <div class="col-md-4">	
                          <div class="row">	
                            ...
                            <div class="col-xs-8">	
                                <select name="expense_category_id" id="expense_category_id" class="pat-select1 m_select_btn" onchange="return form_submit();">	
                                <option value="">Select</option>	
                                   <?php	
                                        if(!empty($expense_category_list)){	
                                             foreach($expense_category_list as $expense_category){?>	
                                                <option value="<?php echo $expense_category->id; ?>"><?php echo $expense_category->exp_category; ?></option>	
                                           <?php  }	
                                        }	
                                   ?> 	
                              </select> 	
                     ...
                     <div class="col-md-4">	
                          <div class="row">	
                            <div class="col-xs-4"><label> Department:</label></div>	
                            <div class="col-xs-8">	
                               <select name="department_type" id="department_type" class="pat-select1 m_select_btn" onchange="return form_submit();">	
      ...
      <?php
       if(in_array('211',$users_data['permission']['action']))
       {
        ...
        <?php
         ...
         <a href="<?php echo base_url('expenses/expenses_excel'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-excel-o"></i> Excel
              </a>

              <a href="<?php echo base_url('expenses/expenses_csv'); ?>" class="btn-anchor m-b-2">
              <i class="fa fa-file-word-o"></i> CSV
              </a>

              <a href="<?php echo base_url('expenses/pdf_expenses'); ?>" class="btn-anchor m-b-2">
             ...
             <!--  <a href="javascript:void(0)" class="btn-anchor m-b-2" id="deleteAll" onClick="return openPrintWindow('< ?php echo base_url("medicine_entry/print_medicine_entry"); ?>', 'windowTitle', 'width=820,height=600');">
              ...
              <a href="javascript:void(0)" class="btn-anchor m-b-2"  onClick="return print_window_page('<?php echo base_url("expenses/print_expenses"); ?>');"> <i class="fa fa-print"></i> Print</a>
      ...
      <?php if(in_array('213',$users_data['permission']['action']))
       ...
       {
        ?>
          <button class="btn-exit" onclick="window.location.href='<?php echo base_url('expenses/archive'); ?>'">
...
$("#search_form_list").on("submit", function(event) { 
  ...
  $.ajax({
    url: "<?php echo base_url('expenses/advance_search/'); ?>",
    type: "post",
    data: $(this).serialize(),
    success: function(result) 
    {
...
$('.start_datepicker').datepicker({
 ...
 {    
    ...
    { 
        $.ajax({
                 url: "<?php echo base_url('expenses/delete/'); ?>"+expenses_id, 
                 success: function(result)
                 {
                    flash_session_msg(result);
                    reload_table(); 
 ...
 function reset_search()
  { 
    ...
    $('#end_date_exp').val('');
    $('#branch_id').val('');
    $.ajax({url: "<?php echo base_url(); ?>expenses/reset_search/", 
      success: function(result)
      { 
