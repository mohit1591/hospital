...
<body onload="get_balance_clearance_list()">
...
function pay_now_to_branch(patient_id,balance)
{
  ...
  $modal.load('<?php echo base_url().'billing/pay_now/' ?>'+patient_id,{/*'id1': '1',//'id2': '2'*/},
     function(){
          $modal.modal('show');
     });
}

 function patient_balance_details(pid)
  {
    var branch_id = $('#sub_branch_id').val(); 
    window.open('<?php echo base_url("billing/patient_balance_details/"); ?>'+branch_id+'/'+pid,'mywin','width=800,height=600');
  }

function get_selected_value(vals)
{ 
