...
class Garbage_stock_item extends CI_Controller {
	...
	function __construct() 
	{
  	...
  	parent::__construct();	
  	 auth_users();  
     //print_r($this->session->userdata('auth_users'));
  	$this->load->model('garbage_stock_item/garbage_stock_item_model','garbage_stock_item');
  	...
  	public function index()
      {
        ...
        $this->load->view('garbage_stock_item/list',$data);
      }
    ...
    {  
        ...
        foreach ($list as $garbage_stock_item) { 
           ...
           // $doctor_type = array('0'=>'Referral','1'=>'Attended','2'=>'Referral/Attended');
            ...
            $print_url = "'".base_url('garbage_stock_item/print_garbage_item/'.$garbage_stock_item->id)."'";
           ...
           // }       
            $row[] = $btnedit.$btnprint.$btndelete;
            $data[] = $row;
            $i++;
          }
    ...
    public function garbage_stock_item_excel()
    {
         $this->load->library('excel');
          ...
          $fields = array('Garbage No.','Remarks','Total Amount','Garbage Date');
    ...
    public function add($ipd_id="",$patient_id="")
    {
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
               ...
               $this->session->set_userdata('garbagess_id',$charge_id);
                $this->session->set_flashdata('success','Garbage item added successfully.');
                redirect(base_url('garbage_stock_item?status=print'));
        ...
        $this->load->view('garbage_stock_item/add',$data);
    }
    ...
    public function edit($id="")
    {
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
               ...
               $this->session->set_userdata('garbagess_id',$purchase_id);
    ...
    private function _validate()
    {
        ...
        $this->form_validation->set_rules('garbage_code', 'garbage No.', 'trim|required'); 
         $garbage_stock_item_list = $this->session->userdata('garbage_stock_item_list');
        ...
        $rows = '<thead class="bg-theme"><tr>           
           ...
           if(isset($garbage_stock_item_list) && !empty($garbage_stock_item_list))
           {
              ...
              foreach($garbage_stock_item_list as $purchase_item_list)
              {
                  ...
                  $purchase_item_serial =$this->garbage_stock_item->purchase_item_serial_by_id($purchase_arr[0],$purchase_item_list['item_id']);
                 ...
                 $rows .= '<tr>
                    ...
                    $rows .= '</td>
                        ...
                        $rows .= "<td> <button id='add_serial_no' value='' class='btn-custom' type='button' onclick='return add_serial(".$purchase_item_list['item_id'].",this.value,1)'> Add </button> </td><input type='hidden' name='serial_no_array[]' id='serial_no_array_".$purchase_item_list['item_id']."' value='".$nre."'><input type='hidden' name='issued_ser_id_no_array[]' id='issued_ser_id_no_array_".$purchase_item_list['item_id']."' value='".$nre_ids."'>"; 
                        
                            
                           $rows .= '<td>'.$purchase_item_list['unit'].'<input type="hidden" value="'.$purchase_item_list['unit'].'" id="unit_'.$purchase_item_list['item_id'].'"/></td>
  ...
  public function archive_ajax_list()
    {
       unauthorise_permission(169,978);
        $this->load->model('garbage_stock_item/garbage_stock_item_archive_model','garbage_stock_item_archive'); 
        $users_data = $this->session->userdata('auth_users');
 ...
 public function payment_calc_all()
    ...
    public function payment_cal_perrow()
    {
      ...
      { 
       $total_price+= $post['quantity']*$post['item_price'];
        $garbage_stock_item_list[$post['item_id']] =  array('item_id'=>$post['item_id'],'category_id'=>$post['category_id'],'total_price'=>$post['total_price'],'item_code'=>$post['item_code'],'item_name'=>$post['item_name'],'amount'=>$post['item_price']*$post['quantity'],'unit'=>$post['unit'],'item_price'=>$post['item_price'],'quantity'=>$post['quantity'],'total_amount'=>'0.00'); 
        $this->session->set_userdata('garbage_stock_item_list', $garbage_stock_item_list);
    ...
    { 
       unauthorise_permission(169,978);
        ...
        if(!empty($post))
        {
            ...
            $response = "Garbage item successfully restore in Garbage item return list.";
            echo $response;
        }
    }
     ...
     public function archive()
    {
       unauthorise_permission(169,978);
        ...
        $this->load->view('garbage_stock_item/archive',$data);
    }
    function deleteall()
    {
        unauthorise_permission(169,978);
        ...
        if(!empty($post))
        {
            ...
            $response = "Garbage item successfully deleted.";
            echo $response;
        }
    }
    ...
    //print receipt 
     public function print_garbage_item($ids="")
     {
        ...
        $purchase= $this->session->userdata('garbagess_id');
        }
        $get_detail_by_id= $this->garbage_stock_item->get_by_id($purchase);
        $get_by_id_data=$this->garbage_stock_item->get_all_detail_print($purchase,$get_detail_by_id['branch_id']);
        ...
        $template_format= $this->garbage_stock_item->template_format(array('section_id'=>9,'types'=>1,'sub_section'=>3,'branch_id'=>$get_detail_by_id['branch_id']));
        
        //print '<pre>';print_r($template_format);die;
        
        $data['template_data']=$template_format;
        $data['payment_mode']= $get_payment_detail;
        $data['user_detail']=$user_detail;
        $data['all_detail']= $get_by_id_data;
        $this->load->view('garbage_stock_item/print_template_stock_garbage',$data);
  }
