...
class Expenses extends CI_Controller {
	...
	{ 
		...
		$dept_list=get_expense_department();
		...
		foreach ($list as $expenses) { 
            ...
            if($expenses->type=='13' && empty($expenses->parent_id))
            {
                $expenses_type='Doctor Commission';
            }

			$row[] = $expenses->vouchar_no;
			$row[] = date('d-m-Y',strtotime($expenses->expenses_date)); 
			$row[] = $expenses_type.' '. $expenses->patient_name.' '.$expenses->book_code;
			$row[] = $dept_list[$expenses->department_type];
			$row[] = $expenses->paid_amount;
			if($expenses->type==13)
			{
			   $row[] = $expenses->doctor_name; 
			}
			else
			{
			   $row[] = $expenses->paid_to_name; 
			}
			
			
			$row[] = $expenses->remarks;
			$row[] = $expenses->payment_from;
            ...
            $btnprint= '';
				...
				$print_url = "'".base_url('expenses/print_expense_report/'.$expenses->id)."'";
				$btnprint = '<a class="btn-custom" href="javascript:void(0)" onClick="return print_window_page('.$print_url.')" title="Print" ><i class="fa fa-print"></i> Print</a>'; 
				}
...
$data['vendor_list'] = $this->expenses->vendor_list();
	    $data['form_error'] = ''; 
         $data['form_data'] = array(
								  ...
								  'department_type'=>$result['department_type'],
								  'paid_amount'=>$result['paid_amount'],
								  'employee_id'=>$result['employee_id'],
								  'emp_type_id'=>$result['emp_type_id'],
								  'payment_mode'=>$result['payment_mode'],
								  'remark'=>$result['remarks'],
								  'payment_from'=>$result['payment_from'],
...
$data['vendor_list'] = $this->expenses->vendor_list();
 ...
 //print_r($get_by_id_data);die;
		...
		if($get_detail_by_id['type']==1)
		{
		    $this->load->model('employee/employee_model','employee');
		    $result = $this->employee->get_by_id($get_detail_by_id['employee_id']); 
		    
		    $data['persone_reg']=$result['reg_no'];
            $data['persone_name']= $result['name'];
            $data['persone_mobile'] = $result['contact_no'];
            $data['employee_id'] = $get_detail_by_id['employee_id'];
		}
		else if($get_detail_by_id['type']==2)
		{
		    $this->load->model('vendor/vendor_model','vendor');
		    $vendor_result = $this->vendor->get_by_id($get_detail_by_id['paid_to_id']); 
            $data['persone_reg']=$vendor_result['vendor_id'];
            $data['persone_name']=$vendor_result['name'];
            $data['persone_mobile']=$vendor_result['mobile'];
            //$data['persone_email']=$vendor_result['email'];
		    
		}
		$data['type'] = $get_detail_by_id['type'];
	...
	public function advance_search()
    {
        ...
        $data['type_list'] = $this->employee->employee_type_list();
	    $data['expense_category_list'] = $this->general_model->expense_category_list();
        $this->load->view('expenses/advance_search',$data);
    }
...
$get_payment_detail= $this->general_model->payment_mode_by_id('',$data['form_data']['payment_mode']);
    ...
    public function archive()
	{
		...
		$this->load->view('expenses/archive',$data);
	}
  ...
  public function expenses_excel()
    {
          ...
          $fields = array('Voucher No.','Expense Date','Expense Type','Department','Paid Amount','Paid To','Remarks','Payment From','Payment Mode');
          $col = 0;
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
						...
						if($reports->type==13)
            			{
            			   $paid_to_name = $reports->doctor_name; 
            			}
            			else
            			{
            			   $paid_to_name = $reports->paid_to_name; 
            			}
					
			
                       array_push($rowData,$reports->vouchar_no,date('d-m-Y',strtotime($reports->expenses_date)),$expenses_type.' '.$reports->patient_name.' '.$reports->book_code,$dept_list[$reports->department_type],$reports->paid_amount,$paid_to_name,$reports->remarks,$reports->payment_from,$reports->p_mode);
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
    ...
    {    
      ...
      // print  '<pre>'; print_r($data['data_list']);die;
        $view_new =$this->load->view('expenses/expenses_report_html',$data); 
    ...
    {    
      ...
      $data['data_list'] = $this->expenses->search_report_data();
      $data['dept_list']=get_expense_department();
      $this->load->view('expenses/expenses_report_html',$data); 
    }
	/* printing data */
