...
class Stock_issue_allotment_collection extends CI_Controller {
    ...
    public function index()
      {
        ...
        $data['page_title'] = 'Issue/Allot Report'; 
        ...
        $this->load->view('stock_issue_allotment/collection_list',$data);
      }
    ...
    {  
        ...
        foreach ($list as $stock_issue_allotment) { 
          ...
          // $row[] = $stock_issue_allotment->discount;
			...
			$print_url = "'".base_url('stock_issue_allotment_collection/print_stock_issue_allotment_report/'.$stock_issue_allotment->id)."'";
...
public function stock_allotment_issue_excel()
    {
        // Starting the PHPExcel library
		  ...
		  $fields = array('Issue Code','Patient / Employee / Doctor Name','Total Amount','Discount','Net Amount','Paid Amount','Balance','Return Date');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
				...
				$grand_balance_amount = $grand_balance_amount + $reports->balance;
                  ...
                  array_push($rowData,$reports->issue_no,$reports->member_name,number_format($reports->total_amount,2),number_format($reports->discount,2),number_format($reports->net_amount,2),number_format($reports->paid_amount,2),number_format($reports->balance,2),$issue_date);
                   $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
          ...
          if(!empty($data))
          {
               foreach($data as $reports_data)
               {
                    ...
                    { 
                          ...
                          $objWorksheet->setCellValueByColumnAndRow(1,$row+1,'Total');
                          // added on 31-jan-2018
                          $objWorksheet->setCellValueByColumnAndRow(2,$row+1,number_format($grand_total_amount,2));
						  $objWorksheet->setCellValueByColumnAndRow(3,$row+1,number_format($grand_total_discount,2));
                          $objWorksheet->setCellValueByColumnAndRow(4,$row+1,number_format($grand_net_amount,2));
                          $objWorksheet->setCellValueByColumnAndRow(5,$row+1,number_format($grand_paid_amount,2));
                          $objWorksheet->setCellValueByColumnAndRow(6,$row+1,number_format($grand_balance_amount,2));
          ...
          header("Content-Disposition: attachment; filename=stock_allotment_issue_collection_report_".time().".xls");  
...
public function stock_allotment_issue_csv()
    {
         ...
         // print_r($objWorksheet);die;
          ...
          // Sending headers to force the user to download the file
          header('Content-Type: application/octet-stream charset=UTF-8');
          header("Content-Disposition: attachment; filename=stock_allotment_issue_collection_report_".time().".csv");
          header("Pragma: no-cache"); 
          header("Expires: 0");
          if(!empty($data))
          {
    ...
    {    
        ...
        $this->load->view('stock_issue_allotment/stock_allotment_issue_collection_report_html',$data);
...
//echo "<pre>"print_r($item_sess);die;
          ...
          $data['page_title'] = 'Stock Allotment Add';
          $data['data_id']='';
          $data['stock_issue_allotment_item_list']=$stock_issue_allotment_item_list;
          $data['form_data']=array(
                                      'data_id'=>'',
    ...
    public function edit($id="")
    {
     ...
     //echo "<pre>" ; print_r($result);die;     
          ...
          $data['vendor_list'] = $this->stock_issue_allotment->vendor_list();
          $data['payment_mode']=$this->general_model->payment_mode();
          $data['page_title'] = 'Stock Allotment Edit';
          $data['data_id']='';
    ...
    public function usertype_list()
    {
        ...
        if(!empty($user_type)){
         ...
         if($user_type==1)
         {
                ...
                $dropdown .='</select></div></div></div> <div class="col-md-4">  <div class="row m-b-5"><div class="col-md-4"><label>Employee Name</label></div><div class="col-md-8"> <select class="w-189px" name="user_type_id" id="employee_list_n" onchange="appned_user_detail(1,this.value);"><option value="">Select</option></select></div></div></div> ';
                echo $dropdown; 
         }
         if($user_type==2)
         {
