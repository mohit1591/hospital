...
class Medicine extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('dental/medicine/medicine_model','medicine');
        $this->load->model('dental/medicine_company/medicine_company_model','medicine_company');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('288','1703');
        $data['page_title'] = 'Medicine List'; 
        $this->load->view('dental/medicine/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $medicine) {
         // print_r($relation);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('1705',$users_data['permission']['action'])){
          ...
          if(in_array('1706',$users_data['permission']['action'])){
    ...
    public function add()
    {
        unauthorise_permission('288','1704');
       ...
       $this->load->view('dental/medicine/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('288','1705');
   ...
   public function brand($str){
          ...
          if(!empty($post['brand']))
          {
               ...
               if(!empty($post['data_id']) && $post['data_id']>0)
               {
                    $data_cat= $this->medicine_company->get_by_id($post['data_id']);
                      ...
                      {
                        $medicine_company = $this->dental_general->check_medicine_company($str);
               ...
               else
               {
                    ...
                    $medicine_company = $this->dental_general->check_medicine_company($post['brand'], $post['data_id']);
    ...
    {
       unauthorise_permission('288','1706');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('eye/medicine/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('288','1707');
        $data['page_title'] = 'Medicine Archive List';
        $this->load->helper('url');
        $this->load->view('dental/medicine/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('229','1297');
        $this->load->model('dental/medicine/medicine_archive_model','medicine_archive'); 
        ...
        foreach ($list as $medicine) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1709',$users_data['permission']['action'])){
          ...
          if(in_array('1708',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('288','1709');
        $this->load->model('dental/medicine/medicine_archive_model','medicine_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('288','1708');
        $this->load->model('dental/medicine/medicine_archive_model','medicine_archive');
       if(!empty($id) && $id>0)
       {
