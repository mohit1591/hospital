...
class Advice extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('dental/advice/Advice_model','advice');
    ...
    { 
       // echo "hi";die;
        unauthorise_permission('282','1667');
        $data['page_title'] = 'Advice List'; 
        $this->load->view('dental/advice/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $chief_complaints) {
         // print_r($chief_complaints);die;
            ...
            $row[] = $chief_complaints->advice;  
            $row[] = $status;
            //$row[] = date('d-M-Y H:i A',strtotime($chief_complaints->created_date)); 
           
          $btnedit='';
          $btndelete='';
          if(in_array('1669',$users_data['permission']['action'])){
               $btnedit = ' <a onClick="return edit_advice('.$chief_complaints->id.');" class="btn-custom" href="javascript:void(0)" style="'.$chief_complaints->id.'" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>';
          }
          if(in_array('1670',$users_data['permission']['action'])){
               $btndelete = ' <a class="btn-custom" onClick="return delete_advice('.$chief_complaints->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-trash"></i> Delete</a> '; 
          }
    ...
    public function add()
    {
       unauthorise_permission('282','1668');
       ...
       $this->load->view('dental/advice/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('282','1669');
    ...
    {
       unauthorise_permission('282','1670');
       if(!empty($id) && $id>0)
       {
    ...
    {
       unauthorise_permission('282','1671');
        $data['page_title'] = 'Advice Archive List';
        $this->load->helper('url');
        $this->load->view('dental/advice/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        unauthorise_permission('282','1671');
        $users_data = $this->session->userdata('auth_users');
        $this->load->model('dental/advice/advice_archive_model','advice_archive'); 
       ...
       // print_r($list);die; 
        ...
        foreach ($list as $chief_complaints) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1673',$users_data['permission']['action'])){
               $btnrestore = ' <a onClick="return restore_advice('.$chief_complaints->id.');" class="btn-custom" href="javascript:void(0)"  title="Restore"><i class="fa fa-window-restore" aria-hidden="true"></i> Restore </a>';
          }
          if(in_array('1672',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('282','1673');
        $this->load->model('dental/advice/advice_archive_model','advice_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('282','1672');
           $this->load->model('dental/advice/advice_archive_model','advice_archive');
       if(!empty($id) && $id>0)
       {
