...
class Test extends CI_Controller {
    ...
    {  
      ...
      $booked_test = $this->session->set_userdata('book_test',[]);
      $profile_data = $this->session->set_userdata('set_profile',[]); 
  ...
  function form_f_print()
   ...
   public function set_prescription_booking_test($prescription_id)
   {
      ...
      if(isset($prescription_test_list) && !empty($prescription_test_list))
      {
          ...
          $this->session->set_userdata('book_test',$post_test);
          echo $result = $this->list_prescription_booked_test($prescription_id);
    ...
    public function getProfileDataBase($id=0)
    {
    ...
    public function setPrescriptionProfile($profile_id)
    {
      $this->session->set_userdata('set_profile',[]);
      $this->load->model('test_profile/test_profile_model');
      $result = $this->getProfileDataBase($profile_id);
      if(!empty($result))
      {
                   $profile_print_status = get_profile_print_status('test_booking');
                    $profile_status = $profile_print_status->profile_status;
                 ...
                 $profile_arr = array('id'=>$result['id'], 'name'=>$profile_name, 'order'=>1, 'price'=>$result['master_rate'],'base_price'=>$result['base_rate']);
                 $this->session->set_userdata('set_profile',$profile_arr);
                 return $profile_arr;
      }
                 // echo "<pre>";print_r($profile_arr);die;
    ...
    public function uniqueByName($array) {
     ...
     public function list_prescription_booked_test($prescription_id)
    {
...
error_reporting(E_ALL);
      ...
      //  if(empty($profile_data)){
        ...
        foreach($prescription_test_list as $key => $pIds){
         $dd = $this->setPrescriptionProfile($pIds->test_id);
         if(!empty($dd)){
          $profileData[] = $dd;
         }
        // }
       ...
       if(isset($profileData) && !empty($profileData))
        {  
          // $profileData = $this->uniqueByName($profileData);
