...
class Stock_purchase_collection extends CI_Controller {
	...
	function __construct() 
	{
  	...
  	public function index()
      {
        ...
        $this->load->view('stock_purchase/collection_list',$data);
      }
...
public function stock_purchase_excel()
    {
        // Starting the PHPExcel library
         ...
         $list = $this->stock_purchase->search_report_data();
          ...
          header("Content-Disposition: attachment; filename=stock_purchase_collection_report_".time().".xls");  
...
public function stock_purchase_csv()
    {
         ...
         //print '<pre>'; print_r($list);die;
          ...
          // Sending headers to force the user to download the file
          header('Content-Type: application/octet-stream charset=UTF-8');
          header("Content-Disposition: attachment; filename=stock_purchase_collection_report_".time().".csv");
          header("Pragma: no-cache"); 
          header("Expires: 0");
          if(!empty($data))
          {
    ...
    {    
        ...
        $this->load->view('stock_purchase/stock_purchase_collection_report_html',$data);
...
//print_r($result);die;
          ...
          $total_values[]= $get_payment_detail[$i]->field_value.'_'.$get_payment_detail[$i]->field_name.'_'.$get_payment_detail[$i]->field_id;
            ...
            // $new_purchase_list = $this->session->userdata('item_id');
            // print_r($new_purchase_list); 
            ...
            //   $this->session->set_userdata('item_id',$stock_purchase_item_list);
...
//print_r($data['stock_purchase_item_list']);die;
 }
       $data['stock_purchase_item_list'] = $stock_purchase_item_list;  
       $data['form_data']=array(
                                      'data_id'=>$result['id'],
