...
class Ipd_collection_report extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('reports/ipd_collection_report_model','ipd_collection_reports');
    ...
    {   
        ...
        $this->session->unset_userdata('ipd_collection_resport_search_data');
        $data['sub_branch'] = $this->session->userdata('sub_branches_data'); 
        $search_date = $this->session->userdata('ipd_collection_resport_search_data');
// Default Search Setting
        ...
        $data['page_title'] = 'IPD Collection Report';
        $this->load->model('general/general_model','general_model');
        $data['employee_list'] = $this->general_model->branch_user_list(); 
        $this->load->view('ipd_collection/list',$data);
    }
    public function expenses(){
       $data['page_title'] = 'IPD Collection Report';
       $this->load->view('ipd_collection/expense_report',$data);
    }
    public function ajax_list()
    {  
        unauthorise_permission('126','772');
        $users_data = $this->session->userdata('auth_users'); 
        $list = $this->ipd_collection_reports->get_datatables();
        //print_r($list);die;
        $data = array();
        $no = $_POST['start'];
        $i = 1;
...
$row[] = date('d-m-Y',strtotime($reports->c_date));  
            ...
            $btn_edit = ' <a class="btn-custom" href="'.base_url("ipd_booking/edit/".$reports->id).'" title="Edit Booking"><i class="fa fa-pencil"></i> Edit</a>';
            /* 22-jan-2018
if($reports->paid_amount_payment>0)
            {
               $print_pdf_url = "'".base_url('ipd_discharge_bill/print_discharge_bill/'.$reports->id.'/'.$reports->patient_new_id)."'";
            ...
            else if($reports->parent_id>0)
            {
                $print_pdf_url = "'".base_url('ipd_booking/print_ipd_booking_recipt/'.$reports->id)."'";
                $btn_print = ' <a class="btn-custom" href="javascript:void(0)" onClick="return print_window_page('.$print_pdf_url.')"  title="Print" ><i class="fa fa-print"></i> Print  </a>';
            }
            else
            {
...
//$print_pdf_url = "'".base_url('ipd_collection_report/print_discharge_receipt/'.$reports->id.'/'.$reports->patient_new_id.'/'.$reports->pay_id.'/'.$reports->c_date)."'";
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->ipd_collection_reports->count_all(),
                        "recordsFiltered" => $this->ipd_collection_reports->count_filtered(),
                        "data" => $data,
                );
    ...
    public function search_data()
    {
       ...
       if(!empty($post))
       {
         ...
         $this->session->set_userdata('ipd_collection_resport_search_data',$search_data);
       }
    }
    ...
    public function ipd_report_excel()
    {
        ...
        $get_date_time_setting = $this->general_model->get_date_time_setting('ipd_collection_report');
        $date_time_status = $get_date_time_setting->date_time_status;
          ...
          $fields = array('IPD No.','IPD Date','Patient Name','Referred By','Paid Amount');
        ...
        $objPHPExcel->getDefaultStyle()->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
          ...
          $list = $this->ipd_collection_reports->search_report_data();
          $rowData = array();
          $data= array();
          $ttl_paid=0;
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
                    ...
                    {
                       $bookingdate = date('d-m-Y',strtotime($reports->admission_date)).' '.date('h:i A',strtotime($reports->admission_time));
                    } 
                    else
                    {
                      $bookingdate = date('d-m-Y',strtotime($reports->admission_date));
                    }
                    array_push($rowData,$reports->ipd_no,$bookingdate ,$reports->patient_name,$reports->doctor_hospital_name,$reports->paid_amount);
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
        ...
        header("Content-Disposition: attachment; filename=ipd_collection_report_".time().".xls"); 
    ...
    public function ipd_report_csv()
    {
          ...
          // Sending headers to force the user to download the file
          header('Content-Type: application/octet-stream; charset=UTF-8');
         header("Content-Disposition: attachment; filename=ipd_collection_report_".time().".csv");
         header("Pragma: no-cache"); 
         header("Expires: 0");
         if(!empty($data))
         {
...
$post = $this->input->post(); 
        ...
        {
            $this->session->set_userdata('ipd_collection_resport_search_data',$post); 
        }
...
$data['referal_hospital_list'] = $this->general_model->referal_hospital_list();
        ...
        //$data['employee_list'] = $this->ipd_booking->employee_list();

        
        $data['profile_list'] = $this->ipd_booking->profile_list();*/
        $data['search_data'] = $this->session->userdata('ipd_collection_resport_search_data');
...
'refered_id'=>$refered_id,
    ...
    {   
        ...
        $data['print_status']="";
        $data['data_list'] = $this->ipd_collection_reports->search_report_data();
        $this->load->view('ipd_collection/ipd_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("ipd_collection_report_".time().".pdf");
    }
