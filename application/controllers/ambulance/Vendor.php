  ...
  public function vendor_excel()
    {
       // Starting the PHPExcel library
          ...
          $fields = array('Vendor Code','Vendor Name','Vendor Type','Mobile No.','Email','Status','created_date');
          ...
          $list = $this->vendor->search_vendor_data();
          $rowData = array();
          $data= array();
          if(!empty($list))
          {
               ...
               foreach($list as $vendor)
               {
                    ...
                    array_push($rowData,$vendor->vendor_id,$vendor->name,$vendor_type,$vendor->mobile,$vendor->email,$status,date('d-M-Y H:i A',strtotime($vendor->created_date)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
    ...
    public function vendor_pdf()
    {    
        $data['print_status']="";
        $data['data_list'] = $this->vendor->search_vendor_data();
        $this->load->view('ambulance/vendor/vendor_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("vendor_list_".time().".pdf");
    }
