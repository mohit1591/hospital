...
class Ambulance_referral_reports extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('ambulance/referral_reports_model','referral_reports');
    ...
    public function reports(){
       unauthorise_permission('349','2086');
          $data['page_title'] = 'Ambulance Referral Reports';
       ...
       $this->load->view('ambulance/referral_report/referral_report',$data);
    }
    ...
    public function get_appointment_data($get="")
    {
      ...
      $data['appointment_list'] = $this->referral_reports->ambulance_report_list($get);
      ...
      $data['get'] = $get;
      $this->load->view('ambulance/referral_report/referral_report_data',$data); 
    }
    ...
    public function referral_report_excel($get="")
    {
      ...
      $parent_id= $users_data['parent_id'];
      $branch_ids = array_column($branch_list, 'id'); 
      $list = $this->referral_reports->ambulance_report_list($get);
      ...
      $fields = array('S. No.','Patient Name','Age','Mobile No.','Referred By','Booking Date','Location','Branch','Remarks');
        $col = 0;
        ...
        if(!empty($list))
        {
            ...
            foreach($list as $reports)
            {
              ...
              if(in_array($reports['appointment_date'],$date_array))
              {
                ...
                array_push($rowData,date('d/m/y',strtotime($reports['booking_date'])),'','','','','','','','',''); 
                $used_date = array_search($reports['booking_date'],$date_array);
             ...
             /* if($reports['reffered']==0 && $reports['reffered']!=''){
                $reference_name=$reports['doctor_name'];
              }
              else if($reports['referred']!='' && $reports['referred']!=0){
                $reference_name=$reports['doctor_name'];
              }else{
                 $reference_name='';
              }*/
              ...
              if($reports['age_y']>0)
              {
                  $ages .= $reports['age_y'].' Years';
              }
              if($reports['age_m']>0)
              {
                  $ages .= $reports['age_m'].' Months';
              }
              if($reports['age_d']>0)
              {
                  $ages .= $reports['age_d'].' Days';
              }
             array_push($rowData,$s,$reports['patient_name'],$ages,$reports['mobile_no'],$reports['doctor_name'],date('d/m/y',strtotime($reports['booking_date'])),$reports['city'],$reports['branch_name'],'');
                $count = count($rowData); 
                for($j=0;$j<$count;$j++)
                {
                    $data[$i][$fields[$j]] = $rowData[$j];
      ...
      //print_r($list);die;
        ...
        if(!empty($list))
        {
            ...
            foreach($list as $reports)
            {
             ...
             array_push($rowData,$s,$reports['patient_name'],$ages,$reports['mobile_no'],$reference_name,date('d/m/y',strtotime($reports['booking_date'])),$reports['city'],$reports['branch_name'],'');
                $count = count($rowData); 
                for($j=0;$j<$count;$j++)
                {
                    $data[$i][$fields[$j]] = $rowData[$j];
