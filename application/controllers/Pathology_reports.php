...
class Pathology_reports extends CI_Controller {
        ...
        foreach ($list as $reports) {
         // print_r($reports);die;
            ...
            {  
              ...
              $tot_row[] = '<input type="text" class="w-100px" value="'.number_format($total_net_amount-$total_paid_amount,2).'" />';
              $tot_row[] = '';
              $data[] = $tot_row;
            }
    ...
    public function path_report_excel()
    {
        ...
        $objPHPExcel->getActiveSheet()->getRowDimension(1)->setRowHeight(20);
          ...
          if(!empty($data))
          {
              ...
              // added on 31-jan-2018
                ...
                $objPHPExcel->getActiveSheet()->setCellValue('J'.$row.'',$ttl_net_amt-$ttl_paid_amt);
        ...
        // Sending headers to force the user to download the file
         ...
         header("Content-Disposition: attachment; filename=pathology_collection_report_".time().".xls"); 
...
if(isset($search_data['branch_id']) && !empty($search_data['branch_id']))
        ...
        $this->load->view('pathology_reports/advance_search',$data);
    }
    ...
    {   unauthorise_permission('147','893');
        ...
        $this->load->view('pathology_reports/path_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("pathology_collection_report_".time().".pdf");
    }
    ...
    unauthorise_permission('43','252');
     ...
     $data['get'] = $get;
     $this->load->view('pathology_reports/list_expenses_reports',$data);  
     ...
     public function print_path_collection_reports()
    {
     ...
     $this->load->view('pathology_reports/list_collection_report_data',$data);  
    ...
    public function print_pdf()
    {
        ...
        $html = $this->load->view('pathology_reports/list_collection_report_data',$data,TRUE); 
        //echo $html; exit;
    ...
    public function advance_report()
    {
       ...
       $this->load->view('pathology_reports/advance_report',$data);
    }
       ...
       if(!empty($post))
       {
          ...
          { 
             ...
             else if($post['collection_group']=='2')
             {
               ...
               /*$total_tt = '0';
               foreach($data['dtl_test_list'] as $ttt)
               { 
                   $total_tt = $total_tt+$ttt['total_amount'];
               }
               echo '<pre>'; print_r($total_tt);die;*/
          ...
          if(!empty($seg))
          {
            $test_report_data = $this->load->view('pathology_reports/advance_report_data',$data,true);
            $users_data = $this->session->userdata('auth_users');
            $this->load->library('m_pdf'); 
            $file_name = str_replace(' ', '-',$users_data['user_name']);
          ...
          { //echo "<pre>"; print_r($data); exit;
            $this->load->view('pathology_reports/advance_report_data',$data);
          }
