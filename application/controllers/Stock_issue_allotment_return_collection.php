...
class Stock_issue_allotment_return_collection extends CI_Controller {
	...
	function __construct() 
	{
  	...
  	public function index()
      {
        ...
        $data['page_title'] = 'Issue/Allot Return Report'; 
        ...
        $this->load->view('stock_issue_allotment_return/collection_list',$data);
      }
 ...
 //echo "<pre>"; print_r( $list); 
        ...
        foreach ($list as $stock_issue_allotment_return) { 
           ...
           // $doctor_type = array('0'=>'Referral','1'=>'Attended','2'=>'Referral/Attended');
            ...
            $print_url = "'".base_url('stock_issue_allotment_return_collection/print_stock_issue_allotment_report/'.$stock_issue_allotment_return->id)."'";
   ...
   public function stock_allotment_return_issue_excel()
    {
		  ...
		  $fields = array('Return Code','Issue Code','Patient / Employee / Doctor Name','Total Amount','Discount','Net Amount','Paid Amount','Balance','Return Date');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
				...
				$grand_balance_amount = $grand_balance_amount + $reports->balance;
                  ...
                  array_push($rowData,$reports->return_no,$reports->issue_no,$reports->member_name,number_format($reports->total_amount,2),number_format($reports->discount,2),number_format($reports->net_amount,2),number_format($reports->paid_amount,2),number_format($reports->balance,2),$issue_date);
                   $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
          ...
          header("Content-Disposition: attachment; filename=stock_allotment_issue_return_collection_report_".time().".xls");  
...
public function stock_allotment_return_issue_csv()
    {
         ...
         // print_r($objWorksheet);die;
          ...
          // Sending headers to force the user to download the file
          header('Content-Type: application/octet-stream charset=UTF-8');
          header("Content-Disposition: attachment; filename=stock_allotment_issue_return_collection_report_".time().".csv");
          header("Pragma: no-cache"); 
          header("Expires: 0");
          if(!empty($data))
          {
    ...
    {    
        ...
        $this->load->view('stock_issue_allotment_return/stock_allotment_issue_return_collection_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("stock_allotment_issue_return_collection_report_".time().".pdf");
    }
...
//echo "<pre>"; print_r($_POST);die;
          ...
          $data['employee_list']= $this->stock_issue_allotment_return->get_employee($post['employee_type']);
