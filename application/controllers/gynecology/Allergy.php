...
class Allergy extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('gynecology/allergy/Allergy_model','allergy');
    ...
    { 
        ...
        unauthorise_permission('300','1781');
        $data['page_title'] = 'Allergy List'; 
        $this->load->view('gynecology/allergy/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $allergy_list) {
         // print_r($chief_complaints);die;
            $no++;
            $row = array();
            if($allergy_list->status==1)
            {
            ...
            ////////// Check list end ///////////// 
            $row[] = '<input type="checkbox" name="employee[]" class="checklist" value="'.$allergy_list->id.'">'.$check_script; 
            $row[] = $allergy_list->allergy_name;  
            $row[] = $status;
            //$row[] = date('d-M-Y H:i A',strtotime($allergy_list->created_date)); 
           
          $btnedit='';
          $btndelete='';
          if(in_array('1783',$users_data['permission']['action'])){
               $btnedit = ' <a onClick="return edit_allergy('.$allergy_list->id.');" class="btn-custom" href="javascript:void(0)" style="'.$allergy_list->id.'" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>';
          }
          if(in_array('1784',$users_data['permission']['action'])){
               $btndelete = ' <a class="btn-custom" onClick="return delete_disease('.$allergy_list->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-trash"></i> Delete</a> '; 
          }
    ...
    public function add()
    {
         unauthorise_permission('300','1782');
       ...
       $this->load->view('gynecology/allergy/add',$data);       
    }
    
    public function edit($id="")
    {
     unauthorise_permission('300','1783');
    ...
    private function _validate()
    {
        ...
        $this->form_validation->set_rules('allergy_name', 'allergy name', 'trim|required|callback_allergy_name'); 
    ...
    public function allergy_name($str){
          ...
          {
               $this->form_validation->set_message('allergy_name', 'allergy field is required.');
               return false; 
          } 
     }
     /* check validation laready exit */
    ...
    {
      unauthorise_permission('300','1784');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('300','1785');
        $data['page_title'] = 'Allergy Archive List';
        $this->load->helper('url');
        $this->load->view('gynecology/allergy/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
       unauthorise_permission('300','1785');
        $users_data = $this->session->userdata('auth_users');
        $this->load->model('gynecology/allergy/allergy_archive_model','allergy_archive'); 
       ...
       // print_r($list);die; 
        ...
        foreach ($list as $chief_complaints) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1787',$users_data['permission']['action'])){
          ...
          if(in_array('1786',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('300','1787');
        $this->load->model('gynecology/allergy/allergy_archive_model','allergy_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('300','1786');
           $this->load->model('gynecology/allergy/allergy_archive_model','allergy_archive');
       if(!empty($id) && $id>0)
       {
