...
class Prescription_setting extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('gynecology/prescription_setting/prescription_setting_model','prescriptionsetting');
    ...
    public function index()
    {
        ...
        unauthorise_permission(316,1897);
        $data['page_title'] = 'Gynecology Prescription Settings'; 
        $post = $this->input->post();
        if(!empty($post))
        { 
            $this->prescriptionsetting->save();
            echo 'Your gynecology Prescription Settings successfully updated.';;
            return false;
        }

        
        $data['prescription_setting_list'] = $this->prescriptionsetting->get_master_unique(0);
        $this->load->view('gynecology/prescription_setting/add',$data);
    }


    public function preview($id="")
    {
        $this->load->model('gynecology/prescription/prescription_model','prescription');
        ...
        $this->load->view('gynecology/prescription_setting/preview',$data);
    } 
