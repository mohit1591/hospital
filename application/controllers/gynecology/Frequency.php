...
class Frequency extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('gynecology/frequency/frequency_model','frequency');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('306','1830');
        $data['page_title'] = 'Frequency List'; 
        $this->load->view('gynecology/frequency/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $frequency) {
         // print_r($relation);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('1832',$users_data['permission']['action'])){
           ...
           if(in_array('1833',$users_data['permission']['action'])){
    ...
    public function add()
    {
        unauthorise_permission('306','1831');
       ...
       $this->load->view('gynecology/frequency/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('306','1832');
    ...
    {
       unauthorise_permission('306','1833');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('gynecology/frequency/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('306','1834');
        $data['page_title'] = 'Frequency Archive List';
        $this->load->helper('url');
        $this->load->view('gynecology/frequency/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('306','1834');
        $this->load->model('gynecology/frequency/frequency_archive_model','frequency_archive'); 
        ...
        foreach ($list as $frequency) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1836',$users_data['permission']['action'])){
          ...
          if(in_array('1835',$users_data['permission']['action'])){
    ...
    {
       unauthorise_permission('306','1836');
        $this->load->model('gynecology/frequency/frequency_archive_model','frequency_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('306','1835');
        $this->load->model('gynecology/frequency/frequency_archive_model','frequency_archive');
       if(!empty($id) && $id>0)
       {
