        ...
        if($users_data['parent_id']==$prescription->branch_id)
        {
            ...
            if($users_data['parent_id']==$prescription->branch_id)
            {
                ...
                elseif($prescription->flag==3)
                {
                    ...
                    {
                    $btn_edit = ' <a class="btn-custom" href="'.base_url("gynecology/gynecology_prescription/edit/".$prescription->flag_id.'/'.$prescription->booking_id).'" title="Edit Prescription"><i class="fa fa-pencil"></i> Edit gynecology Prescription</a>';
                    }
            ...
            else
            {
                ...
                {
                   $print_url = "'".base_url('prescription/print_prescriptions/'.$prescription->id.'/'.$prescription->branch_id)."'";
                  $btn_print_pre = ' <a class="btn-custom" href="'.base_url('prescription/print_prescriptions/'.$prescription->id.'/'.$prescription->branch_id).'" title="Print Prescription"  data-url="512"><i class="fa fa-print"></i> Print Prescription</a>';
                } 
            }
  ...
  public function prescription_dropdown()
  {
    ...
    public function print_prescriptions($prescription_id="",$branch_id='')
    {
        ...
        $data['template_format_bottom']=$template_format_bottom->setting_value;
         $data['vital_show']=$template_format_top->vital_show;
   ...
   if(!empty($signature_image))
   {
     ...
     $signature_reprt_data .='<table border="0" cellpadding="4px" cellspacing="0" width="100%" style="border-collapse: collapse;font-size:13px;margin-top: 5%;"> 
      ...
      <td width="70%"></td>
        ...
        if($template_format_top->vital_show ==1){
        $this->load->library('m_pdf');
        
        $this->m_pdf->pdf->setAutoTopMargin = 'stretch'; 
        $this->m_pdf->pdf->SetHeader('<div style="width:100%;height:100px;text-align:center"></div>');
...
{ 
  ...
  $data['page_title'] = "Prescription Files";
  $data['prescription_id'] = $prescription_id;
