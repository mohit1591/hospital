<?php
class Doctor_handover_template extends CI_Controller
{
    protected $gender;
    public function __construct()
    {
        ...
        $this->load->model('doctor_handover_template/doctor_handover_template_model','doctor_handover_template');
    ...
    {
        $data['page_title'] = 'Doctor Handover Template List'; 
        $this->load->view('doctor_handover_template/list',$data);
    }
    ...
    { 
       ...
       $parent_branch_details = $this->session->userdata('parent_branches_data');
            ...
            $list = $this->doctor_handover_template->get_datatables();
         ...
         // print_r($religion);die;
            ...
            if($users_data['parent_id']==$mc->branch_id){
                 ...
                 if(in_array('52',$users_data['permission']['action'])) 
                 {
                    ...
                    $btnedit = ' <a href="'.base_url('doctor_handover_template/edit/').$mc->id.'" class="btn-custom" href="javascript:void(0)" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>';
                 }
                 ...
                 $print_medication_chart_print = "'".base_url('doctor_handover_template/print_details/'.$mc->id)."'";
            
                // $btn_medication_chart_print = '<a class="btn-custom" href="javascript:void(0)" onClick="return print_window_page('.$print_medication_chart_print.')"  title="Print Doctor Handover Template" ><i class="fa fa-bar-chart"></i> Print</a>';
            }
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->doctor_handover_template->count_all(),
                        "recordsFiltered" => $this->doctor_handover_template->count_filtered(),
                        "data" => $data,
                );
...
// error_reporting(E_ALL);
        ...
        $data['page_title'] = "Add Doctor Handover Template";
        ...
        {   
          ...
          $this->doctor_handover_template->save();
          ...
          redirect(base_url('doctor_handover_template'));
        }   
        $this->load->model('ipd_booking/ipd_booking_model');
        $this->load->model('patient/patient_model');
        if(!empty($booking_id)) {
...
// dd($ipd_booking_data);
        ...
        // $data['templates'] = $this->db->get('hms_ipd_doctor_handover_template_template')->result_array();
        ...
        $this->load->view('doctor_handover_template/add',$data);
    }
    ...
    public function edit($id="")
    {
        ...
        $data['form_error'] = []; 
        $data['all_details'] = $this->doctor_handover_template->get_by_id($id);
        $this->load->model('ipd_booking/ipd_booking_model');
        $this->load->model('patient/patient_model');
        ...
        {   
          ...
          $this->doctor_handover_template->update($id);
    ...
    public function delete($id="")
    {
        ...
        $this->doctor_handover_template->delete($id);
        $this->session->set_flashdata('success','Doctor Handover Template successfully deleted.');
        redirect(base_url('doctor_handover_template'));
    }
    ...
    {
        if(empty($id))
            $id= $this->session->userdata('doctor_handover_template_id');
        // ini_set('display_errors', 1);
        // ini_set('display_startup_errors', 1);
        // error_reporting(E_ALL);
        $data = [];
        ...
        $this->load->view('doctor_handover_template/print_details',$data);
    }
    ...
    $post = $this->input->post();  
        ...
        if(!empty($post))
        {
            $result = $this->doctor_handover_template->deleteall($post['row_id']);
            $response = "Doctor Handover Templates successfully deleted.";
            echo $response;
        }
    }
    ...
    public function get_details_by_id($id) {
        $data = $this->doctor_handover_template->get_by_id($id);
        echo json_encode($data);
    }
}
?>