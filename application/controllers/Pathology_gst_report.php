...
class Pathology_gst_report extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pathology_gst_report/pathology_gst_report_model','reports');
    ...
    {   
        ...
        $data['page_title'] = 'Pathology GST Report';
        ...
        $this->load->view('pathology_gst_report/list',$data);
    }
    ...
    public function path_report_excel()
    {
        ...
        // added on 31-jan-2018
            $amt_ttl=0;
            $data_d= get_setting_value('PATIENT_REG_NO');
            $fields = array('S. No.','Booking Date',$data_d,'Lab Reg. No.','Patient Name','Total Amount','GST');
          $col = 0;
        ...
        if(!empty($list))
        {
            ...
            {
              // added on 31-jan-2018
                $amt_ttl=$reports->gst_amount+$amt_ttl;
                $row[] = date('d M Y',strtotime($reports->booking_date));
            ...
            $row[] = $reports->total_amount;
            $row[] = $reports->gst_amount;
                array_push($rowData,$m,date('d M Y',strtotime($reports->booking_date)),$reports->patient_code,$reports->lab_reg_no,$reports->patient_name,$reports->total_amount,$reports->gst_amount);
                $count = count($rowData);
                for($j=0;$j<$count;$j++)
                {
        ...
        // Sending headers to force the user to download the file
         ...
         header("Content-Disposition: attachment; filename=pathology_gst_report_".time().".xls"); 
    ...
    public function advance_search()
    {
        ...
        $this->load->view('pathology_gst_report/advance_search',$data);
    }
    ...
    {   //unauthorise_permission('147','893');
        ...
        $this->load->view('pathology_gst_report/path_report_html',$data);
    ...
    unauthorise_permission('43','252');
     ...
     $data['get'] = $get;
     $this->load->view('pathology_gst_report/list_expenses_reports',$data);  
     ...
     public function print_path_collection_reports()
    {
     ...
     $this->load->view('pathology_gst_report/list_collection_report_data',$data);  
    ...
    public function print_pdf()
    {
        ...
        $html = $this->load->view('pathology_gst_report/list_collection_report_data',$data,TRUE); 
        //echo $html; exit;
    ...
    public function advance_report()
    {
       ...
       $this->load->view('pathology_gst_report/advance_report',$data);
    }
    ...
    { 
       ...
       if(!empty($post))
       {
          ...
          if(!empty($seg))
          {
            $test_report_data = $this->load->view('pathology_gst_report/advance_report_data',$data,true);
            $users_data = $this->session->userdata('auth_users');
            $this->load->library('m_pdf'); 
            $file_name = str_replace(' ', '-',$users_data['user_name']);
          ...
          { //echo "<pre>"; print_r($data); exit;
            $this->load->view('pathology_gst_report/advance_report_data',$data);
          }
