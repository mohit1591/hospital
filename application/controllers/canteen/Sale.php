...
class Sale extends CI_Controller {
  ...
  function __construct() 
  {
    parent::__construct();  
    auth_users();  
    $this->load->model('canteen/sale/sale_model','sale');
  ...
  public function index()
    {
        ...
        $this->session->unset_userdata('sale_search_item');
// Default Search Setting
        ...
        // End Defaul Search 
         $data['form_data'] = array('sale_no'=>'','patient_id'=>'','paid_amount_to'=>'','paid_amount_from'=>'','balance_to'=>'','balance_from'=>'','start_date'=>$start_date, 'end_date'=>$end_date);
        $data['page_title'] = 'Sale Item List'; 
        $this->load->view('canteen/sale/list',$data);
    }
...
//echo"<pre>"; print_r($list); die;	
        ...
        foreach ($list as $sale) { 
            ...
            $row[] = '<input type="checkbox" name="sale[]" class="checklist" value="'.$sale->id.'">'.$check_script;  
            $row[] = $sale->sale_no;
			if(!empty($sale->customer_code)){ $row[] = $sale->customer_code; } else { $row[] = $sale->patient_code;}
			if(!empty($sale->customer_name)){ $row[] = $sale->customer_name; } else { $row[] = $sale->patient_name.' (Patient)';}
            //$row[] = $sale->customer_code;
            //$row[] = $sale->customer_name;
            //$row[] = $sale->total_amount;
            $row[] = $sale->net_amount;
            $row[] = $sale->paid_amount;
            $row[] = $sale->balance;
            $row[] = date('d-M-Y',strtotime($sale->sale_date));  
            $users_data = $this->session->userdata('auth_users');
            $btnedit='';
            $btndelete='';
            $btnview = '';
            if($session_data['parent_id']==$sale->branch_id){

           //if(in_array('387',$users_data['permission']['action'])){
                 $btnedit = ' <a onClick="return edit_sale('.$sale->id.');" class="btn-custom" href="javascript:void(0)" style="'.$sale->id.'" title="Edit"><i class="fa fa-pencil"></i> Edit</a>';
           // }
		   
            //if(in_array('125',$users_data['permission']['action'])){
                /*$btnview=' <a class="btn-custom" onclick="return view_purchase('.$sale->id.')" href="javascript:void(0)" title="View"><i class="fa fa-info-circle"></i> View </a>';*/
           // }
		   
            //if(in_array('388',$users_data['permission']['action'])){
                $btndelete = ' <a class="btn-custom" onClick="return delete_sale('.$sale->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-trash"></i> Delete</a> ';
           // }
          }

          $print_url = "'".base_url('canteen/sale/print_sale_recipt/'.$sale->id)."'";
    ...
    public function item_sale_excel()
    {
         $this->load->library('excel');
          ...
          $fields = array('Sale No.','Customer No.','Customer Name','Net Amount','Paid Amount','Balance','Sale Date' ,'Sale Time');
          ...
          $list = $this->sale->search_report_data();
          $rowData = array();
          $data= array();
          if(!empty($list))
          {
               ...
               {
				 if(!empty($reports->customer_code)){ $customer_code = $reports->customer_code; } else { $customer_code = $reports->patient_code;}
			     if(!empty($reports->customer_name)){ $customer_name = $reports->customer_name; } else { $customer_name = $reports->patient_name.' (Patient)';} 
			     
			     

                    array_push($rowData,$reports->sale_no,$customer_code,$customer_name,$reports->net_amount,$reports->paid_amount,$reports->balance, 
                    date('d-M-Y',strtotime($reports->sale_date)), date('H:i A',strtotime($data->sale_time)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
    ...
    public function item_sale_csv()
    {
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
			     ...
			     if(!empty($reports->customer_name)){ $customer_name = $reports->customer_name; } else { $customer_name = $reports->patient_name.' (Patient)';} 
                    ...
                    array_push($rowData,$reports->sale_no,$customer_code,$customer_name,$reports->net_amount,$reports->paid_amount,$reports->balance,date('d-M-Y',strtotime($reports->sale_date)) ,date('H:i A',strtotime($reports->sale_time)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
         ...
         header("Content-Disposition: attachment; filename=item_sale_report_".time().".csv");  
         header("Pragma: no-cache"); 
         header("Expires: 0");
         if(!empty($data))
         {
    ...
    public function pdf_item_sale()
    {    
        $data['print_status']="";
        $data['data_list'] = $this->sale->search_report_data();
        $this->load->view('canteen/sale/item_sale_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("item_sale_report_".time().".pdf");
    }
    public function print_item_sale()
    {    
      $data['print_status']="1";
      $data['data_list'] = $this->sale->search_report_data();
      $this->load->view('canteen/sale/item_sale_report_html',$data); 
    ...
    public function ajax_list_item(){
       ...
       $table = '';
        ...
        { 
          ...
          $data['item_new_list'] = $this->sale->item_list($item_ids);
        ...
        if(!empty($post['item']) || !empty($post['item_code']) || !empty($post['bar_code']) || !empty($post['manuf_company']) || !empty($post['conv']) || !empty($post['mfc_date']) || !empty($post['unit1']) ||  !empty($post['unit2']) || !empty($post['mrp']) || $post['mrp']==0 || !empty($post['discount']) || $post['discount']==0 || !empty($post['cgst']) || $post['cgst']==0 || !empty($post['hsn_no']) || !empty($post['igst']) || !empty($post['hsn_no']) || $post['igst']==0 || !empty($post['sgst']) || $post['sgst']==0 ||!empty($post['packing']))
        { 
        
          $result_item = $this->sale->item_list_search();  
    ...
    public function advance_search()
      {
          ...
          if(isset($post) && !empty($post))
          {
              ...
              $this->session->set_userdata('sale_search_item', $marge_post);
          }
          $sale_search_item = $this->session->userdata('sale_search_item');
          if(isset($sale_search_item) && !empty($sale_search_item))
          {
              $data['form_data'] = $sale_search_item;
          }
          $this->load->view('canteen/sale/advance_search',$data);
   }
   ...
   public function reset_search()
    ...
    public function ajax_added_item(){
        ...
        { 
          ...
          $item_ids = implode(',', $ids_arr);
          $result_item = $this->sale->item_list($item_ids);
      ...
      if(!empty($bar_code))
{
 $mbid= $this->input->post('mbid');
      $return= $this->sale->check_bar_code($bar_code,$mbid);
      if($return==2)
      {
        echo '1';exit;
      }
    ...
    public function set_item()
    {
       ...
       {
         $sale = $this->session->userdata('item_id');
         $item_id = [];
         $iid_arr = [];
         ...
         { 
           $total_amount=0;
            ...
            {
               $item_data = $this->sale->item_list($i_ids);
//print_r($item_data);
                $qty=0;
                $perpic_rate=$item_data[0]->mrp;
                $tot_qty_with_rate=$item_data[0]->mrp*$item_data[0]->qty;
                ...
                $post_iid_arr[$i_ids] = array('iid'=>$i_ids,'perpic_rate'=>$perpic_rate,'manuf_date'=>'00-00-0000','batch_no'=>0,'hsn_no'=>$item_data[0]->hsn_no,'qty'=>'1','freeqty'=>'1', 'exp_date'=>'00-00-0000','discount'=>$item_data[0]->discount,'mrp'=>$item_data[0]->mrp,'cgst'=>$item_data[0]->cgst,'igst'=>$item_data[0]->igst,'sgst'=>$item_data[0]->sgst,'qty'=>$qty, 'total_amount'=>$total_amount,'bar_code2'=>$post['barcode'][$i]); 
                $iid_arr[] = $i_ids;
                $i++;
            } 
            
            $item_id = $sale+$post_iid_arr;
         ...
         else
         {
            ...
            foreach($post['item_id'] as $i_ids)
            {
              ...
              $tot_qty_with_rate=$item_data[0]->mrp*$item_data[0]->qty;
                ...
                $total_amount = $total_amount;
                $item_data = $this->sale->item_list($i_ids);
                $post_iid_arr[$i_ids] = array('iid'=>$i_ids,'unit1'=>0,'unit2'=>0,'conversion'=>0,'batch_no'=>0,'manuf_date'=>'00-00-0000','perpic_rate'=>$perpic_rate,'freeunit1'=>0,'freeunit2'=>0,'hsn_no'=>$item_data[0]->hsn_no,'qty'=>'1', 'exp_date'=>'00-00-0000', 'mrp'=>$item_data[0]->mrp,'qty'=>$qty,'discount'=>$item_data[0]->discount,'cgst'=>$item_data[0]->cgst,'igst'=>$item_data[0]->igst,'sgst'=>$item_data[0]->sgst,'total_amount'=>$total_amount,'bar_code'=>$post['barcode'][$i]); 
                $iid_arr[] = $i_ids;
                $i++;
            }
  ...
  public function add($cid="",$patient_id="")
  {
//print_r($_POST); die;
       ...
       if($cid>0)
        {
           $customer = $this->sale->get_customer_by_id($cid);
    //echo"<pre>"; print_r($customer); die;
		...
		else if($pid>0)
       {
           ...
           {
              $patient = $this->sale->get_patient_by_id($pid);
 ...
 //echo "<pre>";print_r($data['item_new_list']);die; 
	...
	//  print_r($data['form_data']);die;							
        ...
        {   
            ...
            {
                $return_id= $this->sale->save();
              if(!empty($return_id))
              {
                
                $get_by_id_data = $this->sale->get_by_id($return_id);
                $get_customer_by_id = $this->sale->get_customer_by_id($get_by_id_data['customer_id']);
  //print_r($get_by_id_data); exit;
                $customer_name = $get_customer_by_id['customer_name'];
               
                $mobile_no = $get_customer_by_id['mobile_no'];
                $email = $get_customer_by_id['customer_email'];
                //print_r($get_by_id_data); exit;
                $sale_id = $get_by_id_data['sale_id'];
                ...
                if(in_array('640',$users_data['permission']['action']))
                {
                  ...
                  if(!empty($mobile_no))
                  {
                    send_sms('purchase_item',6,$customer_name,$mobile_no,array('{Name}'=>$customer_name,'{Amt}'=>$net_amount,'{BillNo}'=>$sale_id,'{PaidAmt}'=>$paid_amount)); 
                  }
              }
              ...
              if(in_array('641',$users_data['permission']['action']))
              {
                if(!empty($email))
                {
                  ...
                  $this->general_functions->email($email,'','','','','1','purchase_item','6',array('{Name}'=>$customer_name,'{Amt}'=>$net_amount,'{BillNo}'=>$sale_id,'{PaidAmt}'=>$paid_amount));
                   
                }
              }
            }
                $this->session->set_userdata('sale_id',$return_id);
              //   echo $return_id;die; 
                $this->session->set_flashdata('success','sale has been successfully added.');
				redirect(base_url('canteen/sale'));
               // redirect(base_url('canteen/sale/add/?status=print'));
            }
            else
            {
      ...
      $this->load->view('canteen/sale/add',$data);
  }
  ...
  public function edit($id="")
    {
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {       
         $post = $this->input->post();
          $result = $this->sale->get_by_id($id);
          $data['max_pur_qty']=get_setting_value('Set_Purchase_Limit_Per_Medicine');

         $item_id_arr=[];
         $result_customer = $this->sale->get_customer_by_id($result['customer_id']);
	//echo "<pre>"; print_r($result_customer); 	die; 	 
		 $result_patient = $this->sale->get_patient_by_id($result['patient_id']);
	 //echo "<pre>"; print_r($result_patient); 	die; 
         if(empty($post))
         { 
            $result_item_list = $this->sale->get_item_by_sale_id($id);
    // echo "<pre>"; print_r($result_item_list); 
     ...
     // print_r($item_new_list);die;
        ...
        $get_payment_detail= $this->sale->payment_mode_detail_according_to_field($result['mode_payment'],$id);
        ...
        //$data['manuf_company_list'] = $this->sale->manuf_company_list();
        $data['page_title'] = "Sale Item";  
        ...
        $data['form_data'] = array(
								...
								"customer_code"=>$result_customer['customer_code'],
								"cname"=>$result_customer['customer_name'],
								"caddress"=>$result_customer['address'],
								"caddress2"=>$result_customer['address2'],
								"caddress3"=>$result_customer['address3'],
								"cmobile"=>$result_customer['mobile_no'],
								"cemail"=>$result_customer['customer_email'],
								 "cremarks"=>$result_customer['remark'],

								  "patient_code"=>$result_patient['patient_code'],	
								  'patient_id'=>$result_patient['id'],	
								  "pname"=>$result_patient['patient_name'],
								  "paddress"=>$result_patient['address'],
                                  "paddress2"=>$result_patient['address2'],
                                  "paddress3"=>$result_patient['address3'],
								  "gender"=>$result_patient['gender'],
								  "pmobile"=>$result_patient['mobile_no'],
								  "pemail"=>$result_patient['patient_email'],
								  "premarks"=>$result_patient['remark'],
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
                 ...
                 $this->session->set_userdata('sale_id',$return_id);
                 $this->session->set_flashdata('success','sale has been successfully updated.');
				 redirect(base_url('canteen/sale'));
                // redirect(base_url('canteen/sale/?status=print'));
    ...
    { 
      ...
      // print_r($item_list);die;
       ...
       //$total_amountwithigst=0;
        ...
        {    
          //print_r($item['sale_id']);die;
          if($item['sale_id']!="")
          {
          $tot_qty_with_rate = $item['mrp']*$item['qty'];
...
// print_r($post); exit;   
       ...
       if(isset($post) && !empty($post))
       {
         ...
         if(!empty($item_list))
         { 
            $item_data = $this->sale->item_list($post['item_id']);
			
            $ratewithqty= $post['mrp']*$post['qty'];
            ...
            $total_amount = $total_amount+$tot_price;
             ...
             $item_list[$post['item_id']] = array('iid'=>$post['item_id'],'item_id'=>$post['item_id'],'manuf_date'=>$post['manuf_date'],'perpic_rate'=>$perpic_rate,'batch_no'=>$post['batch_no'],'hsn_no'=>$post['hsn_no'],'exp_date'=>$post['expiry_date'],'qty'=>$qty,'mrp'=>$post['mrp'],'sgst'=>$post['sgst'],'igst'=>$post['igst'],'cgst'=>$post['cgst'],'discount'=>$post['discount'],'total_amount'=>$total_amount,'total_price'=>$total_amount); 
    ...
    private function _validate($id='')
    {
        ...
        $this->form_validation->set_rules('name', 'Customer name', 'trim|required');
         //$this->form_validation->set_rules('patient_id', 'invoice id', 'trim|required|callback_check_unique_value['.$id.']'); 
    ...
    public function delete($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           ...
           $response = "sale successfully deleted.";
           echo $response;
       }
    }
    ...
    function deleteall()
    {
       // unauthorise_permission(58,388);
        ...
        if(!empty($post))
        {
            $result = $this->sale->deleteall($post['row_id']);
            $response = "sale successfully deleted.";
            echo $response;
        }
    }
    ...
    {  
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('canteen/sale/view',$data);     
      }
    }  
    ...
    public function archive()
    {
      // unauthorise_permission(58,389);
        $data['page_title'] = 'Sale archive list';
        $this->load->helper('url');
        $this->load->view('canteen/sale/archive',$data);
    }

    public function archive_ajax_list()
    {
       // unauthorise_permission(58,389);
        $this->load->model('canteen/sale/sale_archive_model','sale_archive'); 

        $list = $this->sale_archive->get_datatables();  
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        $total_num = count($list);
        foreach ($list as $sale) { 
           ...
           if(in_array('391',$users_data['permission']['action'])){
                $btnrestore = ' <a onClick="return restore_purchase('.$sale->id.');" class="btn-custom" href="javascript:void(0)"  title="Restore"><i class="fa fa-window-restore" aria-hidden="true"></i> Restore </a>';
           }
           if(in_array('390',$users_data['permission']['action'])){
                $btndelete = ' <a onClick="return trash('.$sale->id.');" class="btn-custom" href="javascript:void(0)" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a>';
           }
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->sale_archive->count_all(),
                        "recordsFiltered" => $this->sale_archive->count_filtered(),
                        "data" => $data,
                );
        //output to json format
        echo json_encode($output);
    ...
    public function restore($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           $result = $this->sale_archive->restore($id);
           $response = "Sale successfully restore in item sale list.";
           echo $response;
       }
    }
    ...
    { 
       //unauthorise_permission(58,391);
        ...
        if(!empty($post))
        {
            $result = $this->sale_archive->restoreall($post['row_id']);
            $response = "Sale successfully restore in sale list.";
            echo $response;
        }
    }
    ...
    public function trash($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           $result = $this->sale_archive->trash($id);
           $response = "sale  successfully deleted parmanently.";
           echo $response;
       }
    }
    ...
    function trashall()
    {
     // unauthorise_permission(58,390);
        ...
        if(!empty($post))
        {
            $result = $this->sale_archive->trashall($post['row_id']);
            $response = "sale successfully deleted parmanently.";
            echo $response;
        }
    }
  ...
  /*public function medicine_entry_dropdown()
  {
   ...
   public function print_sale_recipt($ids="")
  {
     $user_detail= $this->session->userdata('auth_users');
     //print '<pre>';print_r($user_detail);die;
      $this->load->model('general/general_model');
      $data['page_title'] = "Add sales Item";
      if(!empty($ids)){
        $sales_id= $ids;
     }else{
       $sales_id= $this->session->userdata('sales_id');
     }
      $get_detail_by_id= $this->sale->get_by_id($sales_id);

      $get_by_id_data=$this->sale->get_all_detail_print($sales_id,$get_detail_by_id['branch_id']);
      $get_payment_detail= $this->general_model->payment_mode_by_id('',$get_detail_by_id['payment_mode']);
      $template_format= $this->sale->template_format(array('section_id'=>15,'types'=>1,'sub_section'=>2,'branch_id'=>$get_detail_by_id['branch_id']));
      $data['template_data']=$template_format;
      $data['payment_mode']= $get_payment_detail;
      $data['user_detail']=$user_detail;
      $data['all_detail']= $get_by_id_data;
      $this->load->view('canteen/sale/print_template_sale',$data);
  ...
  /********** estimate_item**************/
   ...
   function estimate_item($vals="")
    {
        if(!empty($vals))
        {
            $result = $this->sale->estimate_item($vals);  
           // print_r($result);die;
            if(!empty($result))
            {
    ...
    public function set_item_stockitem()
   {
       ...
       $post =  $this->input->post(); 
     // print_r($post['sale_id']);die('fgf');
       if(isset($post['sale_id']) && !empty($post['sale_id']))
       {
        $result = $this->sale->get_estimate_medicine_by_id($post['sale_id']);  
   ...
   //      echo "<pre>";print_r($item_sess);die;
        ...
        { 
         ...
         // $batch_nos = implode(',', $batch_arr); 
          //echo $batch_nos;
        //  $result_item = $this->sale->get_medicine_by_id($key_m_arr->item_id);
    ...
    // die;
       $check_script='';
                     ...
                     <input type="hidden" id="mrp'.$item['id'].$item['batch_no'].'" name="mrp[]" value="'.$item['mrp'].'"/><input type="hidden" id="batch_no_'.$item['id'].$item['batch_no'].'" name="batch_no[]" value="'.$item['batch_no'].'"/><input type="hidden" id="conversion_'.$item['id'].$item['batch_no'].'" name="conversion[]" value="'.$item['conversion'].'"/>';
    ...
    public function get_canteen_vendors()
    {
      ...
      $all_data=$this->sale->get_canteen_vendors($item_id);
      if(!empty($all_data))
      {
        $tr='';
         foreach ($all_data as $values) 
         {
          $tr.='<tr onClick="get_mrp_rates('.$values['mrp'].','.$item_id.');">
                  <td>'.$values['item'].'</td>
                  <td>'.$values['name'].'</td>
                  <td>'.date('d-m-Y', strtotime($values['sale_date'])).'</td>
                  <td>'.$values['mrp'].'</td>
                </tr>';
         }
      }
  ...
  {     
        $users_data = $this->session->userdata('auth_users');
        $result = $this->sale->check_unique_value($users_data['parent_id'], $invoice, $id);
        if($result == 0)
            $response = true;
        else {
