...
//echo"<pre>"; print_r($list); die;	
        ...
        foreach ($list as $sale) { 
          ...
          $new_path=base_url().'canteen/sale/print_sale_recipt';
          //$btnprint = '<a class="btn-custom" onclick="openPrintWindow(123,'.$sale->id.');" target="_blank"><i class="fa fa-print"></i> Print</a>';
          //;$print_url = "'".base_url('canteen/sale/print_purchase_recipt/'.$sale->id)."'";
    ...
    public function item_sale_excel()
    {
         $this->load->library('excel');
          ...
          $fields = array('Sale No.','Customer No.','Customer Name','Net Amount','Paid Amount','Balance','Sale Date');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
			     ...
			     if(!empty($reports->customer_name)){ $customer_name = $reports->customer_name; } else { $customer_name = $reports->patient_name.' (Patient)';} 
                    ...
                    array_push($rowData,$reports->sale_no,$customer_code,$customer_name,$reports->net_amount,$reports->paid_amount,$reports->balance, date('d-M-Y H:i A',strtotime($reports->sale_date)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
...
// print_r($post); exit;   
  ...
  public function print_sale_recipt($ids="")
  {
      ...
      $data['page_title'] = "Add sale";
      $get_detail_by_id= $this->sale->get_by_id($sale_id);

      $get_by_id_data=$this->sale->get_all_detail_print($sale_id,$get_detail_by_id['branch_id']);
      $get_payment_detail= $this->general_model->payment_mode_by_id('',$get_detail_by_id['mode_payment']);
      $template_format= $this->sale->template_format(array('section_id'=>2,'types'=>1,'sub_section'=>1,'branch_id'=>$get_detail_by_id['branch_id']));
