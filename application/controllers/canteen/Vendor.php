...
class Vendor extends CI_Controller {
	...
	public function index()
    {
       unauthorise_permission(57,375);
        ...
        $this->load->view('canteen/vendor/list',$data);
    }
           ...
           if(in_array('377',$users_data['permission']['action'])){
            ...
            if(in_array('378',$users_data['permission']['action'])){
               ...
               if(in_array('378',$users_data['permission']['action'])){
                 ...
                 $btn_estimate = ' <div class="btn-ipd"><a class="btn-custom" href="'.base_url('canteen/purchase_estimate/add/'.$vendor->id).'" style="width:120px;'.$vendor->id.'" title="Estimate Purchase "><i class="fa fa-plus"></i>Estimate Purchase </a></div>';
				  
                 $btn_purchase_return = ' <div class="btn-ipd"><a class="btn-custom" href="'.base_url('canteen/purchase_return/add/'.$vendor->id).'" style="width:120px;'.$vendor->id.'" title="Purchase return"><i class="fa fa-plus"></i>Purchase Return</a></div>';
              
               }       
            $row[] = $btnedit.$btnview.$btndelete.$btn_purchase.$btn_estimate.$btn_purchase_return;
            $data[] = $row;
            $i++;
        }
	...
	public function add($type='')
	{
       ...
       $this->load->view('canteen/vendor/add',$data);
	}
    ...
    private function _validate()
    {
       ...
       if(!empty($_POST['data_id']) && $check_ids['email']==$_POST['vendor_email']){
        ...
        $this->form_validation->set_rules('vendor_email', 'Email', 'trim|valid_email|is_unique[hms_canteen_vendors.email]');
        }
    ...
    {  
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('canteen/vendor/view',$data);     
      }
    }  
    ...
    public function archive()
    {
       unauthorise_permission(57,379);
        ...
        $this->load->view('canteen/vendor/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission(57,379);
        $this->load->model('canteen/vendor/vendor_archive_model','vendor_archive'); 
