...
class Stock_item_unit extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('canteen/stock_item_unit/stock_item_unit_model','stock_item_unit');
    ...
    { 
        ...
        $data['page_title'] = 'Stock item unit list'; 
        $this->load->view('canteen/stock_item_unit/list',$data);
    }
    ...
    public function add()
    {
        ...
        $data['get_stock_item_list']=$this->stock_item_unit->get_stock_item_list(); 
       ...
       $this->load->view('canteen/stock_item_unit/add',$data);       
    }
    ...
    public function edit($id="")
    {
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $data['form_data'] = array(
                                  'data_id'=>$result['id'],
                                  'stock_item_unit'=>$result['unit'],
                                   'second_unit'=>$result['second_id'],  
    ...
    private function _validate()
    {
        ...
        $this->form_validation->set_rules('stock_item_unit', 'Stock Item Unit', 'trim|required|callback_check_stock_unit'); 
  ...
  public function check_stock_unit($str)
  {
    ...
    if(!empty($str))
    {
        ...
        {
              $data_cat= $this->stock_item_unit->get_by_id($post['data_id']);
              if($data_cat['unit']==$str && $post['data_id']==$data_cat['id'])
              {
              return true;  
              }
              else
              {
              $stockunitdata = $this->general->check_stock_unit($str);
              ...
              {
              $this->form_validation->set_message('check_stock_unit', 'The stock unit already exists.');
              return false;
              }
              }
        }
    ...
    {
      $this->form_validation->set_message('check_stock_unit', 'The stock unit field is required.');
            return false; 
    } 
  }
    ...
    {  
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('canteen/stock_item_unit/view',$data);     
      }
    }  
    ...
    {
        //unauthorise_permission('163','944');
        $data['page_title'] = 'Stock Item Unit archive list';
        $this->load->helper('url');
        $this->load->view('canteen/stock_item_unit/archive',$data);
    }

    public function archive_ajax_list()
    {
        //unauthorise_permission('163','944');
        $this->load->model('canteen/stock_item_unit/stock_item_unit_archive_model','stock_item_unit_archive'); 
        ...
        foreach ($list as $stock_item_unit) {
         // print_r($stock_item_unit);die;
            ...
            $row[] = '<input type="checkbox" name="employee[]" class="checklist" value="'.$stock_item_unit->id.'">'.$check_script; 
            $row[] = $stock_item_unit->unit;  
            $row[] = $stock_item_unit->second_unit;   
            $row[] = $status;
            //$row[] = date('d-M-Y H:i A',strtotime($stock_item_unit->created_date)); 
           $users_data = $this->session->userdata('auth_users');
           ...
           $btnrestore='';
           $btndelete='';
          //if(in_array('944',$users_data['permission']['action'])){
  ...
  {
      $unit_list = $this->stock_item_unit->stock_item_unit_list();
      $dropdown = '<option value="">Select Stock Item Unit</option>'; 
      if(!empty($unit_list))
      {
        foreach($unit_list as $stock_item_unit)
        {
           $dropdown .= '<option value="'.$stock_item_unit->id.'">'.$stock_item_unit->unit.'</option>';
        }
      } 
      echo $dropdown; 
  }
