...
// Default Search Setting
      ...
      $data['page_title'] = 'Purchase Canteen Return'; 

      $this->load->view('canteen/purchase_return/collection_list',$data);
    }
    ...
    {  
     ...
     public function canteen_purchase_excel()
    {     
          ...
          header("Content-Disposition: attachment; filename=medicine_purchase_return_coll_report_".time().".xls");
    ...
    public function canteen_purchase_csv()
    {
         ...
         header("Content-Disposition: attachment; filename=canteen_purchase_return_coll_report_".time().".csv");  
         header("Pragma: no-cache"); 
         header("Expires: 0");
         if(!empty($data))
         {
    ...
    public function pdf_canteen_purchase()
    {    
        $data['print_status']="";
        $data['data_list'] = $this->purchase_return->search_report_data();
        $this->load->view('canteen/purchase_return/medicine_purchase_return_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("canteen_purchase_return_report_".time().".pdf");
    }
    public function print_canteen_purchase()
    {    
      $data['print_status']="1";
      $data['data_list'] = $this->purchase_return->search_report_data();
      $this->load->view('canteen/purchase_return/medicine_purchase_return_report_html',$data); 
 ...
 public function print_purchase_return_recipt($ids=""){
    $user_detail= $this->session->userdata('auth_users');
     ...
     if(!empty($ids)){
      ...
      $template_format= $this->purchase_return->template_format(array('section_id'=>15,'types'=>1,'sub_section'=>1,'branch_id'=>$get_detail_by_id['branch_id']));
