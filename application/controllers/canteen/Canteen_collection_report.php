...
class Canteen_collection_report extends CI_Controller {
     ...
     public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('canteen/reports/canteen_collection_report_model','canteen_collection_reports');
        $this->load->library('form_validation');
    }

    public function index()
    { //  unauthorise_permission('159','931');
        $this->session->unset_userdata('medicine_collection_resport_search_data');
        $data['sub_branch'] = $this->session->userdata('sub_branches_data'); 
        $search_date = $this->session->userdata('medicine_collection_resport_search_data');
       ...
       /* $this->load->view('canteen/medicine_collection/list',$data);*/
         $data['page_title'] = 'Canteent Collection'; 
        $this->session->unset_userdata('canteen_report');
        $this->load->view('canteen/medicine_collection/list',$data);
    }
    public function expenses(){
       $data['page_title'] = 'Canteen Collection Reports';
       $this->load->view('canteen/medicine_collection/expense_report',$data);
    }
        ...
        foreach($list as $reports) 
        {
          //print_r($reports);
            ...
            /*if($reports->blnce>0 && $reports->parent_id>0){*/
               $print_url = "'".base_url('canteen/sale/print_sale_recipt/'.$reports->id)."'";
            ...
            $btn_edit = ' <a class="btn-custom" href="'.base_url("canteen/sale/edit/".$reports->id).'" title="Edit Booking"><i class="fa fa-pencil"></i> Edit</a>';
    ...
    public function search_data()
    {
       ...
       $search_data =  array(
         ...
         $this->session->set_userdata('medicine_collection_resport_search_data',$search_data);
       }
    }
    ...
    public function canteen_report_excel()
    {
        ...
        //unauthorise_permission('159','931');
         ...
         // print_r($objWorksheet);die;
          ...
          $fields = array('Sale No','Sale Date','Patient Name','Net Amount','Discount','Paid Amount','Balance');
      ...
      $date_time_status = $get_date_time_setting->date_time_status;
        ...
        else
        {
          $sale_date = date('d-m-Y',strtotime($reports->sale_date));
                  ...
                  array_push($rowData,$reports->sale_no,$sale_date,number_format($reports->total_amount,2),$discount,number_format($reports->paid_amount,2),number_format($reports->balance,2));
                    $count = count($rowData);
          ...
          header("Content-Disposition: attachment; filename=canteen_collection_report_".time().".xls");  
     ...
     public function canteen_report_csv()
    {
        ...
        else
        {
          ...
          // Sending headers to force the user to download the file
          header('Content-Type: application/octet-stream charset=UTF-8');
          header("Content-Disposition: attachment; filename=canteen_collection_report_".time().".csv");
          header("Pragma: no-cache"); 
          header("Expires: 0");
          if(!empty($data))
          {
    ...
    public function advance_search()
    {
      $data['page_title'] = "Advance Search";
        ...
        $data['search_data'] = $this->session->userdata('medicine_collection_resport_search_data');
        ...
        else
        {
          ...
          $this->session->set_userdata('medicine_collection_resport_search_data', $marge_post);
          //$this->session->set_userdata('medicine_collection_resport_search_data',$post); 
          }
       // print_r($data);die;
        $this->load->view('canteen/medicine_collection/advance_search',$data);
    }
    ...
    {   
      //  unauthorise_permission('159','931');
        ...
        $this->load->view('canteen/medicine_collection/medicine_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("canteen_collection_report_".time().".pdf");
    }
   ...
   // unauthorise_permission('159','931');
     ...
     $this->load->view('canteen/medicine_collection/list_medicine_collection',$data);  

    }
    
      public function pdf_daywise_canteen_report()
    {   
      //  unauthorise_permission('159','931');
        $data['print_status']="";
        $this->load->model('general/general_model');
          $get_date_time_setting = $this->general_model->get_date_time_setting('opd_collection_report');
        $data['date_time_status'] = $get_date_time_setting->date_time_status;
        $data['data_list'] = $this->canteen_collection_reports->canteen_search_report_data();
        $this->load->view('canteen/medicine_collection/medicine_report_html',$data);
        $html = $this->output->get_output();
        // Load library
        $this->load->library('pdf');
