...
// Default Search Setting
    ...
    public function item_purchase_excel()
    {
         $this->load->library('excel');
          ...
          header("Content-Disposition: attachment; filename=item_canteen_purchase_list_".time().".xls");
    ...
    public function item_purchase_csv()
    {
         ...
         header("Content-Disposition: attachment; filename=item_canteen_purchase_list_".time().".csv");  
         header("Pragma: no-cache"); 
         header("Expires: 0");
         if(!empty($data))
         {
    ...
    {    
        ...
        $this->pdf->render();
        $this->pdf->stream("item_canteen_purchase_list_".time().".pdf");
    }
...
// print_r($post); exit;   
  ...
  public function print_purchase_recipt($ids="")
  {
      ...
      $template_format= $this->purchase->template_format(array('section_id'=>15,'types'=>1,'sub_section'=>1,'branch_id'=>$get_detail_by_id['branch_id']));
  ...
  /********** estimate_item**************/
   ...
   {    
       $post =  $this->input->post(); 
       //print_r($post['purchase_id']);
       if(isset($post['purchase_id']) && !empty($post['purchase_id']))
       { 
        $result = $this->purchase->get_estimate_item_by_id($post['purchase_id']);  
         ...
         // print_r($item);die;
                       ...
                       if($item['manuf_date']=="00-00-0000"){
                        ...
                        $value="'".$item['id']."'";
                          ...
                          //if(!in_array($item->id,$ids)){ 
                        $table.='<tr><input type="hidden" id="item_id_'.$item['id'].$item['batch_no'].'" name="iid[]" value="'.$item['id'].'.'.$item['batch_no'].'"/>
                         <input type="hidden" id="item_id_'.$item['id'].$item['batch_no'].'" name="item_id[]" value='.$value.'/>
                     <input type="hidden" id="purchase_rate_mrp'.$item['id'].$item['batch_no'].'" name="purchase_rate_mrp[]" value="'.$item['mrp'].'"/><input type="hidden" id="batch_no_'.$item['id'].$item['batch_no'].'" name="batch_no[]" value="'.$item['batch_no'].'"/>';

                        $table.='<td><input type="checkbox" name="item_id[]" class="booked_checkbox" value='.$value.'></td>';
                        $table.='<td>'.$item['item'].'</td>';
                        $table.='<td>'.$item['item_code'].'</td>';

                        $table.='<td><input type="text" class="w-60px" placeholder="HSN No" id="hsn_no_'.$item['id'].$item['batch_no'].'" name="hsn_no[]" value="'.$item['hsn_no'].'" onkeyup="payment_cal_perrow('.$varids.');"/></td>';

                        $table.='<td>'.$item['packing'].'</td>';
                      
					    $table.='<td><input type="text" value="'.$batch_no.'" name="batch_no[]" class="datepicker" placeholder="Batch No" style="width:84px;" id="batch_no_'.$item['batch_no'].$item['batch_no'].'" onchange="payment_cal_perrow('.$varids.');"/>'.$check_script1.'</td>';
                        ...
                        $table.='<td><input type="text" value="'.$date_new.'" name="expiry_date[]" class="datepicker" placeholder="Expiry date" style="width:84px;" id="expiry_date_'.$item['id'].$item['batch_no'].'" onchange="payment_cal_perrow('.$varids.');"/>'.$check_script.'</td>';
						 ...
						 $table.='<td><input type="text" value="'.$item['qty'].'" name="qty[]" class="datepicker" placeholder="Qty" style="width:60px;" id="qty_'.$item['id'].$item['batch_no'].'" onchange="payment_cal_perrow('.$varids.');"/>'.$check_script.'</td>';
                        
                        $table.='<td><input type="text" class="w-60px" id="mrp_'.$item['id'].$item['batch_no'].'" name="mrp[]" value="'.number_format($item['mrp'],2,'.','').'" onkeyup="payment_cal_perrow('.$varids.');"/></td>';
