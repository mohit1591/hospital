...
class Stock_issue_allotment_return extends CI_Controller {
	...
	function __construct() 
	{
  	...
  	public function index()
      {
       // unauthorise_permission(168,970);
        $data['page_title'] = 'Allotment Return list'; 
        ...
        $this->load->view('canteen/stock_issue_allotment_return/list',$data);
      }
          ...
          // $row[] = $stock_issue_allotment_return->discount;
           ...
           //if(in_array('972',$users_data['permission']['action'])){
            ...
            //if(in_array('374',$users_data['permission']['action'])){
               // $btnview=' <a class="btn-custom" onclick="return view_medicine_entry('.$stock_issue_allotment_return->id.')" href="javascript:void(0)" title="View"><i class="fa fa-info-circle"></i> View </a>';
    ...
    public function stock_allotment_return_issue_excel()
    {
         $this->load->library('excel');
          ...
          $fields = array('Return Code','Patient Name/Employee Name/ Doctor Name','Total Amount','Issue Date','Created Date');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
                    ...
                    array_push($rowData,$reports->return_no,$reports->member_name,$reports->total_amount,date('d-m-Y',strtotime($reports->issue_date)), date('d-M-Y H:i A',strtotime($reports->created_date)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
    ...
    {    
        ...
        $this->load->view('canteen/stock_issue_allotment_return/stock_allotment_issue_return_report_html',$data);
    ...
    public function add($ipd_id="",$patient_id="")
    {
        ...
        $this->load->view('canteen/stock_issue_allotment_return/add',$data);
    }
    ...
    public function edit($id="")
    {
          ...
          $get_payment_detail= $this->stock_issue_allotment_return->payment_mode_detail_according_to_field($result['payment_mode'],$id);
            ...
            //print '<pre>'; print_r($stock_issue_allotment_return_item_list);die;
          // if(!isset($post) && empty($post) ||count($stock_issue_allotment_return_item_list)>=1)
          // {
          //   $stock_issue_allotment_return_item_list= $this->stock_issue_allotment_return->get_purchase_to_purchase_by_id($id);
          // $this->session->set_userdata('stock_issue_allotment_return_item_list',$stock_issue_allotment_return_item_list);
          // }
          ...
          {   
            ...
            if(isset( $stock_issue_allotment_return_item_list) && !empty($stock_issue_allotment_return_item_list))
            {
             ...
             else
             {
                  ...
                  if(isset($stock_issue_allotment_return_item_list) && !empty($stock_issue_allotment_return_item_list))
                  {
                  $stock_issue_allotment_return_item_list = $stock_issue_allotment_return_item_list; 
                  }
                  else
                  {
                  $stock_issue_allotment_return_item_list = [];
                  }
                  $stock_issue_allotment_return_item_list[$post['item_id']] = array('item_id'=>$post['item_id'],'category_id'=>$post['category_id'],'total_price'=>$post['total_price'],'item_code'=>$post['item_code'],'unit'=>$post['unit'], 'quantity'=>$post['quantity'], 'amount'=>$post['amount'],'item_price'=>$post['item_price'],'total_amount'=>$post['total_amount'],'item_name'=>$post['item_name']);
                  ...
                  $stock_issue_allotment_return_payment_array = array('total_amount'=>$totamount,'total_amount'=>number_format($total_amount,2,'.',''));
                  $this->session->set_userdata('stock_issue_allotment_return_payment_array', $stock_issue_allotment_return_payment_array);
                  $json = json_encode($response_data,true);
                  echo $json;
  ...
  public function archive_ajax_list()
    {
        ...
        $this->load->model('canteen/stock_issue_allotment_return/stock_issue_allotment_return_archive_model','stock_issue_allotment_archive'); 
...
public function get_payment_mode_data()
{
 ...
 function get_item_values($vals="")
    ...
    { 
        ...
        {
          foreach($stock_issue_allotment_return_item_list as $stock_issue_allotment_return)
          {
              $total_amount+=$stock_issue_allotment_return['quantity']*$stock_issue_allotment_return['item_price'];
          }
        }
    ...
    public function payment_cal_perrow()
    {
      ...
      { 
        ...
        $stock_issue_allotment_return_item_list[$post['item_id']] =  array('item_id'=>$post['item_id'],'category_id'=>$post['category_id'],'item_code'=>$post['item_code'],'item_name'=>$post['item_name'],'amount'=>$post['item_price']*$post['quantity'],'unit'=>$post['unit'],'item_price'=>$post['item_price'],'total_price'=>$total_price,'quantity'=>$post['quantity'],'total_amount'=>''); 
        $this->session->set_userdata('stock_issue_allotment_return_item_list', $stock_issue_allotment_return_item_list);
     ...
     public function archive()
    {
       //unauthorise_permission(168,975);
        ...
        $this->load->view('canteen/stock_issue_allotment_return/archive',$data);
    }
    function deleteall()
    {
