...
class Stock_purchase extends CI_Controller {
	...
	function __construct() 
	{
  	parent::__construct();	
  	 auth_users();  
  	$this->load->model('canteen/stock_purchase/stock_purchase_model','stock_purchase');
  	...
  	public function index()
      {
        ...
        $data['page_title'] = 'Stock purchase List'; 
        ...
        $this->load->view('canteen/stock_purchase/list',$data);
      }
          ...
          //if(in_array('954',$users_data['permission']['action'])){
           ...
           //}
            //if(in_array('374',$users_data['permission']['action'])){
               // $btnview=' <a class="btn-custom" onclick="return view_medicine_entry('.$stock_purchase->id.')" href="javascript:void(0)" title="View"><i class="fa fa-info-circle"></i> View </a>';
    ...
    public function stock_purchase_excel()
    {
         $this->load->library('excel');
          ...
          $fields = array('Purchase Code','Purchase Date','Vendor Name','Net Amount','Paid Amount','Balance','Created Date');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
                    ...
                    array_push($rowData,$reports->purchase_no,date('d-m-Y',strtotime($reports->purchase_date)),$reports->name,$reports->net_amount,$reports->paid_amount,$reports->balance, date('d-M-Y H:i A',strtotime($reports->created_date)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
    ...
    {    
        ...
        $this->load->view('canteen/stock_purchase/stock_purchase_report_html',$data);
    ...
    public function add($ipd_id="",$patient_id="")
    {
         //print_r($_POST);die;
          ...
          $data['page_title'] = 'Stock purchase Add';
          $data['data_id']='';
        ...
        $this->load->view('canteen/stock_purchase/add',$data);
    }
    ...
    public function edit($id="")
    {
          ...
          $total_values[]= $get_payment_detail[$i]->field_value.'_'.$get_payment_detail[$i]->field_name.'_'.$get_payment_detail[$i]->field_id;
            ...
            $new_purchase_list = $this->session->userdata('stock_purchase_item_list');
            ...
            {
              $this->session->unset_userdata('stock_purchase_item_list'); 
              $stock_purchase_item_list= $this->stock_purchase->get_purchase_to_purchase_by_id($id);
              $this->session->set_userdata('stock_purchase_item_list',$stock_purchase_item_list);
            
            }
         //print '<pre>'; print_r($stock_purchase_item_list);die;
          // if(!isset($post) && empty($post) ||count($stock_purchase_item_list)>=1)
          // {
          //   $stock_purchase_item_list= $this->stock_purchase->get_purchase_to_purchase_by_id($id);
          // $this->session->set_userdata('stock_purchase_item_list',$stock_purchase_item_list);
          // }
    ...
    public function item_payment_calculation()
    {
          ...
          {   
            ...
            $stock_purchase_item_list = $this->session->userdata('stock_purchase_item_list');
           // print_r($stock_purchase_item_list);die();
            if(isset( $stock_purchase_item_list))
            {
             ...
             else
             {
                  ...
                  if(isset($stock_purchase_item_list) && !empty($stock_purchase_item_list))
                  {
                  $stock_purchase_item_list = $stock_purchase_item_list; 
                  }
                  else
                  {
                  $stock_purchase_item_list = [];
                  }
                  $stock_purchase_item_list[$post['item_id']] = array('item_id'=>$post['item_id'],'category_id'=>$post['category_id'],'total_price'=>$post['total_price'],'item_code'=>$post['item_code'],'unit'=>$post['unit'], 'quantity'=>$post['quantity'], 'amount'=>$post['amount'],'item_price'=>$post['item_price'],'total_amount'=>$post['total_amount'],'item_name'=>$post['item_name']);
                  $amount_arr = array_column($stock_purchase_item_list, 'amount'); 
                  $total_amount = array_sum($amount_arr);
                  $this->session->set_userdata('stock_purchase_item_list', $stock_purchase_item_list);

                  
                  $html_data = $this->stock_purchase_item_list();
                  $total = $total_amount;
    ...
    private function _validate()
    {
        ...
        if(!isset($stock_purchase_item_list) && empty($stock_purchase_item_list) && empty($post['data_id']))
        {
    ...
    private function stock_purchase_item_list()
    {
        $stock_purchase_item_list = $this->session->userdata('stock_purchase_item_list');
        //print '<pre>';print_r($stock_purchase_item_list);die;
         $check_script = "<script>$('#selectAll').on('click', function () { 
        ...
        $rows = '<thead class="bg-theme"><tr>           
           ...
           if(isset($stock_purchase_item_list) && !empty($stock_purchase_item_list))
           {
              ...
              foreach($stock_purchase_item_list as $purchase_item_list)
              {
           ...
           {
               $rows .= '<tr>  
                          <td colspan="7" align="center" class=" text-danger"><div class="text-center">Particular data not available.</div></td>
                        </tr>';
           }
    ...
    public function remove_stock_purchase_item_list()
    {
       ...
       {
           $stock_purchase_item_list = $this->session->userdata('stock_purchase_item_list');
          //print '<pre>'; print_r($this->session->userdata('stock_purchase_item_list'));die;
           $stock_item_payment_payment_array = $this->session->userdata('stock_item_payment_payment_array'); 
           
           $particular_id_list = array_column($stock_purchase_item_list, 'item_id'); 
           
           foreach($stock_purchase_item_list as $key=>$item_list)
           { 
            
              if(in_array($item_list['item_id'],$post['item_id']))
              {  
                //echo "<pre>"; print_r($perticuller_ids); exit;
                 unset($stock_purchase_item_list[$key]);
  ...
  public function archive_ajax_list()
    {
        ...
        $this->load->model('canteen/stock_purchase/stock_purchase_archive_model','stock_purchase_archive'); 
...
public function get_payment_mode_data()
{
 ...
 public function payment_calc_all()
    { 
        ...
        {
          foreach($stock_purchase_item_list as $stock_purchase)
          {
              $total_amount+=$stock_purchase['quantity']*$stock_purchase['item_price'];
          }
        }
    ...
    public function payment_cal_perrow()
    {
      ...
      { 
        ...
        $stock_purchase_item_list[$post['item_id']] =  array('item_id'=>$post['item_id'],'category_id'=>$post['category_id'],'item_code'=>$post['item_code'],'total_price'=>$total_price,'item_name'=>$post['item_name'],'amount'=>$post['item_price']*$post['quantity'],'unit'=>$post['unit'],'item_price'=>$post['item_price'],'quantity'=>$post['quantity'],'total_amount'=>''); 
        $this->session->set_userdata('stock_purchase_item_list', $stock_purchase_item_list);
       // print_r($this->session->userdata('stock_purchase_item_list'));
     ...
     function trashall()
    {
       //unauthorise_permission(165,957);
        ...
        if(!empty($post))
        {
            ...
            $response = "Stock purchase successfully deleted parmanently.";
            echo $response;
        }
    }
    ...
    { 
       //unauthorise_permission(165,956);
        ...
        if(!empty($post))
        {
            ...
            $response = "Stock purchase successfully restore in stock purchase list.";
            echo $response;
        }
    }
    ...
    {
       //unauthorise_permission(165,956);
        $data['page_title'] = 'Stock purchase archive list';
        $this->load->helper('url');
        $this->load->view('canteen/stock_purchase/archive',$data);
    }
    function deleteall()
    {
        ...
        if(!empty($post))
        {
            ...
            $response = "Stock purchase successfully deleted.";
            echo $response;
        }
    }
