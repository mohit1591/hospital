...
class Purchase_estimate extends CI_Controller {
  ...
  function __construct() 
  {
    parent::__construct();  
    auth_users();  
    $this->load->model('canteen/purchase_estimate/purchase_estimate_model','purchase_estimate');
  ...
  public function index()
    {
        ...
        $this->session->unset_userdata('purchase_estimate_search_item');
// Default Search Setting
        ...
        $data['page_title'] = 'Purchase estimate Item List'; 
        $this->load->view('canteen/purchase_estimate/list',$data);
    }

    public function ajax_list_estimate()
    {  
       //unauthorise_permission(58,385);
        $list = $this->purchase_estimate->get_datatables();    
...
//echo"<pre>"; print_r($list); die;	
        ...
        //$row='';
        foreach ($list as $purchase_estimate) { 
            $no++;
            $row = array();
            ...
            $row[] = '<input type="checkbox" name="purchase[]" class="checklist" value="'.$purchase_estimate->id.'">'.$check_script;  
            $row[] = $purchase_estimate->purchase_id;
            $row[] = $purchase_estimate->invoice_id;
            $row[] = $purchase_estimate->vendor_name;
            //$row[] = $purchase_estimate->total_amount;
            $row[] = $purchase_estimate->net_amount;
            $row[] = $purchase_estimate->paid_amount;
            $row[] = $purchase_estimate->balance;
            $row[] = date('d-M-Y',strtotime($purchase_estimate->purchase_date));  
            $users_data = $this->session->userdata('auth_users');
            $btnedit='';
            $btndelete='';
            $btnview = '';
            if($session_data['parent_id']==$purchase_estimate->branch_id){

           //if(in_array('387',$users_data['permission']['action'])){
                 $btnedit = ' <a onClick="return edit_purchase('.$purchase_estimate->id.');" class="btn-custom" href="javascript:void(0)" style="'.$purchase_estimate->id.'" title="Edit"><i class="fa fa-pencil"></i> Edit</a>';
           // }
		   
            //if(in_array('125',$users_data['permission']['action'])){
                /*$btnview=' <a class="btn-custom" onclick="return view_purchase('.$purchase_estimate->id.')" href="javascript:void(0)" title="View"><i class="fa fa-info-circle"></i> View </a>';*/
           // }
		   
            //if(in_array('388',$users_data['permission']['action'])){
                $btndelete = ' <a class="btn-custom" onClick="return delete_purchase('.$purchase_estimate->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-trash"></i> Delete</a> ';
           // }
          }

          $new_path=base_url().'canteen/purchase_estimate/print_purchase_recipt';
          //$btnprint = '<a class="btn-custom" onclick="openPrintWindow(123,'.$purchase_estimate->id.');" target="_blank"><i class="fa fa-print"></i> Print</a>';
          //;$print_url = "'".base_url('canteen/purchase_estimate/print_purchase_recipt/'.$purchase_estimate->id)."'";
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->purchase_estimate->count_all(),
                        "recordsFiltered" => $this->purchase_estimate->count_filtered(),
                        "data" => $data,
                );
        //print_r($output);
    ...
    public function item_purchase_excel()
    {
         $this->load->library('excel');
          ...
          $list = $this->purchase_estimate->search_report_data();
          $rowData = array();
          $data= array();
          if(!empty($list))
          {
          ...
          header("Content-Disposition: attachment; filename=item_purchase_estimate_report_".time().".xls");
    ...
    public function item_purchase_csv()
    {
         ...
         header("Content-Disposition: attachment; filename=item_purchase_estimate_report_".time().".csv");  
         header("Pragma: no-cache"); 
         header("Expires: 0");
         if(!empty($data))
         {
    ...
    {    
        $data['print_status']="";
        $data['data_list'] = $this->purchase_estimate->search_report_data();
        $this->load->view('canteen/purchase_estimate/item_purchase_report_html',$data);
    ...
    public function ajax_list_item(){
       ...
       $table = '';
        ...
        { 
          ...
          $data['item_new_list'] = $this->purchase_estimate->item_list($item_ids);
        ...
        { 
          ...
          $result_item = $this->purchase_estimate->item_list_search();  
    ...
    public function advance_search()
      {
          ...
          if(isset($post) && !empty($post))
          {
              ...
              $this->session->set_userdata('purchase_estimate_search_item', $marge_post);
          }
          $purchase_estimate_search_item = $this->session->userdata('purchase_estimate_search_item');
          if(isset($purchase_estimate_search_item) && !empty($purchase_estimate_search_item))
          {
              $data['form_data'] = $purchase_estimate_search_item;
          }
          $this->load->view('canteen/purchase_estimate/advance_search',$data);
   }
   ...
   public function reset_search()
    ...
    public function ajax_added_item(){
        ...
        { 
          ...
          $item_ids = implode(',', $ids_arr);
          $result_item = $this->purchase_estimate->item_list($item_ids);
      ...
      if(!empty($bar_code))
{
 $mbid= $this->input->post('mbid');
      $return= $this->purchase_estimate->check_bar_code($bar_code,$mbid);
      if($return==2)
      {
        echo '1';exit;
      }
       ...
       if(isset($post['item_id']) && !empty($post['item_id']))
       {
         ...
         { 
           $total_amount=0;
            ...
            {
               $item_data = $this->purchase_estimate->item_list($i_ids);
//print_r($item_data);
                $qty=0;
  ...
  public function add($pid="",$estimate_id="")
  {
		...
		$this->load->model('general/general_model'); 
		$data['page_title'] = "Purchase Estimate Item";
       ...
       if($pid>0)
        {
           $vendor = $this->purchase_estimate->get_vendor_by_id($pid);
           //print_r($purchase);
           if(!empty($vendor))
           {
              $vendor_id = $vendor['id'];
 ...
 //echo "<pre>";print_r($data['item_new_list']);die; 
       ...
       // echo $invoice_no;
        ...
        {   
            ...
            {
                $return_id= $this->purchase_estimate->save();
              if(!empty($return_id))
              {
                
                $get_by_id_data = $this->purchase_estimate->get_by_id($return_id);
                $get_vendor_by_id = $this->purchase_estimate->get_vendor_by_id($get_by_id_data['vendor_id']);
                ...
                //check permission
                if(in_array('660',$users_data['permission']['action']))
                {
              ...
              //   echo $return_id;die; 
                $this->session->set_flashdata('success','purchase estimate has been successfully added.');
				redirect(base_url('canteen/purchase_estimate'));
               // redirect(base_url('canteen/purchase_estimate/add/?status=print'));
            }
            else
            {
      ...
      $this->load->view('canteen/purchase_estimate/add',$data);
  }
  ...
  public function edit($id="")
    {
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {       
         $post = $this->input->post();
          $result = $this->purchase_estimate->get_by_id($id);
          $data['max_pur_qty']=get_setting_value('Set_Purchase_Limit_Per_Medicine');

         $item_id_arr=[];
         $result_vendor = $this->purchase_estimate->get_vendor_by_id($result['vendor_id']);
         if(empty($post))
         { 
            $result_item_list = $this->purchase_estimate->get_item_by_purchase_id($id);
    // echo "<pre>"; print_r($result_item_list); 
     ...
     // print_r($item_new_list);die;
        ...
        $get_payment_detail= $this->purchase_estimate->payment_mode_detail_according_to_field($result['mode_payment'],$id);
        ...
        //$data['manuf_company_list'] = $this->purchase_estimate->manuf_company_list();
        $data['page_title'] = "Purchase Estimate Item";  
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
                 ...
                 $this->session->set_userdata('purchase_id',$return_id);
                 $this->session->set_flashdata('success','Purchase estimate has been successfully updated.');
				 redirect(base_url('canteen/purchase_estimate'));
                // redirect(base_url('canteen/purchase_estimate/?status=print'));
...
// print_r($post); exit;   
       ...
       if(isset($post) && !empty($post))
       {
         ...
         if(!empty($item_list))
         { 
            $item_data = $this->purchase_estimate->item_list($post['item_id']);
    ...
    public function delete($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           $result = $this->purchase_estimate->delete($id);
    ...
    function deleteall()
    {
       // unauthorise_permission(58,388);
        ...
        if(!empty($post))
        {
            $result = $this->purchase_estimate->deleteall($post['row_id']);
            $response = "Purchase successfully deleted.";
            echo $response;
        }
    }
    ...
    {  
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        $data['form_data'] = $this->purchase_estimate->get_by_id($id);  
        $data['page_title'] = $data['form_data']['item']." detail";
        $this->load->view('canteen/purchase_estimate/view',$data);     
      }
    }  
    ...
    public function archive()
    {
      // unauthorise_permission(58,389);
        $data['page_title'] = 'Purchase Estimate archive list';
        $this->load->helper('url');
        $this->load->view('canteen/purchase_estimate/archive',$data);
    }

    public function archive_ajax_list()
    {
       // unauthorise_permission(58,389);
        $this->load->model('canteen/purchase_estimate/purchase_archive_model','purchase_archive'); 
        ...
        foreach ($list as $purchase) { 
           ...
           if(in_array('391',$users_data['permission']['action'])){
                $btnrestore = ' <a onClick="return restore_purchase('.$purchase_estimate->id.');" class="btn-custom" href="javascript:void(0)"  title="Restore"><i class="fa fa-window-restore" aria-hidden="true"></i> Restore </a>';
           }
           if(in_array('390',$users_data['permission']['action'])){
                $btndelete = ' <a onClick="return trash('.$purchase_estimate->id.');" class="btn-custom" href="javascript:void(0)" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a>';
           }
  ...
  public function print_purchase_recipt($ids="")
  {
      ...
      $data['page_title'] = "Add estimate purchase";
      $get_detail_by_id= $this->purchase_estimate->get_by_id($purchase_id);

      $get_by_id_data=$this->purchase_estimate->get_all_detail_print($purchase_id,$get_detail_by_id['branch_id']);
      $get_payment_detail= $this->general_model->payment_mode_by_id('',$get_detail_by_id['mode_payment']);
      $template_format= $this->purchase_estimate->template_format(array('section_id'=>15,'types'=>1,'sub_section'=>1,'branch_id'=>$get_detail_by_id['branch_id']));
      $data['template_data']=$template_format;
      $data['user_detail']=$user_detail;
      $data['all_detail']= $get_by_id_data;
      $data['payment_mode']= $get_payment_detail;
      $this->load->view('canteen/purchase_estimate/print_template_medicine',$data);
  ...
  /********** estimate_item**************/
   ...
   function estimate_item($vals="")
    {
        if(!empty($vals))
        {
            $result = $this->purchase_estimate->estimate_item($vals);  
           // print_r($result);die;
            if(!empty($result))
            {
    ...
    public function set_item_stockitem()
   {
     ...
     // print_r($post['purchase_id']);die('fgf');
       ...
       {
        $result = $this->purchase_estimate->get_estimate_medicine_by_id($post['purchase_id']);  
   ...
   //      echo "<pre>";print_r($item_sess);die;
        ...
        { 
         ...
         // $batch_nos = implode(',', $batch_arr); 
          //echo $batch_nos;
        //  $result_item = $this->purchase_estimate->get_medicine_by_id($key_m_arr->item_id);
    ...
    public function get_canteen_vendors()
    {
      ...
      $all_data=$this->purchase_estimate->get_canteen_vendors($item_id);
      if(!empty($all_data))
      {
        $tr='';
  ...
  {     
        $users_data = $this->session->userdata('auth_users');
        $result = $this->purchase_estimate->check_unique_value($users_data['parent_id'], $invoice, $id);
        if($result == 0)
            $response = true;
        else {
