...
class Advance_payment extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('advance_payment/advance_payment_model','advancepayment');
    ...
    { 
        ...
        $data['page_title'] = 'Advance Payment Details'; 
        $this->session->unset_userdata('advance_payment_serach');
        ...
        $this->load->view('advance_payment/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $ot) {
         // print_r($relation);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('775',$users_data['permission']['action'])){
               $btnedit = ' <a onClick="return edit_advance_payment('.$ot->id.');" class="btn-custom" href="javascript:void(0)" style="'.$ot->id.'" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>';
          }
           if(in_array('463',$users_data['permission']['action'])){
               $btndelete = ' <a class="btn-custom" onClick="return delete_advance_payment('.$ot->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-trash"></i> Delete</a> '; 
          } 
          $btnprint='';
          if(in_array('776',$users_data['permission']['action'])){
          $print_url = "'".base_url('advance_payment/print_advance_payment_report/'.$ot->id)."'";
    ...
    public function advance_search()
    {
            ...
            if(isset($post) && !empty($post))
            {
                ...
                $this->session->set_userdata('advance_payment_serach', $marge_post);
            }

            $advance_payment_serach = $this->session->userdata('advance_payment_serach');
 ...
 //print '<pre>'; print_r($data);die;
    ...
    public function print_advance_payment_report($ids="")
    {
        ...
        $data['re_number'] =$re_number;
         ...
         $this->load->model('general/general_model'); 
     $transaction_id=$this->general_model->get_transaction_id($ids,3,10);   //3 section id and 10 type
    ...
    public function advance_excel()
    {
        ...
        $fields = array('Patient Reg. No.','IPD No.','Patient Name','Admission Date','Amount','Deposit Date');
        ...
        $list = $this->advancepayment->search_report_data();
			...
			foreach($list as $reports)
            {
             ...
             if($reports->payment_date =='1970-01-01'){
              ...
              $pay_date=date('d-m-Y',strtotime($reports->payment_date));
               ...
               array_push($rowData,$reports->patient_code,$reports->ipd_no,$reports->patient_name,$adm_date,$reports->net_price,$pay_date);
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
        ...
        // Fetching the table data
          ...
          header("Content-Disposition: attachment; filename=advance_payment_list_".time().".xls");
    ...
    {    
        ...
        $this->load->view('advance_payment/advance_html',$data);
        $html = $this->output->get_output();
        $this->load->library('pdf');
        $this->pdf->load_html($html);
        $this->pdf->render();
        $this->pdf->stream("advance_payment_list_".time().".pdf");
    }
