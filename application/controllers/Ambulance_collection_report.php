...
class Ambulance_collection_report extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('reports/ambulance_collection_report_model','ambulance_collection_reports');
        $this->load->library('form_validation');
    }

    public function index()
    {   unauthorise_permission('350','2107');
        $this->session->unset_userdata('ambulance_collection_resport_search_data');
        $data['sub_branch'] = $this->session->userdata('sub_branches_data'); 
        $search_date = $this->session->userdata('ambulance_collection_resport_search_data');
// Default Search Setting
        ...
        $this->load->view('ambulance_collection/list',$data);
    }
    public function expenses(){
       $data['page_title'] = 'Ambulance Collection Reports';
       $this->load->view('ambulance_collection/expense_report',$data);
    }
    public function ajax_list()
    {  
        unauthorise_permission('350','2107');
        $users_data = $this->session->userdata('auth_users'); 
        $list = $this->ambulance_collection_reports->get_datatables();
        //print_r($list);die;
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        ...
        foreach ($list as $reports) 
        {
         // print_r($reports);die;
            ...
            $print_pdf_url = "'".base_url('ambulance/booking/print_booking_report/'.$reports->id.'/'.$users_data['parent_id'])."'";
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->ambulance_collection_reports->count_all(),
                        "recordsFiltered" => $this->ambulance_collection_reports->count_filtered(),
                        "data" => $data,
                );
    ...
    public function search_data()
    {
       ...
       if(!empty($post))
       {
         ...
         $this->session->set_userdata('ambulance_collection_resport_search_data',$search_data);
       }
    }
    ...
    public function ambulance_report_excel()
    {
        unauthorise_permission('350','2107');
          ...
          $fields = array('Booking No.','Booking Date','Patient Name','Referred By','Total Amount','Discount','Paid Amount','Balance');
        ...
        $objPHPExcel->getDefaultStyle()->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
          ...
          foreach ($fields as $field)
          {
              ...
              $list = $this->ambulance_collection_reports->search_report_data();
              $this->load->model('general/general_model');
              $get_date_time_setting = $this->general_model->get_date_time_setting('ambulance_collection_report');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
                  ...
                  // added on 02-feb-2018
                    ...
                    array_push($rowData,$reports->booking_no,$booking_date,$reports->patient_name,$reports->doctor_hospital_name,$reports->total_amount,$reports->discount,$reports->paid_amount,$reports->balance);
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
        ...
        header("Content-Disposition: attachment; filename=ambulance_collection_report_".time().".xls"); 
    ...
    public function ambulance_report_csv()
    {
         unauthorise_permission('350','2107');
          // Starting the PHPExcel library
          $this->load->library('excel');
          ...
          // Field names in the first row
          $fields = array('OPD No.','Booking Date','Patient Name','Consultant','Referred By','Total Amount','Discount','Paid Amount','Balance');
          ...
          if(!empty($list))
          {
              ...
              foreach($list as $reports)
              {
                  ...
                  else
                  {
                   ...
                   array_push($rowData,$reports->booking_no,$bookingdate,$reports->patient_name,$reports->doctor_hospital_name,$reports->total_amount,$reports->discount,$reports->paid_amount,$reports->balance);
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
                      $data[$i][$fields[$j]] = $rowData[$j];
    ...
    public function advance_search()
    {
        ...
        {
            $this->session->set_userdata('ambulance_collection_resport_search_data',$post); 
        }
...
//$data['attended_doctor_list'] = $this->booking->attended_doctor_list();
        ...
        $data['search_data'] = $this->session->userdata('ambulance_collection_resport_search_data');
        $this->load->model('general/general_model','general_model');
        $data['employee_list'] = $this->general_model->branch_user_list();
        if(isset($data['search_data']) && !empty($data['search_data']))
        {
           ...
           $data['form_data'] = array(
                                   ...
                                   'attended_doctor'=>$search_data['reffered'],
        ...
        $this->load->view('ambulance_collection/advance_search',$data);
    }

    public function pdf_ambulance_report()
    {   
        unauthorise_permission('350','2107');
        $data['print_status']="";
        $this->load->model('general/general_model');
        ...
        $data['data_list'] = $this->ambulance_collection_reports->search_report_data();
        $this->load->view('ambulance_collection/ambulance_report_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("ambulance_collection_report_".time().".pdf");
    }

    public function print_ambulance_report()
    {   
       unauthorise_permission('350','2107');
        $data['print_status']="1";
        $this->load->model('general/general_model');
    ...
    { 
     ...
     {
        $data['opd_collection_list'] = $this->ambulance_collection_reports->get_opd_collection_list_details($get);
     } 
     $this->load->view('ambulance_collection/list_ambulance_collection',$data);  
