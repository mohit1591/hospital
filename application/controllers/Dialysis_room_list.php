...
class Dialysis_room_list extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('dialysis_room_list/dialysis_room_list_model','dialysis_room_list');
    ...
    { 
     // unauthorise_permission('108','670');
      ...
      $this->load->view('dialysis_room_list/list',$data);
    }
    ...
    { 
      ...
      $list = $this->dialysis_room_list->get_datatables();
        ...
        foreach ($list as $dialysis_room_list) {
         // print_r($dialysis_room_list);die;
            $no++;
            $row = array();
            ...
            ////////// Check list end ///////////// 
            $checkboxs = "";
            if($users_data['parent_id']==$dialysis_room_list->branch_id)
            {
               $row[] = '<input type="checkbox" name="employee[]" class="checklist" value="'.$dialysis_room_list->id.'">'.$check_script;
            }else{
               $row[]='';
            }
            $row[] = $dialysis_room_list->room_no;
            $row[] = $dialysis_room_list->room_category;
            // $row[] = $dialysis_room_list->panel_type;
            // $row[] = $dialysis_room_list->panel_company;
            ...
            if(!empty($get_charges))
            {
              ...
              {
                  $room_charges = get_dialysis_room_charge_accordint_to_id($users_data['parent_id'], $charge_type->id, $dialysis_room_list->room_type_id, $dialysis_room_list->id);
            ...
            $get_bad_number= count_bad($dialysis_room_list->id);
            $row[]='<a href="'.base_url().'dialysis_room_list/get_bed_to_room_list/roomno/'.$dialysis_room_list->id.'/roomcategory/'.$dialysis_room_list->room_category_id.'"><u>'.$get_bad_number[0]->total_bad.'</u></a>';
            //$row[] = date('d-M-Y H:i A',strtotime($dialysis_room_list->created_date)); 
            $btnedit='';
            $btndelete='';
            $btnview='';
            ...
            if($users_data['parent_id']==$dialysis_room_list->branch_id)
            {
               //if(in_array('672',$users_data['permission']['action'])){
                   $btnedit = ' <a class="btn-custom" onClick="return edit_dialysis_room_list('.$dialysis_room_list->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-pencil"></i> Edit</a> ';
              // }
               //if(in_array('673',$users_data['permission']['action'])){
                    $btndelete = ' <a class="btn-custom" onClick="return delete_dialysis_room_list('.$dialysis_room_list->id.')" href="javascript:void(0)" title="Delete" data-url="512"><i class="fa fa-trash"></i> Delete</a> ';  
               //} 
                    $btnview =' <a onClick="return view_dialysis_room_list('.$dialysis_room_list->id.');" class="btn-custom" href="javascript:void(0)" style="'.$dialysis_room_list->id.'" title="Edit"><i class="fa fa-info-circle" aria-hidden="true"></i> View</a>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->dialysis_room_list->count_all(),
                        "recordsFiltered" => $this->dialysis_room_list->count_filtered(),
                        "data" => $data,
                );
    ...
    public function room_list_excel()
    {
        ...
        if(!empty($get_charges))
          ...
          $rowData = array();
          $data= array();
          $list = $this->dialysis_room_list->search_report_data();
          if(!empty($list))
          { 
              $i=0;
          ...
          header("Content-Disposition: attachment; filename=dialysis_room_list_".time().".xls");
    ...
    public function room_list_csv()
    {
        ...
        if(!empty($get_charges))
          ...
          header("Content-Disposition: attachment; filename=dialysis_room_list_".time().".csv");
          header("Pragma: no-cache"); 
          header("Expires: 0");
         if(!empty($data))
         {
    ...
    {    
        $data['print_status']="";
        $data['data_list'] = $this->dialysis_room_list->search_report_data();
        $this->load->view('dialysis_room_list/dialysis_room_list_html',$data);
    ...
    //$this->load->view('dialysis_room_list/advance_search',$data);
    }
    ...
    public function add()
    {
        ...
        $data['room_charge_type_list'] = $this->general_model->dialysis_room_charge_type_list();
       ...
       $this->load->view('dialysis_room_list/add',$data);       
    }
     // -> function to find gender according to selected dialysis_room_list
    // -> change made by: mahesh:date:23-05-17
    // -> starts:
    public function find_gender(){
         $this->load->model('general/general_model'); 
         $dialysis_room_list_id = $this->input->post('dialysis_room_list_id');
         $data='';
          if(!empty($dialysis_room_list_id)){
               $result = $this->general_model->find_gender($dialysis_room_list_id);
               if(!empty($result))
               {
                  $male = "";
                  $female = ""; 
    ...
    public function edit($id="")
    {
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {     
        ...
        $data['room_type_list'] = $this->general_model->dialysis_room_type_list(); 
         $data['room_charge_type_list'] = $this->general_model->dialysis_room_charge_type_list();
         $result = $this->dialysis_room_list->get_by_id($id);  

        //print_r($result);die;
        $data['page_title'] = "Update Dialysis Room";  
        $post = $this->input->post();
        $data['form_error'] = ''; 
        $data['form_data'] = array(
                                  'data_id'=>$result['id'],
         ...
         if(!empty($room_charge_type_list)){ 
               ...
               $data['form_data'][strtolower(str_replace(' ','_',$room_charge_type_list[$i]['charge_type']))] =get_dialysis_charges_according($id,$room_charge_type_list[$i]['id']);
    ...
    private function _validate()
    {
     ...
     public function check_room_no($str){
          ...
          if(!empty($post['room_no']))
          {
               ...
               {
                    $room_data = $this->general->check_dialysis_room_no($post['room_no'],$post['room_category_id']);
                    if(empty($room_data))
                    {
                         return true;
                    }
    ...
    public function delete($id="")
    {
      //unauthorise_permission('108','673');
       if(!empty($id) && $id>0)
       {
           ...
           $result = $this->dialysis_room_list->delete($id);
           $response = "dialysis_room_list successfully deleted.";
           echo $response;
       }
    }
    ...
    function deleteall()
    {
        ...
        if(!empty($post))
        {
            $result = $this->dialysis_room_list->deleteall($post['row_id']);
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        $data['form_data'] = $this->dialysis_room_list->get_dialysis_room_details($id);  
      //print '<pre>'; print_r($data['form_data'][0]['total_bad']);die;
        $data['page_title'] ="Dialysis Room Detail";
       
        $this->load->view('dialysis_room_list/view_dialysis_room',$data);     
      }
    }  
    ...
    public function archive()
    {
        ...
        $this->load->view('dialysis_room_list/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        //unauthorise_permission('108','674');
         $users_data = $this->session->userdata('auth_users');
        $this->load->model('dialysis_room_list/dialysis_room_list_archive_model','dialysis_room_list_archive'); 
         $list = $this->dialysis_room_list_archive->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        $total_num = count($list);
        foreach ($list as $dialysis_room_list) {
         // print_r($dialysis_room_list);die;
            $no++;
            $row = array();
            // if($dialysis_room_list->status==1)
            // {
            ...
            $row[] = '<a href="'.base_url().'dialysis_room_list/get_bed_to_room_list/roomno/'.$dialysis_room_list->id.'/roomcategory/'.$dialysis_room_list->room_category_id.'">'.$dialysis_room_list->room_no.'</a>'; 
            $row[] = $dialysis_room_list->room_category;
            // $row[] = $dialysis_room_list->panel_type;
            // $row[] = $dialysis_room_list->panel_company;
            $get_charges= get_room_charge_according_to_branch($users_data['parent_id']);
            if(!empty($get_charges))
            {
              ...
              foreach($get_charges as $charge_type)
              {
                  $room_charges = get_room_charge_accordint_to_id($users_data['parent_id'], $charge_type->id, $dialysis_room_list->room_type_id, $dialysis_room_list->id);
                  if(!empty($room_charges[0]->room_charge))
                  {
                     $row[] = $room_charges[0]->room_charge;
                  }
            ...
            $btndelete='';
             //if(in_array('676',$users_data['permission']['action'])){
               $btnrestore = ' <a onClick="return restore_dialysis_room_list('.$dialysis_room_list->id.');" class="btn-custom" href="javascript:void(0)"  title="Restore"><i class="fa fa-window-restore" aria-hidden="true"></i> Restore </a>';
               //}
               //if(in_array('675',$users_data['permission']['action'])){
                    $btndelete = ' <a onClick="return trash('.$dialysis_room_list->id.');" class="btn-custom" href="javascript:void(0)" title="Delete"><i class="fa fa-trash" aria-hidden="true"></i> Delete</a>'; 
               // }
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->dialysis_room_list_archive->count_all(),
                        "recordsFiltered" => $this->dialysis_room_list_archive->count_filtered(),
                        "data" => $data,
                );
        //output to json format
        echo json_encode($output);
    ...
    public function restore($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           $result = $this->dialysis_room_list_archive->restore($id);
           $response = "Dialysis Room successfully restore in Dialysis Room.";
           echo $response;
       }
    }
    ...
    { 
        ...
        if(!empty($post))
        {
            $result = $this->dialysis_room_list_archive->restoreall($post['row_id']);
            $response = "Dialysis Room  successfully restore in Dialysis Room.";
            echo $response;
        }
    }
    ...
    public function trash($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           $result = $this->dialysis_room_list_archive->trash($id);
    ...
    function trashall()
    {
        ...
        if(!empty($post))
        {
            $result = $this->dialysis_room_list_archive->trashall($post['row_id']);
            $response = "Dialysis Room successfully deleted parmanently.";
            echo $response;
        }
    }
  ...
  public function dialysis_room_list_dropdown()
  {
     $dialysis_room_list_list = $this->dialysis_room_list->dialysis_room_list();
     $dropdown = '<option value="">Select Room</option>'; 
     //$dialysis_room_lists_array = array('Mr','mR','mr','MR','Mr.','mR.','mr.','MR.');
     
     if(!empty($dialysis_room_list_list))
     {
          foreach($dialysis_room_list_list as $dialysis_room_list)
          {
               
               $selected_dialysis_room_list = '';   
               $dropdown .= '<option value="'.$dialysis_room_list->id.'" '.$selected_dialysis_room_list.' >'.$dialysis_room_list->dialysis_room_list.'</option>';
          }
     } 
     echo $dropdown; 
  }
  public function bed_status()
  {
      $data['page_title']="Room Bed Status";
       $this->load->model('general/general_model'); 
      $data['room_type_list']=$this->general_model->dialysis_room_type_list();
      $this->load->view('dialysis_room_list/room_bed_status',$data);
  }
  public function select_bed_no_number()
  {
        ...
        $beds_total_count = $this->dialysis_room_list->get_all_beds_counts('',$room_id,$room_no_id);
        $total_bed_in_hosp = $beds_total_count[0]->total_bad;

        $beds_count = $this->dialysis_room_list->get_all_beds_counts(1,$room_id,$room_no_id);
        $total_bed_filled = $beds_count[0]->total_bad;

        $beds_free_count = $this->dialysis_room_list->get_all_beds_counts(2,$room_id,$room_no_id);
        ...
        $dropdown ="";
          ...
          {
            $data['number_bed']= $this->general_model->number_dialysis_bed_all_list($room_id,$room_no_id,$dialysis_id);
          }
   ...
   $dropdown .='<div> <table align="right" cellpadding="0" cellspacing="0" border="0" width="200px" style="border:1px solid #EEE;margin-bottom:10px;">
         ...
         <tr><td style="padding:4px;"><div class="m_alert_orange_mark" style="background-color:orange;"></div></td> <td> <a href="javascript:void(0);"  onClick="return search_filled('.$room_id.','.$room_no_id.',2);"> VACANT  '.$total_bed_free.'</a></td> </tr> <tr><td style="padding:4px;"><div class="m_alert_orange_mark" style="background-color:green;"></div></td><td><a href="javascript:void(0);"  onClick="return search_filled('.$room_id.','.$room_no_id.',1);"> Full  '.$total_bed_filled.'</a></td> </tr></table></div><br><br><br><br><br><br>';
            ...
            foreach($data['number_bed'] as $number_bed)
            {
              ...
              {
                 $get_patient_detail=$this->dialysis_room_list->get_patient_detail($number_bed->id,$number_bed->dialysis_id);
                 $free_button='';
                ...
                $bad_no_iddd = $number_bed->bad_no;
                 ...
                 $dialysis_id = $get_patient_detail[0]->id;
                 $free_button = ' <a href="javascript:void(0);"  onClick="return free_room('.$dialysis_id.','.$room_iddd.','.$room_no_iddd.','.$bad_no_iddd.');"> Free Room</a>';
                 ...
                 //print_r()
                $dropdown .='<div class="bed green">Name : '.$get_patient_detail[0]->patient_name.' <br> Dialysis No. : '.$get_patient_detail[0]->booking_code.' <br> Bed No. : '.$bad_name.' '.$free_button.'</div>';
              }
              else
              {
                  if(!empty($number_bed->bad_name))
  ...
  public function free_room()
  {
       ...
       // echo "<pre>"; print_r($post); exit;
        ...
        if(!empty($bed_id) && !empty($dialysis_id))
        {
            $this->db->set('status','0');
            $this->db->set('dialysis_id','0');
          ...
          if(!empty($room_id) && !empty($room_no_id))
          {
            $data['number_bed']= $this->general_model->number_dialysis_bed_all_list($room_no_id,$room_id,$dialysis_id);
          }
  ...
  { 
    ...
    $status = $post['status'];
        ...
        $this->load->model('general/general_model');
        $table ='';
        $room_list = $this->dialysis_room_list->get_room_list();

        $beds_total_count = $this->dialysis_room_list->get_all_beds_counts();
        $total_bed_in_hosp = $beds_total_count[0]->total_bad;

        $beds_count = $this->dialysis_room_list->get_all_beds_counts(1);
        $total_bed_filled = $beds_count[0]->total_bad;

        $beds_free_count = $this->dialysis_room_list->get_all_beds_counts(2);
        $total_bed_free = $beds_free_count[0]->total_bad;
        ...
        if((isset($room_list) && !empty($room_list)))
        {
            ...
            {  
                ...
                foreach ($number_room_list as $room_list) 
                {
                    ...
                    $bed_list = $this->general_model->all_filter_number_dialysis_bed_all_list($particularlist->id,$room_list->id,$status);
                    ...
                    if(!empty($bed_list))
                    {
                      ...
                      foreach($bed_list as $number_bed)
                      {
                        ...
                        if($number_bed->status==1 &&  !empty($number_bed->dialysis_id))
                        {
                          ...
                          $table.='';
                          $table .='<div style="background-color:green;   color:#FFF;   font-family:calibri;   font-size:13px;   border-radius:3px;padding:5px;"><p>Name : '.$get_patient_detail[0]->patient_name.'</p><p>Dialysis No. : '.$get_patient_detail[0]->booking_code.'</p><p>Bed No. : '.$bad_name.'</p> </div>';
                           $table.=' ';
                        }
                        else
                        {
  ...
  { 
        ...
        $this->load->model('general/general_model');
        $table ='';
        $room_list = $this->dialysis_room_list->get_room_list($room_id);

        $beds_total_count = $this->dialysis_room_list->get_all_beds_counts(0,$room_id);
        $total_bed_in_hosp = $beds_total_count[0]->total_bad;

        $beds_count = $this->dialysis_room_list->get_all_beds_counts(1,$room_id);
        $total_bed_filled = $beds_count[0]->total_bad;

        $beds_free_count = $this->dialysis_room_list->get_all_beds_counts(2,$room_id);
        $total_bed_free = $beds_free_count[0]->total_bad;
    ...
    $table .='<div> <table align="right" cellpadding="0" cellspacing="0" border="0" width="200px" style="border:1px solid #EEE;margin-bottom:10px;">
        ...
        if((isset($room_list) && !empty($room_list)))
        {
            ...
            {  
                ...
                $number_room_list = $this->general_model->dialysis_room_no_list($particularlist->id); 
                foreach ($number_room_list as $room_list) 
                {
                    $table.='<tr style="background-color:lightgray;">';
                    $table.='<td style="text-align:left;padding:4px;"><b>Room No.:'.$room_list->room_no.'</b></td></tr>';

                    $bed_list = $this->general_model->number_dialysis_bed_all_list($particularlist->id,$room_list->id);
                    //echo "<pre>";print_r($bed_list);
  ...
  public function get_bed_to_room_list(){
        ...
        $room_to_bed_data = $this->dialysis_room_list->get_bed_to_room_list($room_data);
        // echo "<pre>";
        // print_r($room_to_bed_data);
        // die;
        $data['bed_to_room_list'] = $this->format_room_to_bed_data($room_to_bed_data);
        $this->load->view('dialysis_room_list/bed_to_room_list',$data);
  }
  ...
  public function edit_bed($id="")
  {
    ...
    if(isset($id) && !empty($id) && is_numeric($id))
      {     
        ...
        $result = $this->dialysis_room_list->get_bed_by_id($id);
        $post= $this->input->post();
        ...
        {   
             ...
             $this->dialysis_room_list->bed_save();
             echo 1;
             return false;
       ...
       $this->load->view('dialysis_room_list/edit_dialysis_bed',$data);       
      }
  }
