...
class Day_care extends CI_Controller {
  ...
  public function index()
  {
    ...
    {   
        ...
        foreach ($list as $test) 
        {
            ...
            $row[] = number_format($test->total_amount1,2);
            $row[] = number_format($test->net_amount1,2);
            $row[] = number_format($test->paid_amount1,2);
            $row[] = number_format($test->discount1,2);
            $row[] = $test->policy_no;
            ...
            $blank_btn_print =''; 
             ...
             if(in_array('524',$users_data['permission']['action'])){
               ...
               $btndischarge_summary = '<a class="btn-custom" href='.base_url('daycare_discharge_summary/add/'.$test->id.'/'.$test->patient_id).' title="Discharge Summary" data-url="512"> <i class="fa fa-bed" aria-hidden="true"></i> Discharge Summary</a>';
  ...
  if($test->discharge_status==1){
     ...
     $running_bill = ' <a class="" href="'.base_url('day_care/running_bill_info/'.$test->id.'/'.$test->patient_id.'').'" title="Modify Bill" data-url="512"><i class="fa fa-plus"></i> Modify Bill</a>'; 
      }else{
      ...
      // $discharge_bill = ' <a class="" href="'.base_url('day_care/discharge_bill_info?opd_id='.$test->id.'&patient_id='.$test->patient_id.'').'" title="Running Bill" data-url="512"><i class="fa fa-database"></i> Discharge Bill</a>';
        ...
        <ul class="slidedown-content">
            ...
            // End Action Button //
            $row[] = $btn_confirm.$btn_edit.$btndischarge_summary.$btn_delete.$btn_print.$btn_a;    
            $data[] = $row;
            $i++;
        }
...
//echo "<pre>";print_r($result); die;
  ...
  /* sample collected data */
      public function sample_import_opd_excel()
      {
              ...
              // Field names in the first row
              $fields = array('Day Care No.','Patient Reg.No.(*)','Simulation','Patient Name(*)','Mobile No.(*)','Address','Gender (Male=1,Female=0,Others=2)','Age Year(yyyy)(*)','Age Month(mm)(*)','Age Day(dd)(*)','Age Hours','Remarks','Doctor Name(*)','Specilization(*)','Payment mode','Amount(*)','Discount(*)','Paid Amount','Created Date(yyyy-mm-dd)');
              $col = 0;
              foreach ($fields as $field)
              {
                  $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, 1, $field);
              ...
              // Sending headers to force the user to download the file
              header('Content-Type: application/vnd.ms-excel charset=UTF-8');
              header("Content-Disposition: attachment;filename=daycare_import_sample_".time().".xls");  
              header("Pragma: no-cache"); 
              header("Expires: 0");
              ob_end_clean();
              $objWriter->save('php://output');
...
/* Running bill info 10-1-20*/ 
    ...
    public function running_bill_info($opd_id="",$patient_id="")
     {
          ...
          $data['day_care_details'] = $this->daycare->get_by_id($data['opd_id']);
          //echo "<pre>"; print_r($data['day_care_details']);die;
          $this->load->model('general/general_model');
          $data['payment_mode']=$this->general_model->payment_mode();
            ...
            { 
                ...
                if($this->form_validation->run() == TRUE)
                {
                  //echo "dsdsd"; exit;
                    ...
                    redirect(base_url('day_care/running_bill_info/'.$opd_id.'/'.$patient_id.'/?status=print&billstatus=printbill')); 
  ...
  public function print_letterhead_running_bill($opd_id="",$patient_id="")
   ...
   { 
        ...
        $total_reg_amount = $post['total_reg_amount'];
        $total_net_amount = $total_reg_amount+$post['total_amount']-$total_discount;
        $net_amount = $total_net_amount;
        $payamount= $post['pay_amount'];//$net_amount;
        $blance_dues=$total_net_amount-$payamount-$total_discount;
    ...
    function get_particular_data($vals="")
    {
        if(!empty($vals))
        {
            $result = $this->daycare->get_particular_data($vals);  
        if(!empty($result))
        {
            echo json_encode($result,true);
        } 
    ...
    function add_perticulars($opd_id="",$patient_id="",$discharge_date='')
    {
     $post= $this->input->post();
     if(!empty($post['particular']))
     {
        $this->daycare->save_particulars($opd_id,$patient_id);
        $this->session->set_flashdata('success','Particular has been successfully added.');
        $data=array('success'=>1,'opd_id'=>$opd_id,'patient_id'=>$patient_id,'discharge_date'=>$discharge_date);
