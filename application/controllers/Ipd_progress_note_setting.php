...
class Ipd_progress_note_setting extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        
        $this->load->model('ipd_progress_note_setting/ipd_progress_note_setting_model','prescriptionsetting');
    ...
    public function index()
    {
        ...
        unauthorise_permission(125,764);
        $data['page_title'] = 'Progress Note Settings'; 
        $post = $this->input->post();
        if(!empty($post))
        { 
            $this->prescriptionsetting->save();
            echo 'Your progress note Settings successfully updated.';;
            return false;
        }

        
        $data['prescription_setting_list'] = $this->prescriptionsetting->get_master_unique(0);
        $this->load->view('ipd_progress_note_setting/add',$data);
    }


    public function preview($id="")
    {
        $this->load->model('ipd_progress_report/ipd_progress_report_model','prescription');

        $template_format = $this->prescription->template_format(array('setting_name'=>'PROGRESS_REPORT_SETTING','unique_id'=>5,'type'=>0));
        ...
        $this->load->view('ipd_progress_note_setting/preview',$data);
    } 
