...
class Test_name extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pediatrician/test_name/test_name_model','test_name');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('336','1985');
        $data['page_title'] = 'Test Name List'; 
        $this->load->view('pediatrician/test_name/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $test_name) {
         // print_r($relation);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('1987',$users_data['permission']['action'])){
           ...
           if(in_array('1988',$users_data['permission']['action'])){
    ...
    public function add()
    {
        unauthorise_permission('336','1986');
       ...
       $this->load->view('pediatrician/test_name/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('336','1987');
    ...
    {
       unauthorise_permission('336','1988');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('pediatrician/test_name/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('336','1989');
        $data['page_title'] = 'Test Name Archive List';
        $this->load->helper('url');
        $this->load->view('pediatrician/test_name/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        unauthorise_permission('336','1989');
        $users_data = $this->session->userdata('auth_users');
        $this->load->model('pediatrician/test_name/test_name_archive_model','test_name_archive'); 
        ...
        foreach ($list as $test_name) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1991',$users_data['permission']['action'])){
          ...
          if(in_array('1990',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('336','1991');
        $this->load->model('pediatrician/test_name/test_name_archive_model','test_name_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('336','1990');
        $this->load->model('pediatrician/test_name/test_name_archive_model','test_name_archive');
       if(!empty($id) && $id>0)
       {
