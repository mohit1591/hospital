...
class Suggestion extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pediatrician/suggestion/suggestion_model','suggestion');
    ...
    {  
        ...
        unauthorise_permission(328,2045);
        $data['page_title'] = 'Suggestion List'; 
       $this->load->view('pediatrician/suggestion/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $suggestion) {
         // print_r($religion);die;
           ...
           if(in_array('2047',$users_data['permission']['action'])) 
            {
           ...
           if(in_array('2048',$users_data['permission']['action'])) 
            {
    ...
    public function add()
    {
        unauthorise_permission(328,2046); 
       ...
       $this->load->view('pediatrician/suggestion/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission(328,2047); 
    ...
    {
       unauthorise_permission(328,2048); 
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission(328,2049); 
        $data['page_title'] = 'Suggestion Archive List';
        $this->load->helper('url');
        $this->load->view('pediatrician/suggestion/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        unauthorise_permission(328,2049);
        $users_data = $this->session->userdata('auth_users');
        $this->load->model('pediatrician/suggestion/suggestion_archive_model','suggestion_archive'); 
        ...
        foreach ($list as $suggestion) {
         // print_r($religion);die;
            ...
            if(in_array('2051',$users_data['permission']['action'])) 
            {
            ...
            if(in_array('2050',$users_data['permission']['action'])) 
            {
    ...
    {
        unauthorise_permission(328,2051);
        $this->load->model('pediatrician/suggestion/suggestion_archive_model','suggestion_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission(328,2050);
        $this->load->model('pediatrician/suggestion/suggestion_archive_model','suggestion_archive');
       if(!empty($id) && $id>0)
       {
