...
class Diagnosis extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pediatrician/diagnosis/diagnosis_model','diagnosis');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('337','1978');
        $data['page_title'] = 'Diagnosis List'; 
        $this->load->view('pediatrician/diagnosis/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $diagnosis) {
         // print_r($diagnosis);die;
            ...
            if($users_data['parent_id']==$diagnosis->branch_id)
            {
              if(in_array('1980',$users_data['permission']['action'])){
              ...
              if(in_array('1981',$users_data['permission']['action'])){
    ...
    public function add()
    {
      //echo json_encode("hi");die;
        unauthorise_permission('337','1979');
       ...
       $this->load->view('pediatrician/diagnosis/add',$data);       
    }

    public function edit($id="")
    {
     unauthorise_permission('337','1980');
    ...
    {
       unauthorise_permission('337','1981');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('pediatrician/diagnosis/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('337','1982');
        $data['page_title'] = 'Diagnosis Archive List';
        $this->load->helper('url');
        $this->load->view('pediatrician/diagnosis/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('337','1982');
        $this->load->model('pediatrician/diagnosis/diagnosis_archive_model','diagnosis_archive'); 
        ...
        foreach ($list as $diagnosis) {
         // print_r($diagnosis);die;
            ...
            $btnrestore='';
            $btndelete='';
            if(in_array('1984',$users_data['permission']['action'])){
              ...
              if(in_array('1983',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('337','1984');
        $this->load->model('pediatrician/diagnosis/diagnosis_archive_model','diagnosis_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('337','1983');
        $this->load->model('pediatrician/diagnosis/diagnosis_archive_model','diagnosis_archive');
       if(!empty($id) && $id>0)
       {
