...
class Dosage extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pediatrician/dosage/dosage_model','dosage');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('332','2013');
        $data['page_title'] = 'Dosage List'; 
        $this->load->view('pediatrician/dosage/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $dosage) {
         // print_r($dosage);die;
            ...
            if($users_data['parent_id']==$dosage->branch_id)
            {
              if(in_array('2015',$users_data['permission']['action'])){
              ...
              if(in_array('2016',$users_data['permission']['action'])){
    ...
    public function add()
    {
      //echo json_encode("hi");die;
        unauthorise_permission('332','2014');
       ...
       $this->load->view('pediatrician/dosage/add',$data);       
    }

    public function edit($id="")
    {
     unauthorise_permission('332','2015');
    ...
    {
       unauthorise_permission('332','2016');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('pediatrician/dosage/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('332','2017');
        $data['page_title'] = 'Dosage Archive List';
        $this->load->helper('url');
        $this->load->view('pediatrician/dosage/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('332','2017');
        $this->load->model('pediatrician/dosage/dosage_archive_model','dosage_archive'); 
       ...
       $data = array();
        ...
        foreach ($list as $dosage) {
         // print_r($dosage);die;
            ...
            $btnrestore='';
            $btndelete='';
            if(in_array('2019',$users_data['permission']['action'])){
              ...
              if(in_array('2018',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('332','2019');
        $this->load->model('pediatrician/dosage/dosage_archive_model','dosage_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('332','2018');
        $this->load->model('pediatrician/dosage/dosage_archive_model','dosage_archive');
       if(!empty($id) && $id>0)
       {
