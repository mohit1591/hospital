...
class Personal_history extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pediatrician/personal_history/personal_history_model','personal_history');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('334','1999');
        $data['page_title'] = 'Personal History List'; 
        $this->load->view('pediatrician/personal_history/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $personal_history) {
         // print_r($relation);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('2001',$users_data['permission']['action'])){
           ...
           if(in_array('2002',$users_data['permission']['action'])){
    ...
    public function add()
    {
        unauthorise_permission('334','1301');
       ...
       $this->load->view('pediatrician/personal_history/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('334','2001');
    ...
    {
       unauthorise_permission('334','2002');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('pediatrician/personal_history/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('334','2003');
        $data['page_title'] = 'Personal History Archive List';
        $this->load->helper('url');
        $this->load->view('pediatrician/personal_history/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        unauthorise_permission('334','2003');
         $users_data = $this->session->userdata('auth_users');
         $this->load->model('pediatrician/personal_history/personal_history_archive_model','personal_history_archive'); 
        ...
        foreach ($list as $personal_history) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('2005',$users_data['permission']['action'])){
          ...
          if(in_array('2004',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('334','2005');
        $this->load->model('pediatrician/personal_history/personal_history_archive_model','personal_history_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('334','2004');
        $this->load->model('pediatrician/personal_history/personal_history_archive_model','personal_history_archive');
       if(!empty($id) && $id>0)
       {
