...
class Previous_history extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('pediatrician/previous_history/Previous_history_model','previoushis');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('340','1957');
        $data['page_title'] = 'Previous History List'; 
        $this->load->view('pediatrician/previous_history/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $previoushis) {
         // print_r($previoushis);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('1959',$users_data['permission']['action'])){
          ...
          if(in_array('1960',$users_data['permission']['action'])){
    ...
    public function add()
    {
        unauthorise_permission('340','1958');
       ...
       $this->load->view('pediatrician/previous_history/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('340','1959');
    ...
    {
       unauthorise_permission('340','1960');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('pediatrician/previous_history/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('340','1961');
        $data['page_title'] = 'Previous History Archive List';
        $this->load->helper('url');
        $this->load->view('pediatrician/previous_history/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        unauthorise_permission('340','1961');
         $users_data = $this->session->userdata('auth_users');
        $this->load->model('pediatrician/previous_history/previous_history_archive_model','previoushis_archive'); 
        ...
        foreach ($list as $previoushis) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1963',$users_data['permission']['action'])){
          ...
          if(in_array('1962',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('340','1963');
        $this->load->model('pediatrician/previous_history/previous_history_archive_model','previoushis_archive');
       if(!empty($id) && $id>0)
       {
