...
class Stock_issue_allotment extends CI_Controller {
  ...
  function __construct() 
  {
    ...
    public function index()
      {
        unauthorise_permission(167,969);
        $data['page_title'] = 'Issue/Allot list'; 
        ...
        // End Defaul Search  
        $data['form_data'] = array('patient_name'=>'','purchase_no'=>'','invoice_id'=>'','paid_amount_to'=>'','paid_amount_from'=>'','balance_to'=>'','balance_from'=>'','start_date'=>$start_date, 'end_date'=>$end_date);
    ...
    {  
        ...
        foreach ($list as $stock_issue_allotment) { 
           ...
           // $doctor_type = array('0'=>'Referral','1'=>'Attended','2'=>'Referral/Attended');
            $issue_date = '';
            if(date('d-m-Y',strtotime($stock_issue_allotment->issue_date))!='01-01-1970')
            {
              $issue_date = date('d-m-Y',strtotime($stock_issue_allotment->issue_date));
            }
            $row[] = '<input type="checkbox" name="stock_issue_allotment[]" class="checklist" value="'.$stock_issue_allotment->id.'">';
            $row[] = $stock_issue_allotment->issue_no;
            $row[] = $stock_issue_allotment->items;
            ...
            $btndelete='';
            $btnview = '';
           if(in_array('967',$users_data['permission']['action'])){
           ...
           if(in_array('966',$users_data['permission']['action'])){
			 ...
			 $print_url = "'".base_url('stock_issue_allotment/print_stock_issue_allotment_report/'.$stock_issue_allotment->id)."'";
		  ...
		  $grand_total_amount = $grand_total_amount+$stock_issue_allotment->total_amount;
		  $grand_total_discount = $grand_total_discount + $stock_issue_allotment->discount;
		  $grand_net_amount = $grand_net_amount + $stock_issue_allotment->net_amount;
		  $grand_paid_amount = $grand_paid_amount + $stock_issue_allotment->paid_amount;
		  $grand_balance_amount = $grand_balance_amount + $stock_issue_allotment->balance;
		  $tot_row = array();
           if($i==$total_num)
           {
    ...
    public function stock_allotment_issue_excel()
    {
        // Starting the PHPExcel library
		  ...
		  $fields = array('Issue No.','Items','Patient / Employee / Doctor Name','Total Amount','Discount','Net Amount','Paid Amount','Balance','Issue Date');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
				...
				$grand_balance_amount = $grand_balance_amount + $reports->balance;
                  ...
                  array_push($rowData,$reports->issue_no,$reports->items,$reports->member_name,number_format($reports->total_amount,2),number_format($reports->discount,2),number_format($reports->net_amount,2),number_format($reports->paid_amount,2),number_format($reports->balance,2),$issue_date);
                   $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
          ...
          if(!empty($data))
          {
               foreach($data as $reports_data)
               {
                    ...
                    { 
                          ...
                          // added on 31-jan-2018
                          $objWorksheet->setCellValueByColumnAndRow(3,$row+1,number_format($grand_total_amount,2));
						  $objWorksheet->setCellValueByColumnAndRow(4,$row+1,number_format($grand_total_discount,2));
                          $objWorksheet->setCellValueByColumnAndRow(5,$row+1,number_format($grand_net_amount,2));
     ...
     public function advance_search()
      {
        ...
        // $this->load->view('stock_issue_allotment/advance_search',$data);
        }

    public function add($ipd_id="",$patient_id="")
    {
          // echo '<pre>';print_r($_POST);die;
          unauthorise_permission(167,968);
          $post = $this->input->post();
          $issue_code = generate_unique_id(30);
          $this->load->model('general/general_model'); 
          $data['unit_list'] = $this->stock_issue_allotment->unit_list();
          ...
          $data['employee_list']= $this->stock_issue_allotment->get_employee();
          ...
          { 
          $item_id_arr = [];
          foreach($stock_issue_allotment_item_list as $key=>$item_sess)
          {
            $item_id_arr[] = $key;
          } 
          $item_ids = implode(',', $item_id_arr);
          $data['stock_issue_allotment_item_list'] = $this->stock_issue_allotment->item_list($item_ids);
          }


          $data['page_title'] = 'Issue/Allot';
          $data['data_id']='';
          $data['stock_issue_allotment_item_list']=$stock_issue_allotment_item_list;
          $data['form_data']=array(
                                      'data_id'=>'',
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
               $charge_id = $this->stock_issue_allotment->save();
                $this->session->set_flashdata('success','Issue/allotment successfully added.');
    ...
    public function edit($id="")
    {
          ...
          unauthorise_permission(167,967);
          $post = $this->input->post();
     ...
     //echo "<pre>" ; print_r($result);die;     
          ...
          $data['vendor_list'] = $this->stock_issue_allotment->vendor_list();
          $data['payment_mode']=$this->general_model->payment_mode();
          $data['page_title'] = 'Issue/Allot Edit';
          $data['data_id']='';
          
          
    //echo $result['payment_mode'];die;
...
$result_item_list = $this->stock_issue_allotment->get_purchase_to_purchase_by_id($id);

 if(empty($post))
 { 
    $result_item_list = $this->stock_issue_allotment->get_purchase_to_purchase_by_id($id); 
    $this->session->set_userdata('stock_issue_allotment_item_list',$result_item_list);

  }

 $item_list = $this->session->userdata('stock_issue_allotment_item_list');
 $data['stock_issue_allotment_item_list'] = $item_list;
 $data['id'] = $id;
...
{ 
  ...
  $item_ids = implode(',', $item_id_arr);
  $data['stock_issue_allotment_item_list'] = $this->stock_issue_allotment->item_list($item_ids,$item_id_arr); 
...
//echo "<pre>";print_r($item_list);die;
 ...
 if(isset($result['user_type']))
  ...
  $data['type_list'] = $this->employee_model->employee_type_list();
     ...
     // $data['stock_issue_allotment_item_list'] = $stock_issue_allotment_item_list;  
  $issue_date = '';
  if(date('d-m-Y',strtotime($result['issue_date']))!='01-01-1970')
  {
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
               ...
               $purchase_id = $this->stock_issue_allotment->save();
                $this->session->set_flashdata('success','Issue/allotment successfully updated.');
                redirect(base_url('stock_issue_allotment'));
            }
            else
            {
  ...
  public function archive_ajax_list()
    {
        unauthorise_permission(167,965);
        ...
        foreach ($list as $stock_issue_allotment) { 
           ...
           if(in_array('965',$users_data['permission']['action'])){
           ...
           if(in_array('964',$users_data['permission']['action'])){
  ...
  function get_emp_data($vendor_id="",$user_type="")
  {
    ...
    $html='<div class="row">
      ...
      <div class="row m-b-5">
          ...
          <div class="col-xs-7"> '.$data['vendor_list'][0]->name.'<input type="hidden" value="'.$data['vendor_list'][0]->name.'" name="user_name"/></div>
        </div>
                
                 <div class="row m-b-5">
          <div class="col-xs-5"><label>Mobile No.</label></div>
          <div class="col-xs-7">  '.$data['vendor_list'][0]->mobile.'<input type="hidden" value="'.$data['vendor_list'][0]->mobile.'" name="vendor_mobile"/></div></div>

               <div class="row m-b-5">
          <div class="col-xs-5">  <label>Email</label>
                </div>
          <div class="col-xs-7">  '.$data['vendor_list'][0]->vendor_email.'<input type="hidden" value="'.$data['vendor_list'][0]->vendor_email.'" name="vendor_email"/></div></div> 
                
        <div class="row m-b-5">
          <div class="col-xs-5">
                  <label>Address</label></div>
          <div class="col-xs-7"> '.$data['vendor_list'][0]->address.'<input type="hidden" value="'.$data['vendor_list'][0]->address.'" name="address"/> </div>
        </div></div>';
    echo $html;exit;
  }
...
public function get_payment_mode_data()
{
 ...
 function get_item_values($vals="")
  ...
  function trashall()
    {
       unauthorise_permission(167,964);
        $this->load->model('stock_issue_allotment/stock_issue_allotment_archive_model','stock_issue_allotment_archive');
        $post = $this->input->post();  
        if(!empty($post))
        {
            $result = $this->stock_issue_allotment_archive->trashall($post['row_id']);
            $response = "Issue/allotment successfully deleted parmanently.";
            echo $response;
        }
    }
    ...
    { 
       unauthorise_permission(167,965);
        ...
        if(!empty($post))
        {
            ...
            $response = "Issue/allotment successfully restore in stock purchase return list.";
            echo $response;
        }
    }
    ...
    function deleteall()
    {
        unauthorise_permission(167,966);
        $post = $this->input->post();  
        if(!empty($post))
        {
    ...
    public function usertype_list()
    {
        ...
        if(!empty($user_type)){
         ...
         if($user_type==1)
         {
                ...
                $dropdown .='</select></div></div></div> <div class="col-md-4">  <div class="row m-b-5"><div class="col-md-4"><label>Employee Name</label></div><div class="col-md-8"> <select class="w-189px selectpicker" name="user_type_id" id="employee_list_n" onchange="appned_user_detail(1,this.value);"><option value="">Select</option></select></div></div></div> ';
                echo $dropdown; 
         }
         if($user_type==2)
         {
                $dropdown = '<div class="col-md-4"><div class="row m-b-5"><div class="col-md-4"> <label>Patient Name</label></div><div class="col-md-4">';
                
                $dropdown = '<div class="col-md-2"><input type="text" name="mobile_no"  id="mobile_no"  placeholder="Search By Mobile No." class="number m_number tooltip-text numeric"  value="" maxlength="10"   onkeyup="get_patient_detail_by_mobile();" autocomplete="off"></div>';
                $dropdown.= '<div class="col-md-2"><select name="user_type_id" class="selectpicker" id="user_type_id" onchange="appned_user_detail('.$user_type.',this.value);" data-live-search="true" ><option value="">Select Patient</option>'; 
                if(!empty($data['user_list']))
                {
                foreach($data['user_list'] as $user_list)
                {
...
public function ajax_list_item()
{
       ...
       $table = '';
        ...
        { 
          ...
          $data['item_new_list'] = $this->stock_issue_allotment->get_item_values($item_ids);
        ...
        { 
            ...
            $result_item = $this->stock_issue_allotment->item_list_search();  
       ...
       {
        //$this->load->model('item_manage/item_manage_model','item_manage');
         $item_sess = $this->session->userdata('stock_issue_allotment_item_list');
         //echo "<pre>"; print_r($item_sess); exit;
         $check_script="";
         $result_item = [];
        if(!empty($item_sess))
        { 
          ...
          $item_ids = implode(',', $ids_arr);
          $result_item = $this->stock_issue_allotment->item_list($item_ids);
                      ...
                      $table.='<th class="40" align="center">';
                       ...
                       $table.='</th>';
                        ...
                        foreach($result_item as $item)
                        {
                           ...
                           $purchase_item_serial =$this->stock_issue_allotment->purchase_item_serial_by_id($purchase_arr[0],$item->id);
...
/*$('#discount_".$item->id."').keyup(function(e){
                        ...
                        $table.='<td><input type="text" id="qty_'.$item->id.'" class="w-60px" name="qty[]" value="'.$item_sess[$item->id]["qty"].'" onkeyup="payment_cal_perrow('.$item->id.'); validation_check(qty,'.$item->id.');"/><div id="qty_error_'.$item->id.'" style="color:red;"></div></td>';
                       ...
                       // $table.='<td></td>';
                        $table.='<td><input type="text" id="mrp_'.$item->id.'" class="w-60px" name="mrp[]" value="'.number_format($item_sess[$item->id]["perpic_rate"],2,'.','').'" onkeyup="payment_cal_perrow('.$item->id.');"/></td>';
                       ...
                       $table.='<td><input type="text" class="price_float" name="discount[]" placeholder="Discount" style="width:59px;" id="discount_'.$item->id.'" value="'.$item_sess[$item->id]["discount"].'" onkeyup="payment_cal_perrow('.$item->id.');"/></td>';
                        ...
                        $table.=' <td><input type="text" value="'.number_format($item_sess[$item->id]["total_amount"],2,'.','').'" name="total_amount[]" placeholder="Total" style="width:59px;" readonly id="total_amount_'.$item->id.'" /></td>';
 ...
 public function check_stock_avability()
   {
      ...
      $return= $this->stock_issue_allotment->check_stock_avability($explode_ids[0]);
      $tot_val= $qty;
      if($return >= $tot_val){
      echo '0'; 
      }else{
 ...
 public function set_item()
    {
       ...
       {
         $purchase = $this->session->userdata('stock_issue_allotment_item_list');
         $item_id = [];
         $iid_arr = [];
         ...
         { 
           $total_amount=0;
            ...
            {
               $item_data = $this->stock_issue_allotment->item_list($ids);
                //print_r($item_data); die;
                //echo $item_data[0]->conversion; die;
         ...
         else
         {
            ...
            foreach($post['item_id'] as $ids)
            {
             ...
             // echo "<pre>" ; print_r($item_data); die;
                ...
                $post_iid_arr[$ids] = array('item_id'=>$ids,'qty'=>'1','item_name'=>$item_data[0]->item_name,'conversion'=>$item_data[0]->conversion,'manuf_date'=>'00-00-0000','perpic_rate'=>$perpic_rate,'exp_date'=>'00-00-0000', 'mrp'=>$item_data[0]->mrp,'discount'=>$item_data[0]->discount,'cgst'=>$item_data[0]->cgst,'igst'=>$item_data[0]->igst,'sgst'=>$item_data[0]->sgst,'total_amount'=>$total_amount,'category_id'=>$item_data[0]->category_id); 
                $iid_arr[] = $ids;
                $i++;
            }
         ...
         $this->session->set_userdata('stock_issue_allotment_item_list',$item_id);
     ...
     public function remove_item_list()
    {
       ...
       if(isset($post['item_id']) && !empty($post['item_id']))
       {
           $ids_list = $this->session->userdata('stock_issue_allotment_item_list');
             ...
             $this->session->set_userdata('stock_issue_allotment_item_list',$ids_list);
 ...
 public function payment_cal_perrow()
    {
     ...
     //print_r($item_list);die;
         ...
         if(!empty($item_list))
         { 
            $item_data = $this->stock_issue_allotment->item_list($post['item_id']);
           ...
           // print_r($ratewithunit1);die;
            ...
            $item_list[$post['item_id']] =  array('item_id'=>$post['item_id'],'item_name'=>$post['item_name'],'qty'=>$post['qty'],'manuf_date'=>$post['manuf_date'],'conversion'=>$post['conversion'],'perpic_rate'=>$perpic_rate,'exp_date'=>$post['expiry_date'],'mrp'=>$post['mrp'],'sgst'=>$post['sgst'],'igst'=>$post['igst'],'cgst'=>$post['cgst'],'discount'=>$post['discount'], 'total_amount'=>$total_amount,'total_price'=>$total_amount,'category_id'=>$post['category_id'],'item_code'=>$post['item_code']); 
            $this->session->set_userdata('stock_issue_allotment_item_list', $item_list);
  ...
  {     
        $users_data = $this->session->userdata('auth_users');
        $result = $this->stock_issue_allotment->check_unique_value($users_data['parent_id'], $invoice, $id);
        if($result == 0)
            $response = true;
        else {
...
public function print_stock_issue_allotment_report($ids=""){
	 ...
	 $this->load->model('stock_issue_allotment/stock_issue_allotment_model','stock_issue_allotment');
      ...
      $allotment= $this->session->userdata('allotment');
      }
      $get_detail_by_id= $this->stock_issue_allotment->get_by_id($allotment);
      $get_by_id_data=$this->stock_issue_allotment->get_all_detail_print($allotment,$get_detail_by_id['branch_id']);
  ...
  //print '<pre>';print_r($get_by_id_data);die;
      ...
      $template_format= $this->stock_issue_allotment->template_format(array('section_id'=>9,'types'=>1,'sub_section'=>2,'branch_id'=>$get_detail_by_id['branch_id']));
      $data['template_data']=$template_format;
      $data['payment_mode']= $get_payment_detail;
      $data['user_detail']=$user_detail;
      $data['all_detail']= $get_by_id_data;
      $data['ids'] = $ids;
      $this->load->view('stock_issue_allotment/print_template_stock_issue_allotment',$data);
  }
...
//////added function 3-3-20 ////////
    ...
    function search_serial_no($vals="")
    {
        ...
        if(!empty($vals))
        {
            $result = $this->stock_issue_allotment->search_serial_no($vals,$issue_allot_id,$item_id);  
            if(!empty($result))
            {
              echo json_encode($result,true);
            } 
