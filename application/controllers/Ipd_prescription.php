...
class Ipd_prescription extends CI_Controller {
	...
	function __construct() 
	{
        ...
        $this->load->model('ipd_prescription/prescription_file_model','prescription_file');
  ...
  public function index()
  {
      ...
      $data['page_title'] = 'Daily Notes Report List';
      ...
      $this->load->view('ipd_prescription/list',$data);
  }
    ...
    {   
        ...
        foreach ($list as $prescription) { 
            ...
            if($users_data['parent_id']==$prescription->branch_id)
            {
                ...
                $btn_edit = ' <a class="btn-custom" href="'.base_url("ipd_prescription/edit/".$prescription->id).'" title="Edit Daily Notes Report"><i class="fa fa-pencil"></i> Edit Daily Notes Report</a>';
               ...
               $btn_view_pre='';
                /*$btn_view_pre = ' <a class="btn-custom"  href="'.base_url('/ipd_prescription/view_prescription/'.$prescription->id.'/'.$prescription->branch_id).'" title="View Daily Notes Report" target="_blank" data-url="512"><i class="fa fa-info-circle"></i> View Daily Notes Report</a>';*/
                
                $print_url = "'".base_url('ipd_prescription/print_prescriptions/'.$prescription->id.'/'.$prescription->branch_id)."'";
                $btn_print_pre = ' <a class="btn-custom" onClick="return print_window_page('.$print_url.')" href="javascript:void(0)" title="Print Daily Notes Report"  data-url="512"><i class="fa fa-print"></i> Print Daily Notes Report</a>';
                
                 $print_single_page_url = "'".base_url('ipd_prescription/print_single_prescriptions/'.$prescription->booking_id.'/'.$prescription->branch_id)."'";
                $btn_print_single_pre = '<a class="btn-custom" onClick="return print_window_page('.$print_single_page_url.')" href="javascript:void(0)" title="Print Daily Notes Report"  data-url="512"><i class="fa fa-print"></i> Single Page Daily Notes Report</a>';
        ...
        if($users_data['parent_id']==$prescription->branch_id)
        {
            ...
            $btn_upload_pre = '<a class="btn-custom" onclick="return upload_prescription('.$prescription->id.')" href="javascript:void(0)" title="View"><i class="fa fa-info-circle"></i> Upload Daily Notes Report </a>';
               
            $btn_view_upload_pre = '<a class="btn-custom" href="'.base_url('/ipd_prescription/view_files/'.$prescription->id.'/'.$prescription->branch_id).'" title="View Daily Notes Report" data-url="512"><i class="fa fa-circle"></i> View Daily Notes Report Files</a>';
            ...
            $row[] =   $btn_view_pre.$btn_print_pre.$btn_print_single_pre.$btn_upload_pre.$btn_view_upload_pre.$btn_edit.$btn_view.$btn_delete;                
    ...
    public function advance_search()
    {
        ...
        $this->load->view('ipd_prescription/advance_search',$data);
    }
    ...
    public function delete($id="")
    {
      // unauthorise_permission(86,535);
       if(!empty($id) && $id>0)
       {
           ...
           $response = "Daily Notes Report successfully deleted.";
           echo $response;
       }
    }
    ...
    {  
     ...
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('ipd_prescription/view',$data);     
      }
    }  
    ...
    public function archive()
    {
        ...
        $data['page_title'] = 'Daily Notes Report Archive List';
        $this->load->helper('url');
        $this->load->view('ipd_prescription/archive',$data);
    }
    ...
    public function archive_ajax_list()
    {
        //unauthorise_permission(86,536);
        $users_data = $this->session->userdata('auth_users'); 
        $this->load->model('ipd_prescription/prescription_archive_model','prescription_archive'); 
    ...
    $this->load->model('ipd_prescription/prescription_archive_model','prescription_archive');
       if(!empty($id) && $id>0)
       {
           ...
           $response = "Daily Notes Report successfully restore in Progress list.";
           echo $response;
       }
    }
    ...
    { 
        ...
        if(!empty($post))
        {
            ...
            $response = "Daily Notes Report successfully restore in Daily Notes Report list.";
            echo $response;
        }
    }
    ...
    public function trash($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           ...
           $response = "Daily Notes Report successfully deleted parmanently.";
           echo $response;
       }
    }
  ...
  public function prescription_dropdown()
  {
    ...
    { 
        ...
        $this->load->view('ipd_prescription/print_template1',$data);
    ...
    public function print_prescriptions($prescription_id="",$branch_id='')
    {
        ...
        $prescriptions_id= $this->session->userdata('ipd_prescription_id');
        ...
        $data['prescription_tab_setting'] = get_ipd_prescription_print_tab_setting('',$branch_id);
        //echo "<pre>";print_r($data['prescription_tab_setting']); exit;
        $data['prescription_medicine_tab_setting'] = get_ipd_prescription_medicine_print_tab_setting('',$branch_id);
...
$signature_reprt_data .= '<tr>
    ...
    <tr>
        ...
        $this->load->view('ipd_prescription/print_prescription_template',$data);
    }
    ...
    public function print_prescription_pdf($id)
    {
        ...
        if(!empty($appointment_date) && $appointment_date!=='0000-00-00')
        {
         ...
         $pdf_template = $this->load->view('ipd_prescription/pdf_template',$pdf_data,true);
         $result['success'] = true;
         $result['msg'] = 'Print Daily Notes Report  success';
...
{ 
    ...
    {   
        ...
        {
        	$this->form_validation->set_rules('prescription_files', 'Daily Notes Report file', 'trim|required');  
        }
        ...
        if($this->form_validation->run() == TRUE)
        {
             ...
             $config['upload_path']   = DIR_UPLOAD_PATH.'ipd_prescription/';  
    ...
    $this->load->view('ipd_prescription/add_file',$data);
}
...
{ 
	...
	$data['page_title'] = "Daily Notes Report Files";
	$data['prescription_id'] = $prescription_id;
	$this->load->view('ipd_prescription/view_prescription_files',$data);
}
...
{ 
        ...
        foreach ($list as $prescription) { 
            ...
            $sign_img = "";
            if(!empty($prescription->prescription_files) && file_exists(DIR_UPLOAD_PATH.'ipd_prescription/'.$prescription->prescription_files))
            {
                $sign_img = ROOT_UPLOADS_PATH.'ipd_prescription/'.$prescription->prescription_files;
    ...
    public function delete_prescription_file($id="")
    {
      // unauthorise_permission(86,116);
       if(!empty($id) && $id>0)
       {
           ...
           $response = "Daily Notes Report file successfully deleted.";
           echo $response;
       }
    }
    ...
    { 
      ...
      {      
        ...
        $data['vitals_list']=$this->general_model->vitals_list();
        $data['page_title'] = "Update Daily Notes Report";  
        $post = $this->input->post();
        $data['form_error'] = ''; 
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $this->session->set_flashdata('success','Daily Notes Report updated successfully.');
        ...
        $this->load->view('ipd_prescription/prescription',$data);       
      }
    }
	 ...
	 public function print_consolidate_prescription()
    {
        ...
        $branch_id = $users_data['parent_id'];
         ...
         $data['page_title'] = 'Progress Note List';
        ...
        $this->load->view('ipd_prescription/consolidate_prescription',$data);
    }
    
    public function print_single_prescriptions($ipd_id="",$branch_id='')
    {
         
        $data['type'] = 6;
        $data['page_title'] = "Print Prescription";
        $opd_prescription_info = $this->prescription->get_detail_by_ipd_id($ipd_id);
        ...
        $data['template_data']=$template_format->setting_value;
        $data['newtemplate_data']=$template_format->setting_value;
        $data['all_prescription_detail']= $opd_prescription_info;
   ...
   if(!empty($signature_image))
   {
     ...
     $signature_reprt_data .='<table border="0" cellpadding="4px" cellspacing="0" width="100%" style="border-collapse: collapse;font-size:13px;margin-top: 5%;"> 
      ...
      <td width="70%"></td>
        ...
        $this->load->view('ipd_prescription/print_single_page_prescription_template',$data);
    }
