...
class Billing extends CI_Controller {
    ...
    public function total_doctor_comission()
    {
      ...
      if(!empty($post) && !empty($post))
      {
        ...
        if(!empty($post) && !empty($post))
        { 
            if(!empty($post) && $post['doctor_id']=='-1') // for all doctor
            {
                $doctor_list = $this->billing->commission_doctor_list($users_data['parent_id']);
                //echo "<pre>"; print_r($doctor_list); exit;
                if(!empty($doctor_list))
                {
                    foreach($doctor_list as $doctor)
                    {
                      $commission_list = $this->billing->all_doctor_comission($doctor->id,$post['start_date'],$post['end_date'],$users_data['parent_id']);
                      //echo "<pre>"; print_r($commission_list); exit;
                      if(!empty($commission_list))
                      {
                          $total_credit = number_format(array_sum(array_column($commission_list, 'total_credit')),2,'.', ''); 
                          $debit_list = $this->billing->all_doctor_debit($doctor->id,$post['start_date'],$post['end_date'],$users_data['parent_id']);
                         /* if(!empty($debit_list))
                          {
                          ...
                          $total_debit = $debit_list[0]->total_debit;
                          $balance = $total_credit-$total_debit;
                           $start_date="'".$post['start_date']."'";
                            $end_date="'".$post['end_date']."'";
                          $html .= '<tr>
                                    <td style="width:20%;text-align:left;"><input type="radio" name="doctors_id" class="checklist" value="'.$doctor->id.'" onclick="return get_indivisual_doctor('.$doctor->id.','.$start_date.','.$end_date.','.$users_data['parent_id'].')"></td>
            ...
            else  //for individual doctor
            {
              ...
              if(!empty($commission_list))
              {
                  ...
                  $balance = $total_credit-$total_debit;
                  $html = '<tr>
                  <td style="width:20%;text-align:left;"><input type="radio" name="doctors_id" class="checklist" value="'.$post['doctor_id'].'" onclick="return get_indivisual_doctor('.$post['doctor_id'].','.$post['start_date'].','.$post['end_date'].','.$users_data['parent_id'].')"></td>
                            <td style="width:20%;text-align:left;">'.$commission_list[0]['doctor_name'].'</td>
                            <td>'.$total_credit.'</td>
                            <td>'.$total_debit.'</td>
                           </tr> ';
        ...
        {
            $html = '<tr>
                    <td colspan="4" align="center" class="text-danger"><div class="text-center">No record found.</div></td>
                    </tr>';
        }
        //print_r($commission_list);die;
...
function print_doctor_commission_letter_head()
{
 ...
 if(!empty($list))
 {
  ...
  if($type=='Download')  
  {
    ...
    public function total_doctor_comission_excel()
    {
      ...
      if(!empty($post) && !empty($post))
      {
        ...
        $balance = '0.00'; 
            ...
            if(!empty($post) && $post['doctor_id']=='-1') // for all doctor
            {
                  ...
                  // Field names in the first row
                  $fields = array('S. No.','Doctor Name','Commission','Paid Amount');
                  $objPHPExcel->getDefaultStyle()->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                  $objPHPExcel->getDefaultStyle()->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                  $col = 0;
                  $row_heading =1;
                       ...
                       foreach($doctor_list as $doctor)
                        {
                          ...
                          if(!empty($commission_list))
                          {
                              ...
                              $balance = $total_credit-$total_debit;
                               ...
                               $ttl_amt=$ttl_amt+$total_credit;
                               $ttlf_amt=$ttlf_amt+$total_debit;
                            
                            array_push($rowData,$m,$commission_list[0]['doctor_name'],$total_credit,$total_debit);
                             $m++;
                          }
                            $count = count($rowData);
                            for($j=0;$j<$count;$j++)
          ...
          if(!empty($data))
          {
               ...
               //$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
                ...
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2,$row,$ttl_amt);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3,$row,$ttlf_amt);
              ...
              //header('Content-Type: application/octet-stream');
              header('Content-Type: application/vnd.ms-excel charset=UTF-8');
              header("Content-Disposition: attachment; filename=doctor_commission_excel_".time().".xls");
              header("Pragma: no-cache"); 
              header("Expires: 0");
             if(!empty($data))
             {
            ...
            else  //for individual doctor
            {
                  ...
                  $commission_list = $this->billing->all_total_doctor_comission($post);
                 
                 //echo "<pre>"; print_r($commission_list); die;
                  $rowData = array();
                  $data= array();
                  $ttl_amt=0;
                 $ttls_amt=0;
                  if(!empty($commission_list))
                  {
                       $i=0;
                          ...
                          //$total_debit = number_format($commission_list[0]['total_debit'],2,'.', ''); 
                          //echo $total_credit; die;
                          $debit_list = $this->billing->all_total_doctor_debit($post);
                          if(!empty($debit_list))
                          {
                            $total_debit = number_format($debit_list[0]->total_debit,2,'.', '');
                          }
                          ...
                          $balance = $total_credit-$total_debit;
                               $ttl_amt=$ttl_amt+$total_credit;
                               $ttls_amt=$ttls_amt+$total_debit;
                            
                            array_push($rowData,1,$commission_list[0]['doctor_name'],$total_credit,$total_debit);
          ...
          if(!empty($data))
          {
               ...
               //$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
                ...
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1,$row,'Total');
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2,$row,$ttl_amt);
                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3,$row,$ttls_amt);
    ...
    public function total_doctor_comission_print()
    {
        $users_data = $this->session->userdata('auth_users'); 
      $post = $this->input->get();
      //echo "<pre>"; print_r($post); exit;
                       ...
                       foreach($doctor_list as $doctor)
                        {
                          ...
                          if(!empty($commission_list))
                          {
                           ...
                           $html .= '<div style="float:left;width:100%;display:inline-flex;">
        <div style="width:40%;padding-bottom:10px;padding-left:5px;">'.$commission_list[0]['doctor_name'].'</div>
        <div style="width:30%;padding-bottom:10px;">'.$total_credit.'</div>
        <div style="width:30%;padding-bottom:10px;">'.$total_debit.'</div>
        </div>';
                             $m++;
                          }
      ...
      $data['end_date'] = $post['end_date'];
        $this->load->view('billing/doctor_commission_details_print',$data);
       
    }
    
    public function total_doctor_comission_pdf()
    {
       $users_data = $this->session->userdata('auth_users'); 
      $post = $this->input->get();
      //echo "<pre>"; print_r($post); exit;
                       ...
                       foreach($doctor_list as $doctor)
                        {
                          ...
                          if(!empty($commission_list))
                          {
                           ...
                           $html .= '<tr> 
      		 	<td>'.$commission_list[0]['doctor_name'].'</td>
      		 	<td>'.$total_credit.'</td>
      		 	<td>'.$total_debit.'</td>
      		 	<tr>';
                             $m++;
      ...
      $data['start_date'] = $post['start_date'];
      $data['end_date'] = $post['end_date'];
        $this->load->view('billing/doctor_commission_details_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("doctor_commission_list_".time().".pdf");
