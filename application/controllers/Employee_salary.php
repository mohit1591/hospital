...
class Employee_salary extends CI_Controller {
	...
	public function __construct()
	{
		parent::__construct();
		auth_users();
		$this->load->model('employee_salary/employee_salary_model','employee_salary');
	...
	public function index()
	{
		...
		$this->load->view('employee_salary/list',$data);
	}
	...
	{ 
       ...
       $parent_branch_details = $this->session->userdata('parent_branches_data');
		...
		foreach ($list as $employee_salary) { 
	        ...
	        $print_url = "'".base_url('expenses/print_expense_report/'.$employee_salary->id)."'";
	...
	{  
			...
			if($this->form_validation->run() == TRUE)
			{
				...
				//check permission
					...
					if(in_array('640',$users_data['permission']['action']))
					{
						...
						if(!empty($mobile_no))
						{
							send_sms('employee_salary',8,$name,$mobile_no,array('{Name}'=>$name,'{Amt}'=>$employee_pay_now)); 
       ...
       $this->load->view('employee_salary/add',$data);		
	}
	...
	{  
	 ...
	 if(isset($id) && !empty($id) && is_numeric($id))
	  { 	 
        ...
        $this->load->view('employee_salary/view',$data);		
      }
    }  
    ...
    public function archive()
	{
		...
		$this->load->view('employee_salary/archive',$data);
	}
	...
	{ 
		unauthorise_permission('36','207');
		$users_data = $this->session->userdata('auth_users');
		$this->load->model('employee_salary/employee_salary_archive_model','employee_salary_archive');
   ...
   public function get_payment_mode_data()
    ...
    public function salary_excel()
    {
       // Starting the PHPExcel library
          ...
          $fields = array('Employee Name','Expense Date','Paid Amount','Payment Mode','Created Date');
          ...
          $list = $this->employee_salary->search_data();
          $rowData = array();
          $data= array();
          if(!empty($list))
          {
               ...
               $i=0;
               foreach($list as $employeesalary)
               {
                 array_push($rowData,$employeesalary->emp_names,date('d-m-Y',strtotime($employeesalary->expenses_date)),$employeesalary->employee_pay_now,$employeesalary->payment_mode,date('d-M-Y H:i A',strtotime($employeesalary->created_date)));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
          ...
          header("Content-Disposition: attachment; filename=salary_list_".time().".xls");
    ...
    public function salary_csv()
    {
         ...
         header("Content-Disposition: attachment; filename=salary_list_".time().".csv");
         header("Pragma: no-cache"); 
         header("Expires: 0");
         if(!empty($data))
         {
    ...
    {    
        $data['print_status']="";
        $data['data_list'] = $this->employee_salary->search_data();
        $this->load->view('employee_salary/salary_html',$data);
        ...
        $this->pdf->render();
        $this->pdf->stream("salary_list_".time().".pdf");
    }
