 ...
 public function check_result_range($booking_id="",$test_id="")
  ...
  $print_name_status = $profile_master_status->print_status;
    ...
    {  
       ...
       /*$data = $html->part;
       $data .= '<script type="text/javascript" src="'.ROOT_JS_PATH.'jquery.min.js"></script>';
       $data .= ' <script type="text/javascript">
          var html_height = $( window ).height();
          $.ajax({
                   url: "'.base_url("test/write_height/").'"+html_height,  
        ...
        $myfile = fopen(DIRECTORY_PATH."views/test/height_script.php", "w") or die("Unable to open file!"); 
        fwrite($myfile, $data);  
        fclose($myfile);*/
       $this->load->view('test/report_html_height',$data); 
       // echo $_COOKIE['myJavascriptVar']; 
        /*$url = base_url('test/load_height_script');
        $ch = curl_init();
