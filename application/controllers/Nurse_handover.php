<?php
class Nurse_handover extends CI_Controller
{
    protected $gender;
    public function __construct()
    {
        ...
        $this->load->model('nurse_handover/nurse_handover_model','nurse_handover');
    ...
    {
        $data['page_title'] = 'Nurse Handover List'; 
        $this->load->view('nurse_handover/list',$data);
    }
    ...
    { 
       ...
       $parent_branch_details = $this->session->userdata('parent_branches_data');
            ...
            $list = $this->nurse_handover->get_datatables();
         ...
         // print_r($religion);die;
            ...
            if($users_data['parent_id']==$mc->branch_id){
                 ...
                 if(in_array('52',$users_data['permission']['action'])) 
                 {
                    ...
                    $btnedit = ' <a href="'.base_url('nurse_handover/edit/').$mc->id.'" class="btn-custom" href="javascript:void(0)" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>';
                 }
                 ...
                 $print_medication_chart_print = "'".base_url('nurse_handover/print_details/'.$mc->id)."'";
            
                $btn_medication_chart_print = '<a class="btn-custom" href="javascript:void(0)" onClick="return print_window_page('.$print_medication_chart_print.')"  title="Print Nurse Handover" ><i class="fa fa-bar-chart"></i> Print</a>';
            }
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->nurse_handover->count_all(),
                        "recordsFiltered" => $this->nurse_handover->count_filtered(),
                        "data" => $data,
                );
    ...
    public function add($booking_id="")
    {
        ...
        $data['page_title'] = "Add Nurse Handover";
        ...
        {   
          ...
          redirect(base_url('nurse_handover/?status=print'));
        }   
        $this->load->model('ipd_booking/ipd_booking_model');
        $this->load->model('patient/patient_model');
        if(!empty($booking_id)) {
        ...
        $data['templates_morning'] = $this->db->get('hms_ipd_nurse_handover_template')->result_array();
        $data['templates_night'] = $this->db->get('hms_ipd_nurse_handover_template')->result_array();
        ...
        $this->load->view('nurse_handover/add',$data);
    }
    ...
    public function edit($id="")
    {
        ...
        $data['form_error'] = []; 
        $data['all_details'] = $this->nurse_handover->get_by_id($id);
        $this->load->model('ipd_booking/ipd_booking_model');
        $this->load->model('patient/patient_model');
    ...
    public function delete($id="")
    {
        ...
        $this->nurse_handover->delete($id);
        $this->session->set_flashdata('success','Nurse Handover successfully deleted.');
    ...
    public function print_details($id="")
    {
        ...
        foreach($ids as $key => $i){
            $all_details[] = $this->nurse_handover->get_by_id($i);
            $ipd_booking_data = $this->ipd_booking_model->get_by_id($this->nurse_handover->get_by_id($i)['ipd_id']);
            $d = $this->nurse_handover->get_by_id($i);
            $data['diagnosis_name'][] = $this->db->where('id',$d['diagnosis_id'])->get('hms_opd_diagnosis')->row_array()['diagnosis'];
        }
        ...
        $this->load->view('nurse_handover/print_details',$data);
    }
    ...
    $post = $this->input->post();  
        ...
        if(!empty($post))
        {
            $result = $this->nurse_handover->deleteall($post['row_id']);
            $response = "Nurse Handovers successfully deleted.";
            echo $response;
        }
    }
