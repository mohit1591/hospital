...
class Advice extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('eye/advice/advice_model','advice');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('235','1335');
        $data['page_title'] = 'Advice List'; 
        $this->load->view('eye/advice/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $advice) {
         // print_r($relation);die;
          ...
          $btnedit='';
          $btndelete='';
          if(in_array('1337',$users_data['permission']['action'])){
          ...
          if(in_array('1338',$users_data['permission']['action'])){
    ...
    public function add()
    {
        unauthorise_permission('235','1336');
       ...
       $this->load->view('eye/advice/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('235','1337');
   ...
   public function medicine_advice($str){
    ...
    {
       unauthorise_permission('235','1338');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('eye/advice/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('235','1339');
        $data['page_title'] = 'Advice archive list';
        $this->load->helper('url');
        $this->load->view('eye/advice/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('235','1339');
        $this->load->model('eye/advice/advice_archive_model','advice_archive'); 
        ...
        foreach ($list as $advice) { 
          ...
          $btnrestore='';
          $btndelete='';
          if(in_array('1341',$users_data['permission']['action'])){
          ...
          if(in_array('1340',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('235','1341');
        $this->load->model('eye/advice/advice_archive_model','advice_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('235','1340');
        $this->load->model('eye/advice/advice_archive_model','advice_archive');
       if(!empty($id) && $id>0)
       {
