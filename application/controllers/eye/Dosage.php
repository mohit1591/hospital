...
class Dosage extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('eye/dosage/dosage_model','dosage');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('232','1314');
        $data['page_title'] = 'Dosage List'; 
        $this->load->view('eye/dosage/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $dosage) {
         // print_r($dosage);die;
            ...
            if($users_data['parent_id']==$dosage->branch_id)
            {
              if(in_array('1316',$users_data['permission']['action'])){
              ...
              if(in_array('1317',$users_data['permission']['action'])){
    ...
    public function add()
    {
      //echo json_encode("hi");die;
        unauthorise_permission('232','1315');
       ...
       $this->load->view('eye/dosage/add',$data);       
    }
    ...
    {
       unauthorise_permission('232','1317');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('232','1318');
        $data['page_title'] = 'Dosage Archive List';
        $this->load->helper('url');
        $this->load->view('eye/dosage/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('232','1318');
        $this->load->model('eye/dosage/dosage_archive_model','dosage_archive'); 
       ...
       $data = array();
        ...
        foreach ($list as $dosage) {
         // print_r($dosage);die;
            ...
            $btnrestore='';
            $btndelete='';
            if(in_array('1320',$users_data['permission']['action'])){
              ...
              if(in_array('1319',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('232','1320');
        $this->load->model('eye/dosage/dosage_archive_model','dosage_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('232','1319');
        $this->load->model('eye/dosage/dosage_archive_model','dosage_archive');
       if(!empty($id) && $id>0)
       {
