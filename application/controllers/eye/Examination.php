...
class Examination extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('eye/examination/examination_model','examination');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('226','1272');
        $data['page_title'] = 'Examination List'; 
        $this->load->view('eye/examination/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $examination) {
         // print_r($examination);die;
            ...
            if($users_data['parent_id']==$examination->branch_id)
            {
              if(in_array('1274',$users_data['permission']['action'])){
              ...
              if(in_array('1275',$users_data['permission']['action'])){
    ...
    public function add()
    {
      //echo json_encode("hi");die;
        unauthorise_permission('226','1273');
       ...
       $this->load->view('eye/examination/add',$data);       
    }

    public function edit($id="")
    {
     unauthorise_permission('226','1274');
    ...
    {
       unauthorise_permission('226','1275');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('eye/examination/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('226','1276');
        $data['page_title'] = 'Examination Archive List';
        $this->load->helper('url');
        $this->load->view('eye/examination/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('226','1276');
        $this->load->model('eye/examination/examination_archive_model','examination_archive'); 
        ...
        foreach ($list as $examination) {
         // print_r($examination);die;
            ...
            $btnrestore='';
            $btndelete='';
            if(in_array('1278',$users_data['permission']['action'])){
              ...
              if(in_array('1277',$users_data['permission']['action'])){
    ...
    {
        unauthorise_permission('226','1278');
        $this->load->model('eye/examination/examination_archive_model','examination_archive');
       if(!empty($id) && $id>0)
       {
    ...
    {
        unauthorise_permission('226','1277');
        $this->load->model('eye/examination/examination_archive_model','examination_archive');
       if(!empty($id) && $id>0)
       {
