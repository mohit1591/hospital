...
class Blank_prescription_setting extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        //auth_users();
        //unauthorise_permission('245','1408');
        $this->load->model('eye/prescription_setting/blank_prescription_setting_model','prescriptionsetting');
...
//print '<pre>'; print_r($users_data);die;
        unauthorise_permission(245,1408);
        $data['page_title'] = 'Eye Prescription Settings'; 
        $post = $this->input->post();
        if(!empty($post))
        { 
            $this->prescriptionsetting->save();
            echo 'Your Eye Prescription Settings successfully updated.';;
            return false;
        }

       
        $data['prescription_setting_list'] = $this->prescriptionsetting->get_master_unique(1);
        $this->load->view('eye/blank_prescription_setting/add',$data);
    }
    ...
    public function preview($id="",$type='1')
    {
        ...
        $template_format = $this->prescription->template_format(array('setting_name'=>'PRESCRIPTION_PRINT_SETTING','unique_id'=>5,'type'=>1));

        $template_format_left = $this->prescription->template_format(array('setting_name'=>'LEFTMARGIN','unique_id'=>1,'type'=>1));
        $template_format_right = $this->prescription->template_format(array('setting_name'=>'RIGHTMARGINS','unique_id'=>2,'type'=>1));
        $template_format_top = $this->prescription->template_format(array('setting_name'=>'TOPMARGIN','unique_id'=>3,'type'=>1));
        $template_format_bottom = $this->prescription->template_format(array('setting_name'=>'TOPMARGIN','unique_id'=>4,'type'=>1));
        ...
        $this->load->view('eye/blank_prescription_setting/preview',$data);
    } 
