...
class Bloodbank_collection_report extends CI_Controller {
    ...
    {  
     unauthorise_permission('268','1560');
        ...
        $data['page_title'] = 'Blood Bank Collection Report list';
    ...
    public function bloodbank_report_excel()
    {
        ...
        unauthorise_permission('268','1568');
         ...
         // print_r($objWorksheet);die;
          ...
          $fields = array('Issue Code','Requirement Date' ,'Patient Name','Net Amount','Discount','Paid Amount','Balance');
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
                ...
                $grand_total_amount = $grand_total_amount+$reports->total_amount;
                  ...
                  array_push($rowData,$reports->issue_code,$requirement_date,$reports->patient_name,number_format($reports->total_amount,2),$discount,number_format($reports->paid_amount,2),number_format($reports->balance,2));
                    $count = count($rowData);
          ...
          if(!empty($data))
          {
               foreach($data as $reports_data)
               {
                    ...
                    { 
                          ...
                          // added on 31-jan-2018
                          $objWorksheet->setCellValueByColumnAndRow(3,$row+1,$grand_total_amount);
                          $objWorksheet->setCellValueByColumnAndRow(4,$row+1,$grand_total_discount);
                          $objWorksheet->setCellValueByColumnAndRow(5,$row+1,$grand_paid_amount);
                          $objWorksheet->setCellValueByColumnAndRow(6,$row+1,$grand_balance_amount);
               ...
               foreach($list as $reports)
               {
				...
				else
				{
                  ...
                  $grand_balance_amount = $grand_balance_amount + $reports->balance;
                    ...
                    array_push($rowData,$reports->issue_code,$requirement_date,$reports->patient_name,number_format($reports->total_amount,2),number_format($reports->discount,2),number_format($reports->paid_amount,2),number_format($reports->balance,2));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
