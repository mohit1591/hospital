...
class Vitals extends CI_Controller {
    ...
    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('blood_bank/vitals/vitals_model','vitals_field');
        $this->load->library('form_validation');
    }

    public function index()
    { 
        unauthorise_permission('296','1757');
        $data['page_title'] = 'Vitals'; 
        $this->load->view('blood_bank/vitals/list',$data);
    }
    ...
    { 
        ...
        foreach ($list as $vital_list) {
         // print_r($simulation);die;
            ...
            if($users_data['parent_id']==$vital_list->branch_id)
            {
             if(in_array('1755',$users_data['permission']['action'])){
            ...
            if(in_array('1754',$users_data['permission']['action'])){
    ...
    public function add()
    {
       ...
       unauthorise_permission('296','1756');
       ...
       $this->load->view('blood_bank/vitals/add',$data);       
    }

    public function edit($id="")
    {
     unauthorise_permission('296','1755');
    ...
    {
        unauthorise_permission('296','1754');
       if(!empty($id) && $id>0)
       {
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        ...
        $this->load->view('blood_bank/vitals/view',$data);     
      }
    }  
    ...
    {
        unauthorise_permission('296','1753');
        $data['page_title'] = 'Vitals Archive List';
        $this->load->helper('url');
        $this->load->view('blood_bank/vitals/archive',$data);
    }

    public function archive_ajax_list()
    {
        unauthorise_permission('296','1753');
        $this->load->model('blood_bank/vitals/vitals_archive_model','vitals_field_archive'); 
      ...
      $list = $this->vitals_field_archive->get_datatables();
        ...
        foreach ($list as $vitals_list) {
         // print_r($simulation);die;
            ...
            $btnrestore='';
            $btndelete='';
           if(in_array('1750',$users_data['permission']['action'])){
             ...
             if(in_array('1752',$users_data['permission']['action'])){
    ...
    {
       unauthorise_permission('296','1750');
        $this->load->model('blood_bank/vitals/vitals_archive_model','vitals_field_archive');
       if(!empty($id) && $id>0)
       {
           $result = $this->vitals_field_archive->restore($id);
           $response = "Vitals successfully restore in vitals list.";
           echo $response;
       }
    }
    ...
    {
       unauthorise_permission('296','1752');
        $this->load->model('blood_bank/vitals/vitals_archive_model','vitals_field_archive');
       if(!empty($id) && $id>0)
       {
