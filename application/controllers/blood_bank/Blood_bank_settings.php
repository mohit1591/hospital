...
class Blood_bank_settings extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $this->load->model('blood_bank/blood_bank_settings/blood_bank_settings_model','blood_bank_settings');
    ...
    { 
        ...
        unauthorise_permission('267','1551');

        $data['page_title'] = 'Blood Bank setting list'; 
        $this->load->view('blood_bank/blood_bank_settings/list',$data);
    }

    public function ajax_list()
    {   
         unauthorise_permission('267','1551');
        $list = $this->blood_bank_settings->get_datatables();  
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        ...
        foreach ($list as $blood_bank_settings) 
        {
          ...
          $row[] = $blood_bank_settings->setting_value1;  
           $row[] = $blood_bank_settings->setting_value2;  

          $users_data = $this->session->userdata('auth_users');
          $btnedit='';
          $btndelete='';
          $btnrestore='';
         if(in_array('1553',$users_data['permission']['action']))
          {
          $btnedit = ' <a onClick="return edit_blood_bank_settings('.$blood_bank_settings->id.');" class="btn-custom" href="javascript:void(0)" style="'.$blood_bank_settings->id.'" title="Edit"><i class="fa fa-pencil" aria-hidden="true"></i> Edit</a>';
          }
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->blood_bank_settings->count_all(),
                        "recordsFiltered" => $this->blood_bank_settings->count_filtered(),
                        "data" => $data,
                );
    ...
    public function add()
    {
        ...
        unauthorise_permission('267','1552');
        $data['page_title'] = "Add Blood Bank Setting";  
       ...
       $this->load->view('blood_bank/blood_bank_settings/add',$data);       
    }
    
    public function edit($id="")
    {
      unauthorise_permission('267','1553');
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        $result = $this->blood_bank_settings->get_by_id($id);  
        $data['page_title'] = "Update Blood Bank Setting";  
        $post = $this->input->post();
        $data['form_error'] = ''; 
        $data['form_data'] = array(
                                  'data_id'=>$result['id'],
    ...
    public function delete($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           $result = $this->blood_bank_settings->delete($id);
           $response = "Blood Bank setting successfully deleted.";
           echo $response;
       }
    }
    ...
    public function restore_setting($id="")
    {
      unauthorise_permission('267','1558');
       if(!empty($id) && $id>0)
       {
           $result = $this->blood_bank_settings->restore_setting($id);
           $response = "Blood Bank setting restore successfully.";
           echo $response;
       }
    }
    ...
    function deleteall()
    {
      unauthorise_permission('267','1554');
        ...
        if(!empty($post))
        {
            $result = $this->blood_bank_settings->deleteall($post['row_id']);
            $response = "Blood Bank setting successfully deleted.";
            echo $response;
        }
    }
    ...
    {  
     if(isset($id) && !empty($id) && is_numeric($id))
      {      
        $data['form_data'] = $this->blood_bank_settings->get_by_id($id);  
    ...
    public function archive()
    {
        ...
        $data['page_title'] = 'Blood bank setting archive list';
