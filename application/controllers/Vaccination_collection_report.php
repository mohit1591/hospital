...
class Vaccination_collection_report extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('reports/vaccination_collection_report_model','vaccination_collection_reports');
        $this->load->library('form_validation');
    }

    public function index()
    {   unauthorise_permission('220','1247');
        $this->session->unset_userdata('vaccination_collection_resport_search_data');
        $data['sub_branch'] = $this->session->userdata('sub_branches_data'); 
        $search_date = $this->session->userdata('vaccination_collection_resport_search_data');
// Default Search Setting
        ...
        $data['page_title'] = 'Vaccination Collection Report';
        $this->load->model('general/general_model','general_model');
        $data['employee_list'] = $this->general_model->branch_user_list();
        $this->load->view('vaccination_collection/list',$data);
    }
    public function expenses(){
       $data['page_title'] = 'Vaccination Collection Reports';
       $this->load->view('vaccination_collection/expense_report',$data);
    }
    public function ajax_list()
    {  
        unauthorise_permission('220','1247');
        $users_data = $this->session->userdata('auth_users'); 
        $list = $this->vaccination_collection_reports->get_datatables();
        //print_r($list);die;
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        ...
        foreach($list as $reports) 
        {
          //print_r($reports);
            ...
            $row[] = $reports->sale_no;  
            $row[] = date('d-m-Y',strtotime($reports->sale_date));  
            ...
            if($reports->blnce>0 && $reports->parent_id>0){
               $print_url = "'".base_url('sales_vaccination/print_sales_report/'.$reports->id)."'";
            ...
            $btn_edit = ' <a class="btn-custom" href="'.base_url("sales_vaccination/edit/".$reports->id).'" title="Edit Booking"><i class="fa fa-pencil"></i> Edit</a>';
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->vaccination_collection_reports->count_all(),
                        "recordsFiltered" => $this->vaccination_collection_reports->count_filtered(),
                        "data" => $data,
                );
    ...
    public function search_data()
    {
       ...
       if(!empty($post))
       {
         ...
         $this->session->set_userdata('vaccination_collection_resport_search_data',$search_data);
       }
    }
    ...
    public function vaccination_report_excel()
    {
        ...
        unauthorise_permission('220','1247');
         ...
         // print_r($objWorksheet);die;
          ...
          $fields = array('Sale No','Sale Date','Patient Name','Referred By','Net Amount','Discount','Paid Amount','Balance');
          ...
          $get_date_time_setting = $this->general_model->get_date_time_setting('vaccination_collection_report');
          $date_time_status = $get_date_time_setting->date_time_status;
          $list =  $this->vaccination_collection_reports->search_report_data();
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
				...
				{
					$sale_date = date('d-m-Y',strtotime($reports->sale_date)).' '.date('h:i A',strtotime($reports->c_date));
				} 
				else
				{
                  ...
                  array_push($rowData,$reports->sale_no,$sale_date,$reports->patient_name,$reports->doctor_hospital_name,number_format($reports->total_amount,2),$discount,number_format($reports->paid_amount,2),number_format($reports->balance,2));
                    $count = count($rowData);
          ...
          header("Content-Disposition: attachment; filename=vaccination_collection_report_".time().".xls");  
     ...
     public function vaccination_report_csv()
    {
         ...
         // print_r($objWorksheet);die;
          ...
          if(!empty($list))
          {
  			...
  			if($date_time_status==1)
				...
				else
				{
                    ...
                    array_push($rowData,$reports->sale_no,$sale_date,$reports->patient_name,$reports->doctor_hospital_name,number_format($reports->total_amount,2),number_format($reports->discount,2),number_format($reports->paid_amount,2),number_format($reports->balance,2));
                    $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
          ...
          // Sending headers to force the user to download the file
          header('Content-Type: application/octet-stream charset=UTF-8');
          header("Content-Disposition: attachment; filename=vaccination_collection_report_".time().".csv");
          header("Pragma: no-cache"); 
          header("Expires: 0");
          if(!empty($data))
          {
    ...
    public function advance_search()
    {
      $data['page_title'] = "Advance Search";
        ...
        $data['search_data'] = $this->session->userdata('vaccination_collection_resport_search_data');
        ...
        else
        {
          ...
          $this->session->set_userdata('vaccination_collection_resport_search_data', $marge_post);
          //$this->session->set_userdata('medicine_collection_resport_search_data',$post); 
          }
       // print_r($data);die;
        $this->load->view('vaccination_collection/advance_search',$data);
    }

    public function pdf_vaccination_report()
    {   
        unauthorise_permission('220','1247');
        $data['print_status']="";
        $this->load->model('general/general_model');
        $data['data_list'] = $this->vaccination_collection_reports->search_report_data();
       $get_date_time_setting = $this->general_model->get_date_time_setting('vaccination_collection_report');
      $data['date_time_status'] = $get_date_time_setting->date_time_status;
        $this->load->view('vaccination_collection/vaccination_report_html',$data);
     
         // print_r( $data['date_time_status']);die;
        ...
        $this->pdf->render();
        $this->pdf->stream("vaccination_collection_report_".time().".pdf");
    }

    public function print_vaccination_report()
    {   
        unauthorise_permission('220','1247');
        $data['print_status']="1";
        $this->load->model('general/general_model');
    ...
    public function print_vaccination_collection_reports()
    { 
     unauthorise_permission('89','563');
     $get = $this->input->get();
     $data['vaccination_collection_list'] = [];
     if(!empty($get['branch_id']))
     {
        $data['vaccination_collection_list'] = $this->vaccination_collection_reports->get_opd_collection_list_details($get);
     } 
     $this->load->view('vaccination_collection/list_vaccination_collection',$data);  
