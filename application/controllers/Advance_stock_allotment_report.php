...
class Advance_stock_allotment_report extends CI_Controller {
 
	function __construct() 
	{
  	parent::__construct();	
  	 auth_users();  
  	$this->load->model('advance_stock_allotment_report/advance_stock_allotment_model','stock_purchase');
  	...
  	public function index()
      {
        ...
        $this->load->view('advance_stock_allotment_report/collection_list',$data);
      }
     ...
     public function advance_search()
      {
          ...
          if(isset($post) && !empty($post))
          {
              ...
              $this->session->set_userdata('stock_purchase_item_list', $marge_post);
          }
          $stock_purchase_search = $this->session->userdata('stock_purchase_item_list');
    ...
    {  
        ...
        {
            $get_items = $this->stock_purchase->get_items_allot($item_stock->parent_id,$item_stock->created_date);
            ...
            $print_url_eye = "'".base_url('advance_stock_allotment_report/print_detail/'.$item_stock->parent_id.'/'.date('Y-m-d',strtotime($item_stock->created_date))).'/'.$item_stock->branch_name."'";  
                //$btn_prescription .= '<div class="btn-ipd">';
              $btn_prescription = '<a  href="javascript:void(0)" onClick="return print_window_page('.$print_url_eye.')" title="Print" ><i class="fa fa-print"></i
                > Print  </a>';
                
            $row[] =$btn_prescription;
			$data[] = $row;
        ...
        foreach ($list as $item_stock) {
            $arr[] = date('d-m-Y',strtotime($item_stock->created_date));
        }
        $unique_data = array_unique($arr);
        //echo "<pre>"; print_r($unique_data); exit;
        $data = array();
        $no = $_POST['start'];
        $i = 1;
	    $total_num = count($unique_data);
        
        foreach ($unique_data as $unique_stock) 
        { 
            $no++;
            $row = array();
            $row[] = $i;//$item_stock->branch_name;
            $items_list='';
            foreach($list as $item_stock)
            {
                
                $date_done = date('d-m-Y',strtotime($item_stock->created_date));
                if(strtotime($unique_stock) == strtotime($date_done)) 
                {
                  
                    $items_list .= $item_stock->item.' - '.$item_stock->item_code.' - '.$item_stock->quantity.'<br>';
    ...
    public function excel()
    {
        // Starting the PHPExcel library
         ...
         $num_rows = $objPHPExcel->getActiveSheet()->getHighestRow();
		  ...
		  $fields = array('Branch Name','Item Name','Date');
         ...
         $list = $this->stock_purchase->search_report_data();
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $item_stock)
               {
                   ...
                   array_push($rowData,$item_stock->branch_name,$get_items,date('d-M-Y',strtotime($item_stock->created_date)));
                   $count = count($rowData);
                    for($j=0;$j<$count;$j++)
                    {
    ...
    {    
        ...
        $this->load->view('advance_stock_allotment_report/stock_report_html',$data);
...
//echo $perpic_rate;
  ...
  public function print_detail($parent_id,$created_date,$branch_name)
  {
    //echo $created_date; die;
      $user_detail= $this->session->userdata('auth_users');
      
      $get_items = $this->stock_purchase->get_items_print_allot($parent_id,$created_date);
      
      $template_format= $this->stock_purchase->template_format(array('section_id'=>9,'types'=>1,'sub_section'=>1,'branch_id'=>$user_detail['parent_id']));
      $data['template_data']=$template_format;
      $data['item_details'] = $get_items;
      $data['branch_name'] = str_replace("%20"," ",$branch_name);
      $this->load->view('advance_stock_allotment_report/print_template_issue',$data);
  }
