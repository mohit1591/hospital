...
class Medicinequantityreport extends CI_Controller {

		public function __construct()
		{
				parent::__construct();
				auth_users();
				$this->load->model('medicinequantityreport/medicinequantityreport_model','medicinequantityreport');
		...
		{   
				...
				$this->load->view('medicinequantityreport/list',$data);
		}
		public function expenses()
		{
		 ...
		 public function medicine_quantity_report(){
             ...
             $get= $this->input->get();
			 $data['page_title'] = 'Medicine Wise Reports List';
		...
		public function report_excel()
		{
				...
				$fields = array('Sr.No.','Unit','Medicine Name','Batch No','MRP','Expiry Date','Stock','Sell','Purchase');
						...
						foreach($list as $medicines)
						{
								 $med_name = trim($medicines['medicine_name']);
								   if($get['module_type']==1)
									...
									$pur_qty = $medicines['purchsase_quantity']['total_qty'];
									$pur_amt =  $medicines['purchsase_quantity']['total_amt'];

									$sale_ret = $medicines['sale_return_quantity']['total_qty'];

									$sale_ret_amt = $medicines['sale_return_quantity']['total_amt'];

								array_push(
									$rowData,
									$k,
									$medicines['medicine_unit'],
									$med_name,
									$medicines['purchsase_quantity']['batch_no'],
									$medicines['purchsase_quantity']['mrp'],
									($medicines['purchsase_quantity']['expiry_date']),
									abs($medicines['purchsase_quantity']['total_qty']),
									abs($medicines['purchsase_quantity']['total_qty_sell']),
									abs($medicines['purchsase_quantity']['total_qty_purchase'])
								);
								$count = count($rowData);
								for($j=0;$j<$count;$j++)
								{
				...
				header("Content-Disposition: attachment; filename=Medicine_Wise_Report".time().".xls");
		...
		{ 
		    ...
		    $data['get'] = $get;
			$this->load->view('medicinequantityreport/list_collection_report_data',$data);  
		}
