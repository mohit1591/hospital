...
class Refund_payment extends CI_Controller {
  ...
  function __construct() 
    ...
    parent::__construct();  
    auth_users();  
    $this->load->model('refund_payment/Refund_payment_type_model','refund_payment');
  ...
  { 
      ...
      $this->load->view('refund_payment/list',$data);     
  }
 ...
 { 
           ...
           // $row[] = $status;
            ...
            if($users_data['parent_id']==$details->branch_id)
            {
              if(in_array('1119',$users_data['permission']['action'])){
               $btnrefund =' <a class="btn-custom" href="'.base_url('refund_payment/pay_now/'.$details->id.'/'.$details->parent_id.'/'.$details->section_id.'/'.$details->branch_id).'" style="'.$details->id.'">Refund</a>';
              }
              if(in_array('1118',$users_data['permission']['action'])){
               $btnedit =' <a class="btn-custom" href="'.base_url('refund_payment/view_refund_details/'.$details->id.'/'.$details->parent_id.'/'.$details->section_id.'/'.$details->branch_id).'" style="'.$details->id.'">View</a>';
              }
        ...
        foreach ($list as $details) 
        {
           ...
           if($details['section_id']==1)
            ...
            elseif ($details['section_id']==13){
             ...
             elseif ($details['section_id']==7){
                   $status = 'Vaccination';
            }
            elseif ($details['section_id']==14){
                   $status = 'Day Care';
            }
            elseif ($details['section_id']==15){
                   $status = 'Dialysis';
            }
            else
            {
           ...
           // $row[] = date('d-M-Y H:i A',strtotime($details['created_date'])); 
            ...
            $print_url = "'".base_url('expenses/print_expense_report/'.$details['expense_id'])."'";
  ...
  public function refund_list()
  {
        ...
        $this->load->view('refund_payment/refund_list',$data);
  }
        ...
        if(isset($post) && !empty($post) && !empty($post['refund_amount']))
        {
             ...
             //print_r($insert_array);die;
               ...
               //echo $this->session->flashdata('success');die;
                redirect('refund_payment/refund_list');
                }
  ...
  public function pay_now($pid="", $parent_id="",$section_id="",$branch_id="")
  {
         ...
         // unauthorise_permission(190,1118);
          ...
          $post = $this->input->post(); 
          $data['dialysis_booking_data']= $this->dialysisbooking->getdialysisbyid($parent_id);
         ...
         //print_r($data['pathology_booking_data']);die;
          ...
          $data['daycare_booking_data']= $this->daycare->get_by_id($parent_id);
          
          $data['ipd_booking_data']= $this->ipdbooking->get_ipd_by_id($parent_id);
        //  print_r($data['medicine_booking_data']);die;
          $balance = '';
        ...
        $data['billing_data']=$this->opdbilling->get_by_id($parent_id);
          $this->load->view('refund_payment/pay_patient_to_branch',$data);
   ...
   public function get_drop_down_value()
   {
      ...
      echo $this->load->view('refund_payment/drop_down_data',$data,true);
   }
...
public function view_refund_details($pid="", $parent_id="",$section_id="",$branch_id="")
    ...
    //print_r($data['refund_data']);die;
          $this->load->view('refund_payment/view_refund_details',$data);
