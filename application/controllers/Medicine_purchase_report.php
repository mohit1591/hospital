...
class Medicine_purchase_report extends CI_Controller {

    public function __construct()
    {
        parent::__construct();
        auth_users();
        $this->load->model('reports/medicine_purchase_collection_report_model','medicine_collection_reports');
    ...
    {  
        ...
        $list = $this->medicine_collection_reports->get_datatables();
        $data = array();
        $no = $_POST['start'];
        $i = 1;
        ...
        foreach($list as $reports) 
        {
          //print_r($reports);
            ...
            $row[] = $reports->name;  
            $row[] = $reports->paid_amount+$reports->balance;
            $grand_total_amount = $grand_total_amount+($reports->paid_amount+$reports->balance);            
            ...
            if($reports->total_amount !='0.00' && $reports->parent_id>0){
               $print_url = "'".base_url('purchase/print_purchase_recipt/'.$reports->id)."'";
               ...
               $print_url = "'".base_url('medicine_purchase_report/print_vendor_balance_receipt/'.$reports->id.'/'.$reports->id.'/'.$reports->paid_amount)."'";
               $btn_print = '<a class="btn-custom" href="javascript:void(0)" onClick="return print_window_page('.$print_url.')" title="Print" ><i class="fa fa-print"></i> Print</a>';
             }
           ...
           if($i==$total_num)
           {
              ...
              $tot_row[] = '<input type="text" class="w-120px" style="text-align:right;" value='.number_format($grand_paid_amount,2).' readonly >'; 
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->medicine_collection_reports->count_all(),
                        "recordsFiltered" => $this->medicine_collection_reports->count_filtered(),
                        "data" => $data,
                );
    ...
    public function medicine_report_excel()
    {
        ...
        unauthorise_permission('159','931');
          ...
          $fields = array('Purchase No','Invoice No','Vendor Name','Net Amount','Paid Amount','Balance','Discount','Purchase Date');
    ...
    /*  $get_date_time_setting = $this->general_model->get_date_time_setting('medicine_collection_report');
      $date_time_status = $get_date_time_setting->date_time_status;*/
          $list =  $this->medicine_collection_reports->search_report_data();
          ...
          if(!empty($list))
          {
               ...
               foreach($list as $reports)
               {
                  ...
                  array_push($rowData,$reports->purchase_id,$reports->invoice_id,$reports->name,number_format($reports->total_amount,2),number_format($reports->paid_amount,2),number_format($reports->balance,2),$discount,$sale_date);
                    $count = count($rowData);
          ...
          if(!empty($data))
          {
               foreach($data as $reports_data)
               {
                    ...
                    { 
                          ...
                          $objWorksheet->setCellValueByColumnAndRow(4,$row+1,number_format($grand_paid_amount,2));
                          $objWorksheet->setCellValueByColumnAndRow(5,$row+1,number_format($grand_balance_amount,2));
                          $objWorksheet->setCellValueByColumnAndRow(6,$row+1,number_format($grand_total_discount,2));
    ...
    public function advance_search()
    {
      $data['page_title'] = "Advance Search";
        ...
        $this->load->view('medicine_purchase_report/advance_search',$data);
    }

    public function pdf_medicine_report()
    {   
        unauthorise_permission('159','931');
        $data['print_status']="";
        $this->load->model('general/general_model');
        ...
        $data['data_list'] = $this->medicine_collection_reports->search_report_data();
        $this->load->view('medicine_purchase_report/medicine_report_html',$data);
    ...
    public function print_medicine_report()
    {   
        unauthorise_permission('159','931');
        $data['print_status']="1";
        $this->load->model('general/general_model');
    ...
    unauthorise_permission('159','931');
     ...
     {
        $data['medicine_collection_list'] = $this->medicine_collection_reports->get_opd_collection_list_details($get);
     } 
     $this->load->view('medicine_purchase_report/list_medicine_collection',$data);  
  ...
  public function print_vendor_balance_receipt($ids="", $purchase_id="",$paid='')
  {
      ...
      $get_detail_by_id= $this->medicine_collection_reports->get_by_id($purchase_id, $ids);
     
      $get_by_id_data=$this->medicine_collection_reports->get_all_detail_print($purchase_id,$get_detail_by_id['branch_id']);
      ...
      $this->load->view('medicine_purchase_report/print_template_medicine',$data);
