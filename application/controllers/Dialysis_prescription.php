...
class Dialysis_prescription extends CI_Controller {
	...
	function __construct() 
	{
        ...
        $this->load->model('dialysis_prescription/dialysis_prescription_file_model','prescription_file');
        $this->load->model('dialysis_prescription/dialysis_prescription_video_model','prescription_video');
  ...
  public function index($dialysis_id='',$patient_id='')
  {
        ...
        $data['page_title'] = 'Dialysis Prescription List';
      ...
      $this->load->view('dialysis_prescription/list',$data);
  }
    ...
    {   
        ...
        foreach ($list as $prescription) { 
            ...
            $row[] = date('d-M-Y',strtotime($prescription->prescription_date)); 
            ...
            if($users_data['parent_id']==$prescription->branch_id)
            {
                    ...
                    //{
                    $btn_edit = ' <a class="btn-custom" href="'.base_url("dialysis_prescription/edit/".$prescription->id).'" title="Edit Prescription"><i class="fa fa-pencil"></i> Edit Prescription</a>';
                    //}
               ...
               // {
                   $print_url = "'".base_url('dialysis_prescription/print_prescriptions/'.$prescription->id.'/'.$prescription->branch_id)."'";
                  $btn_print_pre = ' <a class="btn-custom" onClick="return print_window_page('.$print_url.')" href="javascript:void(0)" title="Print Prescription"  data-url="512"><i class="fa fa-print"></i> Print Prescription</a>';
                   ...
                   /* $print_url_letter = "'".base_url('dialysis_prescription/download_prescription/'.$prescription->booking_id.'/'.$prescription->branch_id)."'";
        ...
        $btn_view_video='';
            ...
            $row[] =   $btn_view_pre.$btn_print_pre.$btn_upload_pre.$btn_view_upload_pre.$btn_edit.$btn_view.$btn_delete.$video_link.$btn_view_video;  //$btn_print_pr_letter               
    ...
    public function advance_search()
    {
        ...
        $this->load->view('dialysis_prescription/advance_search',$data);
    }
...
{ 
    ...
    { 
      ...
      {      
        ...
        if($prescription_data->prescription_date!='0000-00-00')
        {
            $prescription_date = date('d-m-Y',strtotime($prescription_data->prescription_date));
        }
        else
        {
            $prescription_date='';
