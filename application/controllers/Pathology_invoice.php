...
class Pathology_invoice extends CI_Controller {
    public function __construct()
    {
        parent::__construct();
        ob_start();
        //auth_users();
        $this->load->model('pathology_invoice/pathology_invoice_model','test');
    ...
    {   
        unauthorise_permission(409,2472);
        $data['branch_list'] = $this->session->userdata('invoice_sub_branches_data');
        $this->session->unset_userdata('invoice_advance_result');
        $users_data = $this->session->userdata('auth_users');
        $this->session->set_userdata('invoice_complation_status',-1);
        ...
        $data['page_title'] = 'Proforma List'; 
        $this->load->view('pathology_invoice/list',$data);
    }
    ...
    {     
         ...
         //print '<pre>'; print_r($users_data);die;
          ...
          foreach ($list as $test) {
               ...
               {
                 $report_status = '<font class="bg_completed" >Confirmed</font>';
               }
               else if($test->complation_status==0)
               {
                 $report_status = '<font class="bg_pending" >Pending</font>';
               }
 
              

               $row[] = $test->patient_code; 
               $row[] = $test->lab_invoice_no;
         ...
         if($test->complation_status==1)
         {
            ...
            $btn_delete = ' <li><a class="btn-custom" href="javascript:void(0)" disabled title="Already Confirmed"><i class="fa fa-trash"></i> Delete </a></li>';
            
            $btn_edit = '<a class="btn-custom" href="javascript:void(0)" disabled title="Already Confirmed" ><i class="fa fa-pencil"></i> Edit </a>';
         
         }
         else
         {
             $btn_confirm = '<a  href="'.base_url('test/booking/?invoice_id=').$test->id.'" title="Proforma Invoice"><i class="fa fa-plus"></i> Confirm </a>';
             if(in_array('2475',$users_data['permission']['action']))
              { 
              ...
              if(in_array('2474',$users_data['permission']['action']))
              {
       ...
       /* if(!empty($test->mobile_no))
        {
          ...
          $btn_report_print .= '<li><a target="_blanl" href="'.$whatsapp_url.'" title="Report send to whatsapp" ><i class="fa fa-whatsapp" aria-hidden="true"></i> Send Report </a></li>';
            ...
            $print_url = "'".base_url('pathology_invoice/print_test_booking_report/').$test->id."'";
            $btn_print_bill = '<li><a href="javascript:void(0)" onclick = "return print_window_page('.$print_url.');" title="Print Proforma Invoice" ><i class="fa fa-print"></i> Print Proforma Invoice </a></li>';
        ...
        // End Action Button //
          ...
          $btn_a = '<div class="slidedown">
            <button disabled class="btn-custom">More <span class="caret"></span></button>
              ...
              '.$btn_delete.$btn_confirm.$btn_report_download.$btn_print_bill.$opd_consolidated_bill.$btn_upload_pre.'
            </ul>
          </div> ';
          ...
          $output = array(
               "draw" => $_POST['draw'],
               "recordsTotal" => $recordsTotal,//$this->test->count_all(),
    ...
    public function search_date()
    {
       ...
       if(!empty($post))
       {
         ...
         $this->session->set_userdata('invoice_search_booking_date',$marge_post);
       }
    }

    public function reset_date_search()
    {
       $this->session->unset_userdata('invoice_search_booking_date');
    }
    ...
    public function advance_search()
    {
        ...
        $search_data = $this->session->userdata('invoice_search_booking_date');
        ...
        if(isset($search_data) && !empty($search_data))
        {
          ...
          $lab_invoice_no = "";
          if(isset($search_data['lab_invoice_no']))
          {
            $lab_invoice_no = $search_data['lab_invoice_no'];
          }
          $city_id = "";
        ...
        {
            $this->session->set_userdata('invoice_test_search', $post);
        }
        $test_search = $this->session->userdata('invoice_test_search');
        if(isset($test_search) && !empty($test_search))
        {
            $data['form_data'] = $test_search;
        }
        $this->load->view('pathology_invoice/advance_search',$data);
    }
    ...
    {  
        ...
        { 
          ...
          $this->session->set_userdata('invoice_crm_patient_id',$_GET['lid']);
          ...
          if($lead_data['department_id']==8)
          {
            ...
            if(!empty($lead_test_list))
            {
               ...
               if(!empty($data['booked_test_list']))
               {
                 ...
                 $this->session->set_userdata('invoice_book_test', $post_test);
               } 
            } 
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                if(in_array('640',$users_data['permission']['action']))
                {
                  ...
                  { 
                      ...
                      $this->load->model('patient/patient_model');
                    send_sms('test_booking',22,'',$post['mobile_no'],array('{patient_name}'=>$post['patient_name'],'{amount}'=>$post['net_amount'],'{booking_no}'=>$post['lab_invoice_no'], '{booking_date}'=>$post['booking_date']));  
                    $patient_data = $this->patient_model->get_by_id($pid);
                ...
                if(in_array('641',$users_data['permission']['action']))
                {
                  ...
                  { 
                    ...
                    $this->general_library->email($post['patient_email'],'','','','','','test_booking','22',array('{patient_name}'=>$post['patient_name'],'{amount}'=>$post['net_amount'],'{booking_no}'=>$post['lab_invoice_no'],'{booking_date}'=>$post['booking_date'])); 
                  }
                }
                ...
                if(!empty($receiver_device_details))
                {
                  ...
                  $sms_msg=rawurlencode('Proforma Invoice successfully done.');
                ...
                $this->session->set_userdata('invoice_test_booking_id',$booking_id);
                $this->session->set_flashdata('success','Proforma Invoice successfully generated.');
                redirect(base_url('pathology_invoice/booking/?status=print&form_f_status='.$_POST['form_f'].''));
       ...
       $this->load->view('pathology_invoice/booking',$data);       
    }
    ...
    { 
       unauthorise_permission(409,2474);
       $users_data = $this->session->userdata('auth_users'); 
      ...
      {      
        ...
        $this->session->set_userdata('invoice_booking_set_branch', $result['branch_id']);
        $data['country_list'] = $this->general_model->country_list();
        ...
        $data['page_title'] = "Proforma Invoice";  
        $booking_list = $this->session->userdata('invoice_book_test');
       ...
       // print_r($report_tests);die;
        ...
        {  
            $this->session->set_userdata('invoice_book_test',$booked_test_list);
        }
        $booking_list = $this->session->userdata('invoice_book_test');
        if(!empty($booking_list))
        { 
        ...
        if($result['profile_id']>0)
        {
          ...
          $profile_price = $profile_result['master_rate'];*/
           if(empty($post))
           { 
            $this->session->set_userdata('invoice_set_profile',$profile_arr);
            $profile_price = $profile_result['master_rate'];
           }
        }
...
$profile_arr[$booked_profile->profile_id] = array('id'=>$booked_profile->profile_id, 'name'=>$profile_name, 'order'=>$total_test+$p_i, 'price'=>$booked_profile->master_price,'base_price'=>$booked_profile->base_price,'sort_order'=>$booked_profile->sort_order);
        ...
        $data['page_title'] = "Update Proforma Invoice";  
        $post = $this->input->post();
        $data['form_error'] = ''; 
        ...
        $data['form_data'] = array(
                                    ...
                                    'lab_invoice_no'=>$result['lab_invoice_no'],
        ...
        {   
            ...
            if($this->form_validation->run() == TRUE)
            {
                ...
                $booking_id = $this->test->save();
                $this->session->set_userdata('invoice_test_booking_id',$booking_id);
                $this->session->set_flashdata('success','Proforma Invoice successfully updated.');
                redirect(base_url('pathology_invoice/?status=print&form_f_status='.$_POST['form_f'].''));
                /*echo 1;
                return false;*/
    ...
    private function _validate()
    {
        ...
        {  
            ...
            $data['form_data'] = array(
                                        ...
                                        'lab_invoice_no'=>$post['lab_invoice_no'],
    ...
    public function delete($id="")
    {
       ...
       unauthorise_permission(409,2475); 
       if(!empty($id) && $id>0)
       {
           $result = $this->test->delete($id);
           $response = "Proforma Invoice successfully deleted.";
           echo $response;
       }
    }
    public function delivered_test($id="",$status='0')
    {
       ...
       if(!empty($id) && $id>0)
       {
           ...
           if($status==1)
           {
              $response = "Proforma Invoice mark as delivered.";
           }
           else
           {
              $response = "Proforma Invoice mark as un-delivered.";
           }
    ...
    { 
        ...
        $this->load->view('pathology_invoice/archive',$data);
    }
    ...
    { 
           unauthorise_permission(145,875);
        $users_data = $this->session->userdata('auth_users');
        $this->load->model('pathology_invoice/test_archive_model','test_archive'); 
    ...
    public function restore($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           ...
           $response = "Proforma Invoice successfully restore in Test list.";
           echo $response;
       }
    }
    ...
    public function trash($id="")
    {
       ...
       if(!empty($id) && $id>0)
       {
           ...
           $response = "Proforma Invoice successfully deleted parmanently.";
           echo $response;
       }
    }
    ...
    public function set_booking_test()
    {
       ...
       {
         $booked_test = $this->session->userdata('invoice_book_test');
        ...
        // $book_test = array_unique($book_test);
         ...
         $this->session->set_userdata('invoice_book_test',$book_test_arr);
    ...
    public function reset_booked_test()
    {
      ...
      $this->session->set_userdata('invoice_book_test',$booked_test);
      $booked_test = $this->session->userdata('invoice_book_test');
      $this->list_booked_test();   
    ...
    public function check_test_panel_type()
    {
        ...
        $polocy_no= $post['polocy_no'];
          ...
          if(!empty($new_session_array))
          {
            ...
            if($panel_type==1)
            {
              $this->session->set_userdata('invoice_test_panel_rate',$new_session_array);
              $test_panel_payment = $this->session->userdata('invoice_test_panel_rate');
            }
            else
            {
    ...
    public function remove_booked_test()
    {
       ...
       if(isset($post['test_id']) && !empty($post['test_id']))
       {
           ...
           if(!empty($profile_data))
           {
              ...
              $this->session->set_userdata('invoice_set_profile',$profile_data); 
    ...
    {
       $booking_list = $this->session->userdata('invoice_book_test');
       $set_profile = $this->session->userdata('invoice_set_profile');
    ...
    public function advance_report_result($booking_id="",$test_id="")
    {
        ...
        { 
           ...
           $post = $this->input->post();
           //print_r($post); exit;
           $advance_result_data = $this->session->userdata('invoice_advance_result');
           $advance_result = "";
           $advance_remark = "";
           ...
           if(!empty($post))
           {
                   $advance_result = $this->session->userdata('invoice_advance_result'); 
                   ...
                   $this->session->set_userdata('invoice_advance_result',$advance_result); 
                    echo 1; die;
           }

           $this->load->view('pathology_invoice/report_result_advance',$data);
        }
    }
      ...
      public function inventory_report_result($booking_id="",$test_id="")
      {
        ...
        { 
            ...
            $data['item_list']= $this->test->item_list($test_id, $booking_id); 
             ...
             $post = $this->input->post();
             $inventory_data = $this->session->userdata('invoice_inventory_item_ids');
             if(!empty($inventory_data))
             {
                $inventory_data = array();
                $this->session->set_userdata('invoice_inventory_item_ids',$inventory_data);
             }
        ...
        // } 
         ...
         $this->load->view('pathology_invoice/report_result_inventory',$data);
        }
      }
    ...
    public function insert_inventory_item_booking()
    {
          ...
          if(isset($post) && !empty($post))
          {
              if($users_data['users_role']=='2')
              {
                      ...
                      $this->form_validation->set_error_delimiters('<div class="text-danger">', '</div>');
                      $inventory_item_ids = $this->session->userdata('invoice_inventory_item_ids');
...
/* inventory code here by mamta */
 ...
 public function check_result_range($booking_id="",$test_id="")
    ...
    public function set_condition($booking_id="",$test_id="")
  {
    ...
    public function check_doctor_type($type="",$id="",$action="")
    {
      ...
      if(!empty($id) && $id>0)
      {
        ...
        {
          $this->session->set_userdata('invoice_booking_doctor_type',$result_list);
          $booking_doctor_type = $this->session->userdata('invoice_booking_doctor_type');
          $result= 1;  
        } 
        else
    ...
    public function print_test_booking_report($id="")
    {
        ...
        $data['page_title'] = "Print Proforma Invoice";
        $test_booking_id= $this->session->userdata('invoice_test_booking_id');
        ...
        $this->load->view('pathology_invoice/print_test_template',$data);
    }
    ...
    {
       unauthorise_permission(149,902);
        $data['page_title'] = "Print Consolidated Proforma Invoice";
        $test_booking_id= $this->session->userdata('invoice_test_booking_id');
        //profile and print name
        $profile_master_status = get_profile_print_status('print_booking');
  $profile_print_status = $profile_master_status->profile_status;
  $print_name_status = $profile_master_status->print_status;
      ...
      {
        $table = '<table cellpadding="0" cellspacing="0" style="float: left; z-index: 9999; position: absolute; left: 50px; width: 430px; margin-top:5px; " border="1">
                  <thead>
                      <tr>
    ...
    {  
       ...
       $this->load->view('pathology_invoice/report_html_height',$data); 
    }

    public function write_height($height="")
    {
       $this->session->set_userdata('invoice_part_height',$height);
    }
    ...
    public function print_pathology_report($booking_id='',$type='')
    {
      ...
      {  
        ...
        $lab_invoice_no = $booking_data->lab_invoice_no;
                ...
                { 
                        ...
                        if(is_numeric($test_list->result))
                        {
                            ...
                            $test_list1 = $this->check_test_result_range($test_list->booking_id,$test_list->test_id,$test_list->result);
                              ...
                              if(!empty($test_list1) && $test_list->result > $test_list1[0]->range_to && !empty($test_list->result) && !empty($test_list1[0]->range_to))
                              {
                                 $high_text = get_setting_value('TEST_RANGE_HIGH'); //exit;
                                 $test_result='';
                                if(!empty($high_text))
                                {
                                   $test_result="(".$high_text.")"; 
...
//echo $signature_reprt_data; exit;
        ...
        $data['lab_invoice_no'] = $lab_invoice_no;
      ...
      $header_replace_part = str_replace("{lab_invoice_no}",$lab_invoice_no,$header_replace_part);
 ...
 $img_barcode = '<img class="barcode" alt="'.$lab_invoice_no.'" src="'.base_url('barcode.php').'?text='.$lab_invoice_no.'&codetype=code128&orientation=horizontal&size=20&print=false"/>';
          ...
          { 
           ...
           // echo $test_report_data;die;
            ...
            { 
                    ...
                    /* if($profile_test_list->all_range_show==1)
                      {
                         $this->load->model('test_master/test_master_model','test_master');
                         $adv_range_list = $this->test_master->advance_range_list($profile_test_list->test_id);
                         //echo "<pre>"; print_r($adv_range_list);die;
                         if(!empty($adv_range_list))
                         {
                            $range_gender = array('0'=>'Female', '1'=>'Male');
      ...
      {             
        ...
        { 
           if(!empty($profile_test_list->interpratation_data))
           {
            ...
            //$int_inner_text=str_ireplace('<p>','',$profile_test_list->interpratation_data);
                //$int_inner_text=str_ireplace('</p>','',$int_inner_text);
            $test_report_data .='<div style="float:left;width:100%;font-size:12px;font-family:arial;padding:0px 4px;margin:0;">
      ...
      //{
           if(!empty($profile_interpretation))
          {
            /*$test_report_data .='<tr>
               ...
               // $int_inner_text=str_ireplace('<p>','',$profile_interpretation);
                //$int_inner_text=str_ireplace('</p>','',$int_inner_text);
                $test_report_data .='<div style="float:left;width:100%;font-size:12px;font-family:arial;padding:0px 4px;margin:0;">
...
public function check_test_result_range($booking_id="",$test_id="",$result="")
 ...
 public function check_test_result_range20190405($booking_id="",$test_id="",$result="")
 {
  ...
  public function formatted_data($result=array()){
     ...
     if(!empty($result)){
          echo '<pre>';
          $booking_data .= '<td>'.$result[0]->lab_invoice_no.'</td><td>'.$result[0]->patient_name.'</td><td>'.$result[0]->booking_date.'</td><td>'.$result[0]->total_master_amount.'</td><td>'.$result[0]->paid_amount.'</td><td>'.$result[0]->discount.'</td><td>'.$result[0]->created_date.'</td>';
...
$response = $this->general_library->email($email,$subject,$message,'',$attachment,'','','','',$booking_data->branch_id);
            ...
            $this->load->view('pathology_invoice/send_email',$data);
        }
    ...
    { 
        ...
        $fields = array('Patient Reg. no.','Proforma No.','Patient Name' ,'Mobile No.','Gender','Age', 'Referred By' ,'Booking Date','Remark','Total Amount','Discount','Net Amount','Status');
          $col = 0;
        ...
        if(!empty($list))
        {
            ...
            foreach($list as $test)
            {
                ...
                array_push($rowData, $test->patient_code,$test->lab_invoice_no,$test->patient_name,$test->mobile_no,$test->patient_gender,$age,$test->doctor_hospital_name,$booking_date,$test->remarks,$test->total_amount,$test->discount,$test->net_amount,$complate_status[$test->complation_status]);
                $count = count($rowData);
                for($j=0;$j<$count;$j++)
                {
    ...
    public function export_csv()
    {
        ...
        $fields = array('Proforma No.','Patient Name','Referred By','Booking Date','Net Amount','Status');
          $col = 0;
        foreach ($fields as $field)
        {
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, 1, $field);
        ...
        if(!empty($list))
        {
            ...
            foreach($list as $test)
            {
                ...
                $doctor_name = get_doctor($test->referral_doctor);
                array_push($rowData,$test->lab_invoice_no,$test->patient_name, $test->doctor_hospital_name,$booking_date,$test->net_amount,$complate_status[$test->complation_status]);
                $count = count($rowData);
                for($j=0;$j<$count;$j++)
                {
                    $data[$i][$fields[$j]] = $rowData[$j];
    ...
    {     
        ...
        $this->load->view('pathology_invoice/booking_export_print',$data);
  ...
  public function set_profile($profile_id="")
  {
      $post=$this->input->post();
       ...
       {
          $profile_arr = $this->session->userdata('invoice_set_profile'); 
...
/*print and name*/
  ...
  { 
       ...
       if(!empty($booking_id) && $booking_id>0)
       {
          ...
          if(in_array('640',$users_data['permission']['action']))
          {
            ...
            { 
              send_sms('test_report_complete',23,'',$booking_data['mobile_no'],array('{patient_name}'=>$booking_data['patient_name'],'{booking_no}'=>$booking_data['lab_invoice_no'],'{report_complete_date}'=>$booking_data['complation_date'],'{username}'=>'PAT000'.$booking_data['patient_id'],'{password}'=>'PASS'.$booking_data['patient_id']));  
            }
          }
          //////////////////////////////////////////
          ...
          ////////// SEND EMAIL ///////////////////
                if(!empty($booking_data['patient_email']))
                  { 
                    ...
                    $this->general_functions->email($booking_data['patient_email'],'','','','','1','test_report_complete','23',array('{patient_name}'=>$booking_data['patient_name'],'{booking_no}'=>$booking_data['lab_invoice_no'],'{report_complete_date}'=>$booking_data['complation_date']));
   ...
   public function add_interpretation($booking_id="",$test_id="")
   {
     ...
     //echo "<pre>"; print_r($post); exit;
      ...
      $this->load->view('pathology_invoice/add_interpretation',$data);
    }
    ...
    public function booking_set_branch($bid="")
    {
      ...
      {
        $this->session->set_userdata('invoice_booking_set_branch',$bid);
      }
    }
  ...
  public function add_profile_interpretation($booking_id="",$profile_id="")
  {
      ...
      $data['form_error'] = [];
      $interpretation = ""; 
      $interpretation = $this->session->userdata('invoice_profile_interpretation_data');
      ...
      $this->load->view('pathology_invoice/add_profile_interpretation',$data);
  } 
  ...
  function form_f_print()
    {
      ...
      $this->load->view('pathology_invoice/form_f_pathalogy',$data);
    }
    ...
    public function print_barcode($booking_id='')
    {
      ...
      if(!empty($booking_id))
      {
        ...
        $gender = $booking_data['gender'];
            ...
            $data['lab_invoice_no']=$booking_data['lab_invoice_no'];
        //echo "<pre>";print_r($barcode_setting); exit;
        

        $this->load->view('pathology_invoice/print_barcode_template',$data);
      }
    }
    ...
    function status_filter()
    {
      ...
      $this->session->set_userdata('invoice_complation_status',$this->input->post('val'));
    }
     ...
     function open_sample_collected($test_id)
   {
    ...
    $this->load->view('pathology_invoice/sample_collected', $data);
   ...
   function open_sample_received($test_id)
   {
    ...
    $data['testid'] = $test_id;
    $this->load->view('pathology_invoice/sample_received', $data);
...
{ 
   $this->load->model('pathology_invoice/prescription_file_model','prescription_file');
    ...
    $this->load->view('pathology_invoice/add_file',$data);
 }
  ...
  public function view_files($prescription_id='')
  {
     $this->load->model('pathology_invoice/prescription_file_model','prescription_file');
      ...
      $this->load->view('pathology_invoice/view_prescription_files',$data);
...
{ 
   $this->load->model('pathology_invoice/prescription_file_model','prescription_file');
    ...
    public function add_widal_value($booking_id='',$test_id='')
    {
      ...
      $this->load->view('pathology_invoice/widal_value',$data);
    }
    ...
    public function get_sub_branches($parent_id=''){
           ...
           $this->session->set_userdata('invoice_sub_branches_data',$branches_data);
         ...
         public function get_parent_branches($parent_id=''){
           ...
           $this->session->set_userdata('invoice_parent_branches_data',$nested);
