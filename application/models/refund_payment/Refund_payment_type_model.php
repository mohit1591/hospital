	...
	private function _get_datatables_query()
	{
		...
		if(!empty($refund_search_data['section_id']))
		{
			foreach ($refund_search_data['section_id'] as $value) 
			{
				...
				if($value==15)
				{
					$this->db->join('hms_dialysis_booking','hms_dialysis_booking.id=hms_payment.parent_id AND hms_dialysis_booking.is_deleted=0 and hms_dialysis_booking.branch_id="'.$users_data['parent_id'].'"','inner');
				}
			}
		}
