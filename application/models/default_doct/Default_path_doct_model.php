<?php
class Default_path_doct_model extends CI_Model 
{
	var $table = 'hms_default_path_doct_setting'; 
	...
	public function get_master_unique()
	{
		...
		$this->db->select('hms_default_path_doct_setting.*');		
	...
	{  
		...
		{    
           ...
           if(!empty($post['data_id']) && isset($post['data_id']))
             {
            	$this->db->update('hms_default_path_doct_setting',$data);
            }
            else
            {
            	$this->db->insert('hms_default_path_doct_setting',$data);
            }
