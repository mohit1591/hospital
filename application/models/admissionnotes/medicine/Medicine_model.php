...
class Medicine_model extends CI_Model {
	...
	var $column = array('hms_medicine_entry.id','hms_medicine_entry.medicine_name','hms_medicine_entry.unit_id','hms_medicine_entry.salt','hms_medicine_entry.manuf_company', 'hms_medicine_entry.status','hms_medicine_entry.created_date');  
   ...
   public function check_unique_value($branch_id, $medicine_name, $id='')
    {
    	$this->db->select('hms_medicine_entry.*');
		$this->db->from('hms_medicine_entry'); 
		$this->db->where('hms_medicine_entry.branch_id',$branch_id);
		$this->db->where('hms_medicine_entry.medicine_name',$medicine_name);
		if(!empty($id))
		$this->db->where('hms_medicine_entry.id !=',$id);
		$this->db->where('hms_medicine_entry.is_deleted','0');
