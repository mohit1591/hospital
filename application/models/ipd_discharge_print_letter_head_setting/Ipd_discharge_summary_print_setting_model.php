...
class ipd_discharge_summary_print_setting_model extends CI_Model 
{
  ...
  public function get_master_unique() //doctor_signature_position
  {
   ...
   $this->db->where('hms_ipd_discharge_report_print_letter_head_setting.branch_id = "'.$user_data['parent_id'].'"');
  ...
  {  
    ...
    {    
      ...
      $this->db->delete('hms_ipd_discharge_report_print_letter_head_setting');
      ...
      if(isset($post['pixel_value']))
      {
        $pixel_value = $post['pixel_value']; 
            ...
            foreach($post['data'] as $key=>$val)
            {
              ...
              $this->db->insert('hms_ipd_discharge_report_print_letter_head_setting',$data);
