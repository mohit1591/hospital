...
class Expenses_model extends CI_Model {
	...
	var $column = array('hms_expenses.id','hms_expenses.vouchar_no','hms_expenses.expenses_date', 'hms_expenses.paid_to_id', 'hms_expenses.paid_amount','hms_expenses.payment_mode','hms_expenses.payment_from','hms_expenses.created_date','hms_expenses.modified_date');  
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_expenses.*,hms_expenses.dept_id as dep_name,hms_expenses_category.exp_category, (CASE WHEN hms_expenses.type=0 THEN 'Expenses'  WHEN hms_expenses.type=4 THEN 'Inventory Purchase' WHEN hms_expenses.type=1 THEN 'Employee Salary' WHEN hms_expenses.type=2 THEN 'Medicine Purchase' WHEN hms_expenses.type=3 THEN 'Medicine Sale Return' WHEN hms_expenses.type=5 THEN  'Vaccine Purchase' WHEN hms_expenses.type=6 THEN  'Vaccine Billing Return' WHEN hms_expenses.type=7 THEN 'OPD Payment Refund'  WHEN hms_expenses.type=8 THEN 'Pathology Payment Refund'  WHEN hms_expenses.type=9 THEN 'Medicine Payment Refund' WHEN hms_expenses.type=10 THEN 'IPD Refund' WHEN hms_expenses.type=11 THEN 'OT Refund' WHEN hms_expenses.type=12 THEN 'Blood Bank Refund' WHEN hms_expenses.type=13 THEN 'Ambulance Refund' WHEN hms_expenses.type=14 THEN 'OPD Billing' WHEN hms_expenses.type=15 THEN 'Day Care Refund'  WHEN hms_expenses.type=18 THEN 'Dialysis Refund' END) as expenses_type,hms_payment_mode.payment_mode as p_mode,(CASE WHEN hms_expenses.type=0 THEN 'Expenses' WHEN hms_expenses.type=7 THEN (SELECT `hms_opd_booking`.`booking_code` FROM `hms_expenses` as exp left JOIN `hms_refund_payment` ON `hms_refund_payment`.`id` = `exp`.`parent_id`  LEFT JOIN `hms_opd_booking` ON `hms_opd_booking`.`id` = `hms_refund_payment`.`parent_id` AND section_id=2 WHERE `exp`.`id` = hms_expenses.id AND `exp`.`branch_id` = ".$users_data['parent_id']." AND `exp`.`type` = 7) WHEN hms_expenses.type=8 THEN (SELECT `path_test_booking`.`lab_reg_no` FROM `hms_expenses` as exp left JOIN `hms_refund_payment` ON `hms_refund_payment`.`id` = `exp`.`parent_id`  LEFT JOIN `path_test_booking` ON `path_test_booking`.`id` = `hms_refund_payment`.`parent_id`  WHERE `exp`.`id` = hms_expenses.id AND `exp`.`branch_id` = ".$users_data['parent_id']." AND `exp`.`type` = 8) 
...
(SELECT `hms_vaccination_purchase`.`purchase_id` FROM `hms_expenses` as exp  LEFT JOIN `hms_vaccination_purchase` ON `hms_vaccination_purchase`.`id`  = `exp`.`parent_id`  WHERE `exp`.`id` = hms_expenses.id AND `exp`.`branch_id` = ".$users_data['parent_id']." AND `exp`.`type` = 5)
		...
		END) as book_code,(SELECT `hms_patient`.`patient_name` FROM `hms_expenses` as exp left JOIN `hms_refund_payment` ON `hms_refund_payment`.`id` = `exp`.`parent_id`  LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_refund_payment`.`patient_id`  WHERE `exp`.`id` = hms_expenses.id AND `exp`.`branch_id` = ".$users_data['parent_id'].") as patient_name,hms_medicine_vendors.name as paid_to_name,hms_doctors.doctor_name"); 
		...
		$this->db->join("hms_doctors","hms_expenses.doctor_id =hms_doctors.id",'left');
        ...
        if(isset($expenses_search) && !empty($expenses_search))
		{
		    ...
		    if(isset($expenses_search['department_type']) && !empty($expenses_search['department_type']))
			{
				$this->db->where('hms_expenses.department_type',$expenses_search['department_type']);
			}
    		...
    		if(isset($expenses_search['expense_category_id']) && !empty($expenses_search['expense_category_id']))
			{
				$this->db->where('hms_expenses.paid_to_id',$expenses_search['expense_category_id']);
			}
...
(SELECT `hms_vaccination_purchase`.`purchase_id` FROM `hms_expenses` as exp  LEFT JOIN `hms_vaccination_purchase` ON `hms_vaccination_purchase`.`id`  = `exp`.`parent_id`  WHERE `exp`.`id` = hms_expenses.id AND `exp`.`branch_id` = ".$users_data['parent_id']." AND `exp`.`type` = 5)
		...
		END) as book_code,(SELECT `hms_patient`.`patient_name` FROM `hms_expenses` as exp left JOIN `hms_refund_payment` ON `hms_refund_payment`.`id` = `exp`.`parent_id`  LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_refund_payment`.`parent_id`  WHERE `exp`.`id` = hms_expenses.id AND `exp`.`branch_id` = ".$users_data['parent_id'].") as patient_name,hms_medicine_vendors.name as paid_to_name,hms_doctors.doctor_name
		"); 
...
(SELECT `hms_vaccination_purchase`.`purchase_date` FROM `hms_expenses` as exp  LEFT JOIN `hms_vaccination_purchase` ON `hms_vaccination_purchase`.`id`  = `exp`.`parent_id`  WHERE `exp`.`id` = ".$ids." AND `exp`.`branch_id` = ".$users_data['parent_id']." AND `exp`.`type` = 5) END) as book_date,hms_expenses_category.exp_category as exptypes,hms_doctors.doctor_name");
		...
		$this->db->join('hms_expenses_category','hms_expenses_category.id=hms_expenses.paid_to_id','left');
