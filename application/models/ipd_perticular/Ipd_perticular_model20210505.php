...
class Ipd_perticular_model extends CI_Model {
        ...
        public function save()
	{
        ...
        if($grp_code==30)
        {
            $group_name='OT Charges';
                   
        }
        if($grp_code==40)
        {
            $group_name='Medicine and Consumables';
		...
		{    
			...
			$get_all_comapny= $this->get_panel_list();
			 ...
			 {
				$this->db->where(array('branch_id'=>$user_data['parent_id'],'type'=>1,'particular_id'=>$post['data_id'],'panel_company_id'=>$panel_comapny->id));
				$this->db->delete('hms_ipd_particular_charge');
			 	$data_new = array( 
					'particular_id'=>$post['data_id'],
					'branch_id'=>$user_data['parent_id'],
					'charge'=>$post['panel_charge'],
					'type'=>1,
	...
	public function save_old()
	{
		...
		{    
				...
				$this->db->where(array('type'=>0,'particular_id'=>$post['data_id'],'panel_company_id'=>0));
				$this->db->delete('hms_ipd_particular_charge');
			 	$data_ch_new = array( 
					'particular_id'=>$post['data_id'],
					'charge'=>$post['charge'],
			...
			$get_all_comapny= $this->get_panel_list();
			 foreach($get_all_comapny as $panel_comapny)
			 {
				$this->db->where(array('type'=>1,'particular_id'=>$post['data_id'],'panel_company_id'=>$panel_comapny->id));
				$this->db->delete('hms_ipd_particular_charge');
			 	$data_new = array( 
					'particular_id'=>$post['data_id'],
					'charge'=>$post['panel_charge'],
        ...
        if(!empty($ipd_all_data))
        {
            foreach($ipd_all_data as $ipd_perticular_data)
            {
				...
				$ipd_data_array = array( 
	                    ...
	                    'panel_charge'=>$ipd_perticular_data['panel_charge'],
	                    'type'=>0,
					    'panel_company_id'=>0,
						'status'=>1,					
	                    'ip_address'=>$_SERVER['REMOTE_ADDR'],
    ...
    // op 19/08/19 
    public function check_unique_value($branch_id, $particular)
    {
    	$this->db->select('hms_ipd_perticular.*');
		$this->db->from('hms_ipd_perticular'); 
