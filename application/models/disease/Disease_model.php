...
class Disease_model extends CI_Model {
	...
	var $table = 'hms_disease';
	var $column = array('hms_disease.id','hms_disease.disease', 'hms_disease.status','hms_disease.created_date');  
	...
	public function get_by_id($id)
	{
	    $user_data = $this->session->userdata('auth_users');
		$this->db->select('hms_disease.*');
		$this->db->from('hms_disease'); 
		$this->db->where('hms_disease.id',$id);
		$this->db->where('hms_disease.branch_id',$user_data['parent_id']);
		$this->db->where('hms_disease.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
			...
			$this->db->update('hms_disease',$data);  
		}
		else{    
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_disease',$data);               
		} 	
	}
