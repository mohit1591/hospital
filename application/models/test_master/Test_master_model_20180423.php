...
class Test_master_model extends CI_Model {
    ...
    public function downloadall($ids=array()){
    	...
    	{ 
    		...
    		$this->db->select('path_test.*');     
            $this->db->where('path_test.id IN ('.$test_ids.')');  
            $query = $this->db->get('path_test');
            $result = $query->result_array(); 
            $data= array();
            $test_id=array();
            ...
            if(!empty($result)){
	            for($i=0;$i<count($result);$i++){
	            	$test_code[$i] = generate_unique_id(25);

                    if(!empty($result_branch) && $users_data['users_role']==2)
		        	  {
		        	  	 $master_price = $result[$i]['rate']; 

		        	  	 $base_price = $result[$i]['base_rate']; 
		                 
		                 ///////// Master Calculation //////////////////
		                 if($result_branch->master_type==1)
		                 {
		                 	$master_price = "(".$result[$i]['rate']."/100)*".$result_branch->master_rate;   
		                 ...
		                 if($result_branch->base_type==1)
		                 {
		                 	$base_price = "(".$result[$i]['base_rate']."/100)*".$result_branch->base_rate;   
		                 	$base_price = $this->calc_string($base_price);
		                 ...
		                 ///////////////////////////////////////////// 
		                 $master_price = $result[$i]['rate']+$master_price;
		                 $base_price = $result[$i]['base_rate']+$base_price;
		        	  }
		        	  else
		        	  {
						$master_price = $result[$i]['rate'];
	            	 ...
	            	 $data[$i] = array(
								...
								'test_head_id'=>$result[$i]['test_head_id'],
								'test_name'=>$result[$i]['test_name'], 
								'default_value'=>$result[$i]['default_value'],
								'rate'=>$result[$i]['rate'],
								'base_rate'=>$result[$i]['base_rate'],
								'method_id'=>$result[$i]['method_id'],
								'unit_id'=>$result[$i]['unit_id'], 
								'range_to'=>$result[$i]['range_to'],
								'range_from'=>$result[$i]['range_from'], 
								'test_type_id'=>$result[$i]['test_type_id'],
								'interpretation'=>$result[$i]['interpretation'],
								'precaution'=>$result[$i]['precaution'],
								'ip_address'=>$_SERVER['REMOTE_ADDR'],
								'created_by'=>$users_data['id'],
								'created_date'=>date('Y-m-d H:i:s') 
	        	  	           ); 
				    $this->db->insert('path_test',$data[$i]); 
				    $test_id[$i] = $this->db->insert_id();  
                    
                    ////// Formula start /////////////
			        /*
			        $this->db->select('path_test_formula.*');     
			        $this->db->where('path_test_formula.test_id IN ('.$test_ids.')');  
			        $this->db->order_by('id','ASC');
			        $query = $this->db->get('path_test_formula');
			        $formula_list = $query->result_array();
			        if(!empty($formula_list)){
			            for($j=0;$j<count($formula_list);$j++){
			    	        $formula_data = array(
			    	            'test_id'=>$test_id[$i],
			    	            'formula_val'=>$formula_list[$j]['formula_val']
			    	        );
			    	        $this->db->insert('path_test_formula',$formula_data); 
			            }
			        }
			        */
				    ...
				    /*
				    $this->db->select('path_test_condition.*');     
				    $this->db->where('path_test_condition.test_id IN ('.$test_ids.')');  
				    $this->db->order_by('id','ASC');
				    $query = $this->db->get('path_test_condition');
				    $condition_list = $query->result_array();
			        if(!empty($condition_list)){
			    	    for($k=0;$k<count($condition_list);$k++){
			    		    $condition_data = array(
			    			    'test_id'=>$test_id[$i],
			    			    'condition_val'=>$condition_list[$k]['condition_val']
			    			);
			    		    $this->db->insert('path_test_condition',$condition_data); 
			    	    }
			        }
                    */
                    ...
                    ////// Under start /////////////
				   /* $this->db->select('path_test_under.*');     
				    $this->db->where('path_test_under.parent_id IN ('.$test_ids.')');  
				    $this->db->order_by('id','ASC');
				    $query = $this->db->get('path_test_under');
				    $under_list = $query->result_array();
			        if(!empty($under_list)){
			    	    for($l=0;$l<count($under_list);$l++){
			    		    $under_data = array(
			    			    'parent_id'=>$test_id[$i],
			    			    'child_id'=>$under_list[$l]['child_id']
			    		    );
			    		    $this->db->insert('path_test_under',$under_data); 
			    	    }
			        }
	...
	public function save_all_test($test_all_datas)
    {
