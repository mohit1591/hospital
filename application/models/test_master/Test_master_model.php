...
class Test_master_model extends CI_Model {
	...
	var $table = 'path_test';
	var $column = array('path_test.id','path_test.test_code','path_test.test_name','path_test.sort_order', 'path_test.result_heading', 'path_test.test_type_id', 'hms_department.department', 'path_test_heads.test_heads', 'path_unit.unit','path_test.base_rate');  
    ...
    public function get_default_vals($test_id='',$vals='')
    {
    	$response = '';
            ...
            {     
                ...
                if(!empty($p_thead_result))
                {
                    ...
                    else
                    {
                        $test_heads_data = array( 
                                        ...
                                        'dept_id'=>$post['dept_id'],
                                        'test_heads'=>$p_thead_result[0]['test_heads'],
                                        'sort_order'=>$p_thead_result[0]['sort_order'],
              ...
              ///////////////// Set Download Method //////////
                ...
                if(!empty($p_tmethod_result))
                {
                    ...
                    $this->db->where('path_test_method.test_method',$p_tmethod_result[0]['test_method']); 
                    $this->db->where('path_test_method.branch_id',$users_data['parent_id']); 
                    $this->db->where('path_test_method.is_deleted',0); 
                    $c_tmethod_query = $this->db->get('path_test_method');
                    $c_tmethod_result = $c_tmethod_query->result_array();
              ...
              ///////////////// Set Test Unit //////////
                ...
                if(!empty($p_unit_result))
                {
                    ...
                    $this->db->where('path_unit.unit',$p_unit_result[0]['unit']); 
                    $this->db->where('path_unit.branch_id',$users_data['parent_id']);  
                    $this->db->where('path_unit.is_deleted',0); 
                    $c_unit_query = $this->db->get('path_unit');
                    $c_unit_result = $c_unit_query->result_array();
              ...
              ///////////////// Set Test Sample Type //////////
                ...
                if(!empty($p_stype_result))
                {
                    ...
                    $this->db->where('path_sample_type.sample_type',$p_stype_result[0]['sample_type']); 
                    $this->db->where('path_sample_type.branch_id',$users_data['parent_id']);  
                    $this->db->where('path_sample_type.is_deleted.id',0);
                    $c_stype_query = $this->db->get('path_sample_type');
                    
                    if($c_stype_query !== FALSE && $c_stype_query->num_rows() > 0)
                    {
                         $c_stype_result = $c_stype_query->result_array();
                          if(!empty($c_stype_result))
                            {
    ...
    public function download_test20190213($tid)
    {
    	$users_data = $this->session->userdata('auth_users'); 
        ////////////// Rate Plan Data ////////
		 $this->db->select('path_rate_plan.*');     
		       ...
		       // test method 
			    ...
			    ///////////////// Set Download Method //////////
				  ...
				  $data_test = array(  
									...
									'test_name'=>$test_all_data['test_name'],  
									'rate'=>$test_all_data['rate'],
									'base_rate'=>$test_all_data['base_rate'],
									'method_id'=>$test_method_id,
									'unit_id'=>$test_unit_id,  
									'sample_test'=>$sample_type_id, 
		        	  	           ); 
					  ...
					  $this->db->update('path_test',$data_test); 
	...
	public function save_test_result_heading_status($test_id,$status='')
     {
       $user_data = $this->session->userdata('auth_users');
       if(!empty($test_id))
        {   
        	$data = array(  
					'result_heading'=>$status,
				);
            $this->db->set('path_test.result_heading',$status); 
            $this->db->where('path_test.branch_id',$user_data['parent_id']); 
    ...
    public function rate_plan_test_rate($branch_id="",$rate_plan_id="",$test_id="")
	{
		//print_r($test_id);die;
	   $arr = [];
       if(!empty($test_id))
       {
			...
			$cases= '(CASE WHEN path_rate_plan_price.charge>0 THEN path_rate_plan_price.charge ELSE path_test.rate END) as path_price';
			}
			else
			{
			$cases= 'path_test.rate as path_price';
			}
			$this->db->select("path_test.*,".$cases.",path_rate_plan_price.rate_plan_id,path_rate_plan_price.test_head_id,path_rate_plan_price.test_id"); 
			if(!empty($rate_plan_id))
			{
			$this->db->join('path_rate_plan_price','path_rate_plan_price.test_id=path_test.id AND path_rate_plan_price.rate_plan_id = "'.$rate_plan_id.'"','left');
			}
			else
			{
			$this->db->join('path_rate_plan_price','path_rate_plan_price.test_id=path_test.id','left'); 
			}
			if(!empty($branch_id))
			{
               $this->db->where('path_rate_plan_price.branch_id',$branch_id); 
			}  

			
			$this->db->join('path_test_heads','path_test_heads.id = path_test.test_head_id','left');
			$this->db->where('path_rate_plan_price.type',0);
			$this->db->where('path_rate_plan_price.test_id',$test_id);
			$this->db->where('path_rate_plan_price.rate_plan_id',$rate_plan_id);
			$this->db->group_by('path_test.id');   
			$query = $this->db->get('path_test'); 
			...
			if(!empty($result))
			{
               $arr = array('path_price'=>$result->path_price,'rate_plan_id'=>$result->rate_plan_id,'test_head_id'=>$result->test_head_id,'test_id'=>$result->test_id);
			} 
			return $arr;
       }
	} 
	
	public function doctor_plan_test_rate($branch_id="",$doctor_id="",$test_id="")
	{
		//print_r($test_id);die;
	   $arr = [];
       if(!empty($test_id))
       {
			...
			$this->db->select("path_test.*,".$cases.",path_rate_plan_for_doctor.doctor_id,path_rate_plan_for_doctor.test_head_id,path_rate_plan_for_doctor.test_id"); 
			if(!empty($doctor_id))
			{
			$this->db->join('path_rate_plan_for_doctor','path_rate_plan_for_doctor.test_id=path_test.id AND path_rate_plan_for_doctor.doctor_id = "'.$doctor_id.'"','left');
			}
			else
			{
			...
			if(!empty($branch_id))
			{
               $this->db->where('path_rate_plan_for_doctor.branch_id',$branch_id); 
			}  

			
			$this->db->join('path_test_heads','path_test_heads.id = path_test.test_head_id','left');
			$this->db->where('path_rate_plan_for_doctor.type',0);
			$this->db->where('path_rate_plan_for_doctor.test_id',$test_id);
			$this->db->where('path_rate_plan_for_doctor.doctor_id',$doctor_id);
			$this->db->group_by('path_test.id');   
			$query = $this->db->get('path_test'); 
			...
			if(!empty($result))
			{
               $arr = array('path_price'=>$result->path_price,'doctor_id'=>$result->doctor_id,'test_head_id'=>$result->test_head_id,'test_id'=>$result->test_id);
			} 
			return $arr;
       }
	} 
	
	public function doctor_rate_plan_test_rate($branch_id="",$doctor_id="",$test_id="")
	{
		//print_r($test_id);die;
	   $arr = [];
       if(!empty($test_id))
