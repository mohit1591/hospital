...
class Item_opening_stock_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("path_item.*,path_stock_category.category,hms_stock_item_unit.unit,sum(path_stock_item.debit-path_stock_item.credit)as stock_qty,hms_inventory_racks.rack_no,path_stock_item.id as stocks_ids"); 
	...
	public function get_by_id($id)
	{
		...
		$this->db->select('path_item.*,path_stock_item.stock_item_unit,path_stock_item.second_unit');
        $this->db->from('path_stock_item'); 
		$this->db->where('path_stock_item.id',$id);
		$this->db->where('path_stock_item.type',5);
		$this->db->where('path_item.is_deleted','0');
	    //$this->db->join('hms_stock_item_unit','hms_stock_item_unit.id=path_item.unit_id','left');
		$this->db->join('path_item','path_item.id=path_stock_item.parent_id','left');
        $query = $this->db->get(); 
        //echo $this->db->last_query();die;
		return $query->row_array();
        //echo $this->db->last_query();die;	
	...
	public function save()
	{
		...
		$data = array( 
					...
					'item'=>$post['item'],
					//'item_code'=>$post['item_code'], stock_item_unit
		...
		{   
            ...
            $this->db->where('inv_stock_serial_no.module_id',6);
			...
			$data = array( 
                ...
                if(isset($post['serial_no_array']) && !empty($post['serial_no_array']) )
                    {
                       $decoded = json_decode($post['serial_no_array'][0]);
		...
		{ 
		    //echo "<pre>"; print_r($post); exit;
			if(!empty($post['item']) && $post['item']!='')
    		{
            	$this->db->select("path_item.*");
			    $this->db->from('path_item'); 
                $this->db->where('LOWER(path_item.item)',strtolower($post['item'])); 
			    $this->db->where('path_item.branch_id='.$user_data['parent_id']);  
			    $query = $this->db->get(); 
			    //echo $this->db->last_query();die;
				 ...
				 if(isset($post['serial_no_array']) && !empty($post['serial_no_array']) )
                    {
                       ...
                       //print '<pre>'; print_r($decoded); die;
                       $ser_array=explode(',',$decoded);
                       //print '<pre>'; print_r($ser_array); die;
                      foreach ($ser_array as $value) 
                      {
    ...
    public function unit_list($unit_id="")
    {
     ...
     public function get_item_name($vals="")
    {
        //echo $vals; die;
    	$response = '';
    	if(!empty($vals))
    	{
	        ...
	        { 
	        	...
	        	foreach($result as $vals)
	        	{
					...
					$name = $vals->item.'|'.$vals->conversion.'|'.$vals->mrp.'|'.$vals->manuf_company.'|'.$vals->id.'|'.$vals->item_code.'|'.$vals->price;
