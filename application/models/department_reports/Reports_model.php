...
class Reports_model extends CI_Model {
    ...
    var $column = array('path_test_booking.booking_date', 'hms_patient.patient_name','hms_doctors.doctor_name','hms_department.department','path_test_booking.total_amount','path_test_booking.discount','path_test_booking.net_amount','path_test_booking.paid_amount','path_test_booking.balance');  
    ...
    private function _get_datatables_query()
    {
        ...
        $search_data = $this->session->userdata('dept_search_data'); 
        //print_r($search_data);
        $this->db->select("path_test_booking.id, hms_payment.id as payment_id, path_test_booking.lab_reg_no, path_test_booking.booking_date, hms_payment.total_amount, hms_payment.net_amount as net_amount, hms_payment.debit as paid_amount,hms_payment.discount_amount as discount, hms_doctors.doctor_name, hms_patient.patient_name,hms_department.department as department,(CASE WHEN hms_payment.balance > 0  THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.parent_id = path_test_booking.id AND payment.section_id=1) ELSE '0.00' END) as balance, (CASE WHEN hms_payment.balance > 0 THEN '1' ELSE '0' END) as print_type,(CASE WHEN hms_payment.balance > 0 THEN '1' ELSE '0' END) as print_type,
        ...
        if(isset($search_data) && !empty($search_data))
        {
            ...
            {
                $this->db->where('path_test_booking.referral_doctor' ,$search_data['referral_doctor']);
            }
            /* refered by code */
            ...
            {
                $imp_users = implode(',', $search_data['department']);
    ...
    function search_report_data()
    {
        ...
        //(CASE WHEN hms_payment.section_id=1 THEN 'Pathology' ELSE 'Pathology' END) as department
    ...
    function get_department_wise_charge($branch_id='',$department_id='',$booking_id='')
    {
			...
			$search_data = $this->session->userdata('dept_search_data'); 
			$this->db->select('sum(path_test_booking_to_test.amount)
                as total_amount');
            ...
            $this->db->where('path_test.dept_id',$department_id);
