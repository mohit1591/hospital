...
var $test_list_column = array('path_test_booking.id', 'hms_patient.patient_code', 'path_test_booking.lab_reg_no', 'hms_patient.patient_name',   'hms_patient.relation_name', 'hms_patient.mobile_no', 'hms_patient.patient_email','path_test_booking.booking_date', 
'path_test_booking.remarks');  
// added By Nitin Sharma 02/02/2024
  //
 ...
 //referred_by
      ...
      elseif(isset($search_date['refered_id']) && $search_date['referred_by']=='0' && !empty($search_date['refered_id']))
      {
          $this->db->where('path_test_booking.referral_doctor' ,$search_date['refered_id']);
            ...
            if(isset($search_date['profile_id']) && !empty($search_date['profile_id'])
                )
            { 
                $this->db->where('path_test_booking_to_profile.profile_id = "'.$search_date["profile_id"].'"');
            }
            if(isset($search_date['test_ids']) && !empty($search_date['test_ids'])
                )
            { 
                $this->db->where('path_test_booking_to_test.test_id = "'.$search_date["test_ids"].'"');
            }
...
{
    //  dd($this->session->userdata('set_profile'));
    $user_data = $this->session->userdata('auth_users'); 
...
//(CASE WHEN path_profile.print_name="" THEN path_profile.profile_name ELSE path_profile.print_name END) as profile_name
  ...
  public function referal_hospital_list($branch_id="")
    ...
    public function profile_list($branch_id="", $id="")
    {
    $users_data = $this->session->userdata('auth_users');
    $booking_set_branch = $this->session->userdata('booking_set_branch'); 
    $this->db->select('*');   
    ...
    $this->db->select('path_test.test_name,path_test.dept_id, path_test.result_type,  path_test.test_type_id, path_test.test_code, path_test_booking_to_test.*, path_test_booking_to_test.result_date as date_discharge, (CASE WHEN path_test_formula.id > 0 THEN 1 ELSE 0 END) as formula_avl,path_test.test_result_type,hms_department.department,path_test.range_from_pre, path_test.range_from_post, path_test.range_to_pre, path_test.range_to_post, path_test.all_range_show,path_test.range_from,path_test.range_to');  
  ...
  {
    $this->db->select('path_test.*,path_test_booking_to_test.result, path_test_booking_to_test.interpratation_data, path_test_booking_to_test.interp_sec_page, path_test_booking_to_test.interp_thir_page'); 
    $this->db->join('path_test','path_test.id = path_test_booking_to_test.test_id');
    if(!empty($test_id))
         {
...
public function get_transaction_id($booking_id,$section_id,$type)
{
  ...
  public function save_adv_interpretation20220328()
  {
    $post = $this->input->post();
  ...
  public function save_adv_interpretation()
  {
    ...
    //echo "<pre>"; print_r($post); die;
    $this->db->set('interpratation_data',$post['interpretation_data']);
    $this->db->set('interp_sec_page',$post['interp_sec_page']);
    $this->db->set('interp_thir_page',$post['interp_thir_page']);
    $this->db->set('adv_interpretation_status',1);
    $this->db->where('booking_id',$post['booking_id']);
    $this->db->where('test_id',$post['test_id']);
    $this->db->update('path_test_booking_to_test');
...
//end of invoice
    ...
    public function get_all_test_name($vals="")
    {
    	$response = '';
    	if(!empty($vals))
    	{
    ...
    // added By Nitin Sharma 02/02/2024
    private function _get_all_test_by_query($patient_id){
    $users_data = $this->session->userdata('auth_users');
    
    $post = $this->input->post();
    $this->db->select("path_test_booking.id, path_test_booking.branch_id, path_test_booking.complation_status, path_test_booking.lab_reg_no, path_test_booking.booking_date, path_test_booking.net_amount, path_test_booking.total_amount,path_test_booking.paid_amount,path_test_booking.discount, path_test_booking.created_date,hms_patient.patient_name, hms_patient.patient_code"); 
    $this->db->from($this->table); 
    $this->db->join('hms_patient','hms_patient.id = path_test_booking.patient_id','left');
    $this->db->where('path_test_booking.complation_status=1');
    $this->db->where('path_test_booking.branch_id',$users_data['parent_id']);
    ...
    foreach ($this->test_list_column as $item) // loop column 
    {
      if($_POST['search']['value']) // if datatable send POST for search
      {
        ...
        if(count($this->test_list_column) - 1 == $i) //last loop+
          $this->db->group_end(); //close bracket
      }
      $test_list_column[$i] = $item; // set column array variable to order processing
      $i++;
    }
    
    if(isset($_POST['order'])) // here order processing
    {
      $this->db->order_by($test_list_column[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
    }
    else if(isset($this->order))
    {
      $order = $this->order;
    ...
    function get_all_test_by_patient_id($patient_id){
        $this->_get_all_test_by_query($patient_id);
        if($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();   
        // echo $this->db->last_query();die;
    ...
    function count_test_by_patient_id($patient_id){
        $this->_get_all_test_by_query($patient_id);
        $query = $this->db->get();
        return $query->num_rows();
    }
