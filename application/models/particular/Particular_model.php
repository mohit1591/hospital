...
class Particular_model extends CI_Model {
	...
	var $table = 'hms_opd_particular';
	var $column = array('hms_opd_particular.id','hms_opd_particular.particular','hms_opd_particular.charge', 'hms_opd_particular.status','hms_opd_particular.created_date');  
	...
	public function get_by_id($id)
	{
		...
		$this->db->where('hms_opd_particular.id',$id);
		$this->db->where('hms_opd_particular.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
			...
			$this->db->update('hms_opd_particular',$data);  
		}
		else{    
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_opd_particular',$data);               
		} 	
	}
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->update('hms_opd_particular');
			//echo $this->db->last_query();die;
    	} 
    }
        ...
        if(!empty($particular_all_data))
        {
            foreach($particular_all_data as $particular_data)
            {
                ...
                //print_r($vaccination_data_array);
				    $this->db->insert('hms_opd_particular',$particular_data_array);
