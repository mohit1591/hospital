...
class Billing_model extends CI_Model {
    ...
    (CASE WHEN `hms_doctors_to_comission`.`rate_type`=1 THEN (select ((CASE WHEN hms_refund_payment.refund_amount>0 THEN sum(hms_refund_payment.refund_amount) ELSE 0 END)/100)*hms_doctors_to_comission.rate from hms_refund_payment left join hms_medicine_sale med_sale on  med_sale.id = hms_refund_payment.parent_id WHERE hms_refund_payment.section_id='3' AND hms_refund_payment.type='5' AND hms_refund_payment.refund_comission=1 AND med_sale.refered_id='".$post['doctor_id']."' ".$opd_refund_where." ) ELSE 0 END)  
      ...
      //OT
        ...
        //OT comission //
         ...
         ///IPD particular commission  ///
       /* $this->db->select("`hms_doctors`.`doctor_name`, (select sum(hms_ipd_patient_to_charge.net_price) from hms_ipd_patient_to_charge join hms_ipd_booking as ipd_book1 on ipd_book1.id = hms_ipd_patient_to_charge.ipd_id where ipd_book1.discharge_status = 1 AND ipd_book1.is_deleted='0' AND hms_ipd_patient_to_charge.doctor_id = ".$post['doctor_id']." AND hms_ipd_patient_to_charge.is_deleted=0 AND hms_ipd_patient_to_charge.branch_id = ".$post['branch_id'].") as total_debit,(select sum(sub_pay.net_price) from hms_ipd_patient_to_charge as sub_pay join hms_ipd_booking as ipd_book on ipd_book.id = sub_pay.ipd_id where ipd_book.is_deleted='0' AND ipd_book.discharge_status = 1  AND sub_pay.doctor_id = ".$post['doctor_id']." AND sub_pay.doctor_id!=0 AND sub_pay.is_deleted=0 AND sub_pay.branch_id = ".$post['branch_id'].") as total_credit from hms_ipd_patient_to_charge LEFT JOIN `hms_doctors` ON `hms_doctors`.`id` = `hms_ipd_patient_to_charge`.`doctor_id`  
      LEFT JOIN `hms_doctors_to_comission` ON `hms_doctors_to_comission`.`doctor_id` = `hms_ipd_patient_to_charge`.`doctor_id`  
      LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_ipd_patient_to_charge`.`patient_id` LEFT JOIN `hms_ipd_booking` ON `hms_ipd_booking`.`id` = `hms_ipd_patient_to_charge`.`ipd_id`"); 
       ...
       $this->db->where('hms_ipd_patient_to_charge.is_deleted','0');
        ...
        if(!empty($post['branch_id']))
      {
              $this->db->where('hms_ipd_patient_to_charge.branch_id',$post['branch_id']); 
      }
      if(!empty($post['doctor_id']))
      {
              $this->db->where('hms_ipd_patient_to_charge.doctor_id',$post['doctor_id']); 
      }
      if(!empty($post['start_date']) && $post['start_date']!=date('d-m-Y'))
      {
              $this->db->where('hms_ipd_booking.discharge_date >= "'.date('Y-m-d', strtotime($post['start_date'])).'"'); 
      }
      if(!empty($post['end_date']) && $post['end_date']!=date('d-m-Y'))
      {
              $this->db->where('hms_ipd_booking.discharge_date <="'.date('Y-m-d h:i:s', strtotime($post['end_date'])).'"'); 
      } 
  ...
  public function doctor_commission_details($post=array())
  {
    ...
    { 
      ...
      // OPD Billing 
      $this->db->select("(CASE WHEN hms_doctors.id>0 THEN 'OPD Billing' ELSE ' ' END) as commission_type, hms_patient.patient_code, hms_patient.patient_name,hms_opd_booking.net_amount as bill_amount,hms_opd_booking.booking_date, hms_payment.total_comission as total_credit"); 
    ...
    sum(`hms_payment`.`total_comission`) as total_credit from hms_payment
      ...
      $this->db->select("(CASE WHEN hms_doctors.id>0 THEN 'Day Care' ELSE ' ' END) as commission_type, hms_patient.patient_code, hms_patient.patient_name,hms_day_care_booking.net_amount as bill_amount,hms_day_care_booking.booking_date, hms_payment.total_comission as total_credit"); 
