...
class Billing_model extends CI_Model {
  ...
  public function total_doctor_comission()
  {
    ...
    if(!empty($post))
    {
      ...
      $blood_bank_debit_condition = "";
      $blood_bank_credit_condition = "";
      if(!empty($post['start_date']))
      {
        $blood_bank_debit_condition .=  ' AND hms_blood_patient_to_recipient.created_date >= "'.date('Y-m-d', strtotime($post['start_date'])).' 00:00:00'.'"';
        $blood_bank_credit_condition .=  ' AND hms_blood_patient_to_recipient.created_date >= "'.date('Y-m-d', strtotime($post['start_date'])).' 00:00:00'.'"';
      }
      if(!empty($post['end_date']))
      {
        $blood_bank_debit_condition .=  ' AND hms_blood_patient_to_recipient.created_date <= "'.date('Y-m-d', strtotime($post['end_date'])).' 23:59:59'.'"';
        $blood_bank_credit_condition .=  ' AND hms_blood_patient_to_recipient.created_date <= "'.date('Y-m-d', strtotime($post['end_date'])).' 23:59:59'.'"';
      }
    ...
    $this->db->select("`hms_doctors`.`doctor_name`, (select sum(debit) from hms_payment where doctor_id = ".$post['doctor_id']." AND branch_id = ".$post['branch_id']." AND parent_id = 0 ".$debit_condition.") as total_debit, (CASE WHEN hms_doctors_to_comission.rate_type=0 THEN sum(hms_doctors_to_comission.rate) WHEN hms_doctors_to_comission.rate_type=1 THEN sum((hms_opd_booking.net_amount/100)*hms_doctors_to_comission.rate) ELSE 0 END) as total_credit from hms_opd_booking LEFT JOIN `hms_doctors` ON `hms_doctors`.`id` = `hms_opd_booking`.`referral_doctor` LEFT JOIN `hms_doctors_to_comission` ON `hms_doctors_to_comission`.`doctor_id` = `hms_opd_booking`.`referral_doctor` AND `hms_doctors_to_comission`.`dept_id` = 2 LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_opd_booking`.`patient_id` "); 
      ...
      //OT
        ...
        //OT comission //
          ...
          //blood bank comission //
          $this->db->select("`hms_doctors`.`doctor_name`, (select sum(debit) from hms_payment where doctor_id = ".$post['doctor_id']." AND branch_id = ".$post['branch_id']." AND parent_id = 0 ".$blood_bank_debit_condition.") as total_debit, 
          
        (CASE WHEN hms_doctors_to_comission.rate_type=0 THEN sum(hms_doctors_to_comission.rate) WHEN hms_doctors_to_comission.rate_type=1 THEN sum((hms_blood_patient_to_recipient.net_amount/100)*hms_doctors_to_comission.rate) ELSE 0 END)  as total_credit from hms_blood_patient_to_recipient 
          
          LEFT JOIN `hms_doctors` ON `hms_doctors`.`id` = `hms_blood_patient_to_recipient`.`doctor_id`  
          LEFT JOIN `hms_doctors_to_comission` ON `hms_doctors_to_comission`.`doctor_id` = `hms_blood_patient_to_recipient`.`doctor_id` AND `hms_doctors_to_comission`.`dept_id` = 24
          LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_blood_patient_to_recipient`.`patient_id` "); 

          $this->db->where('hms_doctors.doctor_pay_type',1); 

          if(!empty($post['branch_id']))
          {
          $this->db->where('hms_blood_patient_to_recipient.branch_id',$post['branch_id']); 
          }
          if(!empty($post['doctor_id']))
          {
          $this->db->where('hms_blood_patient_to_recipient.doctor_id',$post['doctor_id']); 
          }
          if(!empty($post['start_date']))
          {
          $this->db->where('hms_blood_patient_to_recipient.requirement_date >= "'.date('Y-m-d', strtotime($post['start_date'])).' 00:00:00'.'"'); 
          }
          if(!empty($post['end_date']))
          {
          $this->db->where('hms_blood_patient_to_recipient.requirement_date <="'.date('Y-m-d', strtotime($post['end_date'])).' 23:59:59'.'"'); 
          } 
      ...
      $sql =  $this->db->query("(".$sql1.") UNION ALL (".$sql2.") UNION ALL (".$sql3.") UNION ALL (".$sql4.") UNION ALL (".$sql5.") UNION ALL (".$sql6.") UNION ALL (".$sql7.") "); 
      //echo $this->db->last_query();die; 
    ...
    (CASE WHEN hms_doctors_to_comission.rate_type=0 THEN sum(hms_doctors_to_comission.rate) WHEN hms_doctors_to_comission.rate_type=1 THEN (((select sum(credit) from hms_payment as sub_pay join hms_ipd_booking as ipd_book on ipd_book.id = sub_pay.parent_id where ipd_book.is_deleted='0' AND ipd_book.discharge_status = 1 AND sub_pay.section_id = 5 AND sub_pay.parent_id = hms_payment.parent_id )/100)*hms_doctors_to_comission.rate) ELSE 0 END) as total_credit from hms_payment
      ...
      //pathology
        ...
        $this->db->select("(CASE WHEN `hms_doctors`.`id`>0 THEN 'Pathology' ELSE ' ' END) as commission_type, hms_patient.patient_code, hms_patient.patient_name,  path_test_booking.booking_date, 
          (select SUM(CASE WHEN hms_doctors_to_comission.rate_type=0 THEN hms_doctors_to_comission.rate 
          WHEN hms_doctors_to_comission.rate_type=1 THEN (path_test_booking_to_test.net_amount/100)*hms_doctors_to_comission.rate ELSE 0 END) as total_credit  from path_test_booking_to_test join path_test on path_test.id = path_test_booking_to_test.test_id Join hms_department on hms_department.id = path_test.dept_id join hms_doctors_to_comission on hms_doctors_to_comission.dept_id = hms_department.id AND hms_doctors_to_comission.doctor_id = '".$post['doctor_id']."' where path_test_booking.is_deleted ='0' AND path_test_booking_to_test.parent_status=1 AND path_test_booking_to_test.test_type= 0 AND  path_test_booking_to_test.booking_id = path_test_booking.id ".$path_credit_condition.")
          +
      ...
      //BLOOD COMISSION //
      $this->db->select("(CASE WHEN `hms_doctors`.`id`>0 THEN 'Blood Bank' ELSE '' END) as commission_type, hms_patient`.`patient_code`, `hms_patient`.`patient_name`,`hms_blood_patient_to_recipient`.`requirement_date` as `booking_date`,  
      (CASE WHEN hms_doctors_to_comission.rate_type=0 THEN sum(hms_doctors_to_comission.rate) WHEN hms_doctors_to_comission.rate_type=1 THEN sum((hms_blood_patient_to_recipient.net_amount/100)*hms_doctors_to_comission.rate) ELSE 0 END)  as total_credit from hms_blood_patient_to_recipient 
              LEFT JOIN `hms_doctors` ON `hms_doctors`.`id` = `hms_blood_patient_to_recipient`.`doctor_id`  
      JOIN `hms_doctors_to_comission` ON `hms_doctors_to_comission`.`doctor_id` = `hms_blood_patient_to_recipient`.`doctor_id` AND `hms_doctors_to_comission`.`dept_id` = 24
      LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_blood_patient_to_recipient`.`patient_id`"); 
      ...
      if(!empty($post['start_date']) && $post['start_date']!=date('d-m-Y'))
      {
              $this->db->where('hms_blood_patient_to_recipient.requirement_date >= "'.date('Y-m-d h:i:s', strtotime($post['start_date'])).'"'); 
      }
      if(!empty($post['end_date']) && $post['end_date']!=date('d-m-Y'))
      {
              $this->db->where('hms_blood_patient_to_recipient.requirement_date <="'.date('Y-m-d h:i:s', strtotime($post['end_date'])).'"'); 
      } 
      $this->db->group_by('hms_blood_patient_to_recipient.patient_id');  //hms_doctors.id
      $query = $this->db->get();
      $sql7 = $this->db->last_query();
      //BLOOD COMISSION //
    ...
    $this->db->select("`hms_hospital`.`hospital_name`, (select sum(debit) from hms_payment where hospital_id = ".$post['hospital_id']." AND branch_id = ".$post['branch_id']." AND parent_id = 0 ".$debit_condition.") as total_debit, (CASE WHEN hms_hospital_to_comission.rate_type=0 THEN sum(hms_hospital_to_comission.rate) WHEN hms_hospital_to_comission.rate_type=1 THEN sum((hms_opd_booking.net_amount/100)*hms_hospital_to_comission.rate) ELSE 0 END) as total_credit from hms_opd_booking LEFT JOIN `hms_hospital` ON `hms_hospital`.`id` = `hms_opd_booking`.`referral_hospital` LEFT JOIN `hms_hospital_to_comission` ON `hms_hospital_to_comission`.`hospital_id` = `hms_opd_booking`.`referral_hospital` AND `hms_hospital_to_comission`.`dept_id` = 2 LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_opd_booking`.`patient_id` "); 
      ...
      //Pathology
        ...
        //BLOOD BANK COMISSION//
              ...
              $this->db->select("`hms_hospital`.`hospital_name`, (select sum(debit) from hms_payment where hospital_id = ".$post['hospital_id']." AND branch_id = ".$post['branch_id']." AND parent_id = 0 ".$blood_bank_debit_condition.") as total_debit, 
        (CASE WHEN hms_hospital_to_comission.rate_type=0 THEN sum(hms_hospital_to_comission.rate) WHEN hms_hospital_to_comission.rate_type=1 THEN sum((hms_blood_patient_to_recipient.net_amount/100)*hms_hospital_to_comission.rate) ELSE 0 END)  as total_credit from hms_blood_patient_to_recipient 
                LEFT JOIN `hms_hospital` ON `hms_hospital`.`id` = `hms_blood_patient_to_recipient`.`hospital_id`  
        LEFT JOIN `hms_hospital_to_comission` ON `hms_hospital_to_comission`.`hospital_id` = `hms_blood_patient_to_recipient`.`hospital_id` AND `hms_hospital_to_comission`.`dept_id` = 24
        LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_blood_patient_to_recipient`.`patient_id` "); 
        ...
        if(!empty($post['hospital_id']))
        {
                $this->db->where('hms_blood_patient_to_recipient.hospital_id',$post['hospital_id']); 
        }
        if(!empty($post['start_date']))
        {
                $this->db->where('hms_blood_patient_to_recipient.requirement_date >= "'.date('Y-m-d', strtotime($post['start_date'])).' 00:00:00'.'"'); 
    ...
    (CASE WHEN hms_hospital_to_comission.rate_type=0 THEN sum(hms_hospital_to_comission.rate) WHEN hms_hospital_to_comission.rate_type=1 THEN (((select sum(credit) from hms_payment as sub_pay join hms_ipd_booking as ipd_book on ipd_book.id = sub_pay.parent_id where ipd_book.is_deleted='0' AND ipd_book.discharge_status = 1 AND sub_pay.section_id = 5 AND sub_pay.parent_id = hms_payment.parent_id )/100)*hms_hospital_to_comission.rate) ELSE 0 END) as total_credit from hms_payment
      ...
      //pathology
        ...
        $this->db->select("(CASE WHEN `hms_hospital`.`id`>0 THEN 'Pathology' ELSE ' ' END) as commission_type, hms_patient.patient_code, hms_patient.patient_name,  path_test_booking.booking_date, 
          (select SUM(CASE WHEN hms_hospital_to_comission.rate_type=0 THEN hms_hospital_to_comission.rate 
          WHEN hms_hospital_to_comission.rate_type=1 THEN (path_test_booking_to_test.net_amount/100)*hms_hospital_to_comission.rate ELSE 0 END) as total_credit  from path_test_booking_to_test join path_test on path_test.id = path_test_booking_to_test.test_id Join hms_department on hms_department.id = path_test.dept_id join hms_hospital_to_comission on hms_hospital_to_comission.dept_id = hms_department.id AND hms_hospital_to_comission.hospital_id = '".$post['hospital_id']."' where path_test_booking.is_deleted ='0' AND path_test_booking_to_test.parent_status=1 AND path_test_booking_to_test.test_type= 0 AND  path_test_booking_to_test.booking_id = path_test_booking.id ".$path_credit_condition.")
          +
      ...
      //BLOOD BANK HOSPITAL COMISSION//
         $this->db->select("(CASE WHEN `hms_hospital`.`id`>0 THEN 'Blood Bank' ELSE '' END) as commission_type, hms_patient`.`patient_code`, `hms_patient`.`patient_name`,`hms_blood_patient_to_recipient`.`requirement_date` as `booking_date`,  
      (CASE WHEN hms_hospital_to_comission.rate_type=0 THEN sum(hms_hospital_to_comission.rate) WHEN hms_hospital_to_comission.rate_type=1 THEN sum((hms_blood_patient_to_recipient.net_amount/100)*hms_hospital_to_comission.rate) ELSE 0 END)  as total_credit from hms_blood_patient_to_recipient 
              LEFT JOIN `hms_hospital` ON `hms_hospital`.`id` = `hms_blood_patient_to_recipient`.`hospital_id`  
      LEFT JOIN `hms_hospital_to_comission` ON `hms_hospital_to_comission`.`hospital_id` = `hms_blood_patient_to_recipient`.`hospital_id` AND `hms_hospital_to_comission`.`dept_id` = 24
      LEFT JOIN `hms_patient` ON `hms_patient`.`id` = `hms_blood_patient_to_recipient`.`patient_id`"); 
