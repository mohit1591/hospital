...
class Type_model extends CI_Model {
	...
	var $table = 'hms_medicine_unit';
	var $column = array('hms_medicine_unit.id','hms_medicine_unit.medicine_unit', 'hms_medicine_unit.status','hms_medicine_unit.created_date');  
	...
	private function _get_datatables_query()
	{
		...
		//$this->db->where('hms_medicine_unit.type','1');
	...
	public function get_by_id($id)
	{
		...
		$this->db->where('hms_medicine_unit.id',$id);
		$this->db->where('hms_medicine_unit.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
            if(isset($post['medicine_type']) && !empty($post['medicine_type']))
				{
					// $unit= $this->check_unit($post['medicine_type']);
					// if(!empty($unit))
					// {
					// $unit_id= $unit[0]->id;
					// }
					...
					// {
					     $data_unit = array( 
							...
							$this->db->update('hms_medicine_unit',$data_unit);
		...
		{    
			$result_new_unit= $post['medicine_type'];
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->update('hms_medicine_unit');
			//echo $this->db->last_query();die;
    	} 
    }
