...
$blance ='';
		...
		if($post['total_advance_discount']>$post['total_amount'])
		{
            ...
            if(!empty($post['total_balance']) || $post['total_balance']==0)
            {
                $blance =1;
            }
            else
            {
                $blance =$post['total_balance'];
            }
			...
			$payment_data = array(
						...
						'credit'=>0,
						'debit'=>str_replace(',', '', $post['total_refund_amount']),
            ...
            if(!empty($post['ipd_id']) && !empty($post['patient_id']))
			{
				$this->db->where(array('branch_id'=>$user_data['parent_id'],'patient_id'=>$post['patient_id'],'parent_id'=>$post['ipd_id'],'type'=>6,'section_id'=>5));
				$this->db->delete('hms_payment');
    		...
    		$payment_data = array(
									...
									'net_amount'=>str_replace(',', '', $post['total_net_amount']),
									'credit'=>0,
									'debit'=>str_replace(',', '', $post['total_net_amount']),
									'pay_mode'=>$post['payment_mode'],
									'balance'=>$blance,
									'paid_amount'=>$post['total_paid_amount'],
									'created_date'=>date('Y-m-d H:i:s'),
