...
class Day_discharge_summary_template_model extends CI_Model {

    var $table = 'hms_daycare_discharge_summery_template';
    var $column = array('hms_daycare_discharge_summery_template.id','hms_daycare_discharge_summery_template.name', 'hms_daycare_discharge_summery_template.status','hms_daycare_discharge_summery_template.created_date');   
    ...
    {
        $this->db->select('hms_daycare_discharge_summery_template.*');
        $this->db->from('hms_daycare_discharge_summery_template'); 
        $this->db->where('hms_daycare_discharge_summery_template.id',$id);
        $this->db->where('hms_daycare_discharge_summery_template.is_deleted','0');
        $query = $this->db->get(); 
    ...
    public function save()
    {
        ...
        {    
            ...
            $this->db->update('hms_daycare_discharge_summery_template',$data);  
        }
        else{    
            $this->db->set('created_by',$user_data['id']);
            $this->db->set('created_date',date('Y-m-d H:i:s'));
            $this->db->insert('hms_daycare_discharge_summery_template',$data);               
        }   
    }
    ...
    public function delete($id="")
    {
        if(!empty($id) && $id>0)
        {
            ...
            $this->db->where('id',$id);
            $this->db->update('hms_daycare_discharge_summery_template');
            //echo $this->db->last_query();die;
        } 
    }
