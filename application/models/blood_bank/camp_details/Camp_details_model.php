...
class Camp_details_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
		...
		// $this->db->join('hms_employees','hms_employees.id=hms_blood_camp_details.camp_involved','left');
        $this->db->where('hms_blood_camp_details.is_deleted','0');
	...
	public function get_by_id($id)
	{
		...
		$this->db->where('hms_blood_camp_details.id',$id);
		$this->db->where('hms_blood_camp_details.is_deleted','0');
		$query = $this->db->get(); 
		return $query->row_array();
	}
	...
	else
	{
       ...
       if(!empty($post['start_date']))
		...
		{    
			...
			$this->db->update('hms_blood_camp_details',$data);  
		}
		else{    
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_blood_camp_details',$data); 
			//echo $this->db->last_query();die;              
		} 	
	}
