...
class Bag_type_to_component_archive_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
        ...
        $this->db->where('hms_blood_bag_type_to_component.is_deleted','1');
    ...
    public function restoreall($ids=array())
    {
    	if(!empty($ids))
    	{
			...
			$this->db->set('deleted_by',$user_data['id']);
			$this->db->set('deleted_date',date('Y-m-d H:i:s'));
			$this->db->where('bag_type_id IN ('.$emp_ids.')');
			$this->db->update('hms_blood_bag_type_to_component');
    	} 
    }
