...
class Issue_report_model extends CI_Model {
    ...
    private function _get_datatables_query()
    {
        ...
        $search_data = $this->session->userdata('issue_report_search_data');
           /* for user code */
        ...
        $this->db->select("hms_blood_patient_to_recipient.id, hms_blood_patient_to_recipient.issue_code,hms_payment.debit as paid_amount,hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.section_id,hms_payment.balance as blnce,hms_payment.id as pay_id,hms_blood_patient_to_recipient.requirement_date,  (CASE WHEN hms_payment.balance>0 THEN hms_blood_patient_to_recipient.net_amount ELSE '0.00' END) total_amount,(CASE WHEN hms_payment.balance > 0 THEN (hms_blood_patient_to_recipient.discount_amount) ELSE '0.00' END) as discount,(CASE WHEN hms_payment.balance > 0 THEN (hms_blood_patient_to_recipient.discount_amount) ELSE '0.00' END) as discount,  
            ...
            , hms_patient.patient_name,hms_patient.id as patient_new_id, (CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 10 AND payment.parent_id = hms_blood_patient_to_recipient.id) ELSE '0.00' END) as balance,hms_blood_component_master.component,hms_blood_patient_to_recipient_components.comp_unite,hms_blood_donor.donor_name,hms_blood_group.blood_group ,hms_blood_donor.donor_code,hms_patient.patient_code,hms_blood_patient_to_recipient_components.qty as unit_qty");  
        ...
        $this->db->join('hms_blood_patient_to_recipient_components','hms_blood_patient_to_recipient_components.receipent_id = hms_blood_patient_to_recipient.id','left'); 
        
        $this->db->join('hms_blood_component_master','hms_blood_component_master.id=hms_blood_patient_to_recipient_components.component_id','left');
        
        $this->db->join('hms_blood_stock','hms_blood_stock.recipient_id=hms_blood_patient_to_recipient.id','left');
        ...
        if(isset($search_data) && !empty($search_data))
        {
            ...
            {
                $this->db->where('hms_blood_patient_to_recipient_components.component_id' ,$search_data['component_id']);
            }
    ...
    function search_report_data34()
    {
        ...
        else
        {
         ...
         $this->db->select("hms_blood_patient_to_recipient.id, hms_blood_patient_to_recipient.issue_code,hms_payment.debit as paid_amount,hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.section_id,hms_payment.balance as blnce,hms_payment.id as pay_id,hms_blood_patient_to_recipient.requirement_date,  (CASE WHEN hms_payment.balance>0 THEN hms_blood_patient_to_recipient.net_amount ELSE '0.00' END) total_amount,(CASE WHEN hms_payment.balance > 0 THEN (hms_blood_patient_to_recipient.discount_amount) ELSE '0.00' END) as discount,(CASE WHEN hms_payment.balance > 0 THEN (hms_blood_patient_to_recipient.discount_amount) ELSE '0.00' END) as discount,  
            (CASE WHEN hms_blood_patient_to_recipient.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_blood_patient_to_recipient.referred_by=0 THEN concat('Other ',hms_blood_patient_to_recipient.doctor_id) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name
            , hms_patient.patient_name,hms_patient.id as patient_new_id, (CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 10 AND payment.parent_id = hms_blood_patient_to_recipient.id) ELSE '0.00' END) as balance,hms_blood_component_master.component,hms_blood_patient_to_recipient_components.comp_unite,hms_blood_donor.donor_name,hms_blood_group.blood_group ,hms_blood_donor.donor_code");  
