...
class Sales_vaccination_model extends CI_Model 
{
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_vaccination_sale.*, hms_patient.patient_name, (select count(id) from hms_vaccination_sale_to_vaccination where sales_id = hms_vaccination_sale.id) as total_medicine,

			(CASE WHEN hms_vaccination_sale.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_vaccination_sale.refered_id=0 THEN concat('Other ',hms_vaccination_sale.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name
		...
		{ 
			$this->db->where('hms_vaccination_sale.created_by IN ('.$emp_ids.')');
		}
	...
	public function save()
	{
		...
		if(!empty($blance) || $blance==0)
		{
			...
			/*$payment_data = array(
						...
						'balance'=>$blance,
						'paid_amount'=>$post['pay_amount'],
						'created_date'=>$created_date,//date('Y-m-d H:i:s'),
						'created_by'=>$user_data['id']
					);
            ...
            if(!empty($row_d_pay))
            {
                foreach($row_d_pay as $row_d)
                {
                    ...
                    $comission_type='';
                    $total_comission=0;
                    if(!empty($post['referral_hospital']) || !empty($post['refered_id']))
                    {
                       $comission_arr = get_doc_hos_comission($post['refered_id'],$post['referral_hospital'],$post['pay_amount'],97); 
		            ...
		            $payment_data = array(
						...
						'created_date'=>$row_d->created_date,//$created_date,//date('Y-m-d H:i:s'),
