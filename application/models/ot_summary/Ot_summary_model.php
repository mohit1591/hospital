...
class Ot_summary_model extends CI_Model {

	var $table = 'hms_operation_summery';
	var $column = array('hms_operation_summery.id','hms_operation_summery.name','hms_operation_summery.diagnosis','hms_operation_summery.operation','hms_operation_summery.op_findings','hms_operation_summery.procedures','hms_operation_summery.pos_op_orders', 'hms_operation_summery.status','hms_operation_summery.created_date');  
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_operation_summery.*,hms_ot_management.name as operation_name"); 
		$this->db->from($this->table); 
        $this->db->where('hms_operation_summery.is_deleted','0');
        $this->db->where('hms_operation_summery.branch_id = "'.$users_data['parent_id'].'"');
         $this->db->join('hms_ot_management','hms_ot_management.id=hms_operation_summery.operation');
		$i = 0;
    ...
    public function ot_summary_list()
    {
    	...
    	$query = $this->db->get('hms_operation_summery');
		return $query->result();
    }
	...
	public function get_by_id($id)
	{
		...
		$this->db->where('hms_operation_summery.id',$id);
		$this->db->where('hms_operation_summery.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
			...
			$this->db->update('hms_operation_summery',$data); 
			...
			$this->db->insert('hms_operation_summery',$data);  
		...
		if($medicine_data!="" && !empty($medicine_data))
          {
            ...
            foreach($medicine_data as $data)
            {
                ...
                if($data['medicine_name']!="" && !empty($data['medicine_name']) )
                {
                  ...
                  $this->db->insert('hms_operation_summery_to_medicine',$medicine_array);  
	...
	public function delete_operation_summary_medicines($ot_summary_id)
    {
    	...
    	$this->db->where('branch_id',$user_data['parent_id']);
    	$this->db->delete('hms_operation_summery_to_medicine');
    }
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->update('hms_operation_summery');
			//echo $this->db->last_query();die;
    	} 
    }
    ...
    public function get_vals($vals="")
    {
    	...
    	if(!empty($vals))
    	{
	        ...
	        $query = $this->db->get('hms_operation_summery_type');
	        $result = $query->result(); 
    ...
    public function get_summary_medicine_data($summary_data_id)
    {
    	...
    	$this->db->from('hms_operation_summery_to_medicine');
    	$this->db->where('hms_operation_summery_to_medicine.branch_id',$users_data['parent_id']);
    	$this->db->where('hms_operation_summery_to_medicine.summary_id',$summary_data_id);
    	$res=$this->db->get();
    	if($res->num_rows() > 0)
    		return $res->result();
    	else
