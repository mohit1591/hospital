...
class Medicinequantityreport_model extends CI_Model {
...
*/
    ...
    function medicinequantityreport_list($get="") 
    {
        ...
        $this->db->select(
            "hms_medicine_entry.medicine_code,
            hms_medicine_entry.medicine_name as med_name,
            hms_medicine_entry.id,
            hms_medicine_unit.medicine_unit,
            hms_medicine_stock.batch_no
            ");
        ...
        $this->db->order_by('hms_medicine_unit.medicine_unit asc');
        $this->db->order_by('hms_medicine_entry.medicine_name asc');
        $data['medicine_list']= $this->db->get()->result_array();
        // echo "<pre>"; print_r(count($data['medicine_list']));die;
        // echo $this->db->last_query();die;
        $data_array=[];
        $i=0;
        ...
        foreach($data['medicine_list'] as $medicine_list)
        {
            ...
            $data_array[$i]['medicine_name']=$medicine_list['med_name'];
            $data_array[$i]['med_name']=$medicine_list['med_name'];
            $data_array[$i]['medicine_code']=$medicine_list['medicine_code'];
            $data_array[$i]['medicine_unit']=$medicine_list['medicine_unit'];
            ...
            if($type=1)
            {
                // (sum(debit)-sum(credit)) as total_qty,
                $this->db->select("
                SUM(CASE WHEN hms_medicine_stock.type = 3 THEN (credit - debit) ELSE 0 END) as total_qty_sell,
                SUM(CASE WHEN hms_medicine_stock.type = 1 THEN (debit) ELSE 0 END) as total_qty_purchase,
                sum(total_amount) as total_amt");
                $this->db->where('branch_id',$user_data['parent_id']);
                $this->db->where("hms_medicine_stock.parent_id NOT IN(SELECT `hms_medicine_purchase`.`id` FROM `hms_medicine_purchase` WHERE `hms_medicine_purchase`.`is_deleted` = 2 AND hms_medicine_purchase.branch_id=".$user_data['parent_id'].")");   
                ...
                $this->db->select("sum(hms_medicine_stock.debit)- sum(hms_medicine_stock.credit) as total_qty, batch_no, mrp, expiry_date");//,sum(total_amount) as total_amt
                $this->db->where('branch_id',$user_data['parent_id']);
                $this->db->where("hms_medicine_stock.parent_id NOT IN(SELECT `hms_medicine_purchase`.`id` FROM `hms_medicine_purchase` WHERE `hms_medicine_purchase`.`is_deleted` = 2 AND hms_medicine_purchase.branch_id=".$user_data['parent_id'].")");   
                ...
                // $this->db->where('hms_medicine_stock.type',1);
                $this->db->where('hms_medicine_stock.m_id',$medicine_list['id']);
                $this->db->where('hms_medicine_stock.batch_no',$medicine_list['batch_no']);
                $this->db->where('hms_medicine_stock.branch_id',$user_data['parent_id']);
                $this->db->group_by("batch_no");
                $this->db->group_by('hms_medicine_stock.m_id');
                $customInfo = $this->db->get('hms_medicine_stock')->row_array();
                // print_r($customInfo);die;
                $data_array[$i]['purchsase_quantity']['total_qty'] = $customInfo['total_qty'];
                $data_array[$i]['purchsase_quantity']['batch_no'] = $customInfo['batch_no'];
                $data_array[$i]['purchsase_quantity']['mrp'] = $customInfo['mrp'];
                $data_array[$i]['purchsase_quantity']['expiry_date'] = $customInfo['expiry_date'];
            ...
            if($type=2)
            {
                ...
                $this->db->where('branch_id',$user_data['parent_id']);
                $this->db->where("hms_medicine_stock.parent_id NOT IN(SELECT `hms_medicine_return`.`id` FROM `hms_medicine_return` WHERE `hms_medicine_return`.`is_deleted` = 2 AND `hms_medicine_return`.`branch_id` =".$user_data['parent_id'].")");
                
                    $this->db->where('hms_medicine_stock.type',2);
                    $this->db->where('hms_medicine_stock.m_id',$medicine_list['id']);
                    $this->db->where('hms_medicine_stock.branch_id',$user_data['parent_id']);
                    if(!empty($get['start_date']))
                    {
                    ...
                    $data_array[$i]['purchsase_return_quantity'] = $this->db->get('hms_medicine_stock')->row_array();
            ...
            if($type=3)
            {
                ...
                $this->db->where("hms_medicine_stock.parent_id NOT IN(SELECT `hms_medicine_sale`.`id` FROM `hms_medicine_sale` WHERE `hms_medicine_sale`.`is_deleted` = 2 AND `hms_medicine_sale`.`branch_id` =".$user_data['parent_id'].")");
            ...
            if($type=4)
            {
                    $this->db->select("(sum(debit)-sum(credit)) as total_qty,sum(total_amount) as total_amt");
                    $this->db->where('branch_id',$user_data['parent_id']);
                    $this->db->where("hms_medicine_stock.parent_id NOT IN(SELECT `hms_medicine_sale_return`.`id` FROM `hms_medicine_sale_return` WHERE `hms_medicine_sale_return`.`is_deleted` = 2 AND `hms_medicine_sale_return`.`branch_id` = ".$user_data['parent_id'].")");
