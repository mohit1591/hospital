...
class Advance_payment_model extends CI_Model {
	...
	{
			$search = $this->session->userdata('advance_summary_serach');
			$users_data = $this->session->userdata('auth_users');
			///////////////////////
			
			$this->db->select("hms_ipd_booking.id,hms_payment.type, hms_payment.debit as net_price, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.paid_amount as paid_amount_payment, hms_payment.id as pay_id,hms_ipd_booking.ipd_no, hms_ipd_booking.admission_date,hms_patient.patient_name,hms_patient.id as patient_new_id,hms_payment.reciept_prefix,hms_payment.reciept_suffix,hms_patient.patient_code,hms_payment_mode.payment_mode");  
			...
			///////////////////
			/*$this->db->select("hms_ipd_patient_to_charge.*,hms_ipd_booking.ipd_no,hms_ipd_booking.admission_date,hms_patient.patient_name,hms_ipd_patient_to_charge.patient_id,hms_patient.patient_name,hms_patient.patient_code,hms_branch_hospital_no.reciept_prefix,hms_branch_hospital_no.reciept_suffix,hms_payment_mode.payment_mode");
        	...
        	$this->db->join('hms_branch_hospital_no','hms_branch_hospital_no.payment_id = hms_payment.id AND hms_branch_hospital_no.section_id=7 AND hms_branch_hospital_no.branch_id='.$users_data['parent_id'],'left');
	...
	public function search_report_data()
	{
		...
		$new_self_opd_array=array();
			...
			$new_self_advance_array['self_advance_payment']=$this->db->get()->result();
			
			
			
			/*$this->db->select("sum(hms_ipd_patient_to_charge.net_price) as tot_debit,hms_ipd_booking.ipd_no,hms_ipd_booking.admission_date,hms_patient.patient_name,hms_ipd_patient_to_charge.patient_id,hms_patient.patient_name,hms_patient.patient_code,hms_branch_hospital_no.reciept_prefix,hms_branch_hospital_no.reciept_suffix,hms_payment_mode.payment_mode");
			$this->db->from($this->table); 
			$this->db->where('hms_ipd_patient_to_charge.is_deleted','0');
            $this->db->where('hms_ipd_booking.is_deleted!=','2');
			$this->db->where('hms_ipd_patient_to_charge.type','2');
			...
			$this->db->select("sum(hms_payment.debit) as net_price,hms_payment.section_id,hms_payment.paid_amount as paid_amount_payment,hms_payment_mode.payment_mode");  
    		$this->db->join('hms_ipd_booking','hms_ipd_booking.id = hms_payment.parent_id AND hms_ipd_booking.is_deleted !=2'); 
            $this->db->join('hms_patient','hms_patient.id = hms_ipd_booking.patient_id');
            	$this->db->join('hms_payment_mode','hms_payment_mode.id=hms_payment.pay_mode','left');
			...
			$new_self_advance_array['self_advance_payment_mode']=$this->db->get()->result();
			//echo $this->db->last_query();die;
			//print_r($result);die;
			return $new_self_advance_array;
		...
		{    
			...
			*/
			$payment_data = array(
								...
								'created_date'=>$row_d->created_date,//date('Y-m-d H:i:s'),
