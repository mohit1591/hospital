...
class Vaccine_purchase_model extends CI_Model 
{
	...
	private function _get_datatables_query()
	{
		...
		//$this->db->where('hms_vaccination_purchase.branch_id = "'.$user_data['parent_id'].'"');  hms_vaccination_entry
		...
		{ 
			$this->db->where('hms_vaccination_purchase.created_by IN ('.$emp_ids.')');
		}
		...
		{	
			...
			{
				if(empty($search['vaccination_code']) && empty($search['vaccination_company']) && empty($search['vaccination_name'])  && empty($search['discount'])  && empty($search['cgst'])  && empty($search['igst']) && empty($search['sgst']) && empty($search['batch_no']) && empty($search['unit2']))
				{
					$this->db->join('hms_vaccination_purchase_to_purchase','hms_vaccination_purchase_to_purchase.purchase_id = hms_vaccination_purchase.id','left'); 
			...
			{
				if(empty($search['vaccination_code']) && empty($search['vaccination_company']) && empty($search['vaccination_name'])  && empty($search['discount'])  && empty($search['cgst'])  && empty($search['igst']) && empty($search['sgst']) && empty($search['batch_no']) && empty($search['packing']) && empty($search['unit2']) && empty($search['unit1']))
				{
					$this->db->join('hms_vaccination_purchase_to_purchase','hms_vaccination_purchase_to_purchase.purchase_id = hms_vaccination_purchase.id','left'); 
					$this->db->join('hms_vaccination_entry','hms_vaccination_entry.id = hms_vaccination_purchase_to_purchase.vaccine_id','left');
				} 
	...
	function search_report_data(){
		...
		if(isset($search) && !empty($search))
		{
			...
			{
				if(empty($search['vaccination_code']) && empty($search['vaccination_company']) && empty($search['vaccination_name'])  && empty($search['discount'])  && empty($search['cgst'])  && empty($search['igst']) && empty($search['sgst']) && empty($search['batch_no']) && empty($search['packing'])&& empty($search['conversion']) && empty($search['mrp_from']) && empty($search['unit2']) && empty($search['unit1']))
				{
					$this->db->join('hms_vaccination_purchase_to_purchase','hms_vaccination_purchase_to_purchase.purchase_id = hms_vaccination_purchase.id','left'); 
					$this->db->join('hms_vaccination_entry','hms_vaccination_entry.id = hms_vaccination_purchase_to_purchase.vaccine_id','left'); 
                }
			...
			{
				if(empty($search['vaccination_code']) && empty($search['vaccination_company']) && empty($search['vaccination_name'])  && empty($search['discount'])  && empty($search['cgst'])  && empty($search['igst']) && empty($search['sgst']) && empty($search['batch_no']) && empty($search['packing'])&& empty($search['conversion']) && empty($search['mrp_to']) && empty($search['unit2']) && empty($search['unit1']))
				{
   ...
   public function medicine_list($ids="")
	...
	function get_all_detail_print($ids="",$branch_id=""){
    	...
    	$result_sales=array();
    	//$user_data = $this->session->userdata('auth_users');
		$this->db->select("hms_vaccination_purchase.*,hms_medicine_vendors.*,hms_medicine_vendors.email as v_email,hms_medicine_vendors.address as v_address,hms_users.*,(CASE WHEN hms_users.emp_id>0 THEN hms_employees.name ELSE hms_branch.branch_name END) as user_name"); 
		$this->db->join('hms_medicine_vendors','hms_medicine_vendors.id = hms_vaccination_purchase.vendor_id','left'); 
