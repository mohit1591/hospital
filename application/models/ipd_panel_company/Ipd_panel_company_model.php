...
class Ipd_panel_company_model extends CI_Model {
	...
	{
		$this->db->select('hms_ipd_panel_company.*');
		$this->db->from('hms_ipd_panel_company'); 
		$this->db->where('hms_ipd_panel_company.id',$id);
		$this->db->where('hms_ipd_panel_company.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
			...
			$this->db->update('hms_ipd_panel_company',$data);  
		}
		else{    
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->update('hms_ipd_panel_company');
			//echo $this->db->last_query();die;
    	} 
    }
