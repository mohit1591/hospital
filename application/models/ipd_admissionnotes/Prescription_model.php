...
{
    var $table = 'hms_ipd_patient_admission_prescription';
    var $column = array('hms_ipd_patient_admission_prescription.id','hms_ipd_patient_admission_prescription.ipd_no','hms_ipd_patient_admission_prescription.patient_name', 'hms_ipd_patient_admission_prescription.mobile_no','hms_ipd_patient_admission_prescription.patient_code', 'hms_ipd_patient_admission_prescription.status', 'hms_ipd_patient_admission_prescription.created_date', 'hms_ipd_patient_admission_prescription.modified_date');  
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->select("hms_ipd_patient_admission_prescription.*,hms_ipd_admission_prescription_patient_test.test_name,hms_ipd_admission_prescription_patient_pres.medicine_name,hms_ipd_admission_prescription_patient_pres.medicine_salt,hms_ipd_admission_prescription_patient_pres.medicine_brand,hms_ipd_admission_prescription_patient_pres.medicine_type,hms_ipd_admission_prescription_patient_pres.medicine_dose,hms_ipd_admission_prescription_patient_pres.medicine_duration,hms_ipd_admission_prescription_patient_pres.medicine_frequency,hms_ipd_admission_prescription_patient_pres.medicine_advice"); 
        $this->db->join('hms_ipd_admission_prescription_patient_test','hms_ipd_admission_prescription_patient_test.prescription_id=hms_ipd_patient_admission_prescription.id','left');
        $this->db->join('hms_ipd_admission_prescription_patient_pres','hms_ipd_admission_prescription_patient_pres.prescription_id=hms_ipd_patient_admission_prescription.id','left');
        
        $this->db->where('hms_ipd_patient_admission_prescription.is_deleted','0'); 
        

        if(isset($search['branch_id']) && $search['branch_id']!=''){
        $this->db->where('hms_ipd_patient_admission_prescription.branch_id IN ('.$search['branch_id'].')');
        }else{
        $this->db->where('hms_ipd_patient_admission_prescription.branch_id = "'.$user_data['parent_id'].'"');
        }
        ...
        if(isset($search) && !empty($search))
        {
            ...
            if(!empty($search['start_date']))
            {
                ...
                $this->db->where('hms_ipd_patient_admission_prescription.created_date >= "'.$start_date.'"');
            }

            if(!empty($search['end_date']))
            {
                $end_date = date('Y-m-d',strtotime($search['end_date'])).' 23:59:59';
                $this->db->where('hms_ipd_patient_admission_prescription.created_date <= "'.$end_date.'"');
            }

            
            if(!empty($search['patient_name']))
            {
                $this->db->where('hms_ipd_patient_admission_prescription.patient_name LIKE "'.$search['patient_name'].'%"');
            }

            if(!empty($search['patient_code']))
            {
                $this->db->where('hms_ipd_patient_admission_prescription.patient_code',$search['patient_code']);
            }

            if(!empty($search['mobile_no']))
            {
                $this->db->where('hms_ipd_patient_admission_prescription.mobile_no LIKE "'.$search['mobile_no'].'%"');
            }
        ...
        { 
            $this->db->where('hms_ipd_patient_admission_prescription.created_by IN ('.$emp_ids.')');
        }
        /////// Search query end //////////////
        ...
        foreach ($this->column as $item) // loop column 
        {
         ...
         $this->db->group_by('hms_ipd_patient_admission_prescription.id');
    ...
    {
        $this->db->select("hms_ipd_patient_admission_prescription.*,hms_ipd_admission_prescription_patient_test.test_name,hms_ipd_admission_prescription_patient_pres.medicine_name,hms_ipd_admission_prescription_patient_pres.medicine_salt,hms_ipd_admission_prescription_patient_pres.medicine_brand,hms_ipd_admission_prescription_patient_pres.medicine_type,hms_ipd_admission_prescription_patient_pres.medicine_dose,hms_ipd_admission_prescription_patient_pres.medicine_duration,hms_ipd_admission_prescription_patient_pres.medicine_frequency,hms_ipd_admission_prescription_patient_pres.medicine_advice"); 
        $this->db->from('hms_ipd_patient_admission_prescription'); 
        $this->db->where('hms_ipd_patient_admission_prescription.id',$id);
        $this->db->where('hms_ipd_patient_admission_prescription.is_deleted','0'); 
        $this->db->join('hms_ipd_admission_prescription_patient_test','hms_ipd_admission_prescription_patient_test.prescription_id=hms_ipd_patient_admission_prescription.id','left');
        $this->db->join('hms_ipd_admission_prescription_patient_pres','hms_ipd_admission_prescription_patient_pres.prescription_id=hms_ipd_patient_admission_prescription.id','left');
    ...
    { 
        $this->db->select("hms_ipd_patient_admission_prescription.*,hms_patient.patient_code,hms_patient.id as patient_id,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.gender,hms_patient.dob,hms_patient.age,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_patient.simulation_id,hms_patient.address,hms_patient.city_id,hms_patient.state_id,hms_patient.country_id,hms_patient.address,hms_patient.pincode,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_patient.modified_date as patient_modified_date,hms_patient.dob"); 
        $this->db->from('hms_ipd_patient_admission_prescription'); 
        $this->db->where('hms_ipd_patient_admission_prescription.id',$prescription_id);
        $this->db->join('hms_patient','hms_patient.id=hms_ipd_patient_admission_prescription.patient_id','left');
        
        $result_pre['prescription_list']= $this->db->get()->result();
        
        $this->db->select('hms_ipd_admission_prescription_patient_test.*');
        $this->db->join('hms_ipd_patient_admission_prescription','hms_ipd_patient_admission_prescription.id = hms_ipd_admission_prescription_patient_test.prescription_id'); 
        $this->db->where('hms_ipd_admission_prescription_patient_test.prescription_id = "'.$prescription_id.'"');
        $this->db->from('hms_ipd_admission_prescription_patient_test');
        $result_pre['prescription_list']['prescription_test_list']=$this->db->get()->result();

        $this->db->select('hms_ipd_admission_prescription_patient_pres.*');
        $this->db->join('hms_ipd_patient_admission_prescription','hms_ipd_patient_admission_prescription.id = hms_ipd_admission_prescription_patient_pres.prescription_id'); 
        $this->db->where('hms_ipd_admission_prescription_patient_pres.prescription_id = "'.$prescription_id.'"');
        $this->db->from('hms_ipd_admission_prescription_patient_pres');
    ...
    {
        $this->db->select("hms_ipd_patient_admission_prescription.*,hms_patient.patient_code,hms_patient.id as patient_id,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.gender,hms_patient.dob,hms_patient.age,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_patient.simulation_id,hms_patient.address,hms_patient.city_id,hms_patient.state_id,hms_patient.country_id,hms_patient.address,hms_patient.pincode,hms_ipd_booking.ipd_no,hms_ipd_booking.admission_date,hms_ipd_booking.referral_doctor,hms_ipd_booking.attend_doctor_id,hms_ipd_booking.admission_date,hms_ipd_booking.admission_time,hms_ipd_patient_admission_prescription.patient_bp as patientbp,hms_ipd_patient_admission_prescription.patient_temp as patienttemp,hms_ipd_patient_admission_prescription.patient_weight as patientweight,hms_ipd_patient_admission_prescription.patient_height as patientpr,hms_ipd_patient_admission_prescription.patient_spo2 as patientspo,hms_ipd_patient_admission_prescription.patient_rbs as patientrbs,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation"); 
        ...
        $this->db->join('hms_ipd_booking','hms_ipd_booking.id=hms_ipd_patient_admission_prescription.booking_id','left'); 
        $result_pre['prescription_list']= $this->db->get()->result();
    ...
    {
        $this->db->select("hms_ipd_patient_admission_prescription.*,hms_ipd_booking.ipd_no,hms_ipd_booking.admission_time,hms_ipd_booking.admission_date,hms_ipd_booking.referral_doctor,hms_ipd_booking.attend_doctor_id,hms_ipd_booking.admission_date,hms_ipd_booking.admission_time,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation"); 
        $this->db->from('hms_ipd_patient_admission_prescription'); 
        $this->db->join('hms_ipd_booking','hms_ipd_booking.id=hms_ipd_patient_admission_prescription.booking_id','left');
        $this->db->join('hms_patient','hms_patient.id = hms_ipd_booking.patient_id'); 
        $this->db->join('hms_gardian_relation','hms_gardian_relation.id=hms_patient.relation_type','left');
    ...
    {
        $this->db->select("hms_ipd_patient_admission_prescription.*"); 
        $this->db->from('hms_ipd_patient_admission_prescription'); 
        $this->db->where('hms_ipd_patient_admission_prescription.id',$id);
        $this->db->where('hms_ipd_patient_admission_prescription.is_deleted','0'); 
        $query = $this->db->get(); 
    ...
    public function delete($id="")
    {
        if(!empty($id) && $id>0)
        {
            ...
            $this->db->where('id',$id);
            $this->db->update('hms_ipd_patient_admission_prescription');
        } 
    }
    ...
    public function save_file($filename="")
    {
        ...
        {    
            ...
            $this->db->set('modified_date',date('Y-m-d H:i:s'));
            $this->db->where('id',$post['data_id']);
            $this->db->update('hms_ipd_admission_prescription_files',$data);  
        }
        else{    
            if(!empty($filename))
            {
            ...
            $this->db->set('created_by',$user_data['id']);
            $this->db->set('created_date',date('Y-m-d H:i:s'));
            $this->db->insert('hms_ipd_admission_prescription_files',$data);               
        }   
    }
    ...
    public function get_prescription_files($prescription_id='')
    {
        if(!empty($prescription_id) && $prescription_id>0)
        {
            ...
            $query = $this->db->get('hms_ipd_admission_prescription_files'); 
            $result = $query->result(); 
            //echo $this->db->last_query(); exit;
            return $result;
    ...
    public function save_prescription()
    {
        ...
        $data = array( 
                    ...
                    "local_examination"=>$post['local_examination_text'], // added by nitin sharma 04/02/2024
                    "date_time_new" =>$post['date_time_new'], // added by manoj
                    "status"=>1
                    ); 
            $this->db->set('modified_by',$user_data['id']);
            $this->db->set('modified_date',date('Y-m-d H:i:s'));
            $this->db->where('id',$post['data_id']);
            $this->db->update('hms_ipd_patient_admission_prescription',$data); 

            //echo $this->db->last_query(); exit;
            $this->db->where('prescription_id',$post['data_id']);
            $this->db->delete('hms_ipd_admission_prescription_patient_test'); 
            ...
            $this->db->where('prescription_id',$post['data_id']);
            $this->db->delete('hms_ipd_admission_prescription_patient_pres'); 
    ...
    function template_format($data="",$branch_id='')
    {
        ...
        $this->db->select('hms_ipd_branch_admission_prescription_setting.*');
        $this->db->where($data);
        if(!empty($branch_id))
        {
            $this->db->where('hms_ipd_branch_admission_prescription_setting.branch_id = "'.$branch_id.'"');
        }
        else
        {
            $this->db->where('hms_ipd_branch_admission_prescription_setting.branch_id = "'.$users_data['parent_id'].'"');
        }
         
        $this->db->from('hms_ipd_branch_admission_prescription_setting');
	...
	{
	    $user_data = $this->session->userdata('auth_users');
	    $this->db->select("hms_ipd_patient_admission_prescription.patient_id"); 
        $this->db->from('hms_ipd_patient_admission_prescription'); 
		if(!empty($all_prescription_ids))
		{
		    $this->db->where('hms_ipd_patient_admission_prescription.id IN ('.$all_prescription_ids.')');
		}
		$this->db->join('hms_patient','hms_patient.id=hms_ipd_patient_admission_prescription.patient_id','left');
