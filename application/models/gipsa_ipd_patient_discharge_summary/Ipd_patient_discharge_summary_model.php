...
class Ipd_patient_discharge_summary_model extends CI_Model {
	...
	//	echo "<pre>";print_r($post); die;
		$data = array( 
                    ...
                    'medicine_administered'=>$post['medicine_administered'],
                    'investigations'=>$post['investigations'],
                    'discharge_time_condition'=>$post['discharge_time_condition'],
                     'h_o_past'=>$post['h_o_past'],
	                  'mlc_fir_no'=>$post['mlc_fir_no'],
	                  'ho_any_allergy'=>$post['ho_any_allergy'],
	                  'medicine_prescribed'=>$post['medicine_prescribed'],
	                  'lama_dama_reasons'=>$post['lama_dama_reasons'],
	                  'refered_data'=>$post['refered_data'],
	                  'icd_code'=>$post['icd_code'],
	                  'admission_reason'=>$post['admission_reason'],
	                  'family_history'=>$post['family_history'],
	                  'alcohol_history'=>$post['alcohol_history'],
	                  'procedure_performedand'=>$post['procedure_performedand'],
	                  'nutritional_advice'=>$post['nutritional_advice'],
	                  'consultants_name'=>$post['consultants_name'],
                    ...
                    'date_of_pro'=>date('Y-m-d',strtotime($post['date_of_pro'])),
                    'time_of_pro'=>date('H:i:s', strtotime(date('d-m-Y').' '.$post['time_of_pro'])),
                    'status'=>$post['status'],
	...
	//	print_r($data);die;
		...
		{    
			...
			/*************** Medicine Administered Detail ****************/
			$this->db->where(array('ipd_id'=>$post['ipd_id'],'discharge_summary_id'=>$post['data_id']));
			$this->db->delete('hms_ipd_patient_discharge_summery_medicine_administered');
			if($post['medicine_name1'][0]!="")
			{
				$post_medicine_name= $post['medicine_name1'];
				...
				for($i=0;$i<$counter_name;$i++) 
					{
						$data_medicine= array(
						'medicine_name'=>$post['medicine_name1'][$i],
						'medicine_dose'=>$post['medicine_dose1'][$i],
						'medicine_duration'=>$post['medicine_duration1'][$i],
						'medicine_frequency'=>$post['medicine_frequency1'][$i],
						'medicine_advice'=>$post['medicine_advice1'][$i],
						'medicine_brand'=>$post['medicine_brand1'][$i],
						'medicine_type'=>$post['medicine_type1'][$i],
						'medicine_salt'=>$post['medicine_salt1'][$i],
						'ipd_id'=>$post['ipd_id'],
						'patient_id'=>$post['patient_id'],
						'discharge_summary_id'=>$post['data_id']
				...
				$this->db->insert('hms_ipd_patient_discharge_summery_medicine_administered',$data_medicine);
						//print_r($data_medicine);
				}
...
$users_data = $this->session->userdata('auth_users'); 
  ...
  /********** Medicine Administered List of discharge summary ************/

  public function get_discharge_medicine_administered($id)
	{
		$this->db->select('hms_ipd_patient_discharge_summery_medicine_administered.medicine_name,hms_ipd_patient_discharge_summery_medicine_administered.medicine_dose,hms_ipd_patient_discharge_summery_medicine_administered.medicine_duration,hms_ipd_patient_discharge_summery_medicine_administered.medicine_frequency,hms_ipd_patient_discharge_summery_medicine_administered.medicine_advice');
		$this->db->from('hms_ipd_patient_discharge_summery_medicine_administered'); 
		$this->db->where('hms_ipd_patient_discharge_summery_medicine_administered.discharge_summary_id',$id);
		$query = $this->db->get(); 
		return $query->result();
	}
  /********** Medicine Administered List of discharge summary ************/
