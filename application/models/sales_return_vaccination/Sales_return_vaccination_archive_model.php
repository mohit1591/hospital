...
class Sales_return_vaccination_archive_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_vaccination_sale_return.*, hms_patient.patient_name,(CASE WHEN hms_vaccination_sale_return.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name, (select count(id) from hms_medicine_sale_return_medicine where sales_return_id = hms_vaccination_sale_return.id) as total_medicine"); 
		$this->db->join('hms_patient','hms_patient.id = hms_vaccination_sale_return.patient_id','left'); 
		$this->db->where('hms_vaccination_sale_return.is_deleted','1'); 
