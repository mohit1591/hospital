...
(select sum(credit)-sum(debit) as balance from hms_payment as  payment where payment.parent_id = hms_opd_booking.id AND payment.section_id=2 AND payment.branch_id = ".$user_data['parent_id'].") as balance1");
		...
		if(isset($search) && !empty($search))
		{
			...
			{
				$start_time = date('H:i:s',strtotime($search['start_time']));
				$this->db->where('hms_opd_booking.booking_time >= "'.$start_time.'"');
			}

			if(!empty($search['end_time']))
			{
				$end_time = date('H:i:s',strtotime($search['end_time']));
				$this->db->where('hms_opd_booking.booking_time <= "'.$end_time.'"');
			}
...
//echo $this->db->last_query();
	...
	public function check_booking_code_exits($booking_code="")
	{
		...
		$this->db->where('hms_opd_booking.branch_id',$users_data['parent_id']); 
		$this->db->where('hms_opd_booking.booking_code',$booking_code); 
		$this->db->where('hms_opd_booking.is_deleted',0); 
		$query = $this->db->get('hms_opd_booking')->result();
		return $query;
	}
    ...
    /// upload files
     ...
     {
     	$user_data = $this->session->userdata('auth_users');
		$this->db->select('hms_opd_booking.id,hms_opd_booking.booking_code,hms_opd_booking.booking_type, hms_patient.simulation_id, hms_patient.patient_code, hms_patient.patient_name, hms_patient.mobile_no, hms_patient.gender, hms_patient.age_y, hms_patient.age_m, hms_patient.age_d,hms_patient.patient_code,hms_opd_booking.branch_id,hms_opd_booking.attended_doctor,hms_opd_booking.patient_id,hms_opd_booking.created_by');
		$this->db->from('hms_opd_booking'); 
		$this->db->join('hms_patient','hms_patient.id = hms_opd_booking.patient_id'); 
		$this->db->where('hms_opd_booking.branch_id',$user_data['parent_id']); 
    ...
    {
		$user_data = $this->session->userdata('auth_users');
		$this->db->select('hms_std_opd_patient_status.*, hms_opd_booking.id as booking_id, hms_opd_booking.patient_id  as patient_ids, hms_opd_booking.branch_id as branch_ids, hms_patient.simulation_id, hms_patient.patient_code, hms_patient.patient_name, hms_patient.mobile_no, hms_patient.gender, hms_patient.age_y, hms_patient.age_m, hms_patient.age_d, hms_patient.address,hms_patient.address2,hms_patient.address3, hms_patient.dob');
		$this->db->from('hms_opd_booking'); 
		$this->db->join('hms_patient','hms_patient.id = hms_opd_booking.patient_id');
		$this->db->join('hms_std_opd_patient_status','hms_opd_booking.id = hms_std_opd_patient_status.booking_id AND hms_patient.id = hms_std_opd_patient_status.patient_id','LEFT'); 
		$this->db->where('hms_opd_booking.branch_id',$user_data['parent_id']); 
		$this->db->where('hms_opd_booking.id',$id);
		$this->db->where('hms_opd_booking.is_deleted','0');
		$query = $this->db->get(); 
    ...
    {
    	$this->db->set('hms_opd_booking.cyclo_status',2);
		$this->db->where('hms_opd_booking.id',$id); 
		$query = $this->db->update('hms_opd_booking');
		return $query;
    }
