...
class Ipd_packages_model extends CI_Model {
	...
	public function save($ids=array())
	{
		...
		{     
                //delete package charge and insert a new package charge
                $this->db->where(array('type'=>0,'package_id'=>$post['pack_id'],'panel_company_id'=>0,'branch_id'=>$user_data['parent_id']));
                $this->db->delete('hms_ipd_package_charge');
                $data_ch_new = array(
                    'branch_id'=>$user_data['parent_id'], 
                    'package_id'=>$post['pack_id'],
                    'charge'=>$post['amount'],
                    'type'=>0,
                    'panel_company_id'=>0
                    );
                $this->db->insert('hms_ipd_package_charge',$data_ch_new);
        ...
        { 
                ...
                /*if(!empty($pack_id))
                {
                    ...
                    {
                        $this->db->set('type',1);
                        $this->db->set('panel_company_id',$panel_comapny->id);
                        $this->db->set('created_by',$user_data['id']);
                        $this->db->set('created_date',date('Y-m-d H:i:s'));
                        $this->db->insert('hms_ipd_packages',$data); 
                        $panel_pack_id =$this->db->insert_id();
                        ...
                        if(!empty($particular_ids))  
                        {
                            ...
                            foreach($particular_ids as $particular_id)
                            {  
                                 $this->db->set('package_id',$panel_pack_id);
                                 $this->db->set('particular_id',$particular_id);
                                 $this->db->insert('hms_ipd_particular_to_packages',$particular_data_arr);  
                            }
                        }
    ...
    public function selected_particular($id=''){
        ...
        if(!empty($id)){
            ...
            $this->db->select("hms_ipd_packages.*,hms_ipd_particular_to_packages.particular_id as pert_id");
            $this->db->from("hms_ipd_packages");
