...
class Dialysis_prescription_template_model extends CI_Model 
{
    var $table = 'hms_dialysis_prescription_template';
    var $column = array('hms_dialysis_prescription_template.id','hms_dialysis_prescription_template.template_name','hms_dialysis_prescription_template.status', 'hms_dialysis_prescription_template.created_date', 'hms_dialysis_prescription_template.modified_date');  
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->where('hms_dialysis_prescription_template.branch_id = "'.$user_data['parent_id'].'"');
    ...
    public function get_by_id($id)
    {
        $user_data = $this->session->userdata('auth_users');
        $this->db->select('hms_dialysis_prescription_template.*');
        $this->db->from('hms_dialysis_prescription_template'); 
        $this->db->where('hms_dialysis_prescription_template.branch_id',$user_data['parent_id']); 
        $this->db->where('hms_dialysis_prescription_template.id',$id);
        $this->db->where('hms_dialysis_prescription_template.is_deleted','0');
        $query = $this->db->get(); 
    ...
    {  
        ...
        {    
            ...
            if($total_test > 0)
            {
                ...
                $this->db->delete('hms_dialysis_prescription_patient_test_template');
            }
            ...
            for($i=0;$i<$total_test;$i++)
            {
                  ...
                  $test_data = array(
                        ...
                        "test_name"=>$post['test_name'][$i],
                        "test_id"=>$test_id);
                    $this->db->insert('hms_dialysis_prescription_patient_test_template',$test_data); 
            ...
            {
                $this->db->where('template_id',$post['data_id']);
                $this->db->delete('hms_dialysis_prescription_patient_pres_template');
            }
            /*for($i=0;$i<=$total_prescription-1;$i++)
            {
                    ...
                    $this->db->insert('hms_dialysis_prescription_patient_pres_template',$prescription_data); 
                    $test_data_id = $this->db->insert_id(); 
            }*/
            ...
            $this->db->update('hms_dialysis_prescription_template',$data); 
//echo $this->db->last_query(); exit;
        }
        else
        {    
            ...
            $this->db->set('created_by',$user_data['id']);
            $this->db->set('created_date',date('Y-m-d H:i:s'));
            $this->db->insert('hms_dialysis_prescription_template',$data); 
    ...
    function get_dialysis_prescription_template($template_id='')
    {
        $this->db->select("hms_dialysis_prescription_patient_pres_template.*"); 
        $this->db->from('hms_dialysis_prescription_patient_pres_template'); 
    ...
    function get_dialysis_test_template($template_id='')
    {
        $this->db->select("hms_dialysis_prescription_patient_test_template.*"); 
        $this->db->from('hms_dialysis_prescription_patient_test_template'); 
    ...
    public function delete($id="")
    {
        if(!empty($id) && $id>0)
        {
            ...
            $this->db->where('id',$id);
            $this->db->update('hms_dialysis_prescription_template');
        } 
    }
