...
class Stock_purchase_model extends CI_Model 
{
	...
	var $column = array('path_purchase_item.id','path_purchase_item.purchase_no','path_purchase_item.purchase_date',' hms_medicine_vendors.name','path_purchase_item.net_amount','path_purchase_item.balance','path_purchase_item.created_date','hms_medicine_vendors.name');  
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("path_purchase_item.*,hms_medicine_vendors.name,(SELECT GROUP_CONCAT(path_item.item) FROM `path_purchase_item_to_purchase` join path_item on path_item.id=path_purchase_item_to_purchase.item_id WHERE path_purchase_item_to_purchase.purchase_id=path_purchase_item.id) as items"); 
  ...
  function purchase_item_serial_by_id($request_id="", $sp_id='')
  {
	...
	public function save()
	{
		...
		$data_purchase= array(
                            ...
                            'payment_due_date'=>date('Y-m-d',strtotime($post['payment_due_date'])),
		...
		if(!empty($post['data_id']) && $post['data_id']>0)
		{
			...
			if(!empty($stock_purchase_item_list))
			{
				...
				{  
				    if(!empty($stock_item_list['conversion']))
				    {
				        $conver = $stock_item_list['conversion'];
				    }
				    else
				    {
				        $conver = 1;
                    ...
                    $qty = ($conver*$stock_item_list['unit1'])+$stock_item_list['unit2'];
					$free_qty = ($conver*$stock_item_list['freeunit1'])+$stock_item_list['freeunit2'];
					$qtys = $qty+$free_qty;
                    ...
                    //echo $this->db->last_query(); die;
                   // echo "<pre>"; print_r($post['serial_no_array'][$stock_item_list['item_id']]); 
                    if(isset($post['serial_no_array'][$stock_item_list['item_id']]) && !empty($post['serial_no_array'][$stock_item_list['item_id']]) )
                    {
                       $ser_array=explode(',',$post['serial_no_array'][$stock_item_list['item_id']][0]);
                       
                       //echo "<pre>"; print_r($ser_array); 
		...
		else
		{
			...
			if(!empty($stock_purchase_item_list))
			{
				...
				{ 
			        ...
			        //echo "<pre>";print_r($post['serial_no_array'][$h]);die;		
					if($stock_item_list['conversion']>0)
					{
					...
					{
						$qty = $stock_item_list['unit1']+$stock_item_list['unit2'];
						$free_qty = $stock_item_list['freeunit1']+$stock_item_list['freeunit2'];
					}
...
// add bottom 4 feb 20 
    ...
    public function item_list($ids="")
    {
    	...
    	$users_data = $this->session->userdata('auth_users'); 
		$this->db->select("path_item.*,path_stock_category.category, hms_stock_item_unit.unit as unit1, hms_stock_item_unit_2.unit as unit2, hms_inventory_racks.rack_no, hms_inventory_company.company_name, path_item.price as purchase_rate,path_stock_item.parent_id as purchase_ids_edit");
...
//echo $this->db->last_query();die;	
		$this->db->select('path_purchase_item_to_purchase.*,path_purchase_item_to_purchase.total_amount as amount, path_item.item as item_name, path_item.item_code,path_purchase_item_to_purchase.qty as quantity, hms_stock_item_unit.unit, path_purchase_item_to_purchase.per_pic_price as item_price, path_item.id as item_id, path_item.price as purchase_amount,path_stock_category.category, path_purchase_item_to_purchase.expiry_date as exp_date,path_item.mrp');
		$this->db->from('path_purchase_item_to_purchase'); 
