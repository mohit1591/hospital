<?php
class Reg_card_setting_model extends CI_Model 
{
	var $table = 'hms_card_print_setting'; 
    ...
    $this->db->where('hms_card_print_setting.branch_id',$user_data['parent_id']); 
    $query = $this->db->get();
   ...
   //print_r($post); exit;
    	...
    	$this->db->delete('hms_card_print_setting');
            ...
            foreach($post['data'] as $key=>$val)
            {
            	...
            	$this->db->insert('hms_card_print_setting',$data);
  ...
  function template_format($data="",$branch_id='')
  {
    ...
    {
      $this->db->where('hms_card_print_setting.branch_id = "'.$users_data['parent_id'].'"');
    }
