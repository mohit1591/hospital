...
class Reports_model extends CI_Model {
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->select("path_test_booking.id, hms_payment.id as payment_id, path_test_booking.lab_reg_no, path_test_booking.booking_date, (CASE WHEN path_test_booking.ipd_id > 0 THEN path_test_booking.total_amount ELSE hms_payment.total_amount END) AS total_amount, (CASE WHEN path_test_booking.ipd_id > 0 THEN path_test_booking.net_amount ELSE hms_payment.net_amount END) AS  net_amount, hms_payment.debit as paid_amount,hms_payment.discount_amount as discount, hms_doctors.doctor_name, hms_patient.patient_name,hms_department.department as department,(CASE WHEN hms_payment.balance > 0  THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.parent_id = path_test_booking.id AND payment.section_id=1) ELSE '0.00' END) as balance, (CASE WHEN hms_payment.balance > 0 THEN '1' ELSE '0' END) as print_type,(CASE WHEN hms_payment.balance > 0 THEN '1' ELSE '0' END) as print_type,(CASE WHEN path_test_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN path_test_booking.referral_doctor=0 THEN concat('Other ',path_test_booking.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name");
        ...
        if(isset($search_data['ptype']) && !empty($search_data['ptype']) && $search_data['ptype']==1)
        {
            $this->db->where('path_test_booking.opd_id>0');
        }
        else if(isset($search_data['ptype']) && !empty($search_data['ptype']) && $search_data['ptype']==2)
        {
            $this->db->where('path_test_booking.ipd_id>0');
        }
        else if(isset($search_data['ptype']) && !empty($search_data['ptype']) && $search_data['ptype']==3)
        {
            $this->db->where('path_test_booking.ipd_id=""');
             $this->db->where('path_test_booking.opd_id=""');
        }
        if(isset($search_data) && !empty($search_data))
        {
            ...
            if(isset($search_data['department']) && !empty($search_data['department'])
                )
            { 
                $this->db->where('path_test.dept_id = "'.$search_data["department"].'"');
            }
            if(isset($search_data['profile_id']) && !empty($search_data['profile_id'])
                )
            { 
                $this->db->where('path_test_booking_to_profile.profile_id = "'.$search_data["profile_id"].'"');
            }
            if(isset($search_data['test_ids']) && !empty($search_data['test_ids'])
                )
            { 
                $this->db->where('path_test_booking_to_test.test_id = "'.$search_data["test_ids"].'"');
            }
    ...
    function search_report_data()
    {
        ...
        $this->db->select("path_test_booking.id, hms_payment.id as payment_id, path_test_booking.lab_reg_no, path_test_booking.booking_date, (CASE WHEN path_test_booking.ipd_id > 0 THEN path_test_booking.total_amount ELSE hms_payment.total_amount END) AS total_amount, (CASE WHEN path_test_booking.ipd_id > 0 THEN path_test_booking.net_amount ELSE hms_payment.net_amount END) AS  net_amount, hms_payment.debit as paid_amount,hms_payment.discount_amount as discount, hms_doctors.doctor_name, hms_patient.patient_name,hms_department.department as department,(CASE WHEN hms_payment.balance > 0  THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.parent_id = path_test_booking.id AND payment.section_id=1) ELSE '0.00' END) as balance, (CASE WHEN hms_payment.balance > 0 THEN '1' ELSE '0' END) as print_type,
    ...
    public function details_test_list($post=array())
    {
        ...
        { 
            ...
            $this->db->join('path_test','path_test.id = path_test_booking_to_test.test_id');
            $this->db->join('path_test_booking','path_test_booking.id = path_test_booking_to_test.booking_id AND path_test_booking.is_deleted !=2');
            $this->db->join('hms_patient','hms_patient.id = path_test_booking.patient_id');
            ...
            if(!empty($post['start_date']))
            {
              ...
              $this->db->where('hms_payment.created_date >= "'.$start_date.' 00:00:00"');
            }

            if(!empty($post['end_date']))
            {
               $end_date = date('Y-m-d',strtotime($post['end_date']));
               $this->db->where('hms_payment.created_date <= "'.$end_date.' 23:59:59"');
            }
            ...
            /////////// Profile data ////////
            $this->db->select('path_profile.profile_name as test_name, count(path_test_booking_to_profile.profile_id) as total_test, sum(path_test_booking_to_profile.net_amount) as total_amount');
            $this->db->join('path_profile','path_profile.id = path_test_booking_to_profile.profile_id');  
            
            $this->db->join('path_test_booking','path_test_booking.id = path_test_booking_to_profile.test_booking_id AND path_test_booking.is_deleted !=2','left');
            $this->db->join('hms_patient','hms_patient.id = path_test_booking.patient_id');

            $this->db->join('hms_payment','hms_payment.parent_id = path_test_booking.id AND hms_payment.section_id=1','left');

            
            $this->db->where('path_test_booking_to_profile.parent_id','0');
            //$this->db->where('path_test_booking.is_deleted','0');
            $this->db->where('path_test_booking.branch_id',$users_data['parent_id']);    
            if(!empty($post['start_date']))
            {
        ...
        if(!empty($post['start_date']))
            {
              ...
              $bsub_string .= " AND path_test_booking.booking_date >= '".date('Y-m-d',strtotime($post['start_date']))."'";
            }
            ...
            if(!empty($post['end_date']))
            {
               ...
               $bsub_string .= " AND path_test_booking.booking_date <= '".date('Y-m-d',strtotime($post['end_date']))."'";
            }
            //
            ...
            (select sum(path_test_booking.total_amount) from path_test_booking  where path_test_booking.is_deleted=0 AND path_test_booking.branch_id="'.$users_data['parent_id'].'" '.$bsub_string.' ) as total_amount
            ');    
            //$this->db->join('hms_payment','hms_payment.parent_id = path_test_booking.id AND hms_payment.section_id=1','left');
