...
class Gda_staff_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
		...
		$this->db->where('hms_employees.type =1');
...
//echo $this->db->last_query();die;
	...
	{  
		...
		$data = array(
					...
					'department_id'=>$post['emp_type_id'],
					'status'=>$post['status'],
		...
		{    
			...
			if(in_array('365',$user_data['permission']['section'])) 
              {
					/* New Code 20-jan-2020 */
			$this->db->select('*');
	    	$this->db->where('branch_id',$user_data['parent_id']);
	    	$this->db->where('employee_id',$post['data_id']);
	    	...
	    	if(!empty($leavelist))
			{
				...
				$this->db->insert('prl_leave_details',$data);
    ...
    {
    	$user_data = $this->session->userdata('auth_users');
    	$this->db->select('hms_emp_type.*,hms_emp_type.emp_type as department');
    	$this->db->where('branch_id',$user_data['parent_id']);
    	$this->db->where('status',1); 
    	$this->db->where('is_deleted',0); 
