...
class Referral_reports_model extends CI_Model {
    ...
    public function ambulance_report_list($get="")
    {
        ...
        if(!empty($get))
        {  
            $this->db->select("hms_ambulance_booking.*,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_branch.branch_name,hms_ambulance_booking.created_date as createddate, (CASE WHEN hms_ambulance_booking.reffered_by=0 THEN hms_doctors.doctor_name WHEN hms_ambulance_booking.reffered_by=1 THEN hms_hospital.hospital_name ELSE '' 
          END) as doctor_name,hms_ambulance_location.location_name"); 
            $this->db->join('hms_patient','hms_patient.id=hms_ambulance_booking.patient_id','inner');
