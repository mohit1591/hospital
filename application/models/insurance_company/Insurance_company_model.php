...
class Insurance_company_model extends CI_Model {
	...
	var $column = array('hms_insurance_company.id','hms_insurance_company.insurance_company', 'hms_insurance_company.status','hms_insurance_company.created_date');  
	...
	public function get_by_id($id)
	{
		...
		$this->db->where('hms_insurance_company.id',$id);
		$this->db->where('hms_insurance_company.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
			...
			$this->db->update('hms_insurance_company',$data);  
		}
		else{    
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_insurance_company',$data);               
		} 	
	}
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->update('hms_insurance_company');
			//echo $this->db->last_query();die;
    	} 
    }
        ...
        if(!empty($insurance_all_data))
        {
            foreach($insurance_all_data as $insurance_data)
            {
				...
				$insurance_data_array = array( 
	                    ...
	                    'panel_code'=>$insurance_data['panel_code'],
						'status'=>1,					
	                    'ip_address'=>$_SERVER['REMOTE_ADDR'],
                ...
                //print_r($vaccination_data_array);
				    $this->db->insert('hms_insurance_company',$insurance_data_array);
    ...
    public function check_unique_value($branch_id, $insurance_company, $id='')
    {
    	$this->db->select('hms_insurance_company.*');
		$this->db->from('hms_insurance_company'); 
		$this->db->where('hms_insurance_company.branch_id',$branch_id);
		$this->db->where('hms_insurance_company.insurance_company',$insurance_company);
		if(!empty($id))
		$this->db->where('hms_insurance_company.id !=',$id);
		$this->db->where('hms_insurance_company.is_deleted','0');
		$query = $this->db->get(); 
