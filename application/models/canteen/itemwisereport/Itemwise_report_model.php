...
class Itemwise_report_model extends CI_Model {

  var $table = 'hms_canteen_master_entry';
    var $column = array('hms_canteen_master_entry.id','hms_canteen_master_entry.product_code', 'hms_canteen_master_entry.product_name', 'hms_canteen_master_entry.unit_id','hms_canteen_master_entry.unit_second_id','hms_canteen_master_entry.conversion','hms_canteen_master_entry.min_alrt','hms_canteen_master_entry.packing','hms_canteen_master_entry.rack_no','hms_canteen_master_entry.salt','hms_canteen_master_entry.manuf_company','hms_canteen_master_entry.mrp','hms_canteen_master_entry.purchase_rate','hms_canteen_master_entry.discount','hms_canteen_master_entry.vat','hms_canteen_master_entry.status', 'hms_canteen_master_entry.created_date', 'hms_canteen_master_entry.modified_date');  
    var $order = array('hms_canteen_master_entry.id' => 'desc'); 
...
*/
  ...
  function get_datatables($branch_id='')
  {
    ...
    function medicinequantityreport_list($get="")
    {
     ...
     //   $deleted_purchase_return_medicine= $this->is_deleted_purchase_return_medicine();
       ...
       // $deleted_sale_return_medicine= $this->is_deleted_sale_return_medicine();
        ...
        $this->db->select("hms_canteen_master_entry.product_code,hms_canteen_master_entry.product_name as med_name,hms_canteen_master_entry.id,(CASE WHEN ".$get['module_type']."=1  THEN hms_canteen_manuf_company.company_name ELSE hms_canteen_master_entry.product_name END) as name");
        $this->db->where('hms_canteen_master_entry.is_deleted','0'); 
     if($get['module_type']==2 && !empty($get['vender']))
      {
       $this->db->join('hms_canteen_purchase_to_purchase','hms_canteen_master_entry.id = hms_canteen_purchase_to_purchase.medicine_id','left');
       
       $this->db->join('hms_canteen_purchase','hms_canteen_purchase.id = hms_canteen_purchase_to_purchase.purchase_id','left');
       $this->db->where('hms_canteen_purchase.vendor_id',$get['vender']);
      }
       
        $this->db->join('hms_canteen_manuf_company','hms_canteen_manuf_company.id = hms_canteen_master_entry.manuf_company','left');
        $this->db->group_by('hms_canteen_master_entry.id');
       ...
       //print_r($data['medicine_list']);die();
        ...
        foreach($data['medicine_list'] as $medicine_list)
        {
            ...
            $data_array[$i]['product_name']=$medicine_list['name'];
            $data_array[$i]['med_name']=$medicine_list['med_name'];
            $data_array[$i]['product_code']=$medicine_list['product_code'];
            

            if($type=1)
            {
                $this->db->select("(sum(debit)-sum(credit)) as total_qty,sum(total_amount) as total_amt");
                ...
                if(!empty($deleted_purchase_medicine))
                {
                    ...
                    $this->db->where("hms_canteen_stock_item.parent_id NOT IN(".$new_p_ids.")");   
                }

                    $this->db->where('hms_canteen_stock_item.item_id',$medicine_list['id']);
                    $this->db->where('hms_canteen_stock_item.type',1);
                    $this->db->where('hms_canteen_stock_item.item_id',$medicine_list['id']);
                    ...
                    if(!empty($get['start_date']))
                    {
                        ...
                        $this->db->where('hms_canteen_stock_item.created_date >= "'.$from_c_date.'"');
                    }

                    if(!empty($get['end_date']))
                    {
                        $to_c_date = date('Y-m-d',strtotime($get['end_date'])).' 23:59:59';
                        $this->db->where('hms_canteen_stock_item.created_date <= "'.$to_c_date.'"');
                    }
                    $data_array[$i]['purchsase_quantity'] = $this->db->get('hms_canteen_stock_item')->row_array();
            }
            ...
            if($type=2)
            {
                ...
                if(!empty($deleted_purchase_return_medicine))
                {
                    foreach($deleted_purchase_return_medicine as $purchase_r_ids)
                    {
                        ...
                        $this->db->where("hms_canteen_stock_item.parent_id NOT IN(".$new_p_r_ids.")");
                }
                
                    $this->db->where('hms_canteen_stock_item.type',2);
                    $this->db->where('hms_canteen_stock_item.item_id',$medicine_list['id']);
                    $this->db->where('hms_canteen_stock_item.branch_id',$user_data['parent_id']);
                    if(!empty($get['start_date']))
                    {
                    ...
                    $data_array[$i]['purchsase_return_quantity'] = $this->db->get('hms_canteen_stock_item')->row_array();
            ...
            if($type=3)
            {
                ...
                if(!empty($deleted_sale_medicine))
                {
                    ...
                    $this->db->where("hms_canteen_stock_item.parent_id NOT IN(".$new_s_ids.")");
                }
                
            
               
                    $this->db->where('hms_canteen_stock_item.type',3);
                    $this->db->where('hms_canteen_stock_item.item_id',$medicine_list['id']);
                     $this->db->where('hms_canteen_stock_item.branch_id',$user_data['parent_id']);
                    ...
                    $data_array[$i]['sale_quantity'] = $this->db->get('hms_canteen_stock_item')->row_array();
            }
            ...
            if($type=4)
            {
                    ...
                    if(!empty($deleted_sale_return_medicine))
                    {
                        foreach($deleted_sale_return_medicine as $sale_r_ids)
                        {
                            ...
                            $this->db->where("hms_canteen_stock_item.parent_id NOT IN(".$new_s_r_ids.")");
                    }
                        $this->db->where('hms_canteen_stock_item.type',4);
                        $this->db->where('hms_canteen_stock_item.item_id',$medicine_list['id']);
                        $this->db->where('hms_canteen_stock_item.branch_id',$user_data['parent_id']);
                        ...
                        $data_array[$i]['sale_return_quantity'] = $this->db->get('hms_canteen_stock_item')->row_array();
    ...
    {
        $this->db->select('hms_canteen_purchase.id');
        $this->db->where('hms_canteen_purchase.is_deleted=2');
        $query= $this->db->get('hms_canteen_purchase')->result_array();
        return $query;
    }
    function is_deleted_purchase_return_medicine()
    {
        $this->db->select('hms_canteen_return.id');
        $this->db->where('hms_canteen_return.is_deleted=2');
        $query=$this->db->get('hms_canteen_return')->result_array();
        //print_r($query);die;
        return $query;
    }
    function is_deleted_sale_medicine()
    {
        $this->db->select('hms_canteen_sale.id');
        $this->db->where('hms_canteen_sale.is_deleted=2');
        $query=$this->db->get('hms_canteen_sale')->result_array();
        return $query;
    }
    function is_deleted_sale_return_medicine()
    {
        $this->db->select('hms_canteen_sale_return.id');
        $this->db->where('hms_canteen_sale_return.is_deleted=2');
        $query=$this->db->get('hms_canteen_sale_return')->result_array();
        return $query;
    }
  ...
  public function save()
  {
    ...
    { 
        if(!empty($get))
        {
           ...
           // $this->db->select("(CASE WHEN hms_expenses.type=0 THEN hms_expenses.vouchar_no WHEN hms_expenses.type=2 THEN (select purchase_id from  hms_canteen_purchase where id=hms_expenses.parent_id) ELSE '' END) as vouchar_no, (CASE WHEN hms_expenses.type=0 THEN 'expenses' WHEN hms_expenses.type=3 THEN 'Sale Return' WHEN hms_expenses.type=1 THEN 'Emp. Salary' WHEN hms_expenses.type=2 THEN 'Purchase Medicine' ELSE '' END) as type, hms_expenses.paid_amount,hms_expenses_category.exp_category, hms_expenses.created_date,hms_expenses.expenses_date"); 
            $this->db->select("(CASE WHEN hms_expenses.type=0 THEN hms_expenses.vouchar_no WHEN hms_expenses.type=2 THEN (select purchase_id from  hms_canteen_purchase where id=hms_expenses.parent_id) ELSE 'N/A' END) as vouchar_no, (CASE WHEN hms_expenses.type=0 THEN 'expenses' WHEN hms_expenses.type=3 THEN 'Sale Return' WHEN hms_expenses.type=1 THEN 'Emp. Salary' WHEN hms_expenses.type=2 THEN 'Purchase Medicine' ELSE 'N/A' END) as type, hms_expenses.paid_amount, (CASE WHEN hms_expenses.type=0 THEN hms_expenses_category.exp_category ELSE 'N/A' END) as exp_category, hms_expenses.created_date,hms_expenses.expenses_date,hms_payment_mode.payment_mode");
            ...
            $this->db->select("(CASE WHEN hms_expenses.type=0 THEN hms_expenses.vouchar_no WHEN hms_expenses.type=2 THEN (select purchase_id from  hms_canteen_purchase where id=hms_expenses.parent_id) ELSE 'N/A' END) as vouchar_no, (CASE WHEN hms_expenses.type=0 THEN 'expenses' WHEN hms_expenses.type=3 THEN 'Sale Return' WHEN hms_expenses.type=1 THEN 'Emp. Salary' WHEN hms_expenses.type=2 THEN 'Purchase ITEM' ELSE 'N/A' END) as type, hms_expenses.paid_amount, (CASE WHEN hms_expenses.type=0 THEN hms_expenses_category.exp_category ELSE 'N/A' END) as exp_category, hms_expenses.created_date,hms_expenses.expenses_date,hms_payment_mode.payment_mode,sum(hms_expenses.paid_amount) as total_amount"); 
            $this->db->from('hms_expenses'); 
            $this->db->join("hms_expenses_category","hms_expenses.paid_to_id=hms_expenses_category.id",'left');
            $this->db->join('hms_payment_mode','hms_payment_mode.id=hms_expenses.payment_mode','left');
            $this->db->where('hms_expenses.paid_amount>0');
...
//user collection
    ...
    public function branch_medicine_collection_list($get="",$ids=[])
    {
        ...
        { 
            ...
            $this->db->select("hms_patient.patient_name,hms_branch.branch_name,hms_branch.id as branch_id,hms_doctors.doctor_name, hms_payment.debit, hms_payment.created_date, hms_payment.pay_mode,hms_branch_hospital_no.reciept_prefix,hms_branch_hospital_no.reciept_suffix,(CASE WHEN hms_canteen_sale.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_canteen_sale.refered_id=0 THEN concat('Other ',hms_canteen_sale.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name,hms_payment_mode.payment_mode as mode"); 
            //(CASE WHEN hms_canteen_sale.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name
            $this->db->join('hms_branch','hms_branch.id=hms_payment.branch_id','left');
            $this->db->join('hms_patient','hms_patient.id=hms_payment.patient_id'); 
            $this->db->join('hms_canteen_sale','hms_canteen_sale.id=hms_payment.parent_id AND hms_canteen_sale.is_deleted!=2');
            //$this->db->join('hms_doctors','hms_doctors.id=hms_canteen_sale.refered_id','left');

            $this->db->join('hms_doctors','hms_doctors.id=hms_canteen_sale.refered_id','left');
            $this->db->join('hms_hospital','hms_hospital.id = hms_canteen_sale.referral_hospital','left');
            ...
            // $this->db->join('hms_branch_hospital_no','hms_branch_hospital_no.parent_id = hms_canteen_sale.id AND hms_branch_hospital_no.section_id IN(6,10)','left');
...
//user collection
            ...
            $this->db->join('hms_branch','hms_branch.id=hms_payment.branch_id','left');
            $this->db->join('hms_patient','hms_patient.id=hms_payment.patient_id','left'); 
            $this->db->join('hms_canteen_sale','hms_canteen_sale.id=hms_payment.parent_id');
            //$this->db->join('hms_doctors','hms_doctors.id=hms_canteen_sale.refered_id','left');
      ...
      public function branch_medicine_return_collection_list($get="",$ids=[])
    {
        ...
        { 
           ...
           $this->db->select("hms_patient.patient_name,hms_branch.branch_name, hms_payment.type,hms_payment.debit, hms_payment.created_date, hms_payment.pay_mode,hms_payment_mode.payment_mode as mode"); 
              $this->db->join('hms_patient','hms_patient.id=hms_payment.patient_id','left'); 
             $this->db->join('hms_canteen_return','hms_canteen_return.id=hms_payment.parent_id');
             $this->db->join('hms_branch','hms_branch.id=hms_payment.branch_id','left');
            //$this->db->join('hms_doctors','hms_doctors.id=hms_canteen_sale.refered_id','left'); 
             $this->db->join('hms_payment_mode','hms_payment_mode.id=hms_payment.pay_mode','left');
...
//user collection
    ...
    public function self_hospital_collection_list($get="")
    {
        ...
        {  
            $this->db->select("hms_patient.patient_name, hms_payment.debit, hms_payment.created_date, hms_payment.pay_mode,
                ...
                (CASE WHEN hms_payment.section_id=1 THEN 'Pathology' WHEN hms_payment.section_id=2 THEN 'OPD' WHEN hms_payment.section_id=3 THEN 'Sale ITEM' WHEN hms_payment.section_id=4 THEN 'OPD Billing'  WHEN hms_payment.section_id=5 THEN 'IPD' WHEN hms_payment.section_id=7 THEN 'Vaccination' WHEN hms_payment.section_id=8 THEN 'OT' WHEN hms_payment.section_id=10 THEN 'Blood Bank'  ELSE '' END) as section_name,
                (CASE 
            ...
            //medicine
            $this->db->join('hms_canteen_sale as sell_medicne','sell_medicne.id=hms_payment.parent_id AND sell_medicne.is_deleted != 2','left');
    ...
    /* hospital sub branch report end */
        ...
        public function get_batch_no($mid='')
        {
            $this->db->select('batch_no');
            $this->db->from('hms_canteen_stock_item');
            $this->db->where('hms_canteen_stock_item.item_id',$mid);
            $this->db->group_by('hms_canteen_stock_item.batch_no');
            $query = $this->db->get();
            $result= $query->result();
            return $result;
         }
    
     public function get_opening_stock($mid='')
        {
            $this->db->select('debit');
            $this->db->from('hms_canteen_stock_item');
            $this->db->where('hms_canteen_stock_item.item_id',$mid);
            $this->db->where('hms_canteen_stock_item.type',6);
            $query = $this->db->get();
            $result= $query->result();
            return $result;
         }
