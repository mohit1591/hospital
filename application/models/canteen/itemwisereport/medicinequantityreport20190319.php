...
*/
    ...
    function medicinequantityreport_list($get="")
    {
        ...
        $this->db->select("hms_medicine_entry.medicine_name as med_name,hms_medicine_entry.id,(CASE WHEN ".$get['module_type']."=1  THEN hms_medicine_company.company_name ELSE hms_medicine_entry.medicine_name END) as name");
        ...
        $this->db->join('hms_medicine_company','hms_medicine_company.id = hms_medicine_entry.manuf_company','left');
          if(!empty($get['module_type']) && $get['module_type']==0)
        //{
            $this->db->group_by('hms_medicine_entry.id');
        //}
        // if(!empty($get['module_type']) && $get['module_type']==1)
        // {
        //     $this->db->group_by('hms_medicine_company.id');
        // }
        if(!empty($get['start_date']))
        {
        $from_c_date = date('Y-m-d',strtotime($get['start_date'])).' 00:00:00';
        $this->db->where('hms_medicine_entry.created_date >= "'.$from_c_date.'"');
        }

        if(!empty($get['end_date']))
        {
        $to_c_date = date('Y-m-d',strtotime($get['end_date'])).' 23:59:59';
        $this->db->where('hms_medicine_entry.created_date <= "'.$to_c_date.'"');
        }
        ...
        foreach($data['medicine_list'] as $medicine_list)
        {
            ...
            if($type=1)
            {
                ...
                if(!empty($deleted_purchase_medicine))
                {
                    ...
                    $this->db->where('m_id',$medicine_list['id']);
