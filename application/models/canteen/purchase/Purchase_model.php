...
class Purchase_model extends CI_Model 
{
	...
	private function _get_datatables_query()
	{
		...
		$search = $this->session->userdata('purchase_search_item');

		$this->db->select("hms_canteen_purchase.*,hms_canteen_vendors.name as vendor_name ,hms_canteen_vendors.vendor_id as v_id, hms_canteen_vendors.mobile, (select count(id) from hms_canteen_purchase_to_purchase where purchase_id = hms_canteen_purchase.id) as total_item"); 
		...
		{
			$this->db->where('hms_canteen_purchase.branch_id IN ('.$search['branch_id'].')');
		}
		else
		{
		...
		if(isset($search) && !empty($search))
		{
			...
			{
				$this->db->where('hms_canteen_purchase.invoice_id', $search['invoice_id']);
			}

			if(isset($search['purchase_no']) && !empty($search['purchase_no']))
			{
				$this->db->where('hms_canteen_purchase.purchase_id LIKE "'.$search['purchase_no'].'%"');
			}

			if(isset($search['item_name']) && !empty($search['item_name']))
			{
				$this->db->join('hms_canteen_purchase_to_purchase','hms_canteen_purchase_to_purchase.purchase_id = hms_canteen_purchase.id','left'); 
				$this->db->join('hms_canteen_item','hms_canteen_item.id = hms_canteen_purchase_to_purchase.item_id','left'); 
			...
			{
				$this->db->where('hms_canteen_purchase.purchase_rate',$search['purchase_rate']);
			}
			...
			if(isset($search['discount']) && !empty($search['discount']) && empty($search['item_code']) && empty($search['company']) && empty($search['item_name']))
			{
				if(empty($search['item_code']) && empty($search['company']) && empty($search['item_name']))
				{
                      ...
                      //$this->db->where('hms_canteen_purchase_to_purchase.unit2 LIKE "%'.$search['unit2'].'%"');
				}  
				$this->db->where('hms_canteen_purchase_to_purchase.discount = "'.$search['discount'].'"');
			}
			...
			if(isset($search['batch_no']) && !empty($search['batch_no']))
			{
				...
				$this->db->where('hms_canteen_purchase_to_purchase.batch_no = "'.$search['batch_no'].'"');
			}
			...
			if(isset($search['unit1']) && $search['unit1']!="")
			{
				...
				$this->db->where('hms_canteen_purchase_to_purchase.unit1 LIKE "%'.$search['unit1'].'%"');
			}
			...
			if(isset($search['cgst']) && !empty($search['cgst']))
			{
				...
				$this->db->where('hms_canteen_purchase_to_purchase.cgst', $search['cgst']);
			}
			...
			if(isset($search['igst']) && !empty($search['igst']))
			{
				...
				$this->db->where('hms_canteen_purchase_to_purchase.igst', $search['igst']);
			}
			...
			if(isset($search['sgst']) && !empty($search['sgst']))
			{
				...
				$this->db->where('hms_canteen_purchase_to_purchase.sgst', $search['sgst']);
			}
			...
			{
				$this->db->where('hms_canteen_purchase.paid_amount >="'.$search['paid_amount_to'].'"');
			}
			if(isset($search['paid_amount_from']) && $search['paid_amount_from']!="")
			{
				$this->db->where('hms_canteen_purchase.paid_amount <="'.$search['paid_amount_from'].'"');
			}

			if(isset($search['balance_to']) && $search['balance_to']!="")
			{
				$this->db->where('hms_canteen_purchase.balance >="'.$search['balance_to'].'"');
			}
			if(isset($search['balance_from']) && $search['balance_from']!="")
			{
				$this->db->where('hms_canteen_purchase.balance <="'.$search['balance_from'].'"');
			}

			if(isset($search['total_amount_to']) && $search['total_amount_to']!="")
			{
				$this->db->where('hms_canteen_purchase.total_amount >="'.$search['total_amount_to'].'"');
			}
			if(isset($search['total_amount_from']) && $search['total_amount_from']!="")
			{
				$this->db->where('hms_canteen_purchase.total_amount <="'.$search['total_amount_from'].'"');
			}

			if(isset($search['bank_name']) && $search['bank_name']!="")
			{
				$this->db->where('hms_canteen_purchase.bank_name',$search['bank_name']);
			}
		...
		{ 
			$this->db->where('hms_canteen_purchase.created_by IN ('.$emp_ids.')');
		}
    ...
    //                $this->db->join('hms_canteen_purchase_to_purchase','hms_canteen_purchase_to_purchase.purchase_id = hms_canteen_purchase.id','left'); 
				// } 
				$this->db->where('hms_canteen_purchase.cgst', $search['cgst']);
			}
			if(isset($search['igst']) && !empty($search['igst']))
			{
				// if(empty($search['item_code']) && empty($search['company']) && empty($search['item_name'])  && empty($search['discount']))
    ...
    //                $this->db->join('hms_canteen_purchase_to_purchase','hms_canteen_purchase_to_purchase.purchase_id = hms_canteen_purchase.id','left'); 
				// } 
				$this->db->where('hms_canteen_purchase.igst', $search['igst']);
			}
			if(isset($search['sgst']) && !empty($search['sgst']))
			{
				// if(empty($search['item_code']) && empty($search['company']) && empty($search['item_name'])  && empty($search['discount']))
    ...
    //                $this->db->join('hms_canteen_purchase_to_purchase','hms_canteen_purchase_to_purchase.purchase_id = hms_canteen_purchase.id','left'); 
				// } 
				$this->db->where('hms_canteen_purchase.sgst', $search['sgst']);
			}
	...
	public function get_by_id($id)
	{
		...
		$this->db->from('hms_canteen_purchase'); 
		 $this->db->where('hms_canteen_purchase.id',$id);
		$this->db->where('hms_canteen_purchase.is_deleted','0');
		$query = $this->db->get(); 
	...
	{
		$this->db->select('hms_canteen_purchase_to_purchase.*');
		$this->db->from('hms_canteen_purchase_to_purchase'); 
		if(!empty($id))
		{
          $this->db->where('hms_canteen_purchase_to_purchase.purchase_id',$id);
		} 
	...
	{ 
		...
		{  
           ...
           $blance= str_replace(',', '', $post['net_amount'])-$post['pay_amount'];
			...
			$this->db->set('modified_by',$user_data['id']);
			$this->db->set('modified_date',date('Y-m-d H:i:s'));
			$this->db->update('hms_canteen_purchase',$data);
            ...
            $this->db->delete('hms_canteen_purchase_to_purchase');
...
//	echo"<pre>"; print_r($item_list);die;
					...
					$this->db->insert('hms_canteen_purchase_to_purchase',$data_purchase_topurchase);
//echo ($this->db->last_query());die;			
...
//echo $this->db->last_query();die;
			...
			*/
			...
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_canteen_purchase',$data);
...
//echo $this->db->last_query();die;
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->update('hms_canteen_purchase');
...
//echo $this->db->last_query();die;
    ...
    public function item_list_search()
    {
    	...
    	$users_data = $this->session->userdata('auth_users'); 
    	$post = $this->input->post();  
		$this->db->select("hms_canteen_item.*, hms_canteen_stock_category.category, hms_canteen_manuf_company.company_name, hms_canteen_stock_item_unit.unit, hms_canteen_master_entry.packing, hms_canteen_master_entry.hsn_no, hms_canteen_master_entry.sgst, hms_canteen_master_entry.cgst, hms_canteen_master_entry.igst, hms_canteen_master_entry.discount, hms_canteen_master_entry.expiry_days, 	hms_canteen_master_entry.purchase_rate, hms_canteen_stock_item.price as mrp, (sum(hms_canteen_stock_item.debit)-sum(hms_canteen_stock_item.credit)) as stock_qty"); 
        $this->db->join('hms_canteen_stock_category','hms_canteen_stock_category.id=hms_canteen_item.category_id','left');
        $this->db->join('hms_canteen_stock_item_unit','hms_canteen_stock_item_unit.id=hms_canteen_item.unit_id','left');
        $this->db->join('hms_canteen_stock_item','hms_canteen_stock_item.parent_id=hms_canteen_item.id','left');
		$this->db->join('hms_canteen_master_entry','hms_canteen_master_entry.id=hms_canteen_item.product_id','left');
...
//echo $this->db->last_query();die;
	...
	public function estimate_item($vals)
	{
      ...
      {
          $user_data = $this->session->userdata('auth_users');
          		$this->db->select('hms_canteen_estimate_purchase.*,hms_canteen_vendors.name,hms_canteen_vendors.mobile,hms_canteen_vendors.vendor_gst,hms_canteen_vendors.address,hms_canteen_vendors.address2,hms_canteen_vendors.address3,hms_canteen_vendors.email');
          		$this->db->join('hms_canteen_vendors','hms_canteen_vendors.id=hms_canteen_estimate_purchase.vendor_id');
		$this->db->from('hms_canteen_estimate_purchase'); 

		 $this->db->where('hms_canteen_estimate_purchase.purchase_id LIKE "'.$vals.'%"');
	...
	$result= $query->row_array();
          ...
          if(!empty($result))
          { 
            $data = array($result['purchase_id'].'|'.$result['id'].'|'.$result['name'].'|'.$result['vendor_gst'].'|'.$result['email'].'|'.$result['mobile'].'|'.$result['address'].'|'.$result['address2'].'|'.$result['address3'].'|'.$result['remarks']); 
	...
	public function get_estimate_item_by_id($purchase_id)
	{
		$response = '';
      if(!empty($purchase_id))
      {
          $user_data = $this->session->userdata('auth_users');
          $this->db->select("hms_canteen_estimate_purchase.id,hms_canteen_estimate_purchase.total_amount as grand_total,hms_canteen_estimate_purchase.discount as total_discount,hms_canteen_estimate_purchase.purchase_id,
		  hms_canteen_estimate_purchase_to_purchase.item_id, hms_canteen_estimate_purchase_to_purchase.purchase_rate, hms_canteen_estimate_purchase_to_purchase.discount ,hms_canteen_estimate_purchase_to_purchase.hsn_no, hms_canteen_estimate_purchase_to_purchase.batch_no, hms_canteen_estimate_purchase_to_purchase.qty, hms_canteen_estimate_purchase_to_purchase.mrp, hms_canteen_estimate_purchase_to_purchase.total_amount, hms_canteen_estimate_purchase_to_purchase.manuf_date, hms_canteen_estimate_purchase_to_purchase.expiry_date, hms_canteen_estimate_purchase_to_purchase.sgst sgst, hms_canteen_estimate_purchase_to_purchase.cgst ,hms_canteen_estimate_purchase_to_purchase.igst ,hms_canteen_item.item, hms_canteen_item.item_code"); 
         $this->db->join('hms_canteen_estimate_purchase_to_purchase','hms_canteen_estimate_purchase_to_purchase.purchase_id=hms_canteen_estimate_purchase.id','left');
          ...
          //$this->db->where('hms_canteen_estimate_purchase.id',$estimate_id);
		 //$this->db->where('hms_canteen_item.id',$item_id);
          //$this->db->where('hms_patient.patient_code LIKE "'.$vals.'%"');
          $this->db->where('hms_canteen_estimate_purchase.purchase_id ',$purchase_id);
          $this->db->from('hms_canteen_estimate_purchase'); 
		  $this->db->where('hms_canteen_estimate_purchase.is_deleted','0');
         // $this->db->group_by('hms_canteen_estimate_purchase.id');
          $query = $this->db->get(); 
  //echo $this->db->last_query(); exit;
 ...
 //echo "<pre>"; print_r($result);die;
            foreach($result as $items)
           {
			 ...
			 $post_iid_arr[$items['item_id']] = array('iid'=>$items['item_id'],'item_code'=>$items['item_code'],'item'=>$items['item'],'hsn_no'=>$items['hsn_no'],'batch_no'=>$items['batch_no'],'expiry_date'=>date('d-m-Y',strtotime($items['expiry_date'])),'manuf_date'=>date('d-m-Y',strtotime($items['manuf_date'])),'discount'=>$items['discount'],'qty'=>$items['qty'],'mrp'=>$items['mrp'],'purchase_amount'=>$items['purchase_rate'],'cgst'=>$items['cgst'],'sgst'=>$items['sgst'],'igst'=>$items['igst'],'total_amount'=>$items['total_amount']);
	...
	{
		 $this->db->select("hms_canteen_purchase.vendor_id, hms_canteen_purchase_to_purchase.purchase_id,hms_canteen_purchase_to_purchase.item_id, hms_canteen_item.item, hms_canteen_vendors.name, hms_canteen_purchase_to_purchase.purchase_rate, hms_canteen_purchase.purchase_date"); 

         $this->db->join('hms_canteen_purchase_to_purchase','hms_canteen_item.id=hms_canteen_purchase_to_purchase.item_id','left');

          $this->db->join('hms_canteen_purchase','hms_canteen_purchase.id=hms_canteen_purchase_to_purchase.purchase_id','left'); 
        
          $this->db->join('hms_canteen_vendors','hms_canteen_purchase.vendor_id=hms_canteen_vendors.id','left');
     ...
     public function check_unique_value($branch_id, $invoice_id, $id='')
    {
		...
		$this->db->from('hms_canteen_purchase'); 
		$this->db->where('hms_canteen_purchase.branch_id',$branch_id);
		$this->db->where('hms_canteen_purchase.id',$invoice_id);
		if(!empty($id))
		$this->db->where('hms_canteen_purchase.id !=',$id);
		$this->db->where('hms_canteen_purchase.is_deleted','0');
		$query = $this->db->get(); 
