...
class Employee_salary_model extends CI_Model {
	...
	var $column = array('hms_canteen_expenses.id','hms_canteen_expenses.expenses_date', 'hms_canteen_expenses.paid_amount','hms_canteen_expenses.payment_mode','hms_canteen_expenses.created_date','hms_canteen_expenses.modified_date');  
	...
	private function _get_datatables_query()
	{
        ...
        $this->db->select("hms_canteen_expenses.*,hms_payment_mode.payment_mode, hms_employees.name"); 
        $this->db->from($this->table); 
       
        $this->db->where('(hms_canteen_expenses.is_deleted="0" AND hms_canteen_expenses.type="1")');
       ...
       if(isset($employe_search_detail['branch_id']) && $employe_search_detail['branch_id']!=''){
        $this->db->where('hms_canteen_expenses.branch_id IN ('.$employe_search_detail['branch_id'].')');
        }else{
	...
	public function get_by_id($id)
	{
		$users_data = $this->session->userdata('auth_users');
        $users_data = $this->session->userdata('auth_users');
        $this->db->select("hms_canteen_expenses.*,hms_employees.name,hms_employees.contact_no,hms_emp_type.department,hms_employees.email as employee_email,hms_payment_mode.payment_mode"); 
        $this->db->from($this->table); 
        $this->db->join("hms_employees","hms_canteen_expenses.employee_id=hms_employees.id",'left');
        $this->db->join("hms_emp_type","hms_canteen_expenses.emp_type_id=hms_emp_type.id",'left');
        $this->db->join('hms_payment_mode','hms_payment_mode.id=hms_canteen_expenses.payment_mode','left');
        $this->db->where('hms_canteen_expenses.id',$id);
        $this->db->where('hms_canteen_expenses.is_deleted','0');
        $this->db->where('(hms_canteen_expenses.branch_id = "'.$users_data['parent_id'].'" OR hms_canteen_expenses.branch_id = "0")');
		$query = $this->db->get();
		//echo $this->db->last_query();die;
