...
class Stock_issue_allotment_return_model extends CI_Model 
{
	...
	private function _get_datatables_query()
	{
		...
		if(isset($search) && !empty($search))
		{
			...
			$this->db->where('hms_stock_issue_allotment_return_item.created_date >= "'.$start_date.'"');
			}

			if(!empty($search['end_date']))
			{
			$end_date = date('Y-m-d',strtotime($search['end_date'])).' 23:59:59';
			$this->db->where('hms_stock_issue_allotment_return_item.created_date <= "'.$end_date.'"');
			}
		}
  ...
  //       if(isset($search['discount']) &&$search['discount']!="")
	...
	{
		 $users_data = $this->session->userdata('auth_users');
		 $this->db->select('hms_stock_issue_allotment_return_to_issue_allotment_return.issue_return_id,hms_stock_issue_allotment_return_to_issue_allotment_return.category_id,hms_stock_issue_allotment_return_to_issue_allotment_return.total_amount as amount,path_item.item as item_name,path_item.item_code,hms_stock_issue_allotment_return_to_issue_allotment_return.qty as quantity,hms_stock_item_unit.unit,hms_stock_issue_allotment_return_to_issue_allotment_return.per_pic_price as item_price,path_item.id as item_id,path_stock_category.category');
		...
		if(!empty($query))
		{
		  ...
		  {
            $result[$item_list['item_id']] =  array('item_id'=>$item_list['item_id'],'total_price'=>$item_list['amount'],'category_id'=>$item_list['category_id'],'item_code'=>$item_list['item_code'],'item_name'=>$item_list['item_name'].'-'.$item_list['category'],'amount'=>$item_list['item_price']*$item_list['quantity'],'unit'=>$item_list['unit'],'item_price'=>$item_list['item_price'],'quantity'=>$item_list['quantity'],'total_amount'=>''); 
    ...
    function get_all_detail_print($id="",$branch_id="")
	{
		...
		//echo $this->db->last_query(); exit;
		$result_sales['stock_issue_return_list']= $this->db->get()->result();
