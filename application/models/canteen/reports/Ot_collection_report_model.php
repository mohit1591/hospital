...
class Ot_collection_report_model extends CI_Model {
    ...
    private function _get_datatables_query()
    {
        ...
        // /(CASE WHEN hms_operation_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name
        ...
        //$this->db->where('hms_operation_booking.booking_status',1); 
            
       
        
        /*if(isset($search_data) && !empty($search_data) && !empty($search_data['branch_id']))
        {  
             $this->db->where('hms_operation_booking.branch_id',$search_data['branch_id']);
        }
        else
        {
                ...
                )
        {
            ...
            if($search_data['branch_id']=='all')
            {
                ...
                $this->db->where('hms_operation_booking.branch_id IN ('.$sub_ids.')');
            }
            else
            {
                $this->db->where('hms_operation_booking.branch_id',$search_data['branch_id']);
    ...
    function search_report_data()
    {
        ...
        /*$this->db->select("hms_operation_booking.*, hms_doctors.doctor_name, hms_patient.patient_name"); 
        $this->db->join('hms_patient','hms_patient.id = hms_operation_booking.patient_id');
        //$this->db->join('hms_department','hms_department.id = hms_opd_booking.dept_id'); //, hms_department.department
        $this->db->join('hms_doctors','hms_doctors.id = hms_operation_booking.refered_id','left');
        //$this->db->where('hms_operation_booking.type',0); 
        $this->db->where('hms_operation_booking.is_deleted',0); */
        //$this->db->where('hms_operation_booking.booking_status',1); 
        
          /*$this->db->select("hms_operation_booking.id, hms_operation_booking.sale_no,   hms_payment.debit as paid_amount, hms_operation_booking.sale_date,  (CASE WHEN hms_payment.balance>0 THEN hms_operation_booking.net_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (hms_operation_booking.discount) ELSE '0.00' END) as discount,  hms_doctors.doctor_name, hms_patient.patient_name, (CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 3 AND payment.parent_id = hms_operation_booking.id) ELSE '0.00' END) as balance"); */ 
