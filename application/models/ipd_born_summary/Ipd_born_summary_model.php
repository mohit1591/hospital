...
class Ipd_born_summary_model extends CI_Model {

	var $table = 'hms_born_summery';
	var $column = array('hms_born_summery.id','hms_patient.patient_name','hms_patient.patient_code','hms_patient.mobile_no','hms_born_summery.patient_id', 'hms_born_summery.weight','hms_born_summery.created_date',
	'hms_born_summery.type_of_delivery',
	'hms_born_summery.caste',
	'hms_born_summery.religion',
	'hms_born_summery.para',
	'hms_born_summery.remarks'
);   
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_born_summery.*,hms_ipd_booking.ipd_no,hms_patient.patient_name,hms_patient.age_m,hms_patient.age_y,hms_patient.age_d,hms_patient.patient_code,hms_patient.age_y,hms_patient.mobile_no,hms_patient.address");

		$this->db->join('hms_ipd_booking','hms_ipd_booking.id=hms_born_summery.ipd_id','left');
		$this->db->join('hms_patient','hms_patient.id=hms_ipd_booking.patient_id','left');
		$this->db->from($this->table); 
		$this->db->where('hms_born_summery.branch_id',$users_data['parent_id']);

		if(isset($ipd_born_summary_search) && !empty($ipd_born_summary_search))
        {
            
             if(isset($ipd_born_summary_search['start_date']) && !empty($ipd_born_summary_search['start_date']))
            {
                $start_date = date('Y-m-d',strtotime($ipd_born_summary_search['start_date'])).' 00:00:00';
                $this->db->where('hms_born_summery.created_date >= "'.$start_date.'"');
            }

            if(isset($ipd_born_summary_search['end_date']) && !empty($ipd_born_summary_search['end_date']))
            {
                $end_date = date('Y-m-d',strtotime($ipd_born_summary_search['end_date'])).' 23:59:59';
                $this->db->where('hms_born_summery.created_date <= "'.$end_date.'"');
            }

            if(isset($ipd_born_summary_search['patient_name']) && !empty($ipd_born_summary_search['patient_name']))
			{
				$this->db->where('hms_patient.patient_name LIKE "'.$ipd_born_summary_search['patient_name'].'%"');
			}
	...
	function search_report_data()
    {
        ...
        $this->db->order_by('hms_born_summery.id','desc');
        $query = $this->db->get(); 
        $data= $query->result();
        //echo $this->db->last_query();die;
        return $data;
    ...
    public function delete($id="")
    {
    	if(!empty($id) && $id>0)
    	{
			...
			$this->db->where('id',$id);
			$this->db->delete('hms_born_summery');
			//echo $this->db->last_query();die;
    	} 
    }
	...
	public function get_by_id($id)
	{
		...
		$this->db->where('hms_born_summery.id',$id);
		$this->db->where('hms_born_summery.is_deleted','0');
		$query = $this->db->get(); 
	...
	public function save()
	{
		...
		{    
			...
			$this->db->update('hms_born_summery',$data);
			...
			$this->db->delete('hms_born_summery_medicine');
			if($post['medicine_name'][0]!="")
			{
				...
				$this->db->insert('hms_born_summery_medicine',$data_medicine);
		...
		{    
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_born_summery',$data); 
...
$users_data = $this->session->userdata('auth_users'); 
	...
	public function save_medicine()
	{
		...
		$total_prescription = count(array_filter($post['medicine_name']));  
			...
			{	
					...
					$this->db->insert('hms_born_summery_medicine',$prescription_data);
    ...
    public function deleteall($ids=array())
    {
    	...
    	{ 
			...
			$this->db->update('hms_born_summery');
			//echo $this->db->last_query();die;
    	} 
    }
  ...
  public function get_detail_by_summary_id($discharge_id)
  {
    ...
    $this->db->where('hms_born_summery.id',$discharge_id);
    $this->db->join('hms_patient','hms_patient.id=hms_born_summery.patient_id','left');
   ...
   /* $this->db->select('hms_born_summery_medicine_administered.medicine_name,hms_born_summery_medicine_administered.medicine_dose,hms_born_summery_medicine_administered.medicine_duration,hms_born_summery_medicine_administered.medicine_frequency,hms_born_summery_medicine_administered.medicine_advice');
    $this->db->from('hms_born_summery_medicine_administered');
    $this->db->where('hms_born_summery_medicine_administered.discharge_summary_id',$discharge_id);
   	$result_pre['medicine_administered_detail']= $this->db->get()->result();
*/
    $this->db->select('hms_born_summery_medicine.medicine_name,hms_born_summery_medicine.medicine_dose,hms_born_summery_medicine.medicine_duration,hms_born_summery_medicine.medicine_frequency,hms_born_summery_medicine.medicine_advice');
    $this->db->from('hms_born_summery_medicine');
    $this->db->where('hms_born_summery_medicine.discharge_summary_id',$discharge_id);
  ...
  public function get_doctor_signature($doctor_id='')
  {
	...
	{
		$this->db->select('hms_born_summery_medicine.*');
		$this->db->from('hms_born_summery_medicine'); 
		$this->db->where('hms_born_summery_medicine.discharge_summary_id',$id);
		$query = $this->db->get(); 
