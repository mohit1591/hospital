...
class Pathology_invoice_model extends CI_Model {

  var $table = 'path_test_booking_invoice';
  var $column = array('path_test_booking_invoice.id', 'hms_patient.patient_code', 'path_test_booking_invoice.lab_invoice_no', 'hms_patient.patient_name',   'hms_patient.relation_name', 'hms_patient.mobile_no', 'hms_patient.patient_email', 'hms_patient.insurance_type', 'hms_insurance_company.insurance_company', 'hms_insurance_type.insurance_type', 'hms_patient.polocy_no', 'hms_patient.tpa_id', 'hms_patient.ins_amount', 'hms_patient.ins_authorization_no','hms_patient.gender', 'hms_patient.age_y', 'hms_patient.address','path_test_booking_invoice.form_f', 'path_test_booking_invoice.tube_no', 

    'hms_doctors.doctor_name','hms_employees.name', 'emp.name','path_test_booking_invoice.booking_date', 'path_test_booking_invoice.remarks','hms_payment_mode.payment_mode','path_test_booking_invoice.total_amount','path_test_booking_invoice.home_collection_amount', 'path_test_booking_invoice.discount', 'path_test_booking_invoice.net_amount', 'path_test_booking_invoice.paid_amount', 'path_test_booking_invoice.balance','path_test_booking_invoice.status','path_test_booking_invoice.modified_date','path_test_booking_invoice.created_by');  
  ...
  {
    $search_date = $this->session->userdata('invoice_search_booking_date');

    //print_r($search_date);die;
        $users_data = $this->session->userdata('auth_users');
        $complation_status=$this->session->userdata('invoice_complation_status');

        $post = $this->input->post();
    $this->db->select("path_test_booking_invoice.id, path_test_booking_invoice.branch_id, path_test_booking_invoice.delivered_status, path_test_booking_invoice.remarks,path_test_booking_invoice.complation_status,path_test_booking_invoice.verify_status,path_test_booking_invoice.lab_invoice_no,path_test_booking_invoice.tube_no, path_test_booking_invoice.form_f, path_test_booking_invoice.booking_date, path_test_booking_invoice.net_amount,path_test_booking_invoice.total_amount, path_test_booking_invoice.paid_amount, path_test_booking_invoice.discount,path_test_booking_invoice.created_date, path_test_booking_invoice.modified_date,path_test_booking_invoice.home_collection_amount, hms_patient.patient_name, hms_patient.patient_code, (CASE When hms_patient.relation_type=1 THEN 'Father/o' When hms_patient.relation_type=2 THEN 'Husband/o' When hms_patient.relation_type=3 THEN 'Baby/o' When hms_patient.relation_type=4 THEN 'Son/o' When hms_patient.relation_type=5 THEN 'Daughter/o' Else '' END) as patient_relation, hms_patient.relation_name, hms_patient.mobile_no,
      hms_patient.patient_email,  (CASE WHEN hms_patient.insurance_type=1 THEN 'TPA' Else 'Normal' END ) as ins_type, hms_insurance_company.insurance_company, hms_insurance_type.insurance_type , hms_patient.polocy_no, hms_patient.tpa_id, hms_patient.ins_amount, hms_patient.ins_authorization_no,(CASE WHEN hms_patient.gender=0 THEN 'Female' ELSE 'MALE' END) as patient_gender, hms_patient.age_y, hms_patient.age_m, hms_patient.age_d, hms_patient.age_h, concat_ws(', ', hms_patient.address, hms_patient.address2, hms_patient.address3 ) as patient_address,
        (CASE WHEN path_test_booking_invoice.form_f=1 THEN 'Yes' WHEN path_test_booking_invoice.form_f=2 THEN 'No' ELSE '' END ) as path_form_f, 
        
        (CASE WHEN path_test_booking_invoice.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN path_test_booking_invoice.   referral_doctor=0 THEN concat('Other ',path_test_booking_invoice.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name, 
      ...
      (CASE WHEN created_users.emp_id>0 THEN created_emp.name ELSE 'Admin' END) as created_name,(select sum(payment.debit) as paid_amount1 from hms_payment as payment where payment.parent_id = path_test_booking_invoice.id AND payment.section_id =1 AND payment.branch_id = ".$users_data['parent_id'].") as paid_amount1,
(select sum(credit)-sum(debit) as balance from hms_payment as  payment where payment.parent_id = path_test_booking_invoice.id AND payment.section_id=1 AND payment.branch_id = ".$users_data['parent_id'].") as balance1
      "); 
    
    $this->db->from($this->table); 
    $this->db->join('hms_payment','hms_payment.parent_id = path_test_booking_invoice.id AND hms_payment.section_id = "1" AND hms_payment.branch_id="'.$users_data['parent_id'].'"','left');
    $this->db->join('hms_patient','hms_patient.id = path_test_booking_invoice.patient_id','left');
    $this->db->join('hms_doctors','hms_doctors.id = path_test_booking_invoice.referral_doctor','left');
    $this->db->join('hms_hospital','hms_hospital.id = path_test_booking_invoice.referral_hospital','left');
    ...
    $this->db->join('hms_employees','hms_employees.id=path_test_booking_invoice.sample_collected_by', 'left');

    $this->db->join('hms_employees as emp','emp.id=path_test_booking_invoice.staff_refrenace_id', 'left');
    $this->db->join('hms_users as created_users','created_users.id=path_test_booking_invoice.created_by', 'left');
    $this->db->join('hms_employees as created_emp','created_emp.id=created_users.emp_id', 'left');
      $this->db->join('hms_payment_mode','hms_payment_mode.id=path_test_booking_invoice.payment_mode', 'left');
    /** added on 26feb2018 **/
        ...
        {
           $this->db->where('path_test_booking_invoice.patient_id',$users_data['parent_id']);
        }
        else if($users_data['users_role']==3)
        {
           $this->db->where('path_test_booking_invoice.referral_doctor',$users_data['parent_id']);
        }
        else
        {
          if(isset($post['branch_id']) && !empty($post['branch_id']))
          {
            $this->db->where('path_test_booking_invoice.branch_id',$post['branch_id']);
          }
          else
          {
            $this->db->where('path_test_booking_invoice.branch_id',$users_data['parent_id']);
          } 
        } 
        $this->db->group_by('path_test_booking_invoice.id');
        if(isset($search_date) && !empty($search_date))
        {
            ...
            )
          {
            ...
            $this->db->where('path_test_booking_invoice.booking_date >= "'.$start_date.'"');
          }
            ...
            )
          {
            ...
            $this->db->where('path_test_booking_invoice.booking_date <= "'.$end_date.'"');
          }
          ...
          {
            $this->db->where('path_test_booking_invoice.barcode_text',$search_date['bar_code']);
          }
          if(isset($search_date['tube_no']) && !empty($search_date['tube_no']) )
          {
            $this->db->where('path_test_booking_invoice.tube_no',$search_date['tube_no']);
          }

          if(isset($search_date['lab_invoice_no']) && !empty($search_date['lab_invoice_no']) )
          {
            $this->db->where('path_test_booking_invoice.lab_invoice_no',$search_date['lab_invoice_no']);
          }

          if(isset($search_date['referral_doctor']) && !empty($search_date['referral_doctor']) )
          {
            $this->db->where('path_test_booking_invoice.referral_doctor',$search_date['referral_doctor']);
          }

          if(isset($search_date['attended_doctor']) && !empty($search_date['attended_doctor']) )
          {
            $this->db->where('path_test_booking_invoice.attended_doctor',$search_date['attended_doctor']);
          }
          
           if(isset($search_date['collection_center']) && !empty($search_date['collection_center']) )
          {
            $this->db->where('path_test_booking_invoice.collection_center',$search_date['collection_center']);
          }
...
//print_r($search_date); die;
 //referred_by
      ...
      {
      $this->db->where('path_test_booking_invoice.referral_hospital' ,$search_date['referral_hospital']);
      }
      elseif(isset($search_date['refered_id']) && $search_date['referred_by']=='0' && !empty($search_date['refered_id']))
      {
          $this->db->where('path_test_booking_invoice.referral_doctor' ,$search_date['refered_id']);
      }
    }
        ...
        if($users_data['emp_id']>0)
        {
            if($users_data['record_access']=='1')
            {
                ...
                $this->db->where('path_test_booking_invoice.created_by',$emp_ids);
            }
        }
  ...
  {
    $user_data = $this->session->userdata('auth_users');
    $company_data = $this->session->userdata('invoice_company_data');
    $sub_branches_data = $this->session->userdata('invoice_sub_branches_data');
    $branch_list = '';
    ...
    {
        $users_data = $this->session->userdata('auth_users');
        $booking_set_branch = $this->session->userdata('invoice_booking_set_branch');
      ...
      $booking_list = $this->session->userdata('invoice_book_test');
      ...
      $test_master_rate = 0;
        ...
        $profile_data = $this->session->userdata('invoice_set_profile'); 
         ...
         { 
           foreach($booking_list as $test)
           {
             /////////// Transection doctor payment ///////// 
              ...
              $booking_doctor_type = $this->session->userdata('invoice_booking_doctor_type');
             ...
             $test_panel_rate = $this->session->userdata('invoice_test_panel_rate');
          ...
          $master_rate=$rate_branch_data['path_price'];
            ...
            //////////////////////////////////////////////////
             ...
             if(!empty($gst_val))
             {
                 $test_gst = $master_rate-($master_rate/(1+($gst_val/100)));
                 //$total_gst =$total_gst+round($test_gst,2);
                 $total_amount = round(($total_amount+$master_rate)-$test_gst,2);
...
public function save()
{
    ...
    $user_data = $this->session->userdata('auth_users'); 
     ...
     $dep=array();
     $dep_uni=array();
     $booked_test_list = $this->session->userdata('invoice_book_test'); 
     $post = $this->input->post(); 
    ...
    $total_booked_profile_test = count($this->session->userdata('invoice_book_test'))+count($this->session->userdata('invoice_set_profile'));
    ...
    if(isset($profile_data))
    {
      ...
      ///////////////////////////////////////
        ...
        $lab_invoice_no = generate_unique_id(73);
    ...
    {    
            ...
            $this->db->update('path_test_booking_invoice',$data_test);
            //echo $this->db->last_query(); exit;

            ///////// Set Profile in db ////////
            $profile_sess_data = $this->session->userdata('invoice_set_profile');
            ...
            if(isset($profile_sess_data) && !empty($profile_sess_data))
            {
              ...
              $this->db->delete('path_test_booking_to_profile_invoice');
              ...
              { 
                ...
                $this->db->insert('path_test_booking_to_profile_invoice',$prf_sess);
                ///// Start Child profile  ////////////////
      ...
      $this->db->delete('path_test_booking_to_test_invoice'); 
      $booked_list = $this->session->userdata('invoice_book_test');   
        ...
        {  
          ...
          foreach($unique_test_list as $unique_test)
          {
            ...
            {    
              ...
              $count_total_array = array_merge($this->session->userdata('invoice_book_test'),$profile_all_test);
              $total_tests = count($this->session->userdata('invoice_book_test'));
            ...
            $this->db->insert('path_test_booking_to_test_invoice',$booked_data);
    ...
    { 
      ...
      $is_outsource=0;
        ...
        { 
          ...
          $this->session->set_userdata('invoice_booking_patient_id',$patient_id); 
          ////// Set Patient
          $data = array(     
          "users_role"=>4,
    ...
    ////////// SEND EMAIL ///////////////////
        ...
        ////////////////////////////////////////
          ///// End /////////
            ...
            $this->db->set('lab_invoice_no',$lab_invoice_no);
            $this->db->set('patient_id',$patient_id);
            $this->db->set('created_by',$user_data['id']);
            $this->db->set('created_date',date('Y-m-d H:i:s'));
            $this->db->insert('path_test_booking_invoice',$data_test);    
    ...
    $this->session->unset_userdata('invoice_advance_result');
    $this->session->unset_userdata('invoice_set_profile');
    $this->session->unset_userdata('invoice_booking_set_branch');
    $this->session->unset_userdata('invoice_booking_doctor_type'); 
    $this->session->unset_userdata('invoice_test_panel_rate');
    $this->session->unset_userdata('invoice_book_test');
    $this->session->unset_userdata('invoice_set_profile');
  ...
  function getChildrentest($parent_id) 
    ...
    { 
       ...
       if(!empty($post['result']))
       {
          ...
          foreach($post['result'] as $r_key=>$result)
          {
              ...
              $this->db->update('path_test_booking_to_test_invoice',$data);
    ...
    {
      $users_data = $this->session->userdata('auth_users'); 
      $this->db->select('path_test_booking_to_profile_invoice.*, path_profile.profile_name,path_profile.print_name');  
      $this->db->join('path_profile','path_profile.id = path_test_booking_to_profile_invoice.profile_id');
      if(!empty($booking_id))
      {
        $this->db->where('path_test_booking_to_profile_invoice.test_booking_id',$booking_id);
      }
      if(!empty($profile_id))
      {
        $this->db->where('path_test_booking_to_profile_invoice.profile_id',$profile_id);
      }     
      $this->db->where('path_test_booking_to_profile_invoice.parent_id',$parent_id);       
      //$this->db->order_by('path_profile.sort_order','ASC');  
      $this->db->order_by("path_test_booking_to_profile_invoice.sort_order ASC, path_profile.sort_order ASC"); 
...
//(CASE WHEN path_profile.print_name="" THEN path_profile.profile_name ELSE path_profile.print_name END) as profile_name
    ...
    {  
        $users_data = $this->session->userdata('auth_users');
//(CASE WHEN path_profile.print_name="" THEN path_profile.profile_name ELSE path_profile.print_name END) as profile_name
        $this->db->select('path_test_booking_to_profile_invoice.*,path_profile.profile_name,path_profile.print_name, count(path_test_booking_to_test_invoice.id) as total_test, count(cprofile.id) as total_child_profile');  
        $this->db->join('path_profile','path_profile.id = path_test_booking_to_profile_invoice.profile_id');
        if(!empty($booking_id))
        {
            $this->db->where('path_test_booking_to_profile_invoice.test_booking_id',$booking_id);
        ...
        $this->db->order_by("path_test_booking_to_profile_invoice.sort_order ASC, path_profile.sort_order ASC"); 
        $this->db->where('path_test_booking_to_profile_invoice.parent_id',$parent_id); 
        $this->db->join('path_test_booking_to_test_invoice','path_test_booking_to_test_invoice.booking_id = path_test_booking_to_profile_invoice.test_booking_id AND path_test_booking_to_test_invoice.profile_id=path_test_booking_to_profile_invoice.profile_id  AND path_test_booking_to_test_invoice.print=1', 'left'); 
        $this->db->join('path_test_booking_to_profile_invoice as cprofile','cprofile.test_booking_id = path_test_booking_to_profile_invoice.test_booking_id AND cprofile.parent_id=path_test_booking_to_profile_invoice.profile_id', 'left');
        $this->db->group_by('path_test_booking_to_profile_invoice.profile_id');
        $query = $this->db->get('path_test_booking_to_profile_invoice');
        $result = $query->result();
        //echo $this->db->last_query();
        return $result;
...
//(CASE WHEN path_profile.print_name="" THEN path_profile.profile_name ELSE path_profile.print_name END) as profile_name
        ...
        $this->db->select('path_test_booking_to_test_invoice.*,path_profile.profile_name,path_profile.print_name, (select profile.parent_id from path_test_booking_to_profile_invoice as profile where profile.test_booking_id = path_test_booking_to_test_invoice.booking_id AND profile.profile_id = path_test_booking_to_profile_invoice.profile_id AND profile.parent_id = "'.$parent_id.'" '.$profile_idquery.') as parent_id');  
        $this->db->join('path_profile','path_profile.id = path_test_booking_to_test_invoice.profile_id');
        if(!empty($booking_id))
        {
            $this->db->where('path_test_booking_to_test_invoice.booking_id',$booking_id);
        }  
        $this->db->join('path_test_booking_to_profile_invoice','path_test_booking_to_profile_invoice.profile_id = path_test_booking_to_test_invoice.profile_id', 'left');    
        //$this->db->order_by('path_profile.sort_order','ASC'); 
        $this->db->order_by("path_test_booking_to_profile_invoice.sort_order ASC, path_profile.sort_order ASC"); 
        $this->db->where('path_test_booking_to_test_invoice.profile_id>0');
        $this->db->where('path_test_booking_to_test_invoice.print','1');
        $this->db->where('path_test_booking_to_test_invoice.test_type IN (1,2)');
        //$this->db->where('path_test_booking_to_profile_invoice.parent_id',$parent_id);
        $this->db->group_by('path_test_booking_to_test_invoice.profile_id');
  ...
  public function referal_hospital_list($branch_id="")
    ...
    $this->db->select('path_test.test_name,path_test.dept_id, path_test.result_type,  path_test.test_type_id, path_test.test_code, path_test_booking_to_test_invoice.*, (CASE WHEN path_test_formula.id > 0 THEN 1 ELSE 0 END) as formula_avl,path_test.test_result_type');  
    ...
    if($test_type!="")
    {
          $this->db->where('path_test_booking_to_test_invoice.test_type IN ('.$test_type.')');  
    }

    if($child_profile_status!="")
    {
          $this->db->where('path_test_booking_to_test_invoice.child_profile_status IN ('.$child_profile_status.')');  
    }

    if(!empty($profile_id))
    {
          $this->db->where('path_test_booking_to_test_invoice.profile_id',$profile_id); 
          $this->db->join('path_profile_to_test','path_profile_to_test.test_id = path_test_booking_to_test_invoice.test_id AND path_profile_to_test.profile_id = path_test_booking_to_test_invoice.profile_id', "left");  
    }
        
        $this->db->join('path_test','path_test.id = path_test_booking_to_test_invoice.test_id');
    ...
    {
        $users_data = $this->session->userdata('auth_users');
        $this->db->select('path_test_booking_to_test_invoice.*,path_test.test_name, path_test.interpretation as test_interpretation,path_test.range_from,path_test.range_to,path_test.unit_id,path_test.test_type_id, path_test.range_from_pre, path_test.range_from_post, path_test.range_to_pre, path_test.range_to_post, path_test.all_range_show');
        $this->db->join('path_test','path_test.id = path_test_booking_to_test_invoice.test_id','left'); 
        $this->db->where('path_test_booking_to_test_invoice.booking_id = "'.$booking_id.'"');
        if(!empty($profile_id))
        {
           $this->db->where('path_test_booking_to_test_invoice.profile_id = "'.$profile_id.'"');
        ...
        $this->db->order_by('path_test.sort_order','ASC');
        $this->db->from('path_test_booking_to_test_invoice');
    ...
    $this->db->select('path_test_booking_invoice.*, hms_patient.patient_code, path_profile.profile_name'); 
    if(!empty($id))
    {
      $this->db->where('path_test_booking_invoice.id',$id); 
    }
    $this->db->join('hms_patient','hms_patient.id = path_test_booking_invoice.patient_id');
    $this->db->join('path_profile','path_profile.id = path_test_booking_invoice.profile_id','left');
    if(isset($booking_set_branch) && !empty($booking_set_branch))
        {
          $this->db->where('path_test_booking_invoice.branch_id',$booking_set_branch); 
        }
        else
        {
          $this->db->where('path_test_booking_invoice.branch_id IN ('.$branch_list.')'); 
        } 
    $query = $this->db->get('path_test_booking_invoice');
    //echo $this->db->last_query();die;
    $result = $query->result(); 
  ...
  function update_test_result($booking_id="",$test_id="",$result="")
  {
    ...
    $this->db->update('path_test_booking_to_test_invoice'); 
  }

  public function get_test_details($booking_id,$test_id)
  {
    $this->db->select('path_test.*,path_test_booking_to_test_invoice.result, path_test_booking_to_test_invoice.interpratation_data'); 
    $this->db->join('path_test','path_test.id = path_test_booking_to_test_invoice.test_id');
    if(!empty($test_id))
         {
           $this->db->where('path_test_booking_to_test_invoice.test_id',$test_id); 
         } 
         if(!empty($booking_id))
         {
           $this->db->where('path_test_booking_to_test_invoice.booking_id',$booking_id); 
  ...
  public function formula_test_list($booking_id="", $test_id="")
  {
       if(!empty($booking_id) && !empty($test_id))
       {
         ...
         $this->db->join('path_test_booking_to_test_invoice','path_test_booking_to_test_invoice.test_id = path_test_formula.test_id');
  ...
  public function condition_test_list($booking_id="", $test_id="")
  {
       if(!empty($booking_id) && !empty($test_id))
       {
         ...
         $this->db->join('path_test_booking_to_test_invoice','path_test_booking_to_test_invoice.test_id = path_test_condition.test_id');
    ...
    {
       $this->db->select('path_test_booking_to_test_invoice.*'); 
       if(!empty($booking_id))
       {
         $this->db->where('path_test_booking_to_test_invoice.booking_id',$booking_id); 
       } 
    ...
    function get_all_detail_print($ids="",$branch_id="")
    {
      ...
      $company_data = $this->session->userdata('invoice_company_data');
      $booking_set_branch = $this->session->userdata('invoice_booking_set_branch');
      $this->db->select("path_test_booking_invoice.*,(CASE WHEN path_test_booking_invoice.ipd_id>0 THEN 0.00 ELSE (select sum(credit)-sum(debit) from hms_payment where patient_id=path_test_booking_invoice.patient_id and parent_id=path_test_booking_invoice.id) END) as patient_balance, hms_patient.*,hms_simulation.simulation,hms_users.*, (CASE WHEN hms_users.emp_id>0 THEN (select name from hms_employees where id = hms_users.emp_id) ELSE (select branch_name from hms_branch where id = hms_users.parent_id) END) as signature_name, path_profile.profile_name, ref_doctors.doctor_pay_type as ref_doctor_type,hms_payment_mode.payment_mode,hms_branch_hospital_no.reciept_prefix,hms_branch_hospital_no.reciept_suffix,hms_gardian_relation.relation,hms_patient.relation_simulation_id,hms_patient.relation_type,hms_patient.relation_name,hms_insurance_type.insurance_type as insurance_name, hms_insurance_company.insurance_company,(CASE WHEN path_test_booking_invoice.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN path_test_booking_invoice.   referral_doctor=0 THEN concat('Other ',path_test_booking_invoice.ref_by_other) ELSE concat('Dr. ',ref_doctors.doctor_name) END) END) as doctor_hospital_name,path_test_booking_invoice.payment_mode as pay_mode_id,path_test_booking_invoice.id as test_book_id,hms_countries.country as country_name, hms_state.state as state_name, hms_cities.city as city_name,hms_patient.address as address1"); 
    ...
    $this->db->join('hms_doctors as ref_doctors','ref_doctors.id = path_test_booking_invoice.referral_doctor','left');
    ...
    $this->db->join('hms_users','hms_users.id = path_test_booking_invoice.created_by');  
    $this->db->join('hms_payment_mode','hms_payment_mode.id = path_test_booking_invoice.payment_mode'); 
    $this->db->join('hms_branch_hospital_no','hms_branch_hospital_no.parent_id = path_test_booking_invoice.id AND hms_branch_hospital_no.section_id=4','left');
    $this->db->where('path_test_booking_invoice.is_deleted','0'); 
    if($user_data['users_role']==4)
    {
            $this->db->where('path_test_booking_invoice.branch_id',$company_data['id']); 
    }
    ...
    else
    {
      if(isset($booking_set_branch) && !empty($booking_set_branch))
          ...
          {
            $this->db->where('path_test_booking_invoice.branch_id',$branch_id); 
          }
          else
          {
            $this->db->where('path_test_booking_invoice.branch_id',$user_data['parent_id']); 
          } 
    }
    
    $this->db->where('path_test_booking_invoice.id = "'.$ids.'"');
    $this->db->from($this->table);
    $result_booking['booking_list']= $this->db->get()->result();
    //echo $this->db->last_query(); exit;
    $this->db->select('path_test_booking_to_test_invoice.*');
    $this->db->join('path_test_booking_invoice','path_test_booking_invoice.id = path_test_booking_to_test_invoice.booking_id'); 
    $this->db->where('path_test_booking_to_test_invoice.parent_status =1');
    $this->db->where('path_test_booking_to_test_invoice.test_type IN (0,2)');
    $this->db->where('path_test_booking_to_test_invoice.booking_id = "'.$ids.'"');
    $this->db->from('path_test_booking_to_test_invoice');
    $result_booking['booking_list']['test_booking_list']=$this->db->get()->result();

    $this->db->select('path_profile.*');
    $this->db->join('path_test_booking_invoice','path_profile.id = path_test_booking_invoice.profile_id');  
    $this->db->where('path_test_booking_invoice.id',$ids);
    if(!empty($branch_id))
        {
          $this->db->where('path_test_booking_invoice.branch_id',$branch_id); 
  ...
  {
      $this->db->select('path_test_booking_to_test_invoice.*,path_test_heads.test_heads,path_test_heads.id as thead_id, path_test.dept_id');
    $this->db->join('path_test','path_test.id = path_test_booking_to_test_invoice.test_id','left');
    $this->db->join('path_test_heads','path_test_heads.id = path_test.test_head_id');  
    $this->db->where('path_test_booking_to_test_invoice.booking_id = "'.$booking_id.'"');
    $this->db->where('path_test_booking_to_test_invoice.print','1');
  ...
  public function test_booking_print($booking_id="", $branch_id="")
  {
    ...
    $this->db->join('hms_collection_center','hms_collection_center.id = path_test_booking_invoice.collection_center', 'left');
    
    $this->db->join('hms_branch','hms_branch.id = path_test_booking_invoice.branch_id');  
    $this->db->where('path_test_booking_invoice.is_deleted','0'); 
    if($user_data['users_role']==4)
    {
           $this->db->where('path_test_booking_invoice.branch_id = "'.$company_data['id'].'"');
    ...
    $this->db->where('path_test_booking_invoice.id = "'.$booking_id.'"');
    $this->db->from('path_test_booking_invoice');
    $result_booking['booking_list']= $this->db->get()->result();
    ...
    $result_booking['booking_list']['test_head_list']=$this->db->get()->result();
        ...
        $this->db->select('path_test_footer.*,hms_doctors.doctor_name,hms_doctors.signature as sign_doctor,path_test_booking_invoice.attended_doctor');
        $this->db->join('hms_doctors','hms_doctors.id = path_test_booking_invoice.attended_doctor','left');
        ...
        if($user_data['users_role']==4)
    {
           //$this->db->where('path_test_footer.branch_id = "'.$company_data['id'].'"');
            $this->db->join('path_test_footer','path_test_footer.id = path_test_booking_invoice.signature_id AND path_test_footer.branch_id = "'.$company_data['id'].'"  AND path_test_footer.is_deleted=0','left'); 
    ...
    else
    {
      if(!empty($branch_id))
      {
             //$this->db->where('path_test_footer.branch_id = "'.$branch_id.'"');
              $this->db->join('path_test_footer','path_test_footer.id = path_test_booking_invoice.signature_id AND path_test_footer.branch_id = "'.$branch_id.'"  AND path_test_footer.is_deleted=0','left'); 
      }
      else
      {
              //$this->db->where('path_test_footer.branch_id = "'.$user_data['parent_id'].'"');
               $this->db->join('path_test_footer','path_test_footer.id = path_test_booking_invoice.signature_id AND path_test_footer.branch_id = "'.$user_data['parent_id'].'" AND path_test_footer.is_deleted=0','left'); 
      }  
    }
        $this->db->where('path_test_booking_invoice.id = "'.$booking_id.'"');
        $this->db->where('path_test_booking_invoice.is_deleted','0'); 
...
(select sum(credit)-sum(debit) as balance from hms_payment as  payment where payment.parent_id = path_test_booking_invoice.id AND payment.section_id=1 AND payment.branch_id = ".$users_data['parent_id'].") as balance1
    ...
    {  
      if(!empty($booking_id))
      {
        ...
        $this->db->update('path_test_booking_to_profile_invoice');
        //echo $this->db->last_query();
      }
        ...
        public function test_to_booked_profile_id($test_id)
  {
    $this->db->select('path_profile_to_test.*'); 
      ...
      $profile_session = $this->session->userdata('invoice_set_profile');  
        $profile_id =0;
  ...
  { 
    $this->db->select('path_test_booking_to_profile_invoice.profile_interpretation as interpretation'); 
    $this->db->where('path_test_booking_to_profile_invoice.test_booking_id',$booking_id); 
    $this->db->where('path_test_booking_to_profile_invoice.profile_id',$profile_id); 
    $this->db->from('path_test_booking_to_profile_invoice');
    $result = $this->db->get()->row();
...
public function get_transaction_id($booking_id,$section_id,$type)
{
  ...
  {   
    ...
    $this->db->where('path_test_booking_to_test_invoice.booking_id',$booking_id);
    $data['department_list']= $this->db->get('path_test_booking_to_test_invoice')->result_array();
    //$data = [];
    $data_arr=array();
  ...
  public function get_booked_profiles($booking_id="")
  {
    $users_data = $this->session->userdata('auth_users');
    //(CASE WHEN path_profile.print_name="" THEN path_profile.profile_name ELSE path_profile.print_name END) as profile_name
    $this->db->select('path_test_booking_to_profile_invoice.*');  
    if(!empty($booking_id))
    {
        $this->db->where('path_test_booking_to_profile_invoice.test_booking_id',$booking_id);
    }      
...
public function get_department_id($testid)
{
  ...
  $this->db->from('path_test_booking_to_test_invoice');
  $this->db->join('path_test','path_test.id=path_test_booking_to_test_invoice.test_id');
  $this->db->where('path_test_booking_to_test_invoice.test_id',$testid);
  $query=$this->db->get();
  return $query->row_array();
}
public function get_token_type_by_branch($brachid)
{
 ...
 public function sample_type_list_new($test_id='',$sample_id='')
  ...
  {
    $user_data = $this->session->userdata('auth_users');
    $this->db->select("path_test_booking_invoice.id,path_test_booking_invoice.patient_id,path_test_booking_invoice.ipd_id,path_test_booking_invoice.booking_date"); 
    $this->db->where('path_test_booking_invoice.is_deleted','0'); 
    $this->db->where('path_test_booking_invoice.branch_id = "'.$user_data['parent_id'].'"');
    $this->db->where('path_test_booking_invoice.ipd_id = "'.$ipd_id.'"');
    $this->db->from('path_test_booking_invoice');
    $this->db->order_by('path_test_booking_invoice.booking_date','ASC');
  ...
  {
     $users_data = $this->session->userdata('auth_users');
    $this->db->select('path_test_booking_to_test_invoice.result');
    $this->db->join('path_test_booking_invoice','path_test_booking_invoice.id = path_test_booking_to_test_invoice.booking_id AND path_test_booking_invoice.booking_date="'.$date.'" AND path_test_booking_invoice.branch_id="'.$users_data['parent_id'].'"');
    $this->db->where('path_test_booking_to_test_invoice.test_id',$test_id);
    if(!empty($profile_id))
    {
      $this->db->where('path_test_booking_to_test_invoice.profile_id',$profile_id);
 ...
 /* refered by code */
      ...
      $this->db->where('path_test_booking_invoice.referral_hospital' ,$search_date['referral_hospital']);
        ...
        else
        {
            ...
            $this->db->where('path_test_booking_invoice.booking_date >= "'.date('Y-m-d').'"');
