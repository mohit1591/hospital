...
class Packages_model extends CI_Model {
	...
	var $table = 'hms_packages';
	var $column = array('hms_packages.id','hms_packages.title','hms_packages.amount','hms_packages.status','hms_packages.created_date',  'hms_packages.modified_date');  
    ...
    private function _get_medicine_kit_history_datatables_query()
    {
        ...
        if($post['opt_type']=='1'){
            $this->db->select("hms_packages.id,hms_packages.title,hms_packages.status,hms_packages.created_date,hms_packages.branch_id,hms_packages.amount,hms_medicine_kit_stock.branch_id as medicine_kit_stock_branch_id,hms_medicine_kit_stock.kit_id,hms_medicine_kit_stock.section_id , sum(hms_medicine_kit_stock.debit)-sum(hms_medicine_kit_stock.credit) as qty_kit 
                "); 
            $this->db->from($this->table); 
            $this->db->where('hms_packages.is_deleted','0');
            $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$users_data['parent_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0 or hms_medicine_kit_stock.section_id=4)');
                $this->db->or_where('(hms_medicine_kit_stock.kit_id IN (select package_id from hms_branch_to_package where branch_to_id = '.$users_data['parent_id'].'))');
          
            
            // $this->db->where('hms_medicine_kit_stock.section_id',3);
            // $this->db->where('hms_packages.branch_id',$users_data['parent_id']);
    ...
    function get_medicine_kit_history_datatables()
    {
        $this->_get_medicine_kit_history_datatables_query();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get(); 
        //echo $this->db->last_query();die;
    ...
    private function _get_medicine_kit_datatables_query()
    {
     
        $users_data = $this->session->userdata('auth_users');
        $medicine_kit_search_data = $this->session->userdata('medicine_kit_stock_search');
        //print_r($medicine_kit_search_data);die;
        // $this->db->select("hms_packages.id,hms_packages.title,hms_packages.status,hms_packages.created_date,hms_packages.branch_id,hms_packages.amount,hms_medicine_kit_stock.branch_id as medicine_kit_stock_branch_id,hms_medicine_kit_stock.kit_id,hms_medicine_kit_stock.section_id ,(select sum(kit_stock.debit)-sum(kit_stock.credit) from hms_medicine_kit_stock as kit_stock where kit_stock.kit_id = hms_packages.id) as qty_kit"); 
         $this->db->select("hms_packages.id,hms_packages.title,hms_packages.status,hms_packages.created_date,hms_packages.branch_id,hms_packages.amount,hms_medicine_kit_stock.branch_id as medicine_kit_stock_branch_id,hms_medicine_kit_stock.kit_id,hms_medicine_kit_stock.section_id , sum(hms_medicine_kit_stock.debit)-sum(hms_medicine_kit_stock.credit) as qty_kit 
            "); 
        $this->db->join('hms_packages','hms_packages.id=hms_medicine_kit_stock.kit_id');
        $this->db->from('hms_medicine_kit_stock'); 
        $this->db->where('hms_packages.is_deleted','0');
        if(isset($medicine_kit_search_data) && !empty($medicine_kit_search_data))
        {
            /*if(array_key_exists('sub_branch_id',$medicine_kit_search_data))
            {
                ...
                {
                    $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$medicine_kit_search_data['sub_branch_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0 or hms_medicine_kit_stock.section_id=4)');
                    $this->db->or_where('(hms_medicine_kit_stock.kit_id IN (select package_id from hms_branch_to_package where branch_to_id = '.$medicine_kit_search_data['sub_branch_id'].'))');
                }
                else
                {
                    $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$users_data['parent_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0 or hms_medicine_kit_stock.section_id=4)');
            ...
            if(array_key_exists('start_date',$medicine_kit_search_data))
            {
                if(!empty($medicine_kit_search_data['start_date']))
                {
                    ...
                    $this->db->where('hms_medicine_kit_stock.created_date>="'.$start_date.'"');   
                }
            ...
            if(array_key_exists('end_date',$medicine_kit_search_data))
            {
                if(!empty($medicine_kit_search_data['end_date']))
                {
                    ...
                    $this->db->where('hms_medicine_kit_stock.created_date<="'.$end_date.'"');   
                }
        ...
        if(isset($medicine_kit_search_data['branch_id']) && $medicine_kit_search_data['branch_id']!=''){
        $this->db->where('hms_medicine_kit_stock.branch_id IN ('.$medicine_kit_search_data['branch_id'].')');
        }else{
        $this->db->where('hms_medicine_kit_stock.branch_id = "'.$users_data['parent_id'].'"');
        }
        // $this->db->where('hms_medicine_kit_stock.section_id',3);
    ...
    function get_medicine_kit_datatables()
    {
        $this->_get_medicine_kit_datatables_query();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get(); 
        //echo $this->db->last_query();die;
    ...
    private function _get_medicne_datatables_query($medicine_kit_data=array())
    {
        $users_data = $this->session->userdata('auth_users');
     
        $this->db->select("hms_medicine_entry.*,hms_medicine_company.id as comp_id,hms_medicine_company.company_name"); 
        ...
        if(!empty($medicine_kit_data))
        {
            ...
            { 
                $id_list = [];
                foreach($medicine_kit_data as $id)
                {
                    if(!empty($id) && $id>0)
                    {
                        $id_list[]  = $id;
            ...
            $medicine_id = implode(',', $id_list);
            $this->db->where('hms_medicine_entry.id NOT IN ('.$medicine_id.')');
        }
        $this->db->where('(hms_medicine_entry.is_deleted=0 and hms_medicine_company.is_deleted=0)'); 
        $this->db->where('(hms_medicine_entry.branch_id='.$users_data['parent_id'].' and hms_medicine_company.branch_id='.$users_data['parent_id'].')');
    ...
    function get_medicine_datatables($medicine_kit_data)
    {
        $this->_get_medicne_datatables_query($medicine_kit_data);
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get(); 
        // echo $this->db->last_query();die;
    ...
    function medicine_count_filtered()
    {
    ...
    public function medicine_count_all()
    {
	...
	public function save($ids=array())
	{
		...
		{     
			...
			$this->db->update('hms_packages',$data);  
            ...
            $query = $this->db->get('hms_medicine_kit_stock');
            ...
            if(!empty($medicine_stock_kit))
            {
                ...
                $this->db->where('branch_id',$user_data['parent_id']);
                $this->db->update('hms_medicine_kit_stock');
        ...
        { 
			...
			$this->db->insert('hms_packages',$data); 
            ...
            $this->db->set('created_by',$user_data['id']);
            $this->db->set('created_date',date('Y-m-d H:i:s'));
            $this->db->insert('hms_medicine_kit_stock',$medicine_data_kit);
        ...
        if(!empty($medicine_kit_data))  
        {
            ...
            $this->db->delete('hms_packages_to_medicine');
            // foreach($medicine_kit_data as $medicine)
            // { 
                $medicine_data = array_values($medicine_kit_data);
              
                $medicine_data_count = count($medicine_data);
                for($i=0;$i<$medicine_data_count;$i++)
                {
                    $data_array[$i] = array(
                        'package_id'=>$pack_id,
                        'branch_id'=>$user_data['parent_id'],
                        'medicine_id'=>$medicine_data[$i]['id'],
                        'unit1_qty'=>$medicine_data[$i]['qty1'],
                        'unit2_qty'=>$medicine_data[$i]['qty2'],
                        'total_qty'=>$medicine_data[$i]['total_qty']
                    );
                    // $this->db->set('package_id',$pack_id);
                   // $this->db->set('medicine_id',$medicine_id);
                   $this->db->insert('hms_packages_to_medicine',$data_array[$i]);  
               }
    ...
    public function get_added_medicine_list()
    { 
        $medicine_kit_data = $this->session->userdata('medicine_kit_data');
        ...
        $users_data = $this->session->userdata('auth_users'); 
        $post = $this->input->post();
        $this->db->select('hms_medicine_entry.*,sum(hms_medicine_stock.debit)-sum(hms_medicine_stock.credit) as total_qty, hms_medicine_company.company_name');
        $this->db->from('hms_medicine_entry');
        ...
        {
            $this->db->where('(hms_medicine_entry.medicine_name LIKE "'.$post['search_data'].'%" OR hms_medicine_company.company_name LIKE "'.$post['search_data'].'%" OR hms_medicine_entry.medicine_code LIKE "'.$post['search_data'].'%")'); 
        }
       if(!empty($medicine_kit_data))
        {
          
                $id_list = array_column($medicine_kit_data,'id');
                $mids = implode(',',$id_list);   
            
            $this->db->where('hms_medicine_entry.id NOT IN('.$mids.')');
        } 

        $order = array('hms_medicine_entry.id' => 'desc');
        $this->db->order_by(key($order), $order[key($order)]);
    ...
    public function addallmedicine($ids=array())
    {
        $users_data = $this->session->userdata('auth_users'); 
        ...
        if(!empty($ids))
    	{
          ...
          $this->db->where('hms_medicine_entry.branch_id',$users_data['parent_id']);
           $this->db->group_by('hms_medicine_entry.id');
           $this->db->where('hms_medicine_entry.id IN ('.$medicne_ids.')');
    ...
    public function get_rest_child_test_after_delete(){
		...
		$id_list = [];
        if(!empty($medicine_kit_data)){
    		foreach($medicine_kit_data as $child_test){
    			if(!empty($child_test) && $child_test>=0){
                    $id_list[]  = $child_test;
    			} 
    		}
    		$medicine_kit_data = implode(',', $id_list);
        }
        else{
    		$medicine_kit_data='';
    	}
        if($medicine_kit_data){
	        $this->db->select("test_name,id");
            $this->db->where('id IN ('.$medicine_kit_data.')');
            $this->db->from('path_test');
            // $this->db->group_by("path_test_under.parent_id");
           $query = $this->db->get(); 
           return $query->result_array();
    ...
    public function selected_medicine($id=''){
        ...
        if(!empty($id)){
            ...
            $this->db->select("hms_medicine_entry.*,hms_medicine_company.id as comp_id,hms_medicine_company.company_name,hms_packages_to_medicine.unit1_qty,hms_packages_to_medicine.unit2_qty,hms_packages_to_medicine.total_qty");
            $this->db->from("hms_medicine_entry");
            $this->db->join("hms_packages_to_medicine","hms_medicine_entry.id=hms_packages_to_medicine.medicine_id");
            $this->db->where("hms_packages_to_medicine.package_id='".$id."'");
            $this->db->join('hms_medicine_company','hms_medicine_entry.manuf_company=hms_medicine_company.id','left');
            $this->db->where("hms_medicine_entry.is_deleted",'0');
        ...
        if(!empty($id) && !empty($sort_order_value)){
            ...
            $this->db->where('id',$id);
            $result='';
            if($this->db->update('hms_packages')){
                $result='true';
                return $result;
            }
     ...
     public function get_medicine_kit_list()
    {
        $users_data = $this->session->userdata('auth_users');
        $medicine_kit_ids = $this->session->userdata('alloted_medicine_kit_ids');
        $medicine_kit_search_data = $this->session->userdata('medicine_kit_stock_search');
        $result = array();
        if(!empty($medicine_kit_ids))
        {
            ...
            if(isset($medicine_kit_search_data) && !empty($medicine_kit_search_data))
            {
                if(array_key_exists('sub_branch_id',$medicine_kit_search_data))
                {
                    if(!empty($medicine_kit_search_data['sub_branch_id']))
                    {
                        $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$medicine_kit_search_data['sub_branch_id'].' and hms_medicine_kit_stock.kit_id IN('.$medicine_kit_id.') and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0)');
                    }
                    else
                    {
                        $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$users_data['parent_id'].' and hms_medicine_kit_stock.kit_id IN('.$medicine_kit_id.')  and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0)');
                    }
                }
                else
                {
    ...
    //             $this->db->from('hms_medicine_entry');
    //             $this->db->join('hms_packages_to_medicine','hms_medicine_entry.id=hms_packages_to_medicine.medicine_id and hms_packages_to_medicine.branch_id='.$users_data['parent_id']);
    //              $this->db->join('hms_medicine_unit','hms_medicine_unit.id=hms_medicine_entry.unit_id','left');
               
    //             $this->db->join('hms_medicine_company','hms_medicine_entry.manuf_company=hms_medicine_company.id','left'); 
    //             $this->db->where('hms_packages_to_medicine.package_id',$medicine_kit['mkit_id']);
                
    //             // $this->db->group_by('hms_packages_to_medicine.package_id');
    //             $query = $this->db->get();
    //             // echo $this->db->last_query();die;
    ...
    //                // $this->db->select('hms_medicine_entry.id as med_id,hms_medicine_entry.branch_id as med_branch_id,,hms_medicine_entry.medicine_code,hms_medicine_entry.medicine_name,hms_medicine_entry.unit_id,hms_medicine_entry.unit_second_id,hms_medicine_entry.conversion,hms_medicine_entry.min_alrt,hms_medicine_entry.packing,hms_medicine_entry.rack_no,hms_medicine_entry.salt,hms_medicine_entry.manuf_company,hms_medicine_entry.mrp,hms_medicine_entry.purchase_rate,hms_medicine_entry.hsn_no,hms_medicine_entry.vat,hms_medicine_entry.discount,hms_medicine_entry.igst,hms_medicine_entry.cgst,hms_medicine_entry.status as med_status,hms_medicine_entry.is_deleted as med_is_deleted,hms_medicine_entry.deleted_date as med_deleted_date,hms_medicine_unit.id as units_ids,hms_medicine_unit.medicine_unit,hms_medicine_unit.branch_id as unit_branch_id,hms_medicine_unit.status as unit_status,hms_medicine_unit.deleted_by as unit_deleted_by,hms_medicine_unit.deleted_date as unit_deleted_date,hms_medicine_company.id as com_id,hms_medicine_company.company_name,hms_medicine_company.branch_id as com_branch_id,hms_medicine_company.status as com_status,hms_medicine_company.deleted_by as com_deleted_by,hms_medicine_company.deleted_date as com_deleted_date,hms_packages_to_medicine.total_qty');
    //                // $this->db->from('hms_medicine_entry');
    //                // $this->db->join('hms_packages_to_medicine','hms_medicine_entry.id=hms_packages_to_medicine.medicine_id and hms_packages_to_medicine.branch_id='.$post['sub_branch_id']);
    //                // $this->db->join('hms_medicine_unit','hms_medicine_unit.id=hms_medicine_entry.unit_id','left');
               
    //                // $this->db->join('hms_medicine_company','hms_medicine_entry.manuf_company=hms_medicine_company.id','left'); 
    //                // $this->db->where('hms_packages_to_medicine.package_id',$medicine_kit_id);
    ...
    //                     );
    //                    $this->db->insert('hms_medicine_kit_stock',$data_allot_to_branch);
    //                   //     ///////////////////////////////////////
    ...
    //                     //     $this->db->where('hms_medicine_unit.id',$m_sub_result[$m]['unit_id']); 
    ...
    //                     $this->db->select("hms_medicine_unit.*");
    //                     $this->db->from('hms_medicine_unit'); 
    //                     $this->db->where('hms_medicine_unit.id',$m_result[$m]['unit_id']); 
    ...
    //                     //                  $this->db->select("hms_medicine_unit.*");
    //                     //                 $this->db->from('hms_medicine_unit'); 
    //                     //                 $this->db->where('hms_medicine_unit.id',$m_sub_result[$m]['unit_second_id']); 
    //                     //                 $this->db->where('hms_medicine_unit.branch_id='.$post['sub_branch_id']);  
    //                     //                 $query = $this->db->get(); 
    ...
    //                     $this->db->select("hms_medicine_unit.*");
    //                     $this->db->from('hms_medicine_unit'); 
    //                     $this->db->where('hms_medicine_unit.id',$m_result[$m]['unit_second_id']); 
    //                     $this->db->where('hms_medicine_unit.branch_id='.$users_data['parent_id']);  
    //                     $query = $this->db->get(); 
    ...
    //                         }
    //                         $this->db->insert('hms_medicine_unit',$medicine_sec_unit_data);
    ...
    //                     //                 $this->db->where('hms_medicine_company.company_name',$m_sub_result[$m]['company_name']); 
    ...
    //                     $this->db->select("hms_medicine_company.*");
    //                     $this->db->from('hms_medicine_company'); 
    //                     $this->db->where('hms_medicine_company.company_name',$m_result[$m]['company_name']); 
    ...
    //                     //                 $this->db->from('hms_medicine_entry'); 
    //                     //                 $this->db->where('hms_medicine_entry.medicine_name',$m_sub_result[$m]['medicine_name']);
    //                     //                 if(!empty($m_sub_result[$m]['manuf_company']))
    //                     //                 {
    //                     //                 $this->db->where('hms_medicine_entry.manuf_company',$m_sub_result[$m]['manuf_company']); 
    //                     //                 }
    ...
    //                     $this->db->select("hms_medicine_entry.*");
    //                     $this->db->from('hms_medicine_entry'); 
    //                     $this->db->where('hms_medicine_entry.medicine_name',$m_result[$m]['medicine_name']);
    //                     if(!empty($m_result[$m]['manuf_company']))
    //                     {
    //                     $this->db->where('hms_medicine_entry.manuf_company',$m_result[$m]['manuf_company']); 
    //                     }
    //                     $this->db->where('hms_medicine_entry.branch_id='.$users_data['parent_id']);  
    //                     $query = $this->db->get(); 
    ...
    //                             'branch_id'=>$post['sub_branch_id'],
    //                             'medicine_code'=>$m_result[$m]['medicine_code'],
    //                             'medicine_name'=>$m_result[$m]['medicine_name'],
    //                             'unit_id'=>$m_unit_id,
    ...
    //                     //                 );
    //                     //                 $this->db->insert('hms_packages_to_medicine',$medicine_to_medicine_kit_to_allot_branch);
    ...
    //         //         'created_by'=>$users_data['parent_id'],
    //         //     );
    //         //     $this->db->insert('hms_medicine_kit_stock',$data_allot_by_branch);
    ...
    //         // //     ///////////////////////////////////////
    //         }
    //         $this->session->unset_userdata('alloted_medicine_kit_ids');
     ...
     public function allot_medicine_kit_to_branch()
    {
        ...
        if(isset($post) && !empty($post))
        {
            ...
            foreach($post['medicine'] as $mkit_id=>$medicine_kit)
            {
                ...
                $this->db->select('hms_medicine_entry.id as med_id,hms_medicine_entry.branch_id as med_branch_id,hms_medicine_entry.medicine_code,hms_medicine_entry.medicine_name,hms_medicine_entry.unit_id,hms_medicine_entry.unit_second_id,hms_medicine_entry.conversion,hms_medicine_entry.min_alrt,hms_medicine_entry.packing,hms_medicine_entry.rack_no,hms_medicine_entry.salt,hms_medicine_entry.manuf_company,hms_medicine_entry.mrp,hms_medicine_entry.purchase_rate,hms_medicine_entry.hsn_no,hms_medicine_entry.vat,hms_medicine_entry.discount,hms_medicine_entry.igst,hms_medicine_entry.cgst,hms_medicine_entry.status as med_status,hms_medicine_entry.is_deleted as med_is_deleted,hms_medicine_entry.deleted_date as med_deleted_date,hms_medicine_unit.id as units_ids,hms_medicine_unit.medicine_unit,hms_medicine_unit.branch_id as unit_branch_id,hms_medicine_unit.status as unit_status,hms_medicine_unit.deleted_by as unit_deleted_by,hms_medicine_unit.deleted_date as unit_deleted_date,hms_medicine_company.id as com_id,hms_medicine_company.company_name,hms_medicine_company.branch_id as com_branch_id,hms_medicine_company.status as com_status,hms_medicine_company.deleted_by as com_deleted_by,hms_medicine_company.deleted_date as com_deleted_date,hms_packages_to_medicine.total_qty,hms_packages_to_medicine.unit1_qty,hms_packages_to_medicine.unit2_qty');
                $this->db->from('hms_medicine_entry');
                $this->db->join('hms_packages_to_medicine','hms_medicine_entry.id=hms_packages_to_medicine.medicine_id and hms_packages_to_medicine.branch_id='.$users_data['parent_id'],'left');
                 $this->db->join('hms_medicine_unit','hms_medicine_unit.id=hms_medicine_entry.unit_id','left');
    ...
    public function get_med_kit_qty($mkit_id="",$branch_id="")
    {
    ...
    public function get_medicine_kit_stock_excel_data(){
        ...
        if(isset($medicine_kit_search_data) && !empty($medicine_kit_search_data))
        {
            if(array_key_exists('sub_branch_id',$medicine_kit_search_data))
            {
                if(!empty($medicine_kit_search_data['sub_branch_id']))
                {
                    $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$medicine_kit_search_data['sub_branch_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0)');
                }
                else
                {
                    $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$users_data['parent_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0)');
                }
            }
            else
            {
    ...
    public function get_medicine_kit_stock_csv_data(){
    ...
    public function get_medicine_kit_stock_pdf_data()
    {
        $users_data = $this->session->userdata('auth_users');
        $medicine_kit_search_data = $this->session->userdata('medicine_kit_stock_search');
    ...
    public function get_medicine_kit_branch_details($get=array())
    {
        ...
        if(isset($get) && !empty($get))
        {
            if(array_key_exists('branch_id',$get))
            {
                ...
                {
                    $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$get['branch_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0)');
                }
                else
                {
                    $this->db->join('hms_medicine_kit_stock','hms_packages.id=hms_medicine_kit_stock.kit_id and hms_medicine_kit_stock.branch_id='.$users_data['parent_id'].' and (hms_medicine_kit_stock.section_id=3 or hms_medicine_kit_stock.section_id=0)');
    ...
    public function add_medicine_kit_qty_manage($id=""){
        ...
        $result = array();
        if(!empty($id)){
            $this->db->select('hms_medicine_kit_stock.*,hms_packages.title,hms_packages.amount');
            $this->db->from('hms_medicine_kit_stock');
            $this->db->join('hms_packages','hms_packages.id=hms_medicine_kit_stock.kit_id');
            $this->db->where('hms_medicine_kit_stock.branch_id',$users_data['parent_id']);
            $this->db->where('hms_medicine_kit_stock.kit_id',$id);
            $this->db->where('hms_packages.is_deleted',0);
            $this->db->where('hms_medicine_kit_stock.section_id',4);
            $this->db->order_by('hms_medicine_kit_stock.id','ASC');
            $query = $this->db->get();
            $result = $query->result_array();
        }
        return $result;
