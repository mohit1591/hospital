...
class Discount_authrize_report_model extends CI_Model {
...
//user collection
    ...
    public function self_opd_collection_list($get="")
    {
        ...
        if(!empty($get))
        {  
            $this->db->select("hms_payment.reciept_prefix,hms_payment.reciept_suffix, hms_patient.patient_name, hms_doctors.doctor_name,hms_patient.mobile_no,hms_patient.patient_code,(CASE WHEN hms_patient.insurance_type=1 THEN  'Yes' ELSE 'No' END ) as panel_type,hms_opd_booking.booking_date,hms_opd_booking.booking_code,hms_payment.discount_amount,hms_payment.net_amount,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 2 AND payment.parent_id = hms_opd_booking.id) ELSE '0.00' END) as balance,hms_payment.total_amount,hms_payment.debit,hms_payment.created_date, hms_payment.pay_mode,(CASE WHEN hms_opd_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_opd_booking.referral_doctor=0 THEN concat('Other ',hms_opd_booking.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name,   hms_payment_mode.payment_mode as mode,hms_patient_category.patient_category as patient_category_name,hms_authorize_person.authorize_person as authorize_person_name");
            $this->db->join('hms_patient','hms_patient.id=hms_payment.patient_id','left'); 
            $this->db->join('hms_opd_booking','hms_opd_booking.id=hms_payment.parent_id');
            $this->db->join('hms_doctors','hms_doctors.id=hms_opd_booking.attended_doctor','left');
             $this->db->join('hms_payment_mode','hms_payment_mode.id=hms_payment.pay_mode','left'); 
            ...
            $this->db->where('hms_payment.branch_id',$users_data['parent_id']);
            if(!empty($get['patient_category']))
            {
                $this->db->where('hms_opd_booking.patient_category',$get['patient_category']);
            }
            
            if(!empty($get['authorize_person']))
            {
                $this->db->where('hms_opd_booking.authorize_person',$get['authorize_person']);
            }
            ...
            $this->db->where('hms_payment.section_id IN (2)'); 
            $this->db->where('hms_payment.debit>=0');
            $this->db->where('hms_opd_booking.discount>0');
    ...
    public function self_billing_collection_list($get="")
    {
        ...
        {  
             ...
             $this->db->select("hms_payment.reciept_prefix,hms_payment.reciept_suffix, hms_patient.patient_name, hms_doctors.doctor_name,hms_patient.mobile_no,hms_patient.patient_code,(CASE WHEN hms_patient.insurance_type=1 THEN  'Yes' ELSE 'No' END ) as panel_type,hms_opd_booking.booking_date,hms_opd_booking.reciept_code as booking_code,hms_payment.discount_amount,hms_payment.net_amount,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 4 AND payment.parent_id = hms_opd_booking.id) ELSE '0.00' END) as balance,hms_payment.total_amount, hms_payment.debit, hms_payment.created_date, hms_payment.pay_mode,(CASE WHEN hms_opd_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_opd_booking.referral_doctor=0 THEN concat('Other ',hms_opd_booking.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name,hms_payment_mode.payment_mode as mode,hms_patient_category.patient_category as patient_category_name,hms_authorize_person.authorize_person as authorize_person_name");
    ...
    public function self_medicine_collection_list($get="")
    {
        ...
        {  
            ...
            $this->db->select("hms_payment.reciept_prefix,hms_payment.reciept_suffix, hms_patient.patient_name, hms_doctors.doctor_name,hms_patient.mobile_no,hms_patient.patient_code,(CASE WHEN hms_patient.insurance_type=1 THEN  'Yes' ELSE 'No' END ) as panel_type,hms_medicine_sale.sale_date as booking_date,hms_medicine_sale.sale_no as booking_code,hms_payment.discount_amount,hms_payment.net_amount,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 3 AND payment.parent_id = hms_medicine_sale.id) ELSE '0.00' END) as balance,hms_payment.total_amount,hms_payment.debit, hms_payment.created_date, hms_payment.pay_mode,(CASE WHEN hms_medicine_sale.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_medicine_sale.refered_id=0 THEN concat('Other ',hms_medicine_sale.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name,hms_payment_mode.payment_mode as mode,hms_patient_category.patient_category as patient_category_name,hms_authorize_person.authorize_person as authorize_person_name"); 
           ...
           $this->db->join('hms_patient_category','hms_patient_category.id = hms_medicine_sale.patient_category','left'); 
            ...
            if(!empty($get['patient_category']))
            {
                $this->db->where('hms_medicine_sale.patient_category',$get['patient_category']);
            }
            
            if(!empty($get['authorize_person']))
            {
                $this->db->where('hms_medicine_sale.authorize_person',$get['authorize_person']);
            }
            //user collection
            $emp_ids='';
            if($users_data['emp_id']>0)
            ...
            $this->db->where('hms_payment.debit>0');
            $this->db->where('hms_medicine_sale.discount>0');
            //$this->db->order_by('hms_payment.id','DESC');
            if(!empty($get["order_by"]) && $get['order_by']=='ASC')
            {
                $this->db->order_by('hms_payment.id','ASC');
   ...
   public function all_user_source_report($get="",$user_id='',$source_id='')
    ...
    public function self_ipd_collection_list($get="")
    {
        ...
        {  
            ...
            $this->db->select("hms_payment.reciept_prefix,hms_payment.reciept_suffix, hms_patient.patient_name,hms_patient.mobile_no,hms_patient.patient_code,(CASE WHEN hms_patient.insurance_type=1 THEN  'Yes' ELSE 'No' END ) as panel_type,hms_ipd_booking.discharge_date as booking_date,hms_ipd_booking.ipd_no as booking_code,hms_payment.discount_amount,hms_payment.net_amount,hms_payment.balance,hms_payment.total_amount, hms_payment.debit, hms_payment.created_date,hms_payment_mode.payment_mode as mode,hms_patient_category.patient_category as patient_category_name,hms_authorize_person.authorize_person as authorize_person_name");
           ...
           $this->db->join('hms_patient_category','hms_patient_category.id = hms_ipd_booking.patient_category','left'); 
            $this->db->join('hms_authorize_person','hms_authorize_person.id = hms_ipd_booking.authorize_person','left');
            ...
            $this->db->where('hms_payment.debit>0');
            $this->db->where('hms_ipd_booking.discount_amount_dis_bill>0');
            if(!empty($get["order_by"]) && $get['order_by']=='ASC')
            {
                $this->db->order_by('hms_payment.id','ASC');
            }
            ...
            $this->db->select("hms_payment.reciept_prefix,hms_payment.reciept_suffix, hms_patient.patient_name, sum(hms_payment.debit) as tot_debit, hms_payment.created_date, hms_payment.pay_mode,(CASE WHEN hms_ipd_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE (CASE WHEN hms_ipd_booking.   referral_doctor=0 THEN concat('Other ',hms_ipd_booking.ref_by_other) ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as doctor_hospital_name,hms_payment_mode.payment_mode as mode"); 
...
//user collection
    ...
    public function pathology_self_collection_list($get="")
    {
        ...
        {  
            ...
            if(!empty($get['patient_category']))
            {
                $this->db->where('path_test_booking.patient_category',$get['patient_category']);
            }
            
            if(!empty($get['authorize_person']))
            {
                $this->db->where('path_test_booking.authorize_person',$get['authorize_person']);
            }
           //user collection
           $emp_ids='';
            if($users_data['emp_id']>0)
            ...
            $this->db->where('hms_payment.debit>0');   
            $this->db->where('path_test_booking.discount>0');
            if(!empty($get["order_by"]) && $get['order_by']=='ASC')
            {
                $this->db->order_by('hms_payment.id','ASC');
            }
