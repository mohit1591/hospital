...
class Prescription_model extends CI_Model 
{
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_opd_prescription.*,hms_opd_prescription_patient_test.test_name,hms_opd_prescription_patient_pres.medicine_name,hms_opd_prescription_patient_pres.medicine_salt,hms_opd_prescription_patient_pres.medicine_brand,hms_opd_prescription_patient_pres.medicine_type,hms_opd_prescription_patient_pres.medicine_dose,hms_opd_prescription_patient_pres.medicine_duration,hms_opd_prescription_patient_pres.medicine_frequency,hms_opd_prescription_patient_pres.medicine_advice"); 
		...
		$this->db->where('hms_opd_prescription.branch_id IN ('.$search['branch_id'].')');
		}else{
		$this->db->where('hms_opd_prescription.branch_id = "'.$user_data['parent_id'].'"');
		}
		...
		if(isset($search) && !empty($search))
		{
            ...
            if(!empty($search['start_date']))
			{
				...
				$this->db->where('hms_opd_prescription.created_date >= "'.$start_date.'"');
			}

			if(!empty($search['end_date']))
			{
				$end_date = date('Y-m-d',strtotime($search['end_date'])).' 23:59:59';
				$this->db->where('hms_opd_prescription.created_date <= "'.$end_date.'"');
			}
            
            if(!empty($search['specialization_id']))
			{
				$this->db->where('hms_opd_booking.specialization_id',$search['specialization_id']);
			}
			
			if(!empty($search['patient_name']))
			{
				$this->db->where('hms_opd_prescription.patient_name LIKE "'.$search['patient_name'].'%"');
			}

			if(!empty($search['patient_code']))
			{
				$this->db->where('hms_opd_prescription.patient_code',$search['patient_code']);
			}

			if(!empty($search['mobile_no']))
			{
				$this->db->where('hms_opd_prescription.mobile_no LIKE "'.$search['mobile_no'].'%"');
			}
		...
		{ 
			$this->db->where('hms_opd_prescription.created_by IN ('.$emp_ids.')');
		}
		/////// Search query end //////////////
	...
	public function get_by_prescription_id($prescription_id)
	{
		...
		$this->db->select('hms_opd_prescription_patient_test.*');
		$this->db->join('hms_opd_prescription','hms_opd_prescription.id = hms_opd_prescription_patient_test.prescription_id'); 
		$this->db->where('hms_opd_prescription_patient_test.prescription_id = "'.$prescription_id.'"');
		$this->db->from('hms_opd_prescription_patient_test');
		$result_pre['prescription_list']['prescription_test_list']=$this->db->get()->result();

		$this->db->select('hms_opd_prescription_patient_pres.*');
		$this->db->join('hms_opd_prescription','hms_opd_prescription.id = hms_opd_prescription_patient_pres.prescription_id'); 
		$this->db->where('hms_opd_prescription_patient_pres.prescription_id = "'.$prescription_id.'"');
	...
	{
		$this->db->select("hms_opd_booking.opd_type,hms_opd_prescription.*,hms_patient.patient_code,hms_patient.id as patient_id,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.gender,hms_patient.dob,hms_patient.age,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_patient.simulation_id,hms_patient.address,hms_patient.city_id,hms_patient.state_id,hms_patient.country_id,hms_patient.address,hms_patient.pincode,hms_opd_booking.booking_code,hms_opd_booking.booking_date,hms_opd_booking.specialization_id,hms_opd_booking.referral_doctor,hms_opd_booking.attended_doctor,hms_opd_booking.booking_date,hms_opd_booking.booking_time,hms_opd_prescription.patient_bp as patientbp,hms_opd_prescription.patient_temp as patienttemp,hms_opd_prescription.patient_weight as patientweight,hms_opd_prescription.patient_height as patientpr,hms_opd_prescription.patient_spo2 as patientspo,hms_opd_prescription.patient_rbs as patientrbs,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation,hms_doctors.header_content,hms_doctors.seprate_header,hms_doctors.opd_header,hms_doctors.billing_header,hms_doctors.prescription_header"); 
	...
	{
		$this->db->select("hms_opd_prescription.*,hms_opd_booking.booking_code,hms_opd_booking.booking_time,hms_opd_booking.booking_date,hms_opd_booking.specialization_id,hms_opd_booking.referral_doctor,hms_opd_booking.attended_doctor,hms_opd_booking.booking_date,hms_opd_booking.booking_time,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation");  //,hms_opd_prescription_patient_test.*,hms_opd_prescription_patient_pres.*
		$this->db->from('hms_opd_prescription'); 
		$this->db->join('hms_opd_booking','hms_opd_booking.id=hms_opd_prescription.booking_id','left');
		$this->db->join('hms_patient','hms_patient.id = hms_opd_booking.patient_id'); 
		$this->db->join('hms_gardian_relation','hms_gardian_relation.id=hms_patient.relation_type','left');
	...
	{
		$this->db->select("hms_opd_prescription.*");  //,hms_opd_prescription_patient_test.*,hms_opd_prescription_patient_pres.*
		$this->db->from('hms_opd_prescription'); 
		$this->db->where('hms_opd_prescription.id',$id);
		$this->db->where('hms_opd_prescription.is_deleted','0'); 
		$query = $this->db->get(); 
	...
	{
		$this->db->select("hms_opd_prescription.*");  //,hms_opd_prescription_patient_test.*,hms_opd_prescription_patient_pres.*patient_id
		$this->db->from('hms_opd_prescription'); 
		$this->db->where('hms_opd_prescription.id',$id);
		$this->db->where('hms_opd_prescription.is_deleted','0'); 
		$query = $this->db->get(); 
    ...
    public function save_file($filename="")
	{
		...
		{    
			...
			$this->db->set('modified_date',date('Y-m-d H:i:s'));
            $this->db->where('id',$post['data_id']);
			$this->db->update('hms_prescription_files',$data);  
		}
		else{    
			if(!empty($filename))
			{
			...
			$this->db->set('created_by',$user_data['id']);
			$this->db->set('created_date',date('Y-m-d H:i:s'));
			$this->db->insert('hms_prescription_files',$data);               
		} 	
	}
    ...
    public function save_prescription()
    {
	    ...
	    else
	    {
 		...
 		$data = array( 
			...
			$this->db->set('modified_date',date('Y-m-d H:i:s'));
            $this->db->where('id',$post['data_id']);
			$this->db->update('hms_opd_prescription',$data); 

			//echo $this->db->last_query(); exit;
			$this->db->where('prescription_id',$post['data_id']);
            $this->db->delete('hms_opd_prescription_patient_test'); 
			...
			$this->db->where('prescription_id',$post['data_id']);
            $this->db->delete('hms_opd_prescription_patient_pres'); 
    	...
    	if(!empty($vals))
    	{
	       ...
	       /* if(!empty($result))
	        ...
	        { 
	        	...
	        	{
	               $name = $vals->profile_name.'|'.$vals->id.'-profile';
					array_push($data, $name);
	               //$response[] = $vals->medicine;
	        	}
	        }
...
"doctor_display_info"=>$post['qualification'],"hospital"=>$post['branch_name'], "appointment_utc_time"=>$milliseconds);
    ...
    //get_detail_by_prescription_letterhead //get_detail_by_booking_id
    
    public function get_detail_by_prescription_letterhead($booking_id="",$branch_id="")
    { //echo "dss"; die;
        $this->db->select("hms_opd_prescription.*,hms_patient.patient_code,hms_patient.id as patient_id,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.gender,hms_patient.dob,hms_patient.age,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_patient.simulation_id,hms_patient.address,hms_patient.city_id,hms_patient.state_id,hms_patient.country_id,hms_patient.address,hms_patient.pincode,hms_opd_booking.booking_code,hms_opd_booking.booking_date,hms_opd_booking.specialization_id,hms_opd_booking.referral_doctor,hms_opd_booking.attended_doctor,hms_opd_booking.booking_date,hms_opd_booking.booking_time,hms_opd_prescription.patient_bp as patientbp,hms_opd_prescription.patient_temp as patienttemp,hms_opd_prescription.patient_weight as patientweight,hms_opd_prescription.patient_height as patientpr,hms_opd_prescription.patient_spo2 as patientspo,hms_opd_prescription.patient_rbs as patientrbs,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation,hms_opd_booking.token_no"); 



        $this->db->from('hms_opd_prescription'); 
        $this->db->where('hms_opd_prescription.booking_id',$booking_id);
        $this->db->where('hms_opd_prescription.branch_id',$branch_id);
        $this->db->join('hms_patient','hms_patient.id=hms_opd_prescription.patient_id','left');
        $this->db->join('hms_gardian_relation','hms_gardian_relation.id=hms_patient.relation_type','left');
        $this->db->join('hms_opd_booking','hms_opd_booking.id=hms_opd_prescription.booking_id','left'); 
        $result_pre['prescription_list']= $this->db->get()->result();
        ...
        if(isset($result_pre['prescription_list']) && !empty($result_pre['prescription_list']))
        {
             $this->db->select('hms_opd_prescription_patient_test.*');
        $this->db->join('hms_opd_prescription','hms_opd_prescription.id = hms_opd_prescription_patient_test.prescription_id'); 
        $this->db->where('hms_opd_prescription_patient_test.prescription_id = "'.$result_pre['prescription_list'][0]->id.'"');
        $this->db->from('hms_opd_prescription_patient_test');
        $result_pre['prescription_list']['prescription_test_list']=$this->db->get()->result();

        $this->db->select('hms_opd_prescription_patient_pres.*');
        $this->db->join('hms_opd_prescription','hms_opd_prescription.id = hms_opd_prescription_patient_pres.prescription_id'); 
        $this->db->where('hms_opd_prescription_patient_pres.prescription_id = "'.$result_pre['prescription_list'][0]->id.'"');
        $this->db->from('hms_opd_prescription_patient_pres');
        $result_pre['prescription_list']['prescription_presc_list']=$this->db->get()->result();
        //echo $this->db->last_query(); exit;
    ...
    public function get_consolidate_prescription($all_prescription_ids='')
	{
		...
		{
		    $this->db->where('hms_opd_prescription.id IN ('.$all_prescription_ids.')');
		}
		$this->db->join('hms_patient','hms_patient.id=hms_opd_prescription.patient_id','left');
		$this->db->join('hms_gardian_relation','hms_gardian_relation.id=hms_patient.relation_type','left');
		$this->db->join('hms_opd_booking','hms_opd_booking.id=hms_opd_prescription.booking_id','left'); 
	...
	{
	    $user_data = $this->session->userdata('auth_users');
	    $this->db->select("hms_opd_prescription.patient_id"); 
        $this->db->from('hms_opd_prescription'); 
		if(!empty($all_prescription_ids))
		{
		    $this->db->where('hms_opd_prescription.id IN ('.$all_prescription_ids.')');
	...
	//gynecology
	public function get_gynecology_consolidate_prescription($all_prescription_ids='')
	{
	    $user_data = $this->session->userdata('auth_users');
	    $this->db->select("hms_gynecology_prescription.*,hms_patient.patient_code,hms_patient.id as patient_id,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.gender,hms_patient.dob,hms_patient.age,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_patient.simulation_id,hms_patient.address,hms_patient.city_id,hms_patient.state_id,hms_patient.country_id,hms_patient.address,hms_patient.pincode,hms_opd_booking.booking_code,hms_opd_booking.booking_date,hms_opd_booking.specialization_id,hms_opd_booking.referral_doctor,hms_opd_booking.attended_doctor,hms_opd_booking.booking_date,hms_opd_booking.booking_time,hms_gynecology_prescription.patient_bp as patientbp,hms_gynecology_prescription.patient_temp as patienttemp,hms_gynecology_prescription.patient_weight as patientweight,hms_gynecology_prescription.patient_height as patientpr,hms_gynecology_prescription.patient_spo2 as patientspo,hms_gynecology_prescription.patient_rbs as patientrbs,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation,hms_doctors.header_content,hms_doctors.seprate_header,hms_doctors.opd_header,hms_doctors.billing_header,hms_doctors.prescription_header"); 
		...
		if(!empty($all_prescription_ids))
		{
		    $this->db->where('hms_gynecology_prescription.booking_id IN ('.$all_prescription_ids.')');
		}
