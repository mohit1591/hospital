...
class Ambulance_collection_report_model extends CI_Model {

    var $table = 'hms_payment';
    var $column = array('hms_ambulance_booking.booking_code','hms_ambulance_booking.booking_date', 'hms_patient.patient_name','docs.doctor_name','hms_doctors.doctor_name','hms_ambulance_booking.net_amount','hms_ambulance_booking.discount','hms_ambulance_booking.net_amount','hms_ambulance_booking.paid_amount','hms_ambulance_booking.balance','hms_ambulance_booking.type');  
    ...
    private function _get_datatables_query()
    {
        ...
        $search_data = $this->session->userdata('ambulance_collection_resport_search_data');
        
        $this->db->select("hms_ambulance_booking.id, hms_ambulance_booking.status,hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id,hms_ambulance_booking.booking_no, hms_ambulance_booking.booking_date,hms_ambulance_booking.booking_time, (CASE WHEN hms_payment.balance>0 THEN hms_ambulance_booking.net_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (hms_ambulance_booking.discount) ELSE '0.00' END) as discount, hms_doctors.doctor_name,(CASE WHEN hms_ambulance_booking.reffered_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name,hms_patient.patient_name,hms_patient.id as patient_new_id, (CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 13 AND payment.parent_id = hms_ambulance_booking.id) ELSE '0.00' END) as balance");  
     ...
     //   $this->db->join('hms_doctors as docs','docs.id = hms_ambulance_booking.attended_doctor','left');
       ...
       // $this->db->where('hms_ambulance_booking.type',2); 
        $this->db->where('hms_ambulance_booking.is_deleted !=',2);  
            
       
        if(isset($search_data['branch_id']) && $search_data['branch_id']!=''){
        $this->db->where('hms_ambulance_booking.branch_id IN ('.$search_data['branch_id'].')');
        }else{
        ...
        if(isset($search_data) && !empty($search_data))
        {
            ...
            {
                $this->db->where('hms_ambulance_booking.reffered' ,$search_data['referral_hospital']);
            }
            elseif($search_data['referred_by']=='0' && !empty($search_data['refered_id']))
            {
                $this->db->where('hms_ambulance_booking.reffered' ,$search_data['refered_id']);
            }
        ...
        $this->db->where('hms_ambulance_booking.net_amount>0');
        $i = 0;
    ...
    function search_report_data()
    {
        ...
        $search_data = $this->session->userdata('ambulance_collection_resport_search_data'); 
         ...
         $this->db->select("hms_ambulance_booking.id, hms_ambulance_booking.status, hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id,hms_ambulance_booking.booking_no, hms_ambulance_booking.booking_date, (CASE WHEN hms_payment.balance>0 THEN hms_ambulance_booking.net_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (hms_ambulance_booking.discount) ELSE '0.00' END) as discount, hms_doctors.doctor_name,(CASE WHEN hms_ambulance_booking.reffered_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name, hms_patient.patient_name,hms_patient.id as patient_new_id, (CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 13 AND payment.parent_id = hms_ambulance_booking.id) ELSE '0.00' END) as balance"); 
        //$this->db->select("hms_ambulance_booking.id,hms_ambulance_booking.booking_code,hms_ambulance_booking.booking_date, hms_ambulance_booking.status, hms_ambulance_booking.booking_status, hms_payment.debit as paid_amount, hms_ambulance_booking.booking_code, hms_ambulance_booking.booking_date, hms_ambulance_booking.total_amount, hms_ambulance_booking.discount, hms_ambulance_booking.type, hms_doctors.doctor_name, hms_patient.patient_name, (CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 2 AND payment.parent_id = hms_ambulance_booking.id) ELSE '0.00' END) as balance");  
        $this->db->join('hms_ambulance_booking','hms_ambulance_booking.id = hms_payment.parent_id'); 
        $this->db->join('hms_patient','hms_patient.id = hms_ambulance_booking.patient_id');
        //$this->db->join('hms_doctors','hms_doctors.id = hms_ambulance_booking.attended_doctor','left');
      ...
      //  $this->db->where('hms_ambulance_booking.type',2); 
        ...
        {  
             $this->db->where('hms_ambulance_booking.branch_id',$search_data['branch_id']);
        }
        else
        {
            $this->db->where('hms_ambulance_booking.branch_id',$users_data['parent_id']);
...
if($search_data['referred_by']=='1' && !empty($search_data['referral_hospital']))
            {
                $this->db->where('hms_ambulance_booking.referral_hospital' ,$search_data['referral_hospital']);
            }
            elseif($search_data['referred_by']=='0' && !empty($search_data['refered_id']))
            {
                $this->db->where('hms_ambulance_booking.reffered' ,$search_data['refered_id']);
            ...
            { 
                $this->db->where('hms_ambulance_booking.referral_doctor = "'.$search_data["referral_doctor"].'"');
            ...
            { 
                $this->db->where('hms_ambulance_booking.attended_doctor = "'.$search_data["attended_doctor"].'"');
            }
