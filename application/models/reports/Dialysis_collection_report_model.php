...
class Dialysis_collection_report_model extends CI_Model {

    var $table = 'hms_payment';
    var $column = array('hms_dialysis_booking.booking_code','hms_dialysis_booking.dialysis_date', 'hms_patient.patient_name','hms_doctors.doctor_name','hms_dialysis_booking.advance_payment');  
    ...
    private function _get_datatables_query()
    {
        ...
        $search_data = $this->session->userdata('dialysis_collection_resport_search_data');
        
        $this->db->select("hms_dialysis_booking.id,hms_payment.type, hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.paid_amount as paid_amount_payment, hms_payment.id as pay_id,hms_dialysis_booking.booking_code, hms_dialysis_booking.dialysis_date,hms_doctors.doctor_name, hms_patient.patient_name,hms_patient.id as patient_new_id,(CASE WHEN hms_dialysis_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name");  
        $this->db->join('hms_dialysis_booking','hms_dialysis_booking.id = hms_payment.parent_id AND hms_dialysis_booking.is_deleted !=2'); 
        $this->db->join('hms_patient','hms_patient.id = hms_dialysis_booking.patient_id');
        //$this->db->join('hms_doctors','hms_doctors.id = hms_dialysis_booking.attend_doctor_id','left');
...
$this->db->join('hms_doctors','hms_doctors.id = hms_dialysis_booking.referral_doctor','left');
        ...
        $this->db->where('hms_dialysis_booking.branch_id IN ('.$search_data['branch_id'].')');
        }else{
...
if(isset($search_data['referral_hospital']) && $search_data['referred_by']=='1' && !empty($search_data['referral_hospital']))
            {
                $this->db->where('hms_dialysis_booking.referral_hospital' ,$search_data['referral_hospital']);
            }
            elseif( isset($search_data['refered_id']) && $search_data['referred_by']=='0' && !empty($search_data['refered_id']))
            {
                $this->db->where('hms_dialysis_booking.referral_doctor' ,$search_data['refered_id']);
            }
...
if(isset($search_data['patient_name']) && !empty($search_data['patient_name'])
    ...
    function search_report_data()
    {
        ...
        $this->db->select("hms_dialysis_booking.id, hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id,hms_dialysis_booking.booking_code, hms_dialysis_booking.dialysis_date,hms_doctors.doctor_name, hms_patient.patient_name,hms_patient.id as patient_new_id,(CASE WHEN hms_dialysis_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE concat('Dr. ',hms_doctors.doctor_name) END) as doctor_hospital_name");  
       ...
       // $this->db->group_by('hms_dialysis_booking.id');
        //$this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get(); 
        //echo $this->db->last_query();die;
        return $query->result();
       ...
       if(!empty($id))
        { 
            ...
            $user_data = $this->session->userdata('auth_users');
                ...
                $this->db->select("hms_patient.*,hms_users.*,hms_patient.patient_name as name,hms_dialysis_booking.booking_code as recepit_no,hms_dialysis_booking.dialysis_date as date,hms_doctors.doctor_name,hms_payment_mode.payment_mode,hms_payment.pay_mode,hms_patient.patient_code as code,hms_payment.id as p_id,hms_simulation.simulation,hms_payment.debit, (select sum(credit)-sum(debit) from hms_payment as sub_pay where sub_pay.section_id = hms_payment.section_id AND sub_pay.parent_id = hms_payment.parent_id AND  sub_pay.patient_id = hms_payment.patient_id AND sub_pay.created_date <= hms_payment.created_date) as balance, (select sum(credit)-sum(debit) from hms_payment as total_pay where total_pay.section_id = hms_payment.section_id AND total_pay.parent_id = hms_payment.parent_id AND  total_pay.patient_id = hms_payment.patient_id AND total_pay.created_date < hms_payment.created_date) as total_amount,hms_payment.created_date as c_date,hms_doctors.doctor_name,hms_dialysis_rooms.room_no,hms_dialysis_room_to_bad.bad_no,hms_dialysis_room_category.room_category"); 
                ...
                $this->db->where('hms_payment.section_id',15); 
                //$this->db->where('hms_dialysis_booking.discharge_status =1'); 
