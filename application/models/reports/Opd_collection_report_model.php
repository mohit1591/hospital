...
class Opd_collection_report_model extends CI_Model {
    ...
    var $column = array('hms_opd_booking.id','hms_opd_booking.token_no','hms_opd_booking.booking_code','hms_opd_booking.booking_date', 'hms_patient.patient_name','docs.doctor_name','hms_doctors.doctor_name','hms_opd_booking.total_amount','hms_opd_booking.discount','hms_opd_booking.paid_amount','hms_payment.total_comission','hms_payment.total_comission','hms_opd_booking.balance');  
    ...
    private function _get_datatables_query()
    {
        ...
        if(isset($search_data) && !empty($search_data))
        {
            ...
            {
                $this->db->where('hms_opd_booking.pannel_type',$search_data['insurance_type']);
            }

            if(!empty($search_data['insurance_type_id']))
            {
                $this->db->where('hms_opd_booking.insurance_type_id',$search_data['insurance_type_id']);
            }

            if(!empty($search_data['ins_company_id']))
            {
                $this->db->where('hms_opd_booking.ins_company_id',$search_data['ins_company_id']);
            }
    ...
    function search_report_data_excel()
    {
        $users_data = $this->session->userdata('auth_users');
        $search_data = $this->session->userdata('opd_collection_resport_search_data'); 
        
        $this->db->select("hms_opd_booking.token_no as cl_1,hms_opd_booking.booking_code as cl_2,hms_opd_booking.booking_date as cl_3,hms_patient.patient_name as cl_4,docs.doctor_name as cl_5,(CASE WHEN hms_opd_booking.referred_by =1 THEN concat(hms_hospital.hospital_name,' (Hospital)')  ELSE  
        
        (CASE WHEN hms_opd_booking.referral_doctor='' THEN hms_opd_booking.ref_by_other ELSE concat('Dr. ',hms_doctors.doctor_name) END) END) as cl_6,(CASE WHEN hms_payment.balance>0 THEN hms_opd_booking.total_amount ELSE '0.00' END) cl_7,(CASE WHEN hms_payment.balance > 0 THEN (hms_opd_booking.discount) ELSE '0.00' END) as cl_8,(hms_opd_booking.total_amount-hms_opd_booking.discount) as cl_9,hms_payment.total_comission as cl_10,(hms_payment.debit-hms_payment.total_comission) as cl_11,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 2 AND payment.parent_id = hms_opd_booking.id) ELSE '0.00' END) as cl_12"); 
        /*
        ,hms_opd_booking.id, hms_opd_booking.status, hms_opd_booking.booking_status,  hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.balance as blnce,hms_payment.id as pay_id,   hms_opd_booking.type, hms_doctors.doctor_name,hms_patient.id as patient_new_id
        */
