...
class Daycare_discharge_summary_model extends CI_Model {

    var $table = 'hms_daycare_discharge_summery';
    var $column = array('hms_daycare_discharge_summery.id','hms_patient.patient_name','hms_patient.patient_code','hms_patient.mobile_no','hms_daycare_discharge_summery.patient_id', 'hms_daycare_discharge_summery.status','hms_daycare_discharge_summery.created_date');   
    ...
    private function _get_datatables_query()
    {
        ...
        $this->db->where('hms_daycare_discharge_summery.is_deleted','0');
        ...
        { 
            $this->db->where('hms_daycare_discharge_summery.created_by IN ('.$emp_ids.')');
        }
        $i = 0;
    ...
    {
        $this->db->select('hms_daycare_discharge_summery.*');
        $this->db->from('hms_daycare_discharge_summery'); 
        $this->db->where('hms_daycare_discharge_summery.id',$id);
        $this->db->where('hms_daycare_discharge_summery.is_deleted','0');
        $query = $this->db->get(); 
    ...
    public function save()
    {
        ...
        $data = array( 
                    ...
                    'dischargedate'=>date('Y-m-d',strtotime($post['dischargedate'])),
                    'dischargetime'=>date('H:i:s', strtotime(date('d-m-Y').' '.$post['dischargetime'])),
        ...
        {    
            ...
            $this->db->update('hms_daycare_discharge_summery',$data);
            ...
            $this->db->delete('hms_patient_daycare_field_value');
            if(!empty($post['field_name']))
            {
                ...
                $this->db->insert('hms_patient_daycare_field_value',$data_field_value);
            ...
            /*************** Medicine Detail ****************/
            $this->db->where(array('daycare_id'=>$post['daycare_id'],'discharge_summary_id'=>$post['data_id']));
            $this->db->delete('hms_daycare_discharge_summery_medicine');
            if($post['medicine_name'][0]!="")
            {
                ...
                $this->db->insert('hms_daycare_discharge_summery_medicine',$data_medicine);
            ...
            $this->db->where(array('daycare_id'=>$post['daycare_id'],'discharge_summary_id'=>$post['data_id']));
            $this->db->delete('hms_daycare_discharge_summary_test');
            if($post['test_name'][0]!="")
            {
                ...
                $this->db->insert('hms_daycare_discharge_summary_test',$data_test);
            ...
            if($post['summery_type']=='5')
            {
                ...
                $this->db->where('hms_death_summery.daycare_id',$post['daycare_id']);
                $query = $this->db->get();
        ...
        {    
            $this->db->set('created_by',$user_data['id']);
            $this->db->set('created_date',date('Y-m-d H:i:s'));
            $this->db->insert('hms_daycare_discharge_summery',$data); 
            ...
            /*************** Medicine Detail ****************/
            $this->db->where(array('daycare_id'=>$post['daycare_id'],'discharge_summary_id'=>$discharge_id));
            $this->db->delete('hms_daycare_discharge_summery_medicine');
            if($post['medicine_name'][0]!="")
            {
                $post_medicine_name= $post['medicine_name'];
    ...
    {
        $users_data = $this->session->userdata('auth_users'); 
        $this->db->select('hms_patient_daycare_field_value.*,hms_daycare_discharge_field_master.discharge_lable,hms_daycare_discharge_field_master.type,hms_daycare_discharge_field_master.discharge_short_code'); 
        $this->db->join('hms_patient_daycare_field_value','hms_patient_daycare_field_value.field_id = hms_daycare_discharge_field_master.id AND hms_patient_daycare_field_value.parent_id = "'.$parent_id.'" AND hms_patient_daycare_field_value.branch_id = "'.$users_data['parent_id'].'"','left'); 
        $this->db->where('hms_daycare_discharge_field_master.branch_id',$users_data['parent_id']);
        $this->db->where('hms_daycare_discharge_field_master.status',1);
        $this->db->where('hms_daycare_discharge_field_master.is_deleted',0);  
        $this->db->order_by('hms_daycare_discharge_field_master.sort_order','DESC');
        $query= $this->db->get('hms_daycare_discharge_field_master')->result();
        ...
        public function discharge_master_detail_by_field_print($parent_id="")
    {
        $users_data = $this->session->userdata('auth_users'); 
        $this->db->select('hms_patient_daycare_field_value.*,hms_daycare_discharge_field_master.discharge_lable,hms_daycare_discharge_field_master.type,hms_daycare_discharge_field_master.discharge_short_code');
        $this->db->join('hms_daycare_discharge_field_master','hms_daycare_discharge_field_master.id=hms_patient_daycare_field_value.field_id and hms_daycare_discharge_field_master.status=1');
        $this->db->where('hms_patient_daycare_field_value.branch_id',$users_data['parent_id']);
        $this->db->where('hms_patient_daycare_field_value.parent_id',$parent_id);
        //$this->db->order_by('hms_daycare_discharge_field_master.status',1);
        $this->db->order_by('hms_daycare_discharge_field_master.sort_order','DESC');
        $query= $this->db->get('hms_patient_daycare_field_value')->result();
        //echo $this->db->last_query(); exit;
        return $query;
    }
    ...
    public function save_medicine()
    {
        ...
        $total_prescription = count(array_filter($post['medicine_name']));  
            ...
            {   
                    ...
                    $this->db->insert('hms_daycare_discharge_summery_medicine',$prescription_data);
    ...
    public function template_list()
    {
        ...
        $this->db->where('is_deleted','0');   
        $query = $this->db->get('hms_daycare_discharge_summery_template');
        $result = $query->result(); 
        return $result; 
    }
   ...
   {
        $users_data = $this->session->userdata('auth_users'); 
        $this->db->select('hms_daycare_discharge_summery_template.*');  
        $this->db->where('hms_daycare_discharge_summery_template.id',$template_id);  
        $this->db->where('hms_daycare_discharge_summery_template.is_deleted',0); 
        $this->db->where('hms_daycare_discharge_summery_template.branch_id',$users_data['parent_id']); 
        $query = $this->db->get('hms_daycare_discharge_summery_template');
        $result = $query->row(); 
  ...
  {
    $this->db->select("hms_daycare_discharge_summery.*,hms_patient.patient_code,hms_patient.id as patient_id,hms_patient.patient_name,hms_patient.mobile_no,hms_patient.gender,hms_patient.dob,hms_patient.age,hms_patient.age_y,hms_patient.age_m,hms_patient.age_d,hms_patient.simulation_id,hms_patient.address,hms_patient.address2,hms_patient.address3,hms_patient.city_id,hms_patient.state_id,hms_patient.country_id,hms_patient.address,hms_patient.pincode,hms_day_care_booking.booking_code,hms_day_care_booking.booking_date,hms_day_care_booking.daycare_discharge_date,hms_day_care_booking.attended_doctor,hms_day_care_booking.booking_time,hms_day_care_booking.mlc,hms_doctors.doctor_name,hms_specialization.specialization,hms_patient.relation_type,hms_patient.relation_name,hms_patient.relation_simulation_id,hms_gardian_relation.relation,hms_countries.country as country_name, hms_state.state as state_name, hms_cities.city as city_name,"); 

    $this->db->where('hms_daycare_discharge_summery.id',$discharge_id);
    $this->db->join('hms_patient','hms_patient.id=hms_daycare_discharge_summery.patient_id','left');
    ...
    $this->db->select('hms_daycare_discharge_summery_medicine.medicine_name,hms_daycare_discharge_summery_medicine.medicine_dose,hms_daycare_discharge_summery_medicine.medicine_duration,hms_daycare_discharge_summery_medicine.medicine_frequency,hms_daycare_discharge_summery_medicine.medicine_advice,hms_daycare_discharge_summery_medicine.medicine_type');
    $this->db->from('hms_daycare_discharge_summery_medicine');
    $this->db->where('hms_daycare_discharge_summery_medicine.discharge_summary_id',$discharge_id);
    $result_pre['medicine_list']= $this->db->get()->result();
//echo $this->db->last_query(); exit;
    $this->db->select('hms_daycare_discharge_summary_test.test_name,hms_daycare_discharge_summary_test.test_date,hms_daycare_discharge_summary_test.result');
    $this->db->from('hms_daycare_discharge_summary_test');
    $this->db->where('hms_daycare_discharge_summary_test.discharge_summary_id',$discharge_id);
  ...
  public function get_doctor_signature($doctor_id='')
  {
    ...
    {
      $users_data = $this->session->userdata('auth_users'); 
      $this->db->select('hms_daycare_discharge_medicine_print_setting.*');
      $this->db->where($data);
      if(!empty($branch_id))
      {
        $this->db->where('hms_daycare_discharge_medicine_print_setting.branch_id = "'.$branch_id.'"');
      }
      else
      {
        $this->db->where('hms_daycare_discharge_medicine_print_setting.branch_id = "'.$users_data['parent_id'].'"');
      }
      
      $this->db->from('hms_daycare_discharge_medicine_print_setting');
    ...
    {
      $users_data = $this->session->userdata('auth_users'); 
      $this->db->select('hms_branch_daycare_discharge_summary_setting.*');
      $this->db->where($data);
      if(!empty($branch_id))
      {
        $this->db->where('hms_branch_daycare_discharge_summary_setting.branch_id = "'.$branch_id.'"');
      }
      else
      {
        $this->db->where('hms_branch_daycare_discharge_summary_setting.branch_id = "'.$users_data['parent_id'].'"');
      }
       $this->db->where('type',$type);
      $this->db->from('hms_branch_daycare_discharge_summary_setting');
      $result=$this->db->get()->row();
      //echo $this->db->last_query(); exit;
      return $result;
  ...
  {
        $users_data = $this->session->userdata('auth_users');
        $this->db->select('hms_daycare_discharge_field_master.*');
        $this->db->from('hms_daycare_discharge_field_master');
        $this->db->where('hms_daycare_discharge_field_master.status',1);
        $this->db->where('hms_daycare_discharge_field_master.is_deleted',0);
        $this->db->where('hms_daycare_discharge_field_master.branch_id',$users_data['parent_id']);
        $this->db->order_by('hms_daycare_discharge_field_master.sort_order','ASC');  
  ...
  public function get_discharge_test($id)
    {
        $this->db->select('hms_daycare_discharge_summary_test.test_name,hms_daycare_discharge_summary_test.test_date,hms_daycare_discharge_summary_test.result, hms_daycare_discharge_summary_test.id');
        $this->db->from('hms_daycare_discharge_summary_test'); 
        $this->db->where('hms_daycare_discharge_summary_test.discharge_summary_id',$id);
        $query = $this->db->get(); 
    //  echo $this->db->last_query(); exit;
        return $query->result();
    }
   ...
   public function report_html_template($part="",$branch_id="")
    {
      ...
      if(!empty($branch_id))
      {
          $this->db->where('hms_daycare_discharge_report_print_letter_head_setting.branch_id',$branch_id);
      }
      else
      {
           $this->db->where('hms_daycare_discharge_report_print_letter_head_setting.branch_id',$users_data['parent_id']);
      } 
