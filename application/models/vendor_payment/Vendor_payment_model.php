...
var $column = array('hms_payment.credit','hms_payment.debit', 'hms_doctors.doctor_name');  
	...
	public function vendor_to_balclearlist($type=1)
	{
		...
		{ 
			//$this->db->select('hms_medicine_vendors.id, hms_medicine_vendors.branch_id, hms_medicine_vendors.name, hms_medicine_vendors.vendor_id as code,(sum(hms_payment.credit)-sum(hms_payment.debit)) as balance');
			/*$this->db->from('hms_payment');
			$this->db->join('hms_medicine_vendors','hms_medicine_vendors.id = hms_payment.vendor_id');
			...
			{
			$this->db->like('hms_patient.mobile_no',$post['vendor_mobile_no']);
			}
			$this->db->group_by('hms_payment.vendor_id');*/ 
			$this->db->select('hms_medicine_vendors.*, (sum(hms_payment.credit)-sum(hms_payment.debit)) as balance, hms_medicine_vendors.vendor_id as code');  
	...
	{
	    //(sum(hms_payment.credit)-sum(hms_payment.debit))
	    $search_ven = $this->input->post();
		$users_data = $this->session->userdata('auth_users');
			$this->db->select('hms_medicine_vendors.name,hms_payment.parent_id,hms_payment.section_id,(hms_payment.credit-hms_payment.debit) as totbalance,hms_payment.vendor_id,hms_payment.created_date,hms_payment.type,,hms_payment.branch_id');  
			$this->db->from('hms_payment');
			$this->db->join('hms_medicine_vendors','hms_medicine_vendors.id=hms_payment.vendor_id','left');
			 $this->db->where('hms_payment.type',0);
			 $this->db->where('hms_payment.vendor_id',$vendor_id);
           if(!empty($search_ven))
           {
            	if($search_ven['module']=='1')
            	{
            		$this->db->join('hms_medicine_purchase','hms_medicine_purchase.vendor_id=hms_medicine_vendors.id','left');
	            	if (!empty($search_ven['start_date'])) {
						$start_date = date('Y-m-d 00:00:00', strtotime($search_ven['start_date']));
						$this->db->where('hms_payment.created_date >=', $start_date);
					}
					
					if (!empty($search_ven['end_date'])) {
						$end_date = date('Y-m-d 23:59:59', strtotime($search_ven['end_date']));
						$this->db->where('hms_payment.created_date <=', $end_date);
					}
	            	
	            	$this->db->where('hms_medicine_purchase.is_deleted=0');
	            }
	            if($search_ven['module']=='4')
            	{
            		$this->db->join('hms_medicine_return','hms_medicine_return.vendor_id=hms_medicine_vendors.id','left');
	            	if(!empty($search_ven['start_date']))
	            	{
	            	    $this->db->where('hms_medicine_return.purchase_date >="'.date('Y-m-d H:i:s',strtotime($search_ven['start_date'].' 00:00:00')).'"');
	            	}
	            	if(!empty($search_ven['end_date']))
	            	{
	            	    $this->db->where('hms_medicine_return.purchase_date <="'.date('Y-m-d H:i:s',strtotime($search_ven['end_date'].' 23:59:00')).'"');
	            	}
	            	
	            	$this->db->where('hms_medicine_return.is_deleted=0');
	            }
	            if($search_ven['module']=='2')
            	{
            		$this->db->join('hms_vaccination_purchase','hms_vaccination_purchase.vendor_id=hms_medicine_vendors.id','left');
	            	if(!empty($search_ven['start_date']))
	            	{
	            	    $this->db->where('hms_vaccination_purchase.purchase_date >="'.date('Y-m-d H:i:s',strtotime($search_ven['start_date'].' 00:00:00')).'"');
	            	}
	            	if(!empty($search_ven['end_date']))
	            	{
	            	    $this->db->where('hms_vaccination_purchase.purchase_date <="'.date('Y-m-d H:i:s',strtotime($search_ven['end_date'].' 23:59:00')).'"');
	            	}
	            	$this->db->where('hms_vaccination_purchase.is_deleted=0');
	            }
	            if($search_ven['module']=='3')
            	{
            		$this->db->join('path_purchase_item','path_purchase_item.vendor_id=hms_medicine_vendors.id','left');
	            	if(!empty($search_ven['start_date']))
	            	{
	            	    $this->db->where('path_purchase_item.purchase_date >="'.date('Y-m-d H:i:s',strtotime($search_ven['start_date'].' 00:00:00')).'"');
	            	}
	            	if(!empty($search_ven['end_date']))
	            	{
	            	    $this->db->where('path_purchase_item.purchase_date <="'.date('Y-m-d H:i:s',strtotime($search_ven['end_date'].' 23:59:00')).'"');
	            	}
...
// 			echo $this->db->last_query();die();
	...
	{
		$users_data = $this->session->userdata('auth_users');
		$this->db->select('hms_medicine_vendors.name, hms_expenses.paid_amount, hms_expenses.id, hms_expenses.parent_id as pay_id,hms_expenses.paid_to_id, hms_expenses.expenses_date, hms_expenses.branch_id,hms_expenses.parent_id,hms_payment.section_id,hms_payment.type,hms_payment.parent_id');  
			...
			$this->db->where('hms_expenses.type IN(2,4,5)');
	...
	function delete_paid_records($expense_id='',$payment_id='',$type=''){
	...
	public function get_ledger_history($get='')
	{
	  ...
	  if($get['type']==1)
	  {
        $this->db->select("hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id, (CASE WHEN hms_payment.balance>0 THEN hms_medicine_purchase.total_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (hms_medicine_purchase.discount) ELSE '0.00' END) as discount, CONCAT('MedPur/',hms_medicine_purchase.purchase_id,'/',hms_medicine_vendors.name) as bill_no,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 3 AND payment.parent_id = hms_medicine_purchase.id) ELSE '0.00' END) as balance,hms_payment_mode.payment_mode,hms_medicine_purchase.purchase_time"); 
		...
		$result_expense['purchase_list'] = $result;
		//return $result;
		
		$this->db->select("hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id, (CASE WHEN hms_payment.balance>0 THEN hms_medicine_return.total_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (hms_medicine_return.discount) ELSE '0.00' END) as discount, CONCAT('MedRet/',hms_medicine_return.return_no,'/',hms_medicine_vendors.name) as bill_no,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 3 AND payment.parent_id = hms_medicine_return.id) ELSE '0.00' END) as balance,hms_payment_mode.payment_mode,hms_medicine_return.purchase_time"); 
        $this->db->join('hms_medicine_return','hms_medicine_return.id = hms_payment.parent_id AND hms_medicine_return.is_deleted !=2'); 
        ...
        $query = $this->db->get();
        //echo $this->db->last_query(); exit;
		$result_return = $query->result();
		$result_expense['return_list'] = $result_return;
		//return $result;
	 ...
	 if($get['type']==2)
	  {
        $this->db->select("hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id, (CASE WHEN hms_payment.balance>0 THEN hms_vaccination_purchase.total_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (hms_vaccination_purchase.discount) ELSE '0.00' END) as discount, CONCAT('VacPur/',hms_vaccination_purchase.purchase_id,'/',hms_medicine_vendors.name) as bill_no,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 7 AND payment.parent_id = hms_vaccination_purchase.id) ELSE '0.00' END) as balance,hms_payment_mode.payment_mode"); 
        $this->db->join('hms_vaccination_purchase','hms_vaccination_purchase.id = hms_payment.parent_id AND hms_vaccination_purchase.is_deleted !=2'); 
        $this->db->join('hms_medicine_vendors','hms_medicine_vendors.id = hms_payment.vendor_id AND hms_medicine_vendors.is_deleted !=2'); 
        $this->db->join('hms_payment_mode','hms_payment_mode.id=hms_payment.pay_mode','left');
        $this->db->from('hms_payment');
	 ...
	 if($get['type']==3)
	  {
        $this->db->select("hms_payment.debit as paid_amount, hms_payment.parent_id,hms_payment.created_date as c_date,hms_payment.balance as blnce,hms_payment.section_id,hms_payment.id as pay_id, (CASE WHEN hms_payment.balance>0 THEN path_purchase_item.total_amount ELSE '0.00' END) total_amount, (CASE WHEN hms_payment.balance > 0 THEN (path_purchase_item.discount) ELSE '0.00' END) as discount, CONCAT('InvPur/',path_purchase_item.purchase_no,'/',hms_medicine_vendors.name) as bill_no,(CASE WHEN hms_payment.balance > 0 THEN (select sum(credit)-sum(debit) from hms_payment as payment where payment.section_id = 6 AND payment.parent_id = path_purchase_item.id) ELSE '0.00' END) as balance,hms_payment_mode.payment_mode"); 
        $this->db->join('path_purchase_item','path_purchase_item.id = hms_payment.parent_id AND path_purchase_item.is_deleted !=2'); 
        $this->db->join('hms_medicine_vendors','hms_medicine_vendors.id = hms_payment.vendor_id AND hms_medicine_vendors.is_deleted !=2'); 
        $this->db->join('hms_payment_mode','hms_payment_mode.id=hms_payment.pay_mode','left');
        $this->db->from('hms_payment');
        $this->db->where('hms_payment.section_id',6);
        $this->db->where('hms_payment.created_date >= "'.$start_date.'"');
        $this->db->where('hms_payment.created_date <= "'.$end_date.'"');
        $this->db->where('path_purchase_item.is_deleted',0); 
