...
class Test_profile_model extends CI_Model {
    ...
    public function profile_price($id="")
    {
        if(!empty($id))
        {  
            $user_data = $this->session->userdata('auth_users');
	...
	public function save($ids=array())
	{
        ...
        {
            $test_data_arr = [];
            $child_test_ids = array_values($child_test_ids);
        ...
        {  
            ...
            if(!empty($result))
            {
                for($i=0;$i<count($result);$i++)
                {
                    ...
                    $profile_id = $this->db->insert_id();  
                    //// Check  Test ///////////////
                    $this->db->select('path_test.*, path_test_heads.test_heads, path_test_method.test_method as test_method_name, path_unit.unit, path_sample_type.sample_type');    
                    $this->db->join('path_test','path_test.id=path_profile_to_test.test_id');    
