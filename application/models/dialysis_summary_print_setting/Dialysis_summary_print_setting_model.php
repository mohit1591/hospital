<?php
class Dialysis_summary_print_setting_model extends CI_Model 
{
	var $table = 'hms_branch_dialysis_summary_setting'; 
    ...
    $this->db->where('hms_branch_dialysis_summary_setting.branch_id',$user_data['parent_id']); 
    $this->db->where('hms_branch_dialysis_summary_setting.setting_name','DIALYSIS_SUMMARY_PRINT_SETTING'); 
		$query = $this->db->get();
    return $query->result();
	}
    ...
    if(!empty($post['data_id']))
    {
        $data = array(
                     ...
                     "type"=>$post['type'],
                     "setting_name"=>'DIALYSIS_SUMMARY_PRINT_SETTING',
        ...
        $this->db->update('hms_branch_dialysis_summary_setting',$data);
    }
    ...
    public function get_hms_prescription_page()
    {
        ...
        $query = $this->db->get('hms_branch_dialysis_summary_setting');
        $result = $query->row();
        $data='';
        if(!empty($result))
        {
