...
class Eye_general_model extends CI_Model 
{
    ...
    public function chief_complaint_list($doctor_id="",$type="")
    {
        ...
        if(!empty($type)) {
            $this->db->where('hms_eye_chief_complaints.chief_complaints LIKE "'.$type.'%"');
        }   
        $query = $this->db->get('hms_eye_chief_complaints');
        $result = $query->result(); 
        return $result; 
...
/* prescription setting for eye */
    ...
    function get_eye_prescription_tab_setting($parent_id="",$order_by='')
    {

            $users_data = $this->session->userdata('auth_users');
            $this->db->select('hms_eye_branch_prescription_tab_setting.*,hms_eye_prescription_tab_setting.var_title');   
            $this->db->join('hms_eye_branch_prescription_tab_setting','hms_eye_branch_prescription_tab_setting.unique_id = hms_eye_prescription_tab_setting.id');
            $this->db->where('hms_eye_branch_prescription_tab_setting.branch_id',$parent_id);
            if(!empty($order_by))
            {
               $this->db->order_by('hms_eye_branch_prescription_tab_setting.order_by',$order_by); 
            }
            else
            {
                $this->db->order_by('hms_eye_branch_prescription_tab_setting.order_by','ASC');     
            }
            $this->db->where('hms_eye_branch_prescription_tab_setting.status',1);
            $query = $this->db->get('hms_eye_prescription_tab_setting');
    ...
    function get_eye_prescription_medicine_tab_setting($parent_id="",$order_by='')
    {
            $users_data = $this->session->userdata('auth_users');
            $this->db->select('hms_eye_branch_prescription_medicine_tab_setting.*,hms_eye_prescription_medicine_tab_setting.var_title');   
            $this->db->join('hms_eye_branch_prescription_medicine_tab_setting','hms_eye_branch_prescription_medicine_tab_setting.unique_id = hms_eye_prescription_medicine_tab_setting.id');
            $this->db->where('hms_eye_branch_prescription_medicine_tab_setting.branch_id',$parent_id);
            if(!empty($order_by))
            {
               $this->db->order_by('hms_eye_branch_prescription_medicine_tab_setting.order_by',$order_by); 
            }
            else
            {
                $this->db->order_by('hms_eye_branch_prescription_medicine_tab_setting.order_by','ASC');     
            }
            $this->db->where('hms_eye_branch_prescription_medicine_tab_setting.status',1);
            $query = $this->db->get('hms_eye_prescription_medicine_tab_setting');
    ...
    public function total_camp_booking($parent_id="",$prefix='')
    {
        $users_data = $this->session->userdata('auth_users');
        ...
        if(!empty($prefix))
        {
          $this->db->where('hms_opd_booking.created_date >= (select created_date from hms_branch_unique_ids where unique_id = "45" AND  branch_id = "'.$parent_id.'")');
        }
