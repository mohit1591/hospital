...
class Prescription_file_model extends CI_Model {
	...
	var $table = 'hms_eye_prescription_files';
	var $column = array('hms_eye_prescription_files.id','hms_eye_prescription_files.prescription_files','hms_eye_prescription_files.created_date');  
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("hms_eye_prescription_files.*, hms_eye_prescription_files.prescription_files"); 
		$this->db->from($this->table);  
        $this->db->where('hms_eye_prescription_files.branch_id = "'.$users_data['parent_id'].'"');
    ...
    public function delete($id="")
    {
    	...
    	{ 
			...
			if(!empty($result))
			{
			  ...
			  {
			  	unlink(DIR_UPLOAD_PATH.'eye/prescription/'.$result[0]->prescription_files);
			  }	 
			}  

			$this->db->where('id',$id);
			$this->db->delete('hms_eye_prescription_files'); 
    	} 
    }
