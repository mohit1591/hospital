...
class Ipd_patient_discharge_summary_model extends CI_Model {
	...
	private function _get_datatables_query()
	{
		...
		if(isset($search) && !empty($search))
		{
		    if(!empty($search['start_date']))
			{
				...
				$this->db->where('hms_ipd_patient_discharge_summery.created_date >= "'.$booking_from_date.'"');
			}

			if(!empty($search['end_date']))
			{
				$booking_to_date = date('Y-m-d',strtotime($search['end_date'])).' 23:59:59';
				$this->db->where('hms_ipd_patient_discharge_summery.created_date <= "'.$booking_to_date.'"');
			}
			if($search['summery_type']=="-1")
			{
				
			}
			else
			{
			   $this->db->where('hms_ipd_patient_discharge_summery.summery_type',$search['summery_type']);
	...
	public function save()
	{
		...
		$review_time_date = !empty($post['review_time_date'])? date('Y-m-d',strtotime($post['review_time_date'])): null;
		$data = array( 
                    ...
                    'review_time'=>!empty($post['review_time']) ? date('H:i:s', strtotime(date('d-m-Y').' '.$post['review_time'])): null,
                    'death_date'=>date('Y-m-d',strtotime($post['death_date'])),
                    'death_time'=>date('H:i:s', strtotime(date('d-m-Y').' '.$post['death_time'])),
					'cause_of_death' => $post['cause_of_death'],
                    'status'=>$post['status'],
				...
				for($i=0;$i<$counter_name;$i++) 
					{
						...
						'branch_id'=>$user_data['parent_id'],
						'discharge_summary_id'=>$post['data_id'],
						'interpretation' => $post['interpretation'][$i],
						'interpratation_data' => $post['interpratation_data'][$i]
...
$users_data = $this->session->userdata('auth_users'); 
	...
	public function get_tests_by_patient_id($id,$ipd_id="")
  {
    $user_data = $this->session->userdata('auth_users');
    ...
    $this->db->where('path_test_booking.patient_id',$id);
   ...
   public function template_data($template_id="")
   {
		...
		$this->db->where('discharge_summary_id', $template_id);
...
*/
  ...
  public function get_discharge_test($id)
	{
		$this->db->select('hms_ipd_patient_discharge_summary_test.test_name,hms_ipd_patient_discharge_summary_test.test_date,hms_ipd_patient_discharge_summary_test.result, hms_ipd_patient_discharge_summary_test.id, hms_ipd_patient_discharge_summary_test.interpretation,hms_ipd_patient_discharge_summary_test.interpratation_data,hms_ipd_patient_discharge_summary_test.interpretation,hms_ipd_patient_discharge_summary_test.interpratation_data');
		$this->db->from('hms_ipd_patient_discharge_summary_test'); 
