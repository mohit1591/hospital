...
class Dialysis_schedule_model extends CI_Model {

	var $table = 'hms_dialysis_booking';
	var $column = array('hms_dialysis_booking.id','hms_patient.patient_code','hms_patient.patient_name','hms_dialysis_booking.booking_code','hms_ipd_rooms.room_no','hms_doctors.doctor_name','hms_dialysis_booking.dialysis_date');  
	...
	private function _get_datatables_query() // 
	{
		...
		$this->db->select("hms_dialysis_booking.*,hms_ipd_booking.ipd_no,hms_ipd_booking.patient_id,hms_doctors.doctor_name,hms_ipd_rooms.room_no,hms_ipd_room_to_bad.bad_no,hms_ipd_room_category.room_category as room_type, hms_patient.patient_name as p_name,hms_patient.patient_code as p_code,hms_dialysis_room.room_no as dia_room"); 
		$this->db->from($this->table); 
        $this->db->where('hms_dialysis_booking.is_deleted','0');
        if($users_data['users_role']==4)
		{
		$this->db->where('hms_dialysis_booking.patient_id = "'.$users_data['parent_id'].'"');
		}
		elseif($users_data['users_role']==3)
		{
			$this->db->where('hms_dialysis_booking.referral_doctor = "'.$users_data['parent_id'].'"');
		}
		else
		{
        ...
        $this->db->join('hms_ipd_booking','hms_ipd_booking.id=hms_dialysis_booking.ipd_id','left'); 
		...
		$this->db->join('hms_dialysis_room','hms_dialysis_room.id=hms_dialysis_booking.dialysis_room_no','left');
		...
		$this->db->where('hms_dialysis_booking.dialysis_date >= "'.$start_date.'" AND hms_dialysis_booking.dialysis_time >= "'.$start_time.'"');
		...
		$this->db->where('hms_dialysis_booking.dialysis_date <= "'.$end_date.'"  AND hms_dialysis_booking.dialysis_end_time >= "'.$end_time.'"');
		}
    ...
    public function ot_scheduled_time($dialysis_date="",$start_datetime="",$end_datetime="",$operation_room="")
    {
    	...
    	// //exit;
		$users_data = $this->session->userdata('auth_users'); 
		$this->db->select('hms_dialysis_booking.*,CONCAT(hms_dialysis_booking.dialysis_date," ",hms_dialysis_booking.dialysis_time) as ot_start_datetime, CONCAT(hms_dialysis_booking.dialysis_date," ",hms_dialysis_booking.operation_end_time) as ot_end_datetime ');
		$this->db->where('branch_id  IN ('.$users_data['parent_id'].')'); 
       ...
       //print '<pre>'; print_r($query);die;
		foreach($query as $res)
		{
          if($res->dialysis_date == $dialysis_date  && $res->ot_room_no == $operation_room && ($start_datetime >=$res->ot_start_datetime && $start_datetime <= $res->ot_end_datetime))
          {
          	  $msg['error']=1;
          }
          else
    ...
    public function ot_scheduled_doctor($dialysis_date="",$start_datetime="",$end_datetime="",$doctors="")
    {
    	$users_data = $this->session->userdata('auth_users'); 
		$this->db->select('hms_dialysis_booking.*,CONCAT(hms_dialysis_booking.dialysis_date," ",hms_dialysis_booking.dialysis_time) as ot_start_datetime, CONCAT(hms_dialysis_booking.dialysis_date," ",hms_dialysis_booking.operation_end_time) as ot_end_datetime ');
		$this->db->where('branch_id  IN ('.$users_data['parent_id'].')'); 
